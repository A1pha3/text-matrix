...
- **⚠️ 操作步骤（强制）**：写 frontmatter 前，**必须先执行 `date` 命令确认当前系统时间**。frontmatter 里的 date **必须 ≤ 当前时间**。例如：当前时间是 08:10，但计划 08:30 发布，frontmatter 也必须写 08:10 或更早，绝不能写 08:30（Hugo 会把 08:30 当作未发布而不显示）。
