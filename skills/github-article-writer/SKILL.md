...
1. **准确** — 仓库证据（Stars、Forks、命令、版本号、目录结构、源码片段）必须从 GitHub 页面、README、docs 可追溯；不接受二手转述或猜测。
2. **专业** — 用工程语言描述仓库能力，不堆砌情绪词、不夸张、不网络化；术语首次出现用中文（English）括注。
3. **优雅** — 句式节奏有变化，不堆砌模板腔、机械转场、二元口号句；段落、表格、列表服务于阅读，而非凑结构。
4. **有品味** — 选词有取舍，宁可少说也不要空话；避免泛化抽象词（"核心价值""能力闭环""未来可期"等）；让文字承载判断而非套话。

本铁律与"先取证再写作"、"三轮事实校验"、"去 AI 味"是**叠加约束**，不是替代关系 —— 文章必须在事实可验证的同时具备这四种品质。
