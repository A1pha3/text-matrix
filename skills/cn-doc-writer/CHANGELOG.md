...
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| v6.0.0 | 2026-08-02 | WorkBuddy | **文体包架构（合并替代拆分）**。放弃从未完成的 cn-tech-blog-writer 拆分，改为单 skill + 文体包：新增 `references/styles/` 四包（doc / project-review / video-digest / essay），每包 = frontmatter 参数头（triggers/reader/band/gates）+ 手艺正文；`blog-deep-dive.md` 内容迁入 project-review.md 后删除。`quality.md` 升级为评价内核：权重档注册表（7 档，含 review/video/essay）+ 门槛注册表（事实性/去AI味/观点依据/转写保真），文体包只能引用不能自造。`SKILL.md`：路由键改为读者契约（§1.2 文体包选择，命令×文体正交）；description 纳入博客类触发词；自检 #9 改文体包检查；删除 cn-tech-blog-writer fallback；路由判断 3→4 个。`edge-cases.md` 边界回归（博客/解析/随笔经文体包处理）；`behavior-fixtures.md` 关联指向 project-review.md；README 重写架构节并补 styles/ 结构；测试反转：删除"独立博客 skill 存在性"规格测试，description 关键词断言反转，新增文体包契约测试（frontmatter 必须引用已注册 band/gates）。 |
| v5.16.0 | 2026-08-01 | WorkBuddy | **拆分后遗症清理与单源化**。`quality.md`：明确评分顺序"先求和、再应用封顶门槛、最后取整"，消除 89.6 分读数歧义。`SKILL.md`：异常表新增 `cn-tech-blog-writer` 未安装时的 fallback（声明路由偏差后按 write/optimize 处理）。`edge-cases.md`：适用边界中"技术博客"改为路由说明；删除反馈闭环整节（规则所在文件普通 optimize 不加载、反馈无存储定义、交付锚点与去 AI 味门槛冲突）。`behavior-fixtures.md`：修复两处悬空引用（§3.5 分析型增强回路 → §4 示例与 §1.1 路由；§1 Step 3 → §3 Step 3）。`README.md`：删除内置第二份更新日志（与 CHANGELOG 双向漂移），改为指针；`blog-deep-dive.md` 标注"待迁入 cn-tech-blog-writer"。`test_scripts.py` 新增拆分引用一致性回归测试。 |
| v5.15.0 | 2026-08-01 | WorkBuddy | **对抗性审查修复（P0+P1）与回归测试**。`quality.md`：评分聚合改为"子项得分率 × 维度权重"，子项上限不随档缩放——修复分档后同一缺陷边际代价相同、高正确性档相对宽容的倒挂（一处事实错误：API 档 -5、默认档 -3、教程档 -2.5）。`commands.md`：发布级评审模板分母改为权重档变量；文体判断改为选定权重档，移除博客拆分前的"分析"残留；`optimize-cn-doc` 步骤重排跳号。`SKILL.md`：Step 1 接入权重档选择；选题价值 gate 补"用户坚持先出稿"豁免；删除与 §6 自检重复的 Step 4 交付前回路，Step 4.5 顺位为 Step 4；examples.md 加载触发操作化（默认回路凭主文件内联信号，无法判定/校准/精修时再加载）。`check_ai_tone.py`：门槛消息清除 v5.3.0 残留（"可读性封顶 20/25"），通过语义收敛为"高频信号未命中，完整门槛仍需 quality.md 评估"。`behavior-fixtures.md` 引用同步。`test_scripts.py` 新增评分模型与门槛消息回归测试。 |
| v5.11.0–v5.13.0 | 2026-05 至 2026-08 | — | 变更记录遗失；期间 SKILL.md、quality.md、commands.md、README、skill.json 有实质修改，详情见 git 历史。 |
| v5.14.0 | 2026-08-01 | WorkBuddy | **评分体系第一性原理重构**。`references/quality.md`：权重由固定 30/40/30 改为随文档类型分档（教程 25/45/30、参考与 API 50/25/25、README 与快速上手 30/25/45、默认 30/40/30）并补分档理由；评分细则从给分制改为带锚点扣分制，扣分必须能指认到具体位置；明确双重计算归属（用词漂移只扣正确性，清晰度只管定义与歧义）；所有维度以"对声明的目标读者而言"为前提；新增事实性封顶门槛（≥1 处确凿事实错误即总分 ≤89 且不得评 S），与去 AI 味门槛对称，消除声明优先级与门槛机制的矛盾；可读性量化指标标注"参考带非硬规则"，防止机械达标。`SKILL.md`：评分行改为权重分档表述；`write-cn-doc` 方案新增选题价值 gate（答不上信息增量先收敛选题）；自检表新增第 11 项封顶门槛检查；README 同步权重表述。 |
| v5.10.0 | 2026-05-25 | Codex | **CI 防漂移集成**。`ci/check-docs.yml` 新增可选主副本同步检查：配置 `CN_DOC_WRITER_SOURCE_DIR` 和 `CN_DOC_WRITER_TARGET_DIR` 后，CI 会运行 `scripts/check_skill_sync.py` 并在副本缺失、多余或内容不一致时失败；工作流触发路径补充 `skills/cn-doc-writer/**` 与 `agent/skills/my-skills/cn-doc-writer/**`，确保 skill 文件变更也能触发检查；README 和 dogfooding 测试同步覆盖 CI 接入方式。 |
