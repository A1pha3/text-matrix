# Few-shot 示例与语气框架

> 本文件包含输出质量标准示例、反例和语气/调性框架。由 SKILL.md 按需加载。

---

## 语气/调性框架

根据受众类型选择对应语气（未指定时默认「企业内部」）：

| 受众类型 | 语气特征 | 示例表述 | 禁忌表述 |
|----------|----------|----------|----------|
| **开源社区** | 友好、鼓励、平等 | "恭喜你完成了第一步！" "欢迎提交 PR" | "必须按照规定..." |
| **企业内部** | 简洁、专业、结构化 | "按以下步骤配置..." "注意事项：" | "亲~" "小伙伴们" |
| **初学者教程** | 耐心、细致、引导 | "如果你还不熟悉 X，可以先..." | "这很简单" "显然" |
| **API 参考** | 精确、客观、无废话 | "返回值：string 类型" | "可能会返回..." "大概是..." |
| **技术博客** | 生动、有观点、有故事 | "我们踩过这个坑..." "经过测试发现..." | 过于口语化或夹带私货 |

补充约束：自然表达不等于聊天体。技术文档可以有人味，但不能靠堆口头禅、感叹句、作者自我表态或模板式教学口令来制造“像人写的”错觉。

---

## 完整信号库

去 AI 味时按 3 层顺序处理：先替换泛化、抽象、成组出现的套话；再拆掉过度整齐的判断句和转场句；最后打散“表格 + 解释 + 原则”“定义 + 判断 + 总结”的固定节奏。

### 高频信号

| 信号 | 典型表现 | 修正方向 |
|------|----------|----------|
| 自我声明式元话语 | “本文将”“更准确地说”“换句话说”“这里必须明确”“最重要的是”连续出现 | 直接进入事实、限制和判断 |
| 抽象套话密集 | “核心价值”“落地边界”“能力闭环”“体系化”“赋能”“最佳实践”“可发现、可复用、可验证”成组出现 | 换成命令、字段、文件、页面、会话、输出、错误等具体对象 |
| 二元口号句 | “不是 A，而是 B”“不只是 A，更是 B”“真正 X 的不是 Y，而是 Z”反复推进 | 改成场景判断或操作取舍 |
| 机械总结句 | “这意味着”“这解释了”“这种路径的价值”“这套顺序的本质”频繁收束段落 | 让段落停在具体影响或下一步动作上 |
| 教学控制句 | “如果你只想”“读到这里不妨”“你应该能够”频繁打断正文 | 合并为自然的阅读路径或删去 |
| 模板化信息块 | “学习目标 / 阅读指引 / 自测问题”像表单，而不是正文需要 | 保留结构块，但标题和导语要贴合当前主题 |
| 列表机械对称 | 多条项目句式完全一致、信息密度偏低 | 合并弱项，打散句式，保留真正可执行的信息 |
| 作者在场感失衡 | 非评论文反复出现“我认为 / 我更愿意 / 我会 / 我倾向于” | 只在操作取舍或分析判断处少量保留 |

### 改写原则

- 先保信息，再调语气；不得删掉事实边界、版本差异、错误条件或 caveat。
- 用具体工程对象替代抽象名词：把“能力 / 机制 / 价值 / 边界”落到命令、字段、文件、页面、会话、输出、错误上。
- 用动作替代形容词串：少写“可发现、可复用、可验证”，改成“能被 `opencli list` 找到、能进脚本、能被下一次命令复查”。
- 把高光总结改成场景判断：少写“核心价值在于……”，改成“当团队每周都跑这条流程时，问题就不该停在 browser 原语上”。
- 合并短句，打散过度对称的句式，避免连续“定义句 + 判断句 + 提醒句”模板。
- 允许段落节奏有轻微不均匀：技术文章不必每节都以一句原则收尾。
- 技术文章保持克制，不要为了“像人写的”而改成口语聊天体或情绪化评论。

### 常见替换

| AI 味写法 | 更自然的技术写法 |
| ------ | ------ |
| 核心价值不是 X，而是 Y | 更值得看的地方在 Y；X 只是入口 |
| 可发现、可复用、可验证的能力 | 能被命令列出来、能进脚本、能被下一次运行复查 |
| 这解释了一个常见现象 | 于是会出现一个看似奇怪的现象 |
| 这种路径的价值在于 | 这条路少掉的是具体的维护成本 |
| 落地边界 | 什么时候该用，什么时候不该用 |
| 赋能智能体（Agent）/ 提升效率 | 少猜一次 DOM，少写一段临时脚本 |

---

## 正面示例

以下示例展示期望的输出质量标准，AI 必须达到或超越此水平。

### 示例 0：去 AI 味后的改写方向

```markdown
## ❌ 改写前

更准确地说，最重要的是先理解这里的核心前提。换句话说，如果你只想快速上手，可以先记住这几点。读到这里，你应该已经意识到这个问题并不简单。

## ✅ 改写后

这里有一个前提容易被忽略：系统是否稳定，不取决于配置项是否填全，而取决于调用链里哪些失败会被传播、哪些失败会被吞掉。想先上手，可以先抓住 3 个点：入口参数、失败路径和默认值。把这三处看明白，后面的排查就不会失焦。
```

要点：保留信息密度，但去掉元话语、教学控制句和空泛转场，直接进入事实与判断。

### 示例 1：概念定义段落（标准模式 Level 2）

```markdown
## 📝 中间件（Middleware）

### 一句话定义
**中间件** 是请求到达路由处理函数之前（或响应返回之后）自动执行的函数链。

### 类比理解
> 💡 就像机场安检的多个关卡——每个旅客（请求）必须依次通过身份验证、
> 行李扫描、安全检查，全部通过后才能登机（到达目标路由）。
> 任何一个关卡失败，旅客都会被拦截。

### 为什么需要它
没有中间件时，每个路由都要重复编写认证、日志、错误处理等逻辑：

‌```python
# ❌ 无中间件：每个路由重复认证逻辑
@app.get("/users")
def get_users(token: str):
    if not verify_token(token):    # 重复！
        raise HTTPException(401)
    return db.get_users()

# ✅ 有中间件：认证逻辑集中处理
@app.middleware("http")
async def auth_middleware(request, call_next):
    verify_token(request.headers["Authorization"])
    return await call_next(request)
‌```
```

### 示例 2：翻译段落（translate-cn 输出）

```markdown
原文：
> Middleware functions have access to the request object, the response
> object, and the next middleware function. If the current middleware does
> not end the request-response cycle, it must call next().

译文：
> 中间件（Middleware）函数可以访问请求对象、响应对象和下一个中间件函数。
> 如果当前中间件没有结束请求-响应周期，就必须调用 `next()` 将控制权传递下去。
>
> 💡 **译注**：这是 Express.js 中间件的核心行为——每个中间件要么终结请求，
> 要么把请求传给下一个，否则客户端会一直挂起（hang）。
```

---

## 反例：不要这样输出

以下示例展示常见错误模式，AI **必须避免**：

````markdown
## ❌ 反例 1：术语不一致

第一段说"将应用打包到**容器**中"，第三段却写
"run the Container to start service"。
全文混用"代码仓库""存储库""仓库"三种译法，读者无法确定是否指同一概念。

→ **规则**：首次标注后全文统一，参照 terminology.json 单源

## ❌ 反例 2：只有 How，没有 Why

### 跨域配置

在 `app.py` 中添加以下代码：

```python
app.use(cors())
```

即可完成跨域配置。

→ **问题**：读者不知道什么是跨域、为什么需要处理、不处理会怎样。
→ **修复**：在代码前加一段"为什么需要 CORS"的解释。

## ❌ 反例 3：代码块缺少语言标记

```
npm install express
```

→ **问题**：无法语法高亮，读者分不清这是 shell 命令还是代码。
→ **修复**：添加 `bash` / `python` / `json` 等语言标记。

## ❌ 反例 4：中英文无空格 + 中文标点混乱

使用Docker部署,然后运行npm install安装依赖.

→ **问题**：Docker/npm 前后缺空格、逗号和句号用了半角。
→ **修复**：`使用 Docker 部署，然后运行 npm install 安装依赖。`

## ❌ 反例 5：明显 AI 味 / 模板腔

更准确地说，本文将从多个维度全面介绍这一问题。换句话说，你可以把它理解成一个非常重要的核心机制。最重要的是，如果你是初学者，也不用担心，因为我们会一步一步带你理解。

→ **问题**：连续使用生成式转场、自我声明和教学控制句，但没有增加信息量。
→ **修复**：删掉“本文将 / 换句话说 / 最重要的是 / 不用担心”这类空转句，直接说明机制、边界和影响。
````

---

## 好文档 vs 差文档对比

````markdown
## ❌ 差文档示例

### 安装
运行 npm install 安装依赖。然后配置config.json文件，把apiKey填进去就行了。
接下来看下面的代码：
// ...省略...
如果报错了就检查一下配置。

## ✅ 好文档示例

### 安装（预计 5 分钟）

**前置条件**：Node.js 16+（运行 `node -v` 验证）

```bash
# 克隆项目并安装依赖
git clone https://github.com/example/project.git
cd project
npm install
```

### 配置

创建 `config.json`，填入你的 API Key：

```json
{
  "apiKey": "your-api-key-here",
  "baseUrl": "https://api.example.com/v1"
}
```

> 💡 **获取 API Key**：访问 [开发者后台](https://example.com/dashboard) → 「设置」→「API 密钥」

### ✅ 验证安装

```bash
npm run check
# 预期输出：✓ Configuration valid. Ready to go!
```

### ❓ 常见问题

**Q: 报错 `ENOENT: config.json not found`**
确认 `config.json` 在项目根目录下，而非 `src/` 目录中。
````
