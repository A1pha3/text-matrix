...
**自检 grep 命令**（建议加进交付前脚本）：
```bash
grep -nE "五维评分|结构性.*20|准确性.*25|可读性.*25|教学性.*20|实用性.*10|总分.*100|S 级|铁律登记|铁律#|v1.*v2|迭代记录|迭代日志|质量底线|victorchen96|反写工作流|三备份铁律|USER\.md|TOOLS\.md|HEARTBEAT\.md|memory-tencentdb" <文章文件>
```

**触发背景**：6-19 23:00 AI 写 MIT 6.S184 文章时把"5 维自评表 + 迭代日志"等内部记录写进正文，6-20 18:00 师父飞书 message `om_x100b6c500f4760a4b13b41a19db2c36` 重申铁律，6-30 hotfix 彻底删除（commit `5e0da6b7`）。
