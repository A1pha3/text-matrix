...
**风格铁律：准确、专业、优雅、有品味（2026-07-20 师父拍板，本 skill 全局生效，叠加在五维评分之上）**

所有产物必须同时满足以下四维，缺一不可：

1. **准确** — 事实、数据、版本号、命令、链接、术语必须可验证；不接受"大致"或"应该"，凡是无法验证的内容一律标注或删除。
2. **专业** — 用工程语言描述工程问题，不堆砌情绪词、不夸张、不网络化；术语首次出现用中文（English）括注。
3. **优雅** — 句式节奏有变化，不堆砌模板腔、机械转场、二元口号句；段落、表格、列表服务于阅读，而非凑结构。
4. **有品味** — 选词有取舍，宁可少说也不要空话；避免泛化抽象词（"核心价值""能力闭环""未来可期""可发现、可复用、可验证""不是 A 而是 B"等）；让文字承载判断而非套话。

本铁律与去 AI 味回路、五维评分、自动自检是**叠加约束**，不是替代关系 —— 文章必须在五维 100 分的同时具备这四种品质。任何润色、扩写、改写都必须先经过本铁律的过滤；牺牲风格换信息密度的改写一律回滚。
...
以下 5 类字段是**内部系统记录**，严禁出现在技术文章正文中。任何命中都必须删除或重写：
...
```bash
# v2.1 升级（2026-07-10 00:16 落地）：加入阿拉伯数字变体「5 维」「4 维」「3 维」
# 触发根因：commit e9541bce L283 命中「5 维自评（v2 修订）：结构 20/20」
# 原 grep 只匹配「五维」/「结构性」/「准确性」全名 — 漏掉「5 维」+ 阿拉伯分数
...
- 自检脚本**必须扫全文**，包括末尾"写作笔记 / 元信息 / 备注 / 关于本文"等段
- 不仅扫 `frontmatter` 和 `body`，要包含所有 H2 标题下的全部内容
- e9541bce 漏网教训：5 维自评藏在「## 写作笔记（给后续读者）」段下，frontmatter/body 自检默认不扫末尾元段

**触发背景**：
- 6-19 23:00 AI 写 MIT 6.S184 文章时把"5 维自评表 + 迭代日志"等内部记录写进正文，6-20 18:00 师父飞书 message `om_x100b6c500f4760a4b13b41a19db2c36` 重申铁律，6-30 hotfix 彻底删除（commit `5e0da6b7`）。
...
**正确做法**：5 维评分 100/100 是 AI **自评**的内部质量机制，**只在心里 + 飞书汇报给师父**——不写进正文。铁律登记、迭代日志、质量底线是 AI **写给自己看**的元描述，**不外露**。USER.md/TOOLS.md/HEARTBEAT.md 是**内部 workspace 文件名**，**不外露**。
