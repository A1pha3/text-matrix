...
维护本 skill 的多个拷贝时，以 `prompt_alpha/agent（智能体）/skills/my-skills/cn-doc-writer` 为主版本；`text-matrix/skills/cn-doc-writer` 只作为副本。所有修改都必须先落在主版本，再从主版本同步到副本。

同步后运行 `scripts/check_skill_sync.py` 对比主版本和副本；出现缺失、多余或内容不同的文件，先修正副本漂移，再继续发布或后续优化。若使用 `ci/check-docs.yml`，配置 `CN_DOC_WRITER_SOURCE_DIR` 与 `CN_DOC_WRITER_TARGET_DIR` 后，CI 会自动阻断漂移。
...
| `optimize-cn-doc` | 交付优化后文档（已自动去 AI 味）+ 默认简版报告 | 展示基线问题、主要改动和最多 3 个关键维度变化；发布级完整评审才展开评分表 |
...
自动去 AI 味必须在结构、事实、术语和教学路径稳定后执行；执行后复评各维度分数，不得用自然表达换取信息损失。
...
- `write-cn-doc`：先过选题价值 gate（本文提供了什么读者已有渠道没有的东西？答不上来先收敛选题；用户坚持先出稿时声明价值假设并继续），再定章节骨架 + 目标读者 + 学习路径/级别
...
高频信号、改写原则和替换例见 `references/examples.md` 的"完整信号库"。默认回路凭本文件内联信号（§8 禁止项与下方出口条件）执行；出口条件或自检 #7 无法判定、需要语气校准或发布前精修时，再加载 examples.md。
...
| 命令 | 文件写入规则 |
| ------ | ------ |
| `write-cn-doc` | 写入用户指定的文件路径；若未指定，询问用户而非自行命名 |
| `translate-cn` | 写入用户指定的文件路径；禁止附加快照或副本文件 |
| `optimize-cn-doc` | **直接覆盖原始文件**，禁止创建 `-optimized.md` 或任何带后缀的副本文件 |
| `enhance-learning` | **直接覆盖原始文件**，禁止创建带后缀的副本文件 |

报告输出规则：优化报告、评分表、增强清单等**一律输出在对话中**，禁止保存为独立文件（除非用户明确要求）。禁止在项目目录（如 Hugo `content/`、项目根目录）生成临时报告文件。
