...
默认外显契约只决定是否展示过程；最低交付契约只决定产物是否齐全。内部过程对文件系统零副作用（落盘不变量见 §7 文件输出约束）。
...
**落盘不变量**：内部过程对文件系统零副作用。每个落盘文件必须能指认到用户请求中的对应项（新写或覆盖）；指认不到的过程产物——初稿、版本稿、草稿、快照、评分、报告——只留在对话中。落盘路径的合法来源只有三种：用户显式指定、目标仓库中已存在的同类文档所体现的目录约定、用户答复（前两者不可用时询问）。
