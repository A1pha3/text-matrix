...
omp 不是一个"更多模型的 wrapper"。它的核心差异化在于把工具层作为一等公民来设计：hashline 编辑解决格式损耗、Rust 内联原语解决执行效率、Hindsight 解决跨 session 记忆、子 Agent 协调解决并行任务管理。这四件事单独看都不复杂，但组合在一起，让模型的实际工作质量产生了质的差别——不是模型自己变强了，而是模型输出的东西能完整、准确、低损耗地进入磁盘。
