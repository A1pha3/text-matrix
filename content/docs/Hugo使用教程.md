...
description = '从零搭建 Hugo 静态网站的完整教程，涵盖安装配置、主题定制、内容管理与部署发布全流程。'
+++
...
- **访问性能高**：静态文件可以直接由 CDN 分发
- **攻击面小**：无数据库、无服务端代码，几乎没有可被攻击的入口
- **部署简单**：生成的静态文件可以放到任何 Web 服务器或 CDN

Hugo 的构建速度很快，普通笔记本电脑上每秒可以生成数千个页面。这让它特别适合大型文档站点和文章量大的博客。
...
输出会包含版本号、支持平台与构建日期，看到版本号就说明装好了。
...
把 AdSense 的 Publisher ID 写进 `hugo.toml`，未来改或临时关都更顺手。
...
**步骤 2：在 `<head>` 中引入**
找到主题提供的自定义 head 钩子（LoveIt 主题用 `layouts/partials/custom/head.html`），或者直接覆写基础模板，添加以下调用：
...
**在 Markdown 中这样调用：**
在编写文章时，在需要展示广告的地方插入以下短代码（`slot` 替换为 AdSense 后台创建的真实广告单元 ID）：
...
广告加载不当会拖慢页面速度并影响 SEO，需要执行以下优化：

#### 1. 降低 CLS（累积布局偏移）
上面短代码中给 `<div class="ad-container">` 设置了 `min-height: 100px;`。
广告脚本通常在页面渲染后才异步拉取内容，没有预留高度会让广告突然出现，把正文往下挤，导致 CLS 飙升，直接影响 Core Web Vitals。
