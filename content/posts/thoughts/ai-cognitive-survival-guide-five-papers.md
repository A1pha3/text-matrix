...
lastmod: 2026-09-15T02:45:00+08:00
...
五篇不属于同一证据层级：三篇是实验（Kosmyna、Khan、Ghosh），一篇是调查（Lee），一篇是不报告效应量的框架论文（Vendrell & Johnston）。所以更准确的读法，是把它们当一组方向一致、可靠度不同的信号，而不是五项铁证。

## 一、MIT 的 EEG 实验：认知债务会累积

MIT 媒体实验室的 Kosmyna 等人写的《Your Brain on ChatGPT》流传最广，因为它问得最直接：把写作外包给 LLM，人到底损失了什么。论文给这笔损失起了个名字——认知债务：文章还在交，但参与的脑力活动、对文本的记忆和所有权都在悄悄流失，并且随使用次数累积。

参与者在三种条件下完成写作：`LLM`、`Search Engine`、`Brain-only`，各写三轮。三个条件组共 54 人，其中 18 人继续进入第 4 轮。第 4 轮把条件对调——一直用 LLM 的改纯脑写作，一直纯脑的改用 LLM——专门观察切换后的变化。

EEG 数据显示，`Brain-only` 组脑区连接最强，`Search Engine` 居中，`LLM` 最弱。`LLM` 组对自己文章的归属感更低，对自己刚写的内容也更陌生。每轮写作结束后，访谈会请参与者凭记忆引用自己的文章：第一轮，LLM 组 18 人里有 15 人（83.3%）给不出一句正确引用，全组没有一个人引用正确（0/18）；两个对照组各只有 2 人做不到。到第三轮，LLM 组仍有 6 人给不出正确引用，而纯脑组从第二轮起就几乎全员能引用，第三轮全部做到。

第 4 轮的结果更有价值。从 `LLM` 切回纯脑写作的人，仍表现出较弱的 alpha 和 beta 连接，切过来的 9 个人里有 7 个依旧引不出自己文章里的段落；反过来，从 `Brain-only` 切到 `LLM` 的人，枕顶叶和前额叶保持更强的激活，回忆成绩也更高，激活模式更接近 `Search Engine` 组而不是 `LLM` 组。也就是说，认知债务会累积，切换条件后不会立刻消失；而一直动脑的人接上 LLM，短期能轻松不少。
...
作者综合心理学、教育理论和 AI 伦理研究，提出六个构成批判性参与的核心过程：概念诠释、推理、评价性判断、元认知调节、智识好奇心、认识论诚信。基于这套过程，论文抽出八条设计原则，对普通用户来说几条最直接。

一是主动保留认知摩擦。所谓认知摩擦，就是那些费劲但产出判断的环节：自己定标准、自己找漏洞、自己权衡取舍——别把它们彻底抹平，把 LLM 当成临时协作伙伴，而不是权威答案源。二是在流程里交替安排 AI 参与和纯人工阶段，让人至少有一段必须独立整合并复述自己的判断。三是把“先自己写结论、先自己列假设、先自己复述再看 AI”这些动作固定下来，不交给自动生成。论文还点了一条容易被忽略的：评价工作要看推理质量，别被表面流畅度带走——AI 修饰过的文字往往更“顺”，顺不等于想得清楚。
...
不过，这项研究处理的是有标准答案的阅读判断，不是日常知识工作里那种没有唯一解的决策。它为“让 AI 提供对立论证”提供了机制上的支持，但不等于“让 AI 反驳你一次，批判性思维就一定提升”。
