...
github_repo: "vas3k/TaxHacker"
source_key: "gh:vas3k/TaxHacker"
...
draft: false
categories: ["技术笔记"]
tags: ["自托管", "加密货币", "Docker", "Next.js", "PostgreSQL"]
---
...
- TaxHacker 的定位：自托管 AI 会计，面向自由职业者和独立开发者
- 核心功能：AI 文档识别、多货币、自定义分类、LLM 提示词
- Docker 一键部署和 LLM 提供商配置
- 自定义分类、字段和提示词
- 导出税务报告和数据
- 本地开发环境搭建
...
**TaxHacker**（[GitHub 仓库](https://github.com/vas3k/TaxHacker)）是一款自托管的 AI 会计应用，面向自由职业者、独立开发者（indie hackers）和小型企业，用 AI 自动化费用和收入跟踪。
...
拍摄收据照片或上传发票 PDF，TaxHacker 自动识别、提取、分类并存储到结构化数据库。
...
创建自定义字段、项目和类别，适应特定需求、行业标准或国家要求。
...
完全控制 TaxHacker AI 处理文档的方式。为字段、类别和项目编写自定义 AI 提示，或修改内置提示。
...
TaxHacker 可以适应各种需求——从文档中提取电子邮件、地址、项目代码或其他自定义信息，都能通过自定义提示词实现。

### 3.5 数据过滤和导出

处理文档后，按日期、类别、项目等过滤交易历史，导出为 CSV 或生成税务报告。
