...
四种记忆方案侧重不同：Redis 适合低延迟的会话级记忆；Mem0 的混合存储适合跨会话关联的长期记忆；cognee 的图谱结构适合实体关系密集的领域（如医疗、法律知识库）；Contextual AI 适合不想自己维护检索管线的团队。选型差异落在三个维度上：Redis 的延迟在毫秒级但表达力弱，只能做向量近似检索；Mem0 的混合存储能关联跨会话实体，但写入路径长、成本高；cognee 的图谱查询表达力最强，前提是愿意投入领域建模；Contextual AI 把检索管线托管出去，换来的是厂商依赖和定制空间收窄。

有两类问题在原型阶段看不到，等数据量上来才显现。一是把 HNSW 的 `ef_runtime` 调得太低，近似检索会过早截断邻域，召回率明显下滑，Agent 看似有记忆实则经常答非所问；二是 Mem0 跨会话关联积累到几千条后，图查询延迟可能从毫秒级涨到秒级，需要在写入侧做主动裁剪。用小数据集演示时这两类都测不出来，要按量级去预判。
...
- **想做领域专业化**：看 `fine-tuning-agents`，理解微调与 Prompt 工程的边界。前置检查：准备至少 500 条领域标注数据，确认基线模型在领域任务上的失败模式。最小可验证产物：微调后模型在 held-out 集上的准确率相对基线有可测量提升（可先以 5 个百分点为门槛，达不到先排查数据质量与训练配置）。
