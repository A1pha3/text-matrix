...
- [核心架构：前后端分离的双轨设计](#核心架构前后端分离的双轨设计)
- [画布模式：所见即所得的设计稿编辑](#画布模式所见即所得的设计稿编辑)
- [Vision LLM 生成：关键的系统 Prompt 设计](#vision-llm-生成关键的系统-prompt-设计)
- [语音模式](#语音模式)
- [双 API 兼容：OpenAI 与 Anthropic 的无缝切换](#双-api-兼容openai-与-anthropic-的无缝切换)
- [核心 API 设计](#核心-api-设计)
- [增量更新机制](#增量更新机制)
- [数据持久化与状态恢复](#数据持久化与状态恢复)
- [技术亮点](#技术亮点)
- [局限性与未来展望](#局限性与未来展望)
- [快速上手](#快速上手)
- [常见问题](#常见问题)
...
CodeVinci 的核心思路是：用 Vision 多模态大模型直接"看懂"设计稿图片，然后生成对应的 HTML。与其他工具不同，CodeVinci 提供了一个完整的本地工作环境：**左侧画布编辑 → 右侧实时预览**，还支持**语音驱动**的增量修改。
...
| 创新点 | 实现方式 | 价值 |
|--------|----------|------|
| 语音驱动的增量更新 | Deepgram 实时转写 → 自动 Render | 免去手动操作 |
...
- 添加了**学习目标**，明确阅读后的收获。
- 添加了**目录**，方便快速导航。
- 添加了**常见问题**章节，覆盖支持的 LLM、语音配置、增量更新、本地/云端、框架支持等高频疑问。
