...
自测跑完一轮后，如果发现 stop-slop 的规则对你有用，下一步可以看两个方向：一是把 `phrases.md` 里的英文短语表替换成你常写的中文套话，自己做一张中文黑名单；二是把 5 维评分表抽出来，作为你所有技术文章的定稿前检查项——即使不用 stop-slop 的禁词表，这五个维度本身也是很好的编辑框架。
