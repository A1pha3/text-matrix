...
- 理解 Gateway 在架构中的角色——它不只是代理转发，而是会话、工具和权限的控制平面
...
理解了这三条，再去看 OpenClaw 的架构模块会清楚很多。下面先给一张总览图，再逐个拆开。

---

## 2. 架构总览：Gateway 是控制平面

OpenClaw 的运行时分为两个层面——**不是主从关系，而是控制面与数据面的关系**：
...
简单说：**Agent 决定「用哪个模型、给多少权限」，Session 承载「当前在说什么」，Skill 提供「能做什么」**。
...
这个设计解决了一个实际痛点：你可能希望工作相关的渠道用能力强但更贵的模型，而个人闲聊渠道用轻量模型。

---

## 5. 一条消息的完整旅程

理解架构最有效的方式是追踪一条真实消息从头到尾怎么走。以 Telegram 为例：
...
- **工具调用不是「先执行再检查」**，而是在 Agent 配置阶段就决定了权限边界——主会话有权的一切工具，对非主会话直接不可见。
...
Skills 不是插件二进制包，而是**被 AI 读取的指令文件**。SKILL.md 全文会注入到 Agent 的 System Prompt 中，模型直接根据 Markdown 里的描述决定是否激活和如何调用。这个设计与 Anthropic 的 Claude Code 的 `CLAUDE.md`、Cursor 的 `.cursorrules` 同属一个思路：让 AI 自己读说明书，而不是通过 SDK 注册回调。
...
OpenClaw 的工具权限不是全局开关，而是按 Agent 类型分层：
...
OpenClaw 不是 ChatGPT 或 Claude 的替代品——它在模型能力上依赖外部 API，不提供内置模型或免费 token 额度。它的核心价值在于**渠道整合 + 本地控制 + 工具链扩展**。
...
如果你决定试 OpenClaw，建议按以下顺序推进，避免一次性配置 20 个渠道后陷入排查地狱：
