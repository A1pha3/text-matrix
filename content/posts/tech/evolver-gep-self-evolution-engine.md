...
> **目标读者**：AI Agent 开发者、Prompt 工程师、对 AI 自我进化感兴趣的研究者
> **前置知识**：JavaScript/Node.js 基础、对 AI Agent 概念有了解
...
1. **理解 GEP 基因组进化协议**：为何需要结构化的进化资产
2. **掌握 Evolver 的核心架构**：Gene/Capsule/Event 三元素体系
3. **理解四大 Strategy Presets**：balanced/innovate/harden/repair-only
4. **掌握 Evolver 的部署方式**：独立运行/接入 OpenClaw/接入 EvoMap Hub
5. **能够构建自进化 Agent**：使用 Evolver 实现 Agent 的自我修复与优化
6. **理解安全模型**：Evolver 的验证命令白名单机制

---

## 目录

- [背景与动机](#§2-背景与动机为何-ai-需要自进化)：当前困境、传统局限、GEP 解决思路
- [§4 核心概念：GEP 三元素体系](#§4-核心概念gep-三元素体系)：Gene、Capsule、EvolutionEvent
- [§5 Evolver 工作原理](#§5-evolver-工作原理)：整体流程、Signal 提取、Gene Selection、Solidify 验证
- [§6 Strategy Presets：进化策略配置](#§6-strategy-presets进化策略配置)：四大策略、动态调整
- [§7 部署模式](#§7-部署模式)：独立运行、接入 OpenClaw、生命周期管理、Cron 定时任务
- [§8 EvoMap Hub：网络协作](#§8-evomap-hub网络协作)：Hub 连接配置、功能、Worker Pool 模式
- [§9 安全模型](#§9-安全模型)：命令白名单、验证命令安全矩阵、A2A 资产摄入安全
- [§10 与 OpenClaw 集成](#§10-与-openclaw-集成)：集成架构、本地覆盖机制
- [§11 完整使用示例](#§11-完整使用示例)：修复重复错误、系统加固、紧急修复
- [§12 FAQ](#§12-faq)
- [§13 自测题](#自测题)
- [§14 进阶路径](#进阶路径)
...
| 问题 | 描述 | 影响 |
|------|------|------|
| **Prompt 漂移** | 多次迭代后 Prompt 质量下降 | Agent 行为不稳定 |
| **错误重复** | 同样的错误反复发生 | 效率低下 |
| **缺乏记忆** | 无法从历史失败中学习 | 经验无法积累 |
| **人工维护** | 依赖人工调整 Prompt | 成本高、响应慢 |
...
当 Evolver 运行在 OpenClaw 中时，`sessions_spawn(...)`输出会被 OpenClaw 解释执行：
...
外部 Gene/Capsule 通过`scripts/a2a_ingest.js`摄入时：
- 先存入隔离的候选区（candidate zone）
- 提升到本地存储需要`--validated`标志
- Gene 的验证命令会经过同样的安全检查
...
### 10.1 场景：修复 Agent 的重复错误

**问题**：Agent 反复在 JSON 解析时失败
...
**Q1**：GEP 三元素（Gene、Capsule、Event）各自的作用是什么？
<details>
<summary>参考答案</summary>
Gene 是最小的可复用进化单元（单个变异）；Capsule 是多步进化的组合单元（多个 Gene 组合）；Event 是每一次进化变更的审计日志（追踪原因和结果）。
</details>

**Q2**：Evolver 的四大 Strategy Presets 有什么区别？
<details>
<summary>参考答案</summary>
`balanced`（平衡模式，日常运行）、`innovate`（创新模式，最大化新功能）、`harden`（加固模式，聚焦稳定性）、`repair-only`（仅修复模式，紧急修复）。失败率高时会自适应切换到 `repair-only`。
</details>

**Q3**：Solidify 验证阶段的命令白名单机制是什么？
<details>
<summary>参考答案</summary>
只允许 `node`、`npm`、`npx` 开头的命令；禁止命令替换（`$(...)`）和 shell 操作符（`;`、`&&`、`||`、`>`、`<`）；禁止 `node -e` 参数。这防止了恶意 Gene 执行危险操作。
</details>

**Q4**：Evolver 如何接入 OpenClaw？
<details>
<summary>参考答案</summary>
Evolver 扫描 OpenClaw 的 `memory/` 目录获取 Agent 运行时日志，生成 GEP Prompt 后输出 `sessions_spawn(...)` 指令，OpenClaw 解释执行这些指令完成进化。
</details>

**Q5**：EvoMap Hub 的 Worker Pool 模式如何开启？
<details>
<summary>参考答案</summary>
需要两个条件同时满足：1. 本地配置 `WORKER_ENABLED=1`；2. 在 evomap.ai 网站上打开 Worker 开关。两者都开启后才能接收网络任务。
</details>
...
### 阶段一：理解概念（1-2 天）
- 阅读 GEP 协议文档
- 理解 Gene/Capsule/Event 三元素体系
- 运行 Evolver 示例，观察进化过程

### 阶段二：本地部署（3-7 天）
- 安装 Evolver 并接入 OpenClaw
- 编写自定义 Gene 并验证
- 配置 Strategy Presets 适配你的场景

### 阶段三：生产使用（1-2 周）
- 接入真实 Agent 运行时日志
- 观察进化效果，调整 Gene Selection 算法
- 配置 EvoMap Hub（可选）

### 阶段四：贡献生态（持续）
- 将有效的 Gene/Capsule 发布到 EvoMap Hub 技能商店
- 参与 Evolution Circle 协作进化
- 提交 PR 改进 Evolver 核心
...
**Q1：Evolver 会自动修改我的代码吗？**
A：不会。Evolver 只生成协议约束的 Prompt 和资产，不直接修改源代码。验证通过后才由宿主环境决定是否应用。

**Q2：需要连接 EvoMap Hub 吗？**
A：不需要。核心进化功能完全离线可用。Hub 连接只是用于技能商店和 Worker Pool 网络功能。

**Q3：Evolver 安全吗？**
A：安全。验证命令有严格的白名单限制（仅 node/npm/npx），禁止命令替换和 shell 操作符。

**Q4：可以自定义 Gene 吗？**
A：可以。在`assets/gep/genes.json`中添加你自己的 Gene 即可。

**Q5：支持哪些编程语言的 Agent？**
A：Evolver 是语言无关的。它通过扫描日志生成 Prompt，不依赖特定编程语言。
