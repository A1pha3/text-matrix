...
Babel 的创新点在 EDA 工具之外：把 AI coding agent（Claude Code）引入芯片设计流程，让 agent 以 issue handoff 的方式协作，每个 agent 专注特定设计阶段，用 labeled issue 做状态机驱动。这说到底是把"设计师 → RTL → 验证 → 综合 → PD"的人工流程 AI 化。
