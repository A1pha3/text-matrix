...
Go 1.26 是 1.22 以来改动面最宽的一个版本。语言层补了两处长期被吐槽的语法；运行时把实验了一年的 Green Tea GC 转正，并把 cgo 基线开销砍掉 30%；标准库放出三个新包（一个进入标准的 `crypto/hpke`，两个需要 `GOEXPERIMENT` 开启的实验包）；工具链把 `go fix` 从「打补丁工具」改写成基于 `go vet` 同一套分析框架的 modernizer。如果你只关心升级收益，下面的表格能直接给出答案。
...
> **姊妹篇**：[Go 1.27 前瞻]({{< relref "golang-1-27-preview-and-roadmap.md" >}}) — 泛型方法、字面量类型推导、`crypto/tls` GODEBUG 清理、macOS 13 起跳
...
1. 写出 1.26 的两处语言层新语法，并解释它们各自解决的工程痛点。
2. 描述 Green Tea GC 的核心改动（页面级标记 + AVX-512 扫描）以及它对 P99 延迟的传导路径与边界。
3. 解释 cgo 基线开销的来源、为什么降 30% 在「频繁调用」场景才显著。
4. 在自己的代码里启用 `errors.AsType` 和 `new(expr)`，并理解为什么 `go fix` 现在能跨文件批量迁移。
5. 判断一个 Go 服务应不应该立刻升 1.26，以及升之前需要做的几项验证。
...
| 层级 | 改动 | 一行收益 | 必读章节 |
|------|------|----------|----------|
| **语言** | `new(expr)` 接受表达式 | 可选字段指针一行写完 | [二、1](#21-newexpr可选字段指针一行写完) |
| **语言** | 泛型约束允许自引用 | `Adder[A Adder[A]]` 这类递归约束合法 | [二、2](#22-泛型约束允许自引用) |
| **运行时** | Green Tea GC 默认启用 | GC CPU 开销降 10–40%；新 CPU 上再叠 ~10% | [三、1](#31-green-tea-gc-默认启用) |
| **运行时** | cgo 基线开销降 ~30% | 高频 cgo 调用的延迟和吞吐改善 | [三、2](#32-cgo-调用基线开销降-30) |
| **运行时** | 64 位堆基址随机化 | cgo 场景下更难被利用内存地址漏洞 | [三、3](#33-64-位堆基址随机化) |
| **运行时** | 实验性 goroutine 泄漏 profile | 可发现「永远无法唤醒」的 goroutine | [三、4](#34-实验性-goroutine-泄漏-profile) |
| **编译器** | 切片 backing array 栈分配增多 | 短生命周期切片不再走 GC | [六](#六编译器切片更多走栈) |
| **工具链** | `go fix` 改写为 modernizer | 自动升级旧 API 与新语言特性 | [四、1](#41-go-fix-改写为-modernizer) |
| **标准库** | `crypto/hpke` | RFC 9180 混合公钥加密，含后量子 KEM | [五、1](#51-cryptohpke) |
| **标准库** | 实验性 `simd/archsimd` | amd64 上 128/256/512-bit SIMD | [五、2](#52-实验性-simdarchsimd) |
| **标准库** | 实验性 `runtime/secret` | 擦除寄存器/栈/堆上的敏感残留 | [五、3](#53-实验性-runtimesecret) |
| **标准库** | `errors.AsType[E]` | 类型安全的错误检查 | [五、4](#54-errorsastypet类型安全的错误检查) |

### 按读者角色的阅读路径

不同读者关心的部分差别很大，建议直接跳到对应章节：

- **普通 Go 开发者**：先看 [二、语言层](#二语言层) 和 [五、4 `errors.AsType`](#54-errorsastypet类型安全的错误检查)。这两处对日常代码影响最直接。
- **后端 / SRE**：直奔 [三、运行时](#三运行时green-tea-gc-转正cgo-加速泄漏检测) 和 [八、升级建议](#八升级建议)。Green Tea GC 和 cgo 加速是这次版本里最影响线上指标的两项。
- **AI Infra 工程师**：重点看 [七、Go 在 AI 时代的生态位](#七go-在-ai-时代的生态位) 的「请求流过系统」案例，然后看 [三、1](#31-green-tea-gc-默认启用) 和 [五、2 `simd/archsimd`](#52-实验性-simdarchsimd)——RAG、向量检索、推理服务都对延迟和吞吐敏感。
- **平台 / 库作者**：读 [四、`go fix` 改写为 modernizer](#41-go-fix-改写为-modernizer) 和 [五、3 `runtime/secret`](#53-实验性-runtimesecret)。这两个直接决定你能不能用 1.26 的能力给下游做迁移和安全加固。

---

## 一、为什么 Go 今天又上 trending

GitHub 上 [golang/go](https://github.com/golang/go) 是 134,156 颗星（2026-06-07 数据）的常青树，最近一次进 GitHub Trending 是 6 月 7 日，叠加 [nginx](https://github.com/nginx/nginx) 罕见同期上榜，被中文社区读作「基础设施巨头集体被关注」。把这个信号和语言本身的变化分开看，原因有三：
...
但 trending 只是信号。这一版本本身改了什么，对你下个迭代的工作更有用。
...
这个改动对 `encoding/json` 和 protocol buffers 的 optional 字段场景最直接：
...
- 由于 `new` 内部会复制表达式的结果到新分配的内存，对大结构体做 `new(largeStruct)` 不会比 `&largeStruct{}` 更省。
...
1.26 解除这个限制。规范上，类型参数约束里允许出现被约束的类型本身。语义上没有变复杂——过去需要一条专门的「禁止自引用」规则，现在这条规则去掉了，规范反而更干净。实际效果：
...
这种模式在表达「和自己同类型做运算、返回自身类型」的接口时很自然。可比较树（`Node[T Node[T]]`）、状态机迁移、单调/偏序集合（`Less` 接口）等都受益。但要注意：
...
旧 GC 的标记阶段以单个对象为单位，工作队列里放的是对象指针；扫描时频繁在离散的内存地址之间跳转，CPU 缓存命中率低，官方统计约 35% 的 CPU 周期浪费在内存停顿等待上。
...
在 Intel Ice Lake / AMD Zen 4 及更新的 CPU 上，Green Tea 进一步用 `VGF2P8AFFINEQB`（Galois field 指令）配合 AVX-512，把 span 位图操作放在 512-bit 寄存器里一次处理 64 字节——这是 span 内部对象粒度为 8 字节时的整列扫描。官方用「在市里走走停停 vs 在高速上长距离匀速行驶」比喻这个改动。

#### 性能数字的读法

官方给的区间是「GC CPU 开销降 10–40%」，「10%」是中位数，「40%」是小对象密集工作负载的上限。第三方的代表性数字：

- 地理空间数据库 Tile38：GC 开销降 35%
- 时序数据库 InfluxDB：降 5%
- 内部小对象密集 RPC：18% → 11%（CPU 占用比）

需要注意的是：所有这些数字测的都是「GC 占用 CPU 时间占总 CPU 时间的比例」。这个比例下降通常会传导到 P99 延迟改善，但传导幅度取决于 GC 在请求延迟构成里的占比——如果服务瓶颈在网络 I/O、数据库、外部 API，体感会非常有限。

#### 回退与未来

如需回退旧 GC，构建时设置 `GOEXPERIMENT=nogreenteagc`。官方计划在 1.27 删除该开关，所以这只是过渡期选项，不是长期方案。详见姊妹篇 [4.2 移除 `nogreenteagc` 开关]({{< relref "golang-1-27-preview-and-roadmap.md#42-移除-nogreenteagc-开关" >}})。

### 3.2 cgo 调用基线开销降 ~30%

Go 调 C 函数的固定开销——栈切换、寄存器保存恢复、胶水代码——降低了约 30%。这 30% 来自两部分优化：

- 更轻的 cgo entry 路径：减少进入 C 时的 prologue 指令数
- 更高效的 `GoString` / `GoSlice` 等胶水数据结构的生成方式

对偶尔调一次 C 的服务，体感不大。如果你做的是以下场景，升级即可见改善：
...
如果阻塞原语通过全局变量或可运行 goroutine 的局部变量可达，runtime 无法判定为泄漏。这是理论上不可避免的误报，工具不能完全替代人工 review。标记为「实验性」主要因为 API 形式（独立 profile 类型还是嵌入 goroutine profile）还在收集反馈。官方计划在 1.27 默认启用，详见姊妹篇 [4.1 `goroutineleak` profile 默认启用]({{< relref "golang-1-27-preview-and-roadmap.md#41-goroutineleak-profile-默认启用" >}})。

实现本身是生产可用的，理论依据见 [Vlad Saioc 等人的论文](https://dl.acm.org/doi/pdf/10.1145/3676641.3715990)。

---

## 四、工具链：`go fix` 改写为现代助手

### 4.1 `go fix` 改写为 modernizer

旧版 `go fix` 只做历史 API 迁移（比如 `golang.org/x/net/context` → `context`），所有 fixer 在 1.25 之前就已经过时。1.26 基于 `go vet` 同一套 [`golang.org/x/tools/go/analysis`](https://pkg.go.dev/golang.org/x/tools/go/analysis) 静态分析框架重写了 `go fix`：

- 内置数十个 modernizer，覆盖 `new(expr)`、`errors.AsType`、新的 `slices` / `maps` 用法、loopvar 语义修正等
- 新增 inline 分析器，配合 `//go:fix inline` 指令可以对标记的函数做源级内联，用于团队内部 API 迁移
...
1.26 的实现支持 5 个 KEM，覆盖传统椭圆曲线和后量子：

| KEM | 类型 |
|-----|------|
| `DHKEM(P-256, HKDF-SHA256)` | 传统 |
| `DHKEM(P-384, HKDF-SHA384)` | 传统 |
| `DHKEM(P-521, HKDF-SHA512)` | 传统 |
| `DHKEM(X25519, HKDF-SHA256)` | 传统（X25519 高性能常用） |
| `ML-KEM-768` | 后量子 |
| `MLKEM768X25519` | **混合**（X25519 + ML-KEM-768，1.24 起 TLS 默认） |
| `MLKEM768P256` | 混合 |
| `MLKEM1024P384` | 混合（更高安全等级） |
...
### 5.2 实验性 `simd/archsimd`

通过 `GOEXPERIMENT=simd` 开启。当前仅支持 amd64（Intel/AMD），提供 128/256/512-bit 向量类型与对应操作：
...
在 amd64 上跑 `SumVec` 通常比纯 Go 标量循环快 5–20 倍，取决于 CPU 和切片长度。Go 1.26 RC2 的汇编显示，编译器把这段翻译成 `VEXTRACTI128` / `VPADDQ` / `VPEXTRQ` 序列，干净地落在 AVX2/AVX-512 上。
...
### 5.3 实验性 `runtime/secret`

通过 `GOEXPERIMENT=runtimesecret` 开启。当前支持 linux/amd64 和 linux/arm64，其它平台 `secret.Do` 直接调用 f。它擦除寄存器、栈内存和堆分配中的敏感数据残留，面向密钥处理、加密中间态等短生命周期敏感数据，目标是帮助实现前向保密（Forward Secrecy）。
...
函数签名是 `func AsType[E error](err error) (E, bool)`，类型参数 `E` 约束为 `error` 接口。`E` 可以是接口（如 `io.Reader`）、具体类型（`errors.AsType[io.EOF]`），或 `*T`（如 `*json.SyntaxError`）。
...
这条链路里，Go 不算模型，而是把模型调用编排成可靠的生产系统。每个 hop 的延迟、吞吐、错误率都能用 `runtime/metrics` 和 `pprof` 拿到——而 Green Tea GC 1.26 的 10–40% 开销下降，最直接的受益方就是 [2]、[3]、[5] 这类长跑 goroutine 密集的中间层。

另一个值得注意的方向是 [五、2 `simd/archsimd`](#52-实验性-simdarchsimd)。当 Go 终于能写 SIMD 循环，向量检索的 Go 实现、JSON/Protobuf 解码热路径、字符串处理都会逐步出现「不再走 cgo」的纯 Go 高性能库——这对需要单二进制部署的 AI Infra 工具是个明显加成。
...
1. **机制层**：
   ...
   - [`crypto/hpke` RFC 9180 原文](https://datatracker.ietf.org/doc/html/rfc9180)
...
2. **实验层**：
   ...
   - [`goroutineleak` profile 提案 #74609](https://go.dev/issue/74609)
3. **生产层**：
