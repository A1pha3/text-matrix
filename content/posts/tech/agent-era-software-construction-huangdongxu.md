...
进入正文前，先明确 3 个边界，否则很容易把这篇文章读成"行业定论"：
...
过去软件工程的重心是"怎样把代码写出来"，Agent 时代重心移到了"怎样把目标说对、材料给对、缰绳拉对"。
...
这张表把"为什么 Agent 表现不好"从模糊抱怨变成了可诊断问题：Goal 不清，系统会勤奋地做错事；Context 不够，系统会高置信地瞎补；Constraints 太弱，系统会把局部最优写成整体事故。
...
黄东旭提到的 Memory、RAG、Harness Engineering，本质上都在解决"如何让 Agent 拿到正确的前提"。Context 是最容易被误做成"堆料工程"的方向——把所有相关文档塞进去，反而会稀释信号。

### 6.4 Constraints：决定能不能上线的维度

Goal 决定方向，Context 决定理解质量，Constraints 决定系统有没有资格接近生产环境。
...
他提到 TiDB Cloud 的新产品线 Zero，强调的是一种 Agent-first 的思路：如果未来发起请求的主体越来越多是 Agent，而不是人，那么围绕"人类登录流程"设计的很多默认前提都会变成摩擦。
...
未来被重新设计的不只是软件界面本身，还包括任务如何被组织、能力如何被调动、责任如何被落实。
