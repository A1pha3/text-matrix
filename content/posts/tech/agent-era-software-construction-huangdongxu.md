...
> **定位**：本文把黄东旭在高强度 Agent 实战后的观点，整理成一篇更适合工程师阅读、评估与试运行的方法论笔记，不涉及"AI 会不会替代程序员"的泛泛讨论。
...
1. “Coding 已死”的含义：代码正在从“人类主要思考媒介”退到“AI 负责生产的执行载体”，编程知识本身并没有失效。
2. Agent 时代最稀缺的能力，变成了把 Goal、Context、Constraints 讲清楚，写代码只是其中一环。
3. 多 Agent 协作不自动提升质量；缺少高质量 Spec 和验收标准，系统只会更快跑偏。
4. 软件形态会从固定界面走向意图驱动。UI 不会消失，但会退居辅助层。
...
1. “代码从思考载体变成执行载体”意味着什么。
2. Goal / Context / Constraints 三维分析框架，以及怎么映射到真实工程场景。
3. Agent 时代软件形态为什么会从静态界面转向“意图驱动”。
4. 人机协同的正确姿势：人更像架构师加审稿人，不是甩手掌柜。
5. 哪些观点是经验结论，哪些只是趋势判断。
6. 一套能在团队中试运行的实践清单。
7. 未来 5 年程序员能力结构会怎样迁移。
8. AI 软件产业链可能如何重组，以及为什么治理层比页面层更重要。
...
黄东旭是 TiDB 联合创始人兼 CTO，工程师出身，长期参与一线技术工作。他这轮分享之所以值得注意，它同时具备下面 3 个特征：
...
“复杂工程只能靠人工一点点啃”的旧前提开始松动，不代表所有复杂工程都已经被解决。

### 5.3 人类工作的重心因此转移

在这种框架下，人类的主要职责转向下面几件事：
...
黄东旭提出的 Goal / Context / Constraints，它能把很多零散讨论重新归位。
...
这张表把“为什么 Agent 表现不好”从模糊抱怨变成了可诊断问题：
...
真正有价值的 Context，通常具有 3 个特征：

1. 与当前任务强相关，而不是背景越多越好。
2. 能解释历史选择，而不是只给现状代码。
3. 能帮助模型做取舍，而不是只堆素材。

黄东旭提到的 Memory、RAG、Harness Engineering、本质都在解决“如何让 Agent 拿到正确的前提”。Context 是最容易被误做成“堆料工程”的方向。
...
但实践结果并不理想。这说明一个现实问题：**长程复杂任务不是把智能体数量拉满就能解决，人仍然需要在关键节点上校准方向。**
...
Harness 更像放大器，Spec 才像定盘星。

### 7.4 “大力出奇迹”到底该怎么理解

“大力出奇迹”很容易被读成粗暴堆 Token，更合理的理解：**在复杂问题上，不要过早抠搜探索成本，应该用足够多的试错、重写、比较和评审去换跃迁式提升。**
...
黄东旭强调，在成千上万步的长程任务里，模型能力哪怕只差一点，最终也会被级联放大。它提醒你：
...
这里需要加一个边界：意图驱动不意味着界面消失，界面从“唯一入口”变成“多入口之一”。
...
它揭示的方向：**越来越多基础设施会朝“随调随走、弱人工配置、适合机器调用”的形态演进。**

### 9.3 “Everything is Skill” 提醒了什么

“把一段 Skill 贴进对话框，它就自己跑起来”这类说法，本质上是在描述一种新的交付体验：不用先教人点哪里，而是先让系统知道该做什么。

软件分发、上手和集成的最小单位都可能变化。过去的最小单位可能是安装包、账号和配置页面；未来的最小单位可能更接近一段可执行的任务说明、工作流模板或可组合能力包。
...
黄东旭认为 Skill 很重要，但也很粗糙。Skill 的优点和缺点来自同一个地方：它足够轻，所以传播快；但也因为太轻，所以难以承载复杂系统。
...
未来很多软件产品的核心竞争力，可能是“把专家经验编译成可调用系统”的能力。
...
Agent 时代并没有消灭软件工程，反而把以下能力推到了更前台：
...
真正会被淘汰的是把工程能力误以为只是“手写代码速度”的理解方式，工程能力本身不会。
...
Coding 本身不会消失，变化的是它在工程工作里的位置会下降。代码当然还在，但那些低杠杆、可模板化、可局部优化的实现劳动，未来大概率不会像今天这样值钱。
...
文章前面提到，中间层 SaaS 会被挤压。被压缩的未必是所有中间层，更可能是那些主要靠页面包装提供价值的中间层；相反，和治理、控制、审计相关的中间层，重要性可能会上升。
...
一个产品的竞争力，看的是“能不能把结果闭环做出来”，“功能多不多”反倒是其次。
...
组织被编程，目标、规则、权限、证据链和执行流程可以被结构化表达，但人类仍然承担裁决、问责和价值判断。Agent 会放大组织能力，也会放大组织错误。
