...
# Pi Subagents：Pi AI助手的异步子Agent委托框架完全指南

在日常 AI 辅助编程中，我们经常遇到这样的场景：正在做一个功能，需要有人同时做代码审查、做技术调研、或者实现另一个相关模块。传统的做法是开多个对话窗口、手动复制粘贴上下文——繁琐且容易出错。**Pi Subagents** 提供了另一种可能：让 Pi  Agent 直接在会话内Spawn 子 Agent，分工协作，结果自动汇入主会话。
...
Pi Subagents 提供了一种将多角色协作自然化地融入单一对话界面的方案。它的核心价值不是功能本身（并行审核、代码审查等工作流在很多工具链中都有），而是**将"委托"这件事的认知负担降到最低**——不需要配置、不需要定义 Agent 模板，只需要用你想做事的方式说出来，Pi 会自己完成路由和执行。这种"意图驱动 + 自动委托"的交互范式，是 AI 原生开发工具的一个重要方向。
