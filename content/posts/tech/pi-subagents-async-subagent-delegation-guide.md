---
title: "Pi Subagents：Pi AI助手的异步子Agent委托框架完全指南"
...
# Pi Subagents：Pi AI 助手的异步子 Agent 委托框架完全指南

在日常 AI 辅助编程中，我们经常遇到这样的场景：正在做一个功能，需要有人同时做代码审查、做技术调研、或者实现另一个相关模块。传统的做法是开多个对话窗口、手动复制粘贴上下文——繁琐且容易出错。**Pi Subagents** 提供了另一种可能：让 Pi  Agent 直接在会话内 Spawn 子 Agent，分工协作，结果自动汇入主会话。
...
Pi Subagents 的设计思路值得 AI 工具链开发者关注：它将"委托"这件事做到了极致——**不需要写配置、不需要定义 Agent，只需要用自然语言描述你想做什么**。系统会自动解析意图、选择合适的子 Agent 类型、管理并行或串行执行、并最终将结果返回主会话。这种"意图驱动 + 自动路由"的模式，是 AI 原生工作流的一种有价值的探索方向。
...
Pi Subagents 提供了一种将多角色协作自然化地融入单一对话界面的方案。它的关键价值不是功能本身（并行审核、代码审查等工作流在很多工具链中都有），而是**将"委托"这件事的认知负担降到最低**——不需要配置、不需要定义 Agent 模板，只需要用你想做事的方式说出来，Pi 会自己完成路由和执行。这种"意图驱动 + 自动委托"的交互范式，是 AI 原生开发工具的一个重要方向。
