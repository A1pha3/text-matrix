...
1. 理解 ArcKit 的核心定位和技术边界
2. 说清它的四层架构（文档统一层、流程标准化层、可追溯性层、AI 辅助层）
3. 判断 ArcKit 是否适合你的项目和团队
4. 完成首次安装和基本配置
5. 理解 Phase 0-7 的每个阶段的输入、输出和 Gate
6. 把 ArcKit 引入团队的架构治理流程

## 目录

- [一句话判断](#一句话判断)
- [总览地图](#总览地图)
- [为什么需要 ArcKit](#为什么需要-arkit)
- [三层核心机制](#三层核心机制)
- [任务流案例](#任务流案例)
- [UK 政府合规套件](#uk-政府合规套件)
...
### ArcKit 和 TOGAF 有什么关系？

ArcKit 不是 TOGAF 的替代品，而是把 TOGAF 的部分方法论（特别是架构治理和文档管理）用 AI 辅助和 Git 工作流重新实现。TOGAF 提供完整的企业架构方法论，ArcKit 提供执行工具。如果团队已经用 TOGAF，ArcKit 可以加速文档生成和治理执行；如果团队没有架构框架，ArcKit 可以作为一个轻量级起点。

### Phase 4 为什么是硬 Gate？

Phase 4（SOBC 业务论证）是硬 Gate，因为它决定项目是否值得继续投入。很多项目失败是因为在没拿到预算和战略对齐前就投入需求细化，结果做了一半发现不值得做。Phase 4 把这个论证从"事后补文档"前移到"事前挡板"，避免浪费。

### 非 UK 项目能用 ArcKit 吗？

能，但需要裁剪。ArcKit 的 UK 政府合规命令（`/arckit-tcop`、`/arckit-ai-playbook`、`/arckit-mod-secure`）对非 UK 项目价值不大，可以跳过。但它的通用命令（`/arckit-principles`、`/arckit-stakeholders`、`/arckit-risk`、`/arckit-sobc`、`/arckit-requirements`）对任何受监管行业都有价值。

### `[DOC-CN]` 引用标记会不会增加文档维护负担？

会，但 `stale-artifact-scan` 钩子会自动扫描过期引用，在每次 Claude Code 会话启动时提醒。相比 Jira 链接（人工维护、容易腐化），`[DOC-CN]` 标记嵌在文档正文里，由工具自动检查，维护负担反而更小。

## 自测题

读完后，尝试回答这些问题：

1. ArcKit 的四层架构（文档统一层、流程标准化层、可追溯性层、AI 辅助层）各解决什么问题？
2. Phase 4 为什么是硬 Gate？它产出什么？由谁审批？
3. Wardley Mapping 在平台设计里起什么作用？它和 SWOT 分析有什么区别？
4. 为什么 ArcKit 把文档收进 Git 仓库？相比 Confluence 有什么优势？
5. 如果你的项目不在 UK 公共部门，你会保留哪些 ArcKit 命令？跳过哪些命令？

## 进阶路径

如果你准备在生产环境使用 ArcKit，建议按下面顺序推进：
...
4. **裁剪合规命令** - 非 UK 项目可以跳过 UK 专属命令，保留通用命令。

5. **建立团队规范** - 把 workflow 当仓库资产来维护，和 CI、lint、脚本一样进入版本控制。workflow 变更应该走 code review。
