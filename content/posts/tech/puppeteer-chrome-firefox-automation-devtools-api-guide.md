...
- **locator API**：把"找元素"和"等元素可操作"绑定到一起，比经典的 `page.$` 更稳定。
- **AI Agent 集成**：通过 `chrome-devtools-mcp` 让 AI 助手直接操作浏览器，是 Chrome DevTools 团队的官方路径。

如果你只需要记住一件事：**Puppeteer 不是测试框架，而是挂到任何测试、抓取、Agent 栈里的浏览器驱动原语。**

## §1 本文覆盖范围

本文会讲清楚以下 5 件事：

1. **安装时的真实坑点**：现代包管理器默认拦截 postinstall 脚本，浏览器二进制不会自动下载。
2. **双包机制的设计动机**：为什么要把"浏览器管理"和"API 客户端"拆成两个包。
3. **locator API 为什么稳**：它把选择器从字符串变成"按属性等待 + 重试"的对象。
4. **协议抽象的工程意义**：Puppeteer 如何把 CDP 和 BiDi 的差异吃进 `Connection` 层，让上层业务代码不用改。
5. **AI Agent 集成的两条路径**：`chrome-devtools-mcp`（Agent 操作浏览器）和 WebMCP（页面暴露能力给 Agent）。

本文**不会**展开：

- Puppeteer 的完整 API 列表（请查 [pptr.dev](https://pptr.dev/)）
- Playwright / Selenium 的详细对比（只给选型建议）
- CDP / BiDi 协议的完整规范（只讲 Puppeteer 如何抽象它们）
...
## §6 元素定位：locator API 与无障碍选择器

### 6.1 经典 selector 的脆弱性
...
直接装 `chrome-devtools-mcp`（见 §8.1），让 MCP 客户端列出它的工具集。不需要再写一层 Puppeteer 适配——MCP 服务器本身就是 Puppeteer 的封装。

## §11 学习练习

### 练习 1：跑通最小示例（Level 1）

**任务**：把 §4.3 的最小示例跑起来，成功打印博客标题。

**验证标准**：
- 能成功启动浏览器
- 能导航到 `https://developer.chrome.com/`
- 能打印出博客标题

**常见坑**：
- Chromium 没下载：按 §4.2 修复
- 网络超时：检查是否需要代理
- locator 找不到元素：网站结构可能变了，按 F12 检查选择器

### 练习 2：用 locator 重写旧代码（Level 2）

**任务**：找一个你现有的 Puppeteer 脚本，把所有的 `page.$` / `page.click` 改成 `page.locator`。

**验证标准**：
- 新脚本能跑通原有逻辑
- 稳定性有明显提升（不再需要手写 `waitForSelector`）
- 代码可读性更高

**提示**：优先改那些经常因为"元素还没出现"而失败的脚本。

### 练习 3：接入 chrome-devtools-mcp（Level 3）
...
**任务**：在 Docker 里部署一个使用 Puppeteer 的服务。
...
### 基础概念
- [ ] 我能解释 `puppeteer` 与 `puppeteer-core` 的区别
- [ ] 我知道什么时候该用哪个包
- [ ] 我能解释为什么现代包管理器会拦截 postinstall 脚本

### 元素定位
- [ ] 我能解释 locator API 比经典 selector 稳在哪里
- [ ] 我会用 `::-p-aria`、`::-p-text`、`::-p-xpath` 三种伪类
- [ ] 我知道什么时候该用 locator，什么时候可以用 `page.$`

### 协议抽象
- [ ] 我能解释 CDP 和 BiDi 的区别
- [ ] 我知道 Puppeteer 如何抽象这两套协议
- [ ] 我知道为什么 `launch({ browser: 'firefox' })` 不需要改业务代码

### AI Agent 集成
- [ ] 我知道 chrome-devtools-mcp 是什么
- [ ] 我知道 WebMCP 是什么
- [ ] 我能解释这两条路径的区别

### 选型决策
- [ ] 我知道什么时候该选 Puppeteer
- [ ] 我知道什么时候该让位给 Playwright / Selenium
- [ ] 我能给新项目推荐合适的浏览器自动化方案
...
**必备技能**：
- [ ] 掌握 Puppeteer 核心 API（Page、Browser、Locator）
- [ ] 能够写出稳定的自动化脚本
- [ ] 能够排查常见问题（元素找不到、超时、协议错误）
- [ ] 能够在 CI / Docker 里部署

**推荐资源**：
- 本文（多读几遍）
- [pptr.dev 官方文档](https://pptr.dev/)
...
**推荐资源**：
- CDP 协议文档（[chromedevtools.github.io](https://chromedevtools.github.io/devtools-protocol/)）
- WebDriver BiDi 规范
- Puppeteer 源码（重点看 `cdp/` 和 `bidi/` 模块）
...
- [ ] 深刻理解浏览器自动化本质和演进趋势
- [ ] 能够制定团队的最佳实践
- [ ] 能够解决跨领域复杂问题（反爬、性能、稳定性）
- [ ] 能够贡献到 Puppeteer 社区

**推荐资源**：
- 浏览器内核源码（Chromium / Firefox）
- 行业会议和技术博客
- 参与 Puppeteer 社区（提 Issue、提 PR）
