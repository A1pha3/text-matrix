...
>
> **一句话定位**：让 ML 模型在不同平台和硬件上跑出比框架原生推理更好的性能。

## 项目概览

ONNX Runtime 是微软开源的跨平台机器学习推理和训练加速器。它将 ONNX（Open Neural Network Exchange）格式的模型部署到各种平台和设备上，通过硬件加速后端和图优化降低推理延迟。
...
ONNX Runtime 是一个跨平台 ML 推理加速器，通过硬件加速和图优化技术，帮助开发者将 ML 模型部署到各种平台上。在云端服务器、边缘设备和移动端都能用。
