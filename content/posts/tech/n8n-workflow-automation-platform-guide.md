...
source_key: "gh:en/authentication"
...
n8n 采用 fair-code 许可模式——源码可见、可自托管运行，但商业用途需要购买许可证。你可以完全掌控自己的数据和工作流，同时项目有可持续的商业模式支撑。
...
n8n 的执行模型比较特殊，支持两种数据获取模式，不是简单的"触发-执行"线性流：
...
n8n 在 v0.200+ 版本后引入了原生 AI 支持，基于 LangChain 构建。你可以直接在可视化编辑器里组合：
