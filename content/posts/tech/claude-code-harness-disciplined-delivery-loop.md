...
Claude Code 很能写。但如果你用过它做超过3个文件的任务，大概率遇到过同一个问题：写着写着，Agent 跑偏了。计划散落在6轮对话之前，测试被跳过，Review 变成事后心理安慰，PR 描述靠记忆拼凑。
...
- 项目有安全合规要求——只读敏感文件、禁止危险命令、网络出口过滤不是"最佳实践建议"而是硬约束
...
Harness 的 GitHub 星标不算高，但它的设计思路踩中了一个正在被越来越多人意识到的点：AI 编程工具的生产力瓶颈，已经从"模型不够聪明"变成了"Agent 不够守纪律"。

从工程角度看，它做了三个正确的选择：第一，把约束写成硬规则（Go 守护引擎）而不是软提示（prompt 指令），这让约束不可被模型绕过；第二，把"人员工是合同的审批者"写进工作流，人类判断留在最关键的决策节点；第三，把交付证据和发布门控标准化，让 AI 的输出物像人的输出物一样可审计。
