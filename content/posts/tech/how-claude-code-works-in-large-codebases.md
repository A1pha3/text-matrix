...
还有一个更底层的约束容易被忽略：所有 LLM 都存在上下文衰减（context rot）——上下文窗口里的 token 越多，模型精确召回其中信息的能力就越差。这不是某个模型的 bug，而是 Transformer 架构的 n² 注意力机制在长序列上的固有特性。"把更多东西塞进上下文"从来不是一个可持续的策略——关键是塞进去的每一条信息都足够高信号。
