...
| 组件 | 本质 | 加载时机 | 最适合的场景 | 常见误区 |
|------|------|----------|---------------|----------|
| CLAUDE.md | 上下文文件 | 每个会话 | 项目级约定、代码库知识 | 用来存放本应放入 Skill 的可复用专业知识 |
| Hook | 事件触发脚本 | 由事件触发 | 自动化一致性行为、捕获会话学习成果 | 用提示词做本应自动执行的事 |
| Skill | 任务专用指令包 | 按需加载（任务相关时） | 跨会话和项目可复用专业能力 | 把所有东西都塞进 CLAUDE.md |
| 插件 | 打包的 Skill + Hook + MCP 配置 | 配置后始终可用 | 在组织内部分发已验证的配置 | 让好的配置停留在部落知识层面 |
| LSP | 通过语言服务器实现的实时代码智能 | 配置后始终可用 | 类型化语言的符号级导航和自动错误检测 | 以为它是自动启用的 |
| MCP 服务器 | 连接外部工具和数据 | 配置后始终可用 | 让 Claude 访问无法直接触及的内部工具 | 在基础配置还没跑通时就急着接 MCP |
| 子智能体 | 独立 Claude 实例执行特定任务 | 调用时启动 | 探索与编辑分离、并行工作 | 在同一会话中同时做探索和编辑 |
