...
## 先说结论：CodeGraph 把 discovery 前置成可复用索引
...
原因很简单。CodeGraph 的收益来自“提前建好了索引”。如果问题到了 agent 手里，还是先按老路去扫文件，最贵的 discovery 还是会重新发生一遍，CodeGraph 就只剩额外开销。
...
更稳的顺序是：先问图，再补读细节。只有当 CodeGraph 没覆盖到某个实现细枝末节时，再回退到 `Read` 或 `Grep`。
...
如果仓库本身是 Web 项目，第一跳常常还能多拿一层信息：某个 handler 不只是“被谁调用”，它还会带出绑定它的 URL 模式。这样做的好处很直接，agent 不必再先去猜是从 `routes.ts`、`urls.py`、controller decorator 还是某个 router builder 进去。
...
从当前公开文档看，CodeGraph 更像一层面向 AI Coding Agent 的通用代码理解基础设施。

- 支持的 agent：Claude Code、Cursor、Codex CLI、opencode、Hermes Agent。
- 支持的语言：20+ 种，覆盖 TypeScript、JavaScript、Python、Go、Rust、Java、C#、PHP、Ruby、C、C++、Swift、Kotlin、Scala、Dart、Svelte、Vue、Liquid、Pascal / Delphi、Lua、Luau 等；README 当前还把 Objective-C 列为 Partial support。
- 支持的平台：Windows、macOS、Linux，x64 和 arm64 都有自包含构建。
- 运行方式：100% 本地，数据不离开机器，不依赖外部服务，持久化由 SQLite 完成。

这也是它和“再加一个搜索命令”的根本区别。它已经在往跨 agent、跨语言、跨框架的代码理解层演进。
...
更稳的采用顺序通常是：先安装到常用 agent，再挑 1 到 2 个最大的仓库初始化索引；先把 `context`、`trace`、`impact` 用在架构问答和影响分析上，再把 `affected` 接进测试或 CI，最后才考虑把它写进团队约定。
