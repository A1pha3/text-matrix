...
这篇文章会先把 SAM 3 的 2D 管线、3D 管线和多模态提示三条主线拆开，用一个具体的“自驾场景点云分割”案例把管线串起来，然后对照 SAM 1/2/3 的演进解释各代在精度和泛化上的差异来源。最后给出什么时候该上 SAM 3、什么时候 SAM 2 就够用的判断。
...
三条线共享同一个 mask 解码器，区别全在编码器阶段。这解释了为什么 SAM 3 比 SAM 2 更轻量：不是砍了解码器，而是 3D 编码器用了稀疏卷积，2D 编码器也做了蒸馏压缩。
...
SAM 系列的提示机制从 SAM 1 的“点 + 框”扩展到了 SAM 3 的“文本 + 几何”。这意味着你可以用自然语言描述要分割的对象，模型会找到对应的区域。
...
SAM 3 的真正价值不在“比 SAM 2 更好”，而在“把分割从 2D 平面拉进了 3D 空间”。如果你的任务没有 3D 需求，升级的收益有限。
