...
**Ghost Pepper**是一款macOS上的本地语音转文字工具，核心特点是**100%本地运行**，不依赖任何云端API。用户只需按住`Control`键说话，松开后即可自动将语音转录为文字并粘贴到任意文本框中。
...
**官方Slogan：** "100% local hold-to-talk speech-to-text for macOS. Hold Control to record, release to transcribe and paste. No cloud APIs, no data leaves your machine."
...
所有模型在Apple Silicon本地运行，**没有任何数据离开你的电脑**。语音转录和清理完全离线完成。

### 3. Smart Cleanup

使用本地LLM（Qwen3.5）自动清理转录内容：
- 移除填充词（"uh"、"um"等）
- 处理自我纠正
- 格式化输出
...
Ghost Pepper使用**WhisperKit**作为语音识别引擎，支持多种模型：
...
| 模型 | 大小 | 速度 | 质量 |
|------|------|------|------|
| Qwen 3.5 0.8B (默认) | ~535 MB | ~1-2秒 | 基础 |
| Qwen 3.5 2B | ~1.3 GB | ~4-5秒 | 标准 |
| Qwen 3.5 4B | ~2.8 GB | ~5-7秒 | 最佳 |

### 技术栈

- **语音识别**：WhisperKit (argmaxinc)
- **LLM推理**：LLM.swift (eastriverlee)
- **模型托管**：Hugging Face
- **自动更新**：Sparkle
- **开发框架**：Swift + Xcode
...
1. 下载GhostPepper.dmg
2. 打开DMG，将Ghost Pepper拖到Applications
3. 首次启动时授予麦克风和辅助功能权限
4. 按住**Control**键说话
...
Ghost Pepper的Smart Cleanup功能通过本地LLM提升转录质量：
...
Ghost Pepper的独特之处：
1. **最直觉的操作**：Hold-to-Talk比点击录音更自然
2. **本地LLM Cleanup**：转录质量远超原生Whisper输出
3. **完全隐私保护**：没有任何数据上传
...
Ghost Pepper需要**辅助功能权限**来实现全局快捷键和模拟按键。企业环境可通过MDM（Jamf、Kandji、Mosaic等）预置：

### PPPC Privacy Preferences Policy Control配置
...
Ghost Pepper是一个开源项目，欢迎开发者贡献。贡献者包括：

| 贡献者 | 角色 |
|--------|------|
| @matthartman | 创始人，主要开发 |
| @claude | Claude Code贡献代码 |
| @obra | Jesse Vincent (Perl之父) |
...
- **v2.0.0** (Apr 5, 2026): 新增Pepper Chat语音助手功能
- **v1.x**: 初始版本发布
...
Ghost Pepper代表了**本地AI应用**的一个优秀范例：

1. **隐私优先**：100%本地运行，数据从不离开设备
2. **直觉交互**：Hold-to-Talk比点击录音更自然
3. **质量保证**：本地LLM Cleanup提升转录可读性
4. **轻量高效**：菜单栏应用，资源占用低
5. **开源透明**：代码完全可见，社区共建
...
*本文基于Ghost Pepper项目编写，发布时间：2026-04-08*
