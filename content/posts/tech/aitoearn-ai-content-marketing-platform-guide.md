...
截至 2026 年 6 月，AiToEarn 的 GitHub Stars 约 2 万（参见 [yikart/AiToEarn](https://github.com/yikart/AiToEarn) 仓库）。按官网和 README 的表述，它面向的是 **OPC（一人公司）、创作者、品牌和小团队的 AI 内容营销智能体平台**。
...
1. **AiToEarn 的四件事是什么？它们之间的先后关系是怎样的？**
   提示：从 Create 到 Monetize 是一条流水线，想想为什么 Monetize 排在最后。

2. **5 种接入路径分别适合谁？如果你是一个已经在用 Cursor 写内容的独立开发者，应该优先走哪条？**
   提示：回顾“快速选路”那一节，注意区分“使用路径”和“开发路径”。

3. **接入时遇到 `401` 错误，应该如何排查？**
   提示：从环境和 API Key 的匹配关系入手，先确认 Key 来自中国版还是国际版，再确认接入端点是否对应。

答案不在此处展开，回到“先记住这条前提：API Key 和环境必须匹配”和“5 种接入路径详解”两节对照即可。
