...
result = rag.query("ROE 怎么算？资产负债表里有哪些项目？", use_vlm=True)
```

直接注入模式的代价是需要自己保证内容块的结构和类型正确——类型标错会让内容路由器把表格送进公式分析器，召回时就会出现语义错位。建议在注入前打印一遍 `content_type` 和 `page_idx`，确认每个块归位。
...
1. **替换 VLM**：默认 VLM 在某些垂直领域（医疗影像、工程图纸）可能不够准。把 `RAGAnything` 初始化时的 VLM 替换成自己微调过的模型，对比同一批查询的答案质量变化，能直观看到 VLM 能力对端到端效果的影响边界。
2. **调知识图谱构建策略**：LightRAG 的实体抽取和关系建边有参数可调（抽取 prompt、实体类型白名单）。在专业领域文档上，默认抽取策略可能漏掉关键术语。改 prompt 加上领域词典，再对比召回率，能定位"是图谱建错了还是检索没召回到"。
3. **接前缀缓存**：如果文档集合里有大量重复前缀（比如同一模板的财报、同一期刊的论文），给 MinerU 解析结果加内容哈希缓存，避免重复解析。这一步在文档量上千时收益明显。
4. **写自定义分析器**：三类内置分析器（视觉、表格、公式）覆盖不到所有内容类型。比如化学结构式、电路图、乐谱这些专业图形，可以仿照 `visual_content_analyzer` 的接口写一个专用分析器，注册到内容路由器。
5. **做端到端评估**：搭一个评估集，分别测文档解析准确率、跨模态召回率、多模态问答正确率三层指标。只看最后一层会掩盖前序层的瓶颈——解析错了召回再准也没用。
