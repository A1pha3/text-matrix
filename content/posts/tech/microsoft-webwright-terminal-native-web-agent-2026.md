...
source_key: "gh:microsoft/Webwright"
...
draft: false
categories: ["技术笔记"]
tags: ["Playwright", "Coding Agent", "OpenClaw", "Hermes Agent"]
...
1. 说清 Webwright 和传统 Web Agent 的根本分歧（Code as Action vs 逐动作预测）
2. 解释 Webwright 的四层抽象（terminal / browser / script / skill）如何工作
3. 分析 Webwright 在 Online-Mind2Web（86.7%）和 Odysseys（60.1%）上的 benchmark 结果
4. 配置 Webwright 的 Plugin 集成（Claude Code / Codex / OpenClaw / Hermes）
5. 判断 Webwright 是否适合你的场景（long-horizon 是甜区，短交互有代价）
...
传统 Web Agent 的每一步：
┌─────────────────────────────────────────────┐
│ 1. 截取当前页面（截图 / DOM snapshot / aria tree） │
...
两种范式的分歧在状态载体上。传统范式里，浏览器的持久化 session 扛着所有状态——cookie、DOM、tab、scroll 位置全在里面；Webwright 把状态搬到了本地 workspace 的 Python 文件里，浏览器 session 退化成一次性执行环境：spawn、inspect、discard。
...
### Q1：Webwright 和 browser-use 的核心区别是什么？

browser-use 把模型当成一个「看截图 + 预测下一个点击」的逐动作系统；Webwright 把模型当成一个「写 Python + 跑代码 + 看结果」的程序员。区别在于状态载体：browser-use 的状态在浏览器 session 里（每次重新理解页面），Webwright 的状态在本地 Python 脚本里（可以重跑、可以改代码、可以 fork）。

### Q2：为什么 Webwright 的 benchmark 成绩比 browser-use 高这么多？

不直接可比。Webwright 测的是 long-horizon task（平均 76 步），browser-use 的 benchmark 集不同。最公平的对比是同一模型（GPT-5.4）、同一 task（Odysseys）、两种 harness 形态——Webwright（code-as-action）比 xy-coordinate prediction 高 26.6 个百分点。这个差距来自「把模型当程序员」vs「把模型当逐动作预测器」的范式差异。

### Q3：Webwright 能处理需要登录的网站吗？

能，但有代价。每次 Webwright 起新的 browser context，不继承 cookie。临时解法是第一次手动登录后导出 storage state（Playwright 支持），后续用 `--storage-state` 参数复用。正式解法是等 harness 层面加上 profile 持久化支持（目前在 Early Beta，还没做）。

### Q4：自托管的 Webwright 部署需要多少资源？

取决于你跑多少并发任务。单任务：~2-4 GB RAM（Playwright + Chromium + model context）。如果要用本地模型（Ollama），需要额外 ~8-16 GB RAM（7B-70B 模型）。推荐起点：8 核 CPU + 16 GB RAM + 50 GB 磁盘（用于 trajectory 和 screenshot 存储）。

### Q5：Webwright 的 token 成本比 browser-use 高还是低？

同任务下，Webwright 的 token 成本通常更低。原因：browser-use 每一步都要传完整 accessibility tree（极占 token），Webwright 只传截图（base64，可控大小）+ 文本 observation。但 Webwright 的 step limit 默认 15 步（可以调到 100），browser-use 的 step budget 取决于具体配置。

### Q6：可以把 Webwright 集成到现有的 Playwright 测试套件里吗？

可以。`final_script.py` 就是标准的 Playwright Python 代码，可以：
1. 被 pytest 直接调用（加 `conftest.py` 和 fixture）
2. 被 CI/CD 流水线调用（GitHub Actions / GitLab CI）
3. 被调度系统定时触发（cron + Python scheduler）

关键是 `final_script.py` 是「可重跑的脚本」，不是「一次性 agent 执行的痕迹」。
...
回答下面 5 个问题，检验你对 Webwright 的理解：

1. Webwright 的核心主张是什么？它和传统 Web Agent 的根本分歧在哪？

2. Webwright 的四层抽象（terminal / browser / script / skill）分别解决什么问题？

3. `keep_last_n_observations: 1` 解决了什么问题？为什么只剪 ARIA snapshot 不删整条 message？

4. Webwright 在 Online-Mind2Web 上 86.7% 的成绩反映了什么？不能直接推出什么？

5. 如果你要在团队内采用 Webwright，按什么顺序引入功能最稳？

3 题以上答不准的话，建议重看「§2 系统总览」和「§9 Benchmark 解读」两节。

<details>
<summary>参考答案</summary>

**题 1**：核心主张是"把模型当成一个可以写代码、跑代码、看执行结果的程序员，而不是一个每步预测下一个 token 的分类器"。根本分歧在状态载体：传统 Web Agent 把状态扛在浏览器 session 里（cookie、DOM、tab），Webwright 把状态搬到了本地 workspace 的 Python 文件里。

**题 2**：Terminal 让模型在有文件系统的环境里「思考」（文件持久化、命令执行、错误反馈）；Browser 是一次性环境（spawn → inspect → discard），不是状态载体；Script 是持久化产物（可重跑、可 fork、可 review）；Skill 是跨 host 的统一描述（Claude Code / Codex / OpenClaw / Hermes 通用）。

**题 3**：解决了长 horizon 任务上下文膨胀的问题。ARIA accessibility tree 极占 token，但文本 observation 保留供后续步骤参考——所以只剪 ARIA，不删整条 message。

**题 4**：反映了 GPT-5.4 的 coding ability + Webwright 的 code-as-action 范式 + 100 step budget 的组合成绩。不能直接推出"Webwright 让所有模型变强"（需要同一模型两种 harness 对比），也不能直接推出"Webwright 适合生产环境"（86.7% 是 benchmark 数字）。

**题 5**：先跑通 Google Flights demo → 读 `src/webwright/agents/default.py`（450 行核心 loop）→ 认真看 §9 的 benchmark 解读 → 把你的第一个真实 task 改成 `task_showcase.yaml` 模式 → 如果需要 fork / 改 Webwright，读 `skills/webwright/SKILL.md`。
...
- 跑通 Quick Start 的 Google Flights demo
- 读 `src/webwright/agents/default.py`——450 行核心 loop，30 分钟能读懂
- 认真看 §9 的 benchmark 解读——不要把数字直接搬到生产
- 把你的第一个真实 task 改成 `task_showcase.yaml` 模式

### 阶段二：Host Agent 集成（3-5 天）

- 配置 Claude Code plugin（`.claude-plugin/plugin.json`）
- 配置 OpenAI Codex plugin（`.codex-plugin/plugin.json`）
- 理解 skill 共享机制——一份 `skills/webwright/SKILL.md` 四种 host 通用
- 对比「直接跑 Webwright CLI」vs「通过 Host Agent 调用」的 token 成本差异

### 阶段三：长期任务优化（1-2 周）

- 配置 `step_limit`、`keep_last_n_observations`、`summary_every_n_steps`
- 理解上下文压缩的代价（丢细节）和收益（省 token）
- 在真实 long-horizon task 上测试 self-reflection 开关（30%-50% token 增加换视觉兜底）
- 把 `final_script.py` 集成到 CI/CD 流水线

### 阶段四：框架贡献（2-4 周）

- Fork 仓库，读 `src/webwright/` 四层抽象的实现
- 理解三个 backend（OpenAI / Anthropic / OpenRouter）的归一化错误处理
- 如果你需要新的 browser 能力，改 `environments/` 目录
- 如果你需要新的 model 后端，照着 `models/openai_model.py` 的实现写一个
