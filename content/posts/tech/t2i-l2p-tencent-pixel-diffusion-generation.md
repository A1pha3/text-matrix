...
主流文生图（SD3、FLUX、Sana）都在潜在空间（latent space）做扩散——把图像压缩到低维潜在空间，在那里计算生成，再解码回像素空间。L2P 的判断是：潜在空间扩散对某些能力有隐式约束，如果能直接在像素空间做扩散，可以解锁更强的细粒度控制能力，但代价是计算开销更高。L2P 的目标是找到一种高效传递范式，让像素空间扩散的细粒度和潜在空间扩散的计算效率兼得。
...
两阶段分工明确：潜在扩散做粗略生成，像素级扩散做细粒度增强，计算量可控。
...
L2P 是一个有明确技术路线的研究项目：探索"latent→latent→pixel"这条路径的可行性。它提出了一个问题——像素空间扩散的细粒度优势和潜在空间扩散的计算效率是否可以兼得。