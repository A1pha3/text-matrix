...
description: "ES 9.x 主线：向量搜索与 RAG 一等公民、ES|QL 新查询语言、Lucene 10 升级、Aggregation 性能改进，以及今天该不该上 ES 的判断。"
categories: ["技术笔记"]
tags: ["向量搜索"]
author: "text-matrix"
---

> 范围说明：Elasticsearch 是一个 77k Stars、百万行级 Java 代码的搜索引擎。本文不展开 ES 教程，也不复述入门 CRUD。只回答三件事：今天为什么会再次上榜、9.x 主线改了什么、采用边界在哪里。
...
ES 9.x 发布周期是 ~6 个月一次大版本（9.0 于 2024-04 发布，9.1 于 2024-08，9.2 于 2025-02，9.3 于 2025-08，9.4 计划 2026-Q1）。
...
- **删除性能**：`soft-deletes` 改进，segment merge 期间不再卡写入

Lucene 升级对 ES 用户是无感的（API 不变），但对性能有 5-15% 的整体提升。

### 4. 聚合性能优化

- **`composite aggregation` 内存优化**：分页聚合的内存占用降为 1/3
- **`terms` aggregation 改进**：`global_ordinals` 的并发安全
...
把 `commits/main.atom` 过去 24 小时梳理了一下：
...
`LOOKUP JOIN` 支持跨集群的 lookup table；`RERANK` 命令把 rerank 流程内置到 ES|QL。
...
- **运维成熟**：ES 集群运维复杂度高，需要专门团队

### 不太适合

- **极简搜索**：Meilisearch、Typesense 比 ES 轻很多，部署时间 < 1 小时
- **强一致性事务**：ES 不是事务数据库，跨文档事务不支持
- **超大规模（> 10TB 单索引）**：ES 的 master 节点是瓶颈，> 10TB 要做 federated search
- **预算敏感**：JVM 内存占用大（建议 32GB+），AWS 上的 OpenSearch 价格高于 Postgres + 全文索引
- **不想运维基础设施**：直接用 Elastic Cloud 或者 Algolia、Meilisearch Cloud

### 升级建议

- **ES 7.x → 8.x**：7.x 已 EOL，升级路径有完整文档
...
**Q：ES 还值得学吗？现在上 9.x 会不会很快过时？**
A：9.x 是持续演进的主线，每个大版本都有文档化的 breaking change。7.x 已经 EOL，新项目建议直接上 8.x / 9.x，不存在「刚学就过时」的问题。

**Q：跑向量搜索一定要上 GPU 吗？**
A：不需要。BBQ 把 float32 向量压成 1-bit，内存占用降到 1/32，CPU 也能跑召回。GPU 只在你需要大规模训练或实时生成 embedding 时才必要。

**Q：`semantic_text` 默认用哪个 embedding 模型？**
A：端到端测试用的是 e5 模型，但默认模型会随版本变化。实际接入时以官方文档为准，也可以显式指定自己托管的模型。

**Q：ES 和 OpenSearch 到底怎么选？**
A：需要 ES|QL、最新向量能力选 ES；如果团队已经在 7.10 fork 上跑得很稳且用不到这些新能力，留在 OpenSearch 也合理，但升级前必须验证新特性。

**Q：单节点能直接上生产吗？**
A：不建议。3 节点起步，生产环境保留 9.x 默认的 security 配置。单节点一旦宕机，分片没有冗余。