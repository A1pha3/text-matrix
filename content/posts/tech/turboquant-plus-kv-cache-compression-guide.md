...
source_key: "gh:TheTom/turboquant_plus"
...
[TurboQuant+](https://github.com/TheTom/turboquant_plus) 是对 Google Research [TurboQuant](https://research.google/blog/turboquant-redefining-ai-efficiency-with-extreme-compression/) 论文（ICLR 2026）的开源实现与扩展工程。截至 2026 年 4 月，该项目已获得 **6,482 Stars** 和 **872 Forks**。
...
项目目标：**在不显著损失模型质量的前提下，将 LLM 推理时的 KV Cache 压缩 3.8-6.4 倍**，降低长上下文场景的内存占用和带宽消耗。
...
即使模型权重是量化的，KV Cache 仍可能占满显存，导致无法使用长上下文。

TurboQuant 的切入点：**不需要量化模型权重，只需要压缩 KV Cache 本身**。
...
**关键发现**：turbo4 压缩率接近 q4_0，但质量接近 q8_0。PPL 仅增加 0.23%，远优于同等压缩率的 q4_0。

### 3.3 K 压缩 vs V 压缩：不对称性
...
这一发现直接导向**非对称 K/V 配置**（q8_0-K + turbo-V），这是低 bit 模型（如 Q4_K_M）的关键救星：
...
项目进一步发现：**Transformer 的首尾层对 V 精度极为敏感**，中间层则相对鲁棒。
...
传统 KV Cache 在 decode 时需要对所有缓存位置进行 dequantize 和加权求和。但大多数位置的 softmax 注意力权重 < 1e-6，贡献可以忽略。
...
Sparse V 不是 TurboQuant 专属的——在 q8_0 KV Cache 上也能带来 +5% 的 decode 加速，说明这是一种通用的注意力优化。
...
**turbo4 的 Prefill 速度持平甚至略超 q8_0**——压缩后的 KV Cache 带宽占用更小。
...
104B 模型在 MacBook M5 Max 上跑到 128K 上下文，PPL 仅增加 1.9%。
...
关键教训：**WHT 旋转必须在图侧（graph-side）执行**，而非查询侧（query-side），否则旋转方向不一致会导致质量灾难。
...
2. **所有质量损失来自 K 压缩**：K 控制注意力路由，精度丢失直接导致错误的权重分配，因此非对称配置能救活低 bit 模型
...
TurboQuant+ 是目前 KV Cache 压缩领域最完整的开源实现：

1. **工程覆盖**：Python 原型 → llama.cpp C 集成 → MLX Swift 移植，全平台可用
2. **实验基准**：PPL、NIAH、KL 散度数据完整，6,482 Stars
3. **核心发现**：V 压缩免费 + K 是质量瓶颈 + 边界层保护，三条结论已被 5 个独立团队验证
4. **硬件验证**：M1-M5 Mac、RTX 3080 Ti/3090/4090/5090、AMD RX 9070 XT
