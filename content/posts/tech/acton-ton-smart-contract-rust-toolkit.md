...
description: "Acton 是 ton-blockchain 官方组织出品的智能合约开发工具链，基于 Rust 编写，用单条 CLI 覆盖项目脚手架、编译、测试、脚本、钱包操作、网络部署和调试等合约全生命周期环节。"
draft: false
categories: ["技术笔记"]
tags: ["智能合约", "Rust", "Web3"]
---
...
Acton 把 TON 合约开发里原本散落在 `func`、`fift` 和多个第三方工具之间的命令，收束到一个用 Rust 编写的二进制里，覆盖从项目脚手架到测试网部署的全部环节。本文描述的行为对应 2026 年 5 月 Acton 仓库 `master` 分支的状态，命令与产物路径以仓库 README 和官方文档为准。
...
装好后从官方模板建项目最省事。模板自带完整的 `contracts/`、`wrappers/`、`tests/` 和 `scripts/` 目录结构，新建后即可直接 `acton build`：
...
官方模板包括 `counter`（计数器）、`jetton`（代币）、`nft`（collection 与 item 一对合约）、`w5-extension`（钱包 v5 扩展与订阅示例）和 `empty`（空项目）。`--app` 会额外生成带 Vite + React 前端的项目，适合想要 dApp 完整骨架的人。
...
测试由 `acton test` 承担，是 Acton 的强项：支持 fork 模式（把测试网或主网的链上状态拉到本地执行）、gas 快照、覆盖率、突变测试（mutation testing）和模糊测试（fuzz）。测试文件用 Tolk 编写，默认放在 `tests/`：
...
`acton test --ui` 在浏览器里打开测试结果，失败用例、交易调用轨迹和日志都能可视化查看。调试复杂合约逻辑时这比命令行输出直观——交易在哪一步退出、消息怎么流转，页面上直接可见；命令行下要在几百行文本里 grep 才能拼出同样的信息。

代码质量相关有三条命令：`acton check` 做静态/写法检查，`acton fmt` 做格式化，`acton fmt --check` 只检查不格式化，适合接进 CI。

### 脚本部署与钱包

TON 的部署习惯是“写一段部署脚本，脚本里调用合约方法并处理链上交互”。Acton 用 `acton script` 执行这类 Tolk 脚本：不带 `--net` 时在本地模拟执行，加上 `--net` 才广播真实交易：
...
# 签名外部消息体（支持 hex 或 base64 输入）
acton wallet sign deployer --body <message-body>
```

官方文档里最常见的启动链路是：建项目 → build → 建钱包 → 脚本上测试网。这两节的命令拼起来，就是“从一段源码到一笔链上交易”的完整路径。

### 语言与生态支持

Acton 是 Tolk-first 的工具链：默认语言是 **Tolk**，测试、脚本、部署脚本都用 Tolk 编写，底层用 Rust 实现并以单个二进制分发。

dApp 侧，`acton new --app` 能生成带 Vite + React 前端的项目骨架，并自动生成合约的 TypeScript 封装（wrapper），前端调用合约方法时有类型提示和自动补全，不用对照 ABI 手写每个方法签名。低层工具也内置：`acton disasm contract.boc` 反汇编字节码，`acton retrace <transaction_hash>` 重放链上交易，方便排查。
...
选 `nft` 模板是因为它已经带好 collection 和 item 一对合约、类型定义、wrapper、测试和部署脚本，比从 `empty` 起步省事。

### 2. 编写合约

官方 `nft` 模板的 `contracts/NftCollection.tolk` 已经是一份符合 [TEP-62](https://github.com/ton-blockchain/TEPs/blob/master/text/0062-nft-standard.md) 与 TEP-64 的完整实现，不必从零写。看它的骨架就能看懂 Tolk 合约的结构。合约声明与持久状态（节选）：

```tolk
// contracts/NftCollection.tolk（节选自官方 nft 模板）
import "types"

contract NftCollection {
    author: "__ACTON_AUTHOR__"
    version: "1.0.0"
...
状态结构和入站消息类型定义在 `contracts/types.tolk`（节选）：
...
入站消息统一进 `onInternalMessage`，按 `match` 分支分发。铸造（`DeployNft`）分支的完整逻辑：
...
链下查询通过 `get fun` 暴露，对应 TEP-62 规定的 getter（节选）：
...
几个值得留意的 Tolk 写法：持久状态收拢在一个 struct 里，通过 `load()`/`save()` 与链上存储互转；`lazy` 让字段只在首次访问时从 cell 解析；`assert (条件) throw 错误码` 一行完成权限校验；`get fun` 返回结构体实例而非手拼 tuple。这套高层结构让合约比纯栈式的 FunC 好写很多。
...
```bash
# 先在本地模拟部署脚本，确认逻辑正确
acton script scripts/deploy-collection.tolk

# 再真正上测试网
acton script scripts/deploy-collection.tolk --net testnet
```

部署脚本（`scripts/deploy-collection.tolk`）负责：取 `deployer` 钱包、采集 collection 元数据、构造合约初始状态并广播部署交易，最后打印部署好的 collection 信息（含合约地址）。钱包提前用 `acton wallet` 创建并 airdrop 测试币。
...
TON 智能合约生态里主要有三种开发语言，定位各不相同。Acton 默认并优先支持 **Tolk**。官方资料：[TON 文档站 FunC 章节](https://docs.ton.org/v3/documentation/smart-contracts/func/overview)、[tact-lang.org](https://tact-lang.org/)、[TON 文档站 Tolk 章节](https://docs.ton.org/v3/documentation/smart-contracts/tolk/overview)。

**Tolk** 是 ton-blockchain 组织维护的较新语言，也是 Acton 的主角语言。它静态类型、直接编译到 TVM 字节码，官方文档把它定位为 FunC 的演进与推荐方向。语法是 struct 加 `match` 分支的现代风格，同时保留对底层的控制：直接操作 slice/builder，给消息结构体标注 TL-B 操作码。Acton 的测试、脚本、模板和文档都以 Tolk 为准——语言和工具链出自同一个组织，工具链跟着官方推荐的语言走，是顺理成章的选择。Tolk 仍在快速迭代，标准库与审计工具链尚在完善，主网生产案例还不多；存量且已审计的合约不建议立刻迁移，等生态稳定后再评估。
...
- **从零启动 TON 合约项目**：`acton new --template` 一步建好骨架，编译器、测试框架、部署脚本都不用自己拼接。
...
2. 再用 `acton test` 替换原有测试方式，先跑通现有用例，再逐步引入 fork 模式、gas 快照和突变测试
3. 最后用 `acton script` + `acton wallet` 替换原有的部署脚本和钱包操作，统一运维入口

如果还想把语言从 FunC 换成 Tolk，Acton 自带 `acton func2tolk` 迁移命令。语言替换涉及行为等价性验证，建议放在前三步跑稳、且有审计余量时再做。
...
3. 检查源码语法：Tolk 语法随版本迭代可能变化，对照 TON 文档站的 Tolk 章节检查。
...
4. **深入 TVM**：用 `acton disasm` 反汇编 `.boc`，理解 Tolk 编译器生成的字节码，以及如何在极端 gas 优化场景下手写更精细的逻辑。
