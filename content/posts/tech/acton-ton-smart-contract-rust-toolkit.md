...
下面 10 条用来检验你对全文的掌握程度，每条标注了对应的章节，答不上来的回去翻一遍再继续。

- 能 / 不能：说出 Acton 解决的 TON 工具链碎片化问题，以及四个命令组的职责边界。（→ 项目概览）
- 能 / 不能：复述 `acton new`/`build`/`test`/`deploy`/`verify` 的完整生命周期，并指出每步的产物。（→ 快速上手、核心能力）
- 能 / 不能：区分 FunC、Tact、Tolk 三种语言的适用场景，并解释为什么 NFT 合约示例选了 Tact。（→ FunC、Tact 与 Tolk）
- 能 / 不能：解释 `acton verify` 的验证原理，以及验证失败时的三类常见原因。（→ 任务流案例）
- 能 / 不能：根据自己的合约项目特点，判断是否应该迁移到 Acton，并给出迁移的优先顺序。（→ 适用场景与采用顺序）
- 能 / 不能：在 CI/CD 里配置 `acton test` 的 gas 快照对比，并解释为什么这能检测回归。（→ 动手练习）
- 能 / 不能：对照"建议现在上 / 可以暂缓"两类用户，给自己或团队归类并说明理由。（→ 决策建议）
- 能 / 不能：排查 `acton deploy` 失败的三种常见原因，并给出对应的修复命令。（→ 常见问题与排查）
- 能 / 不能：解释为什么 Acton 选择 Rust 编写测试运行器，以及这对大型测试套件的意义。（→ 核心能力）
- 能 / 不能：描述一次完整的 NFT 合约从创建到验证的任务流，并说明每个环节的输入和输出。（→ 任务流案例）
