...
在 `sources/contracts/nft_collection.tact` 中定义 `NFTCollection` 合约，包含 `mint`、`transfer`、`get_item_info` 三个方法。下面是合约骨架（已包含 storage 读写和消息处理的关键逻辑，完整实现还需补全 NFT 标准 TL-B 序列化与 item 合约部署；完整 NFT 标准实现参考 [TEP-62](https://github.com/ton-blockchain/TEPs/blob/master/text/0062-nft-standard.md)）。代码中 `reply`/`asString`/`asComment` 等调用形式以 Tact 官方文档为准，不同版本语法可能存在差异：
...
- 你的团队现在是不是同时维护 func、fift 和 tonos-cli 三套工具的配置和版本？
...
- **编译器版本不一致**：本地用的 Tact/FunC 编译器版本与浏览器端重新编译时使用的版本不同，导致字节码差异。排查命令：参考 Acton 仓库 README 的编译器版本查询命令，把输出与浏览器端声明的版本对比。
- **源码未提交完整**：`sources/` 下有未跟踪的 include 文件或 `.tact` 中引用的 stdlib 路径在提交时被 `.gitignore` 排除，浏览器端拿不到完整源码。排查命令：`git status` 检查未跟踪文件，本地模拟浏览器端编译的调试命令参考 Acton 仓库 README（命令名以仓库为准）。
- **参数顺序错误**：`Acton.toml` 里的 `init_data` 字段顺序与合约 `init` 函数签名不匹配，拼装出的 cell 与链上部署时的 cell 不一致。排查命令：参考 Acton 仓库的调试命令打印实际拼装的 cell（命令名以仓库 README 为准），与部署时的 `init_data` cell 逐字节对比。

### FAQ

**Q1：`acton build` 产物与 `func` 直接编译的结果不一致怎么办？**

先对比编译器版本（参考 Acton 仓库 README 的编译器版本查询命令与 `func -V`），再对比编译选项（`-O` 优化级别、`-c` 配置文件路径）。Acton 默认开启的优化级别可能与你的 `func` 命令不同，把两边对齐后产物应当一致；若仍不一致，到 Acton 仓库提 issue 附上最小复现用例。
...
Acton 跟随 [ton-blockchain/tolk](https://github.com/ton-blockchain/tolk) 主线，通常在 Tolk 发布新版本后一到两周内适配。若你的项目锁定了某个 Tolk 旧版本，在 `Acton.toml` 的 `[toolchain]` 段指定 `tolk = "0.7.x"`，Acton 会下载对应版本，不用内置最新版。
...
时间预期：单人完成约需 3-5 小时。FunC 的栈操作和 cell 序列化对没接触过 TVM 的开发者有学习曲线，第一次写 `recv_internal` 入口和 `load_msg_addr` 这类切片解析大约要花 1-2 小时查文档和调试；后续 getter 注册和 storage 读写再花 1-2 小时；最后跑通测试和 gas 对比约 1 小时。如果练习一用的是 FunC 模板，反向改写为 Tact 的时间会短一些（约 2-3 小时），因为 Tact 的抽象层次更高。

这个练习揭示高级语言编译器的"隐形成本"——Tact 帮你省掉的代码由编译器自动生成，但自动生成的 FunC 中间代码在极端场景下可能多消耗 gas。
...
| 3 | 能解释 `acton test` 为什么用 Rust 实现而不用 Node.js——以及这对万级测试用例的编译耗时意味着什么 | [编译与测试](#编译与测试) |
...
3. **用 `acton debug` 深入 TVM 指令级**：打开 `acton debug --ui`，跑一遍 mint 操作，在浏览器里逐条看 TVM 指令执行。跟踪一次 `recv_internal` 调用中消息解析、cell 操作和存储写入的完整指令序列。这比读文档更直接地建立"FunC 代码→TVM 指令→Gas 消耗"的映射直觉。
4. **模糊测试先行**：用 [ton-blockchain/tolk-fuzzer](https://github.com/ton-blockchain/tolk-fuzzer) 对 FunC 合约做模糊测试，覆盖消息格式异常、边界值和并发场景。

**未来路线（尚未实现，不可用）**：Acton 计划引入形式化验证能力（命令名待定，将与现有的源码验证 `acton verify` 区分），目前仍在开发中。在它落地之前，模糊测试是更现实的安全性补充手段。

每一步对应一个具体的工程能力——从"会用工具"到"理解工具背后的虚拟机"再到"能用工具验证合约安全性"。不必一次走完，但建议至少走到第二步再开始写生产合约。
