...
**Q1**：插件系统的三层结构（plugins/、external_plugins/、marketplace.json 远程插件）分别是什么？

<details>
<summary>查看答案</summary>

→ plugins/ 是 Anthropic 官方维护的插件；external_plugins/ 是第三方合作插件（经审核）；marketplace.json 远程插件是第三方插件（指向外部 Git 仓库）
</details>

**Q2**：MCP 服务器的四种类型（stdio、SSE、HTTP、WebSocket）分别适用什么场景？

<details>
<summary>查看答案</summary>

→ stdio：本地工具，调起子进程（如 PostgreSQL、clangd）；SSE：托管服务，支持 OAuth（如 GitHub、GitLab）；HTTP：REST API 直连；WebSocket：实时双向通信（消息推送场景）
</details>

**Q3**：插件安全策略的三项检查（has_broad_scope_hooks、has_undisclosed_telemetry、description_matches_behavior）分别防什么？

<details>
<summary>查看答案</summary>

→ has_broad_scope_hooks：防止插件监听全局事件窃取数据；has_undisclosed_telemetry：强制声明所有外向网络调用；description_matches_behavior：确保 README 与实际行为一致，避免伪装）
</details>
...
选择一个官方插件（如 `github` 或 `code-review`），完成以下任务：

1. 使用 `/plugin install` 命令安装插件
2. 阅读插件的 `.claude-plugin/plugin.json` 配置文件
3. 在对话中测试插件的功能（如创建 Issue、审查 PR）
4. 观察插件如何调用 MCP 服务器，理解插件的工作机制

**目标**：掌握插件的安装和配置流程，理解插件与 MCP 服务器的关系。

### 练习 2：开发一个自定义插件

参考 `plugin-dev` 插件的开发流程，开发一个简单的自定义插件：
...
选择一个第三方插件（从 `external_plugins/` 或 `marketplace.json`），完成以下评估：

1. 阅读插件的源代码（`.mcp.json`、`.claude-plugin/plugin.json`）
2. 检查是否包含 `has_broad_scope_hooks`（危险的钩子）
3. 检查是否包含 `has_undisclosed_telemetry`（未披露的遥测）
4. 评估 `description_matches_behavior`（描述与行为是否匹配）

**目标**：掌握插件安全评估的基本方法，理解 Anthropic 的三项安全检查策略。
...
- 安装 GitHub、GitLab、Playwright、AWS 等开箱即用插件
- 确认插件是否真的提升工作效率

### 阶段二：学习插件开发（2-4 周）
- 当发现现成插件不够用，或有重复性工作流时，使用 `plugin-dev` 工具包
- 学习 7 个专项 skill，掌握插件开发全生命周期
- 开发一个自定义插件，解决团队的具体问题
...
本文基于 Anthropic 官方仓库（[chrislgarry/Apollo-11](https://github.com/chrislgarry/Apollo-11)）公开文档整理，需要说明的边界：

1. **性能数据来源**：文中提到的性能数据来自官方文档和社区反馈，未在标准化测试环境中验证，实际性能因硬件配置而异。
2. **版本时效性**：Zvec 处于活跃开发阶段，API 可能变化，请以[官方 GitHub 仓库](https://github.com/alibaba/zvec)的最新代码为准。
...
**主要优化点：**
1. 将"自测题"改为标准格式（5道题，含`<details>`标签参考答案）
2. 添加"资料口径说明"章节
3. 添加"优化说明"章节
4. 使用 humanizer 检查并移除AI味道
5. 修正中英文空格规范
