...
1. AI Agent 系统的四条并行主线（单 Agent、多 Agent、生产架构、前沿实践）各自解决什么问题，为什么四条线之间可以独立阅读。
2. ReAct 循环的三个步骤（观察 → 思考 → 行动）和终止条件设计的工程意义。
3. DAG、Swarm、Handoff 三种编排模式分别适合什么场景，怎么根据任务特征选择。
4. Shannon 三层架构（Orchestrator / Agent Core / LLM Service）的职责划分，以及分层后容错策略为什么能独立设计。
5. 根据读者背景（后端开发者 / 架构师 / 技术负责人）给出分阶段的阅读路径，并说明主线三为什么依赖主线一和主线二。
...
如果你已经能调用 OpenAI 或 Anthropic 的 API 跑通一个 Chatbot Demo，却卡在"怎么把它变成一个能上线、能审计、能控制成本的真实系统"，Wayland Zhang 这本《AI Agent 架构：从单体到企业级多智能体》（英文副标题 *From Concept to Production: Framework-Agnostic AI Agent Architecture Patterns*）是目前少数能直接回答这个问题的开源资料。

书里不绑定 LangGraph、CrewAI 或 AutoGen 中的任何一个，而是把 Agent 系统拆成 ReAct 循环、工具协议、上下文管理、编排模式、生产架构、企业治理这几条独立演化的主线，每条线都有明确的边界和取舍判断。配套的参考实现 Shannon（Apache 2.0）用 Go/Rust/Python 三层架构落地了这些模式，可以对照代码验证。

本文先给总览地图，再按"单 Agent → 多 Agent → 生产架构 → 企业治理 → 前沿实践"的顺序拆解每部分的核心机制，最后给不同读者的采用建议。
...
四条主线之间没有严格的依赖：你可以只读主线一和主线二来理解 Agent 的运行机制，也可以只读主线三来补生产化短板。但主线三依赖主线一和主线二的概念，主线四依赖前三条的全部基础，所以不建议跳读 Part 9。
...
第 1 章先回答"Agent 和普通软件的区别在哪"——区别在于**自主决策循环**。普通软件的分支由开发者写死，Agent 的下一步行动由 LLM 在运行时根据观察决定。第 2 章把这个循环具体化为 ReAct（Reason-Act）：
...
这四个策略互不冲突——可以单独优化任何一个，但组合使用时需要权衡。例如 Compress 会损失细节，Isolate 会增加总 Token 消耗。第 7 章还覆盖 Prompt Cache 这一工程优化，第 8 章讲短期/长期记忆的存储与检索（含向量存储），第 9 章讲多轮对话的状态管理。
...
分三层是因为这三类职责的失败模式不同：编排层失败需要快速重启，执行层失败需要隔离爆炸半径，LLM 调用失败需要重试和降级。分层后可以针对每层独立设计容错策略。Go 选型偏性能和并发，Rust 选型偏安全和隔离，Python 选型偏 LLM 生态——这是工程权衡，按团队实际情况调整。
...
这个案例中 ReAct 循环、MCP 工具、Swarm 编排、HITL、沙箱执行、预算控制、可观测性、策略治理等机制同时工作，展示了这些模式在真实任务中如何组合。
...
每个阶段都能拿到一个可运行的中间产物，不用读完 33 章才动手。
...
下面 7 道题覆盖了《AI Agent 架构》的核心设计要点。点击参考答案前的三角展开查看解析。
