...
> **本文回答三个问题**：n8n-MCP 到底解决了什么具体问题、它的 7 个核心工具分别承担什么角色、以及你在自己的项目里该从哪里开始用。
...
n8n-MCP 要解决的问题比"让 AI 能调用 n8n API"具体得多：让 AI 助手在 1,650 个节点、上万种参数组合面前，不依赖默认值、不犯配置错误，一步到位生成可运行的工作流。
...
```
1. 调用 tools_documentation() 获取推荐做法
...
- 正在教团队成员上手 n8n——把它当成交互式学习工具，AI 助手可以实时解释每个节点的用法和实践建议。
