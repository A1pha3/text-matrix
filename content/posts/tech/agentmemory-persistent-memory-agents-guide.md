...
- **很多高级能力默认关闭**。README 的配置章节明确写到，像 `AGENTMEMORY_AUTO_COMPRESS`、`AGENTMEMORY_INJECT_CONTEXT`、`AGENTMEMORY_SLOTS`、`GRAPH_EXTRACTION_ENABLED` 这类功能默认关闭。设计思路是先保证系统可控，再把更激进的自动化能力交给用户按需启用。
...
这个模型的重点在于，agentmemory 让项目知识逐步沉淀成不同密度、不同稳定性的记忆层，而不是把所有信息平铺存起来。README 还明确提到记忆会衰减、强化、自动淘汰，并尝试解决陈旧记忆和矛盾事实的问题。
...
这些数字适合帮助读者建立预期，不能脱离场景直接当成结论。benchmark 口径、适用边界和默认配置，最好一起交代。
...
agentmemory 的 README 自己就把 mem0 和 Letta 放在对比表里，结论很清楚：它强调的是 **memory engine + MCP server** 这一路径，也就是继续使用现有 Agent，但把记忆变成共享服务；mem0 更偏 **memory layer API**；Letta 更偏 **full agent runtime**。三种方案切入的层级不同，不宜简单归为同类竞品。
...
从工程视角看，agentmemory 把 AI Agent 的长期记忆做成了**本地可运行、可检索、可观察、可扩展**的一层基础设施。session 中零散的 observation 会沉淀成四层记忆，再通过 BM25、vector、graph 三路融合把相关上下文找回来，最后用 MCP、REST、skills、viewer 和 iii console 交付整套能力。

只需要给 Agent 一份项目说明时，静态记忆文件已经够用。到了"一个项目要跑很多轮、很多天、很多 Agent"的阶段，agentmemory 的差异才会真正显出来。它解决的是把过去的工作变成下一轮可用的上下文。

## 11. 练习

### 练习1：评估你的项目是否需要 agentmemory

回顾你最近一个月使用 AI 编程 Agent 的经历，回答以下问题：

1. 你有没有遇到过"上次已经解释过的问题，新会话又要重新解释"的情况？
2. 你的 `CLAUDE.md` 或 `MEMORY.md` 是否已经超过 500 行，并且很难维护？
...
|-----------|--------------|--------------|----------|
| Claude Code | hooks + MCP + skills | ✅ | 完整服务 + 插件 |
| Cursor | MCP | ❌ | 独立 MCP server |
| 你的 Agent | ？ | ？ | ？ |
...
1. 你会开启哪些自动功能？（auto-compress、inject-context、slots、graph extraction）
2. 你会为哪些信息创建手动记忆？（项目架构、编码规范、常见错误）
3. 你会如何组织记忆的优先级？（哪些信息应该在每次会话开始时被召回）
...
- **行动**：
  ...
  3. 尝试手动触发各层的巩固过程，观察效果
- **参考资源**：
  - [agentmemory GitHub - consolidation 实现](https://github.com/rohitg00/agentmemory)
  - [记忆巩固的神经科学原理](https://en.wikipedia.org/wiki/Memory_consolidation)

### 阶段2：优化检索质量和性能

- **目标**：让你的记忆召回更准确、更快速
- **行动**：
  1. 调整 BM25、vector、graph 的权重比例
  2. 为你的项目选择最合适的 embedding 模型
  3. 配置 token 预算，避免注入过多无关记忆
  4. 使用 session replay 功能分析召回质量
- **参考资源**：
  - [agentmemory benchmark 结果](https://github.com/rohitg00/agentmemory/tree/main/benchmark)
  - [Reciprocal Rank Fusion 原理](https://en.wikipedia.org/wiki/Reciprocal_rank_fusion)

### 阶段3：构建团队级记忆共享

- **目标**：让团队成员共享项目记忆，避免重复踩坑
- **行动**：
  1. 配置 team share 功能，让团队成员共享记忆层
  2. 建立记忆治理规范（什么应该记录、什么不应该记录）
  3. 使用 memory_governance_delete 和 memory_verify 进行记忆审计
  4. 将 agentmemory 集成到团队的 CI/CD 流程中
- **参考资源**：
  - [agentmemory team sharing 文档](https://agent-memory.dev/docs/team-sharing)
  - [AI Agent 记忆治理最佳实践](https://github.com/rohitg00/agentmemory/blob/main/docs/governance.md)
...
2. 完善自测问题：将开篇的3道自测问题扩展为5道标准格式（含`<details>`标签参考答案）
