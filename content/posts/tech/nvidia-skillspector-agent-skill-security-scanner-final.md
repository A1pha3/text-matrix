...
npm 生态有 package-lock.json 锁定版本、有 npm audit 扫漏洞、有 Snyk 做 SCA；但 agent skill 生态目前没有等价物。一个 agent skill 通常只是一段 Markdown + 几行脚本，没有任何安全保证。
...
npm 上有 package-lock、有 SCA、有 Snyk；但一个 agent skill 通常是一段从 GitHub gist 复制过来的 SKILL.md，加几行「推荐先 pip install X」的提示。
...
SkillSpector 把「这个 skill 安不安全」从直觉判断转换成 SARIF 2.1.0 标准报告。
...
SkillSpector 用统一的 `Finding` 形状来对齐所有 17 大类漏洞：

- 不管是正则匹配的「skill 文字里出现 'ignore all previous instructions'」
- 还是 AST 树遍历的「Python 文件里有 `exec()` 调用 + 字符串拼接」
- 还是网络请求的「OSV.dev 返回这条依赖有 CVE」

最后都变成同一个 `Finding`。这种统一让下游的报告节点不用关心上游是哪种分析。

## 总览图：22 个分析器怎么并行协作

SkillSpector v2 的 LangGraph workflow 形状非常标准——`resolve_input` → `build_context` → 22 个分析器并行（fan-out）→ `meta_analyzer`（fan-in）→ `report`。
...
E2 是 HIGH，因为它通常意味着 skill 在主动收集密钥。
...
OH1 在 OWASP LLM Top 10 2025 的 LLM02（Insecure Output Handling）里有对应。

### System Prompt Leakage (3)

- **P6**：直接泄露
- **P7**：间接提取（要求翻译/复述 system prompt）
- **P8**：通过工具（file write / HTTP 请求）把系统提示外传
...
MP1+MP3 联合起来就是「skill 永久改写 agent 行为」的攻击模式。
...
传统静态扫描会把 `getattr(os, "system")(cmd)` 漏掉，因为 `getattr` 的第二参数是字面量；SkillSpector 显式枚举了「外层是 getattr、第二参数是字面量、内层名属于危险集合（exec / eval / system / popen / __import__）」的反射模式。
...
**TT5 的典型场景**：「间接 prompt injection 落地执行」的检测模式——邮件内容 → LLM → exec sink。
...
**TP1-TP4** 检测 MCP tool metadata 是否含：

- 隐藏指令
- Unicode 欺骗（homoglyphs、RTL）
- 参数描述注入
- 声明行为与代码行为不匹配
...
如果用户开了 `--no-llm`，meta_analyzer 直接跳过 LLM 调用，把 `findings` 原样当作 `filtered_findings`。

**设计动机**：「CI 默认开 LLM、本地快速跑用 `--no-llm`」——LLM 调用增加延迟和成本，但能减少静态分析器的过度报警。
...
**不能推出**：「skill 在所有攻击场景下都安全」——评分只覆盖 SkillSpector 已知的 68 个模式；新型攻击（novel injection、零日 MCP 漏洞、未入库的恶意包）不在评分范围内。

### 几个边界条件

1. **同一条 finding 不会被去重**（设计上保留，让 SARIF 完整呈现）。如果 13 个 static_patterns_* 都报告了「ignore all previous instructions」，score 会按多条算。

2. **baseline 抑制不进 score**——被压制的 finding 只在 `--show-suppressed` 或 JSON 报告的 `suppressed` 字段可见。这避免了「塞满 baseline 假装安全」的反模式。

3. **executable multiplier 只触发一次**（不叠加），但 ×1.3 在 HIGH（25 分）级别上等于多加 7.5 分，等于把一个 HIGH finding 升一档。
...
**不能推出**：「这个 skill 没有漏洞」——OSV.dev 只覆盖已公开的 advisory；私有漏洞、零日、依赖本身没问题但调用方式有问题的场景，SC4 看不到。

#### SC4 重构的启示

这个 SC4 重构是个值得借鉴的范例：它把「人工维护的小清单」换成「实时对接权威数据库 + 离线 fallback」，解决了所有四类问题（staleness、maintenance、coverage、version logic），代价是增加了一个网络依赖。

SkillSpector 用 graceful degradation 把这个代价压到了最小——网络失败时不报错，只是降级。
...
两阶段分析测的是「已知漏洞模式 + LLM 判断的语义漏洞」。

**不能推出**：「skill 在生产环境 100% 安全」——LLM 本身有幻觉和判断不一致问题；静态分析器有覆盖率盲区；新型攻击每天都在出现。

SkillSpector 的设计取舍是：把已知的、模式化的攻击覆盖到，把语义判断交给 LLM，把决策交给人（review SARIF 报告）。

## Baseline 抑制与增量 CI

LLM 语义分析器（特别是 SQP-1、SDI-2）经常会产生「技术上正确但对你没意义」的报警。

**例**：某个 skill 的 SKILL.md 描述里有「这个工具会调用 API」触发了 SDI 报警，但你知道这是你内部框架的约定写法。
...
**drift-tolerant**：即使 skill 行号变了、文字改写了，规则还能继续工作。
...
这个设计把 SkillSpector 从「CI 里的一个 step」升级成了「agent 的安全 sensor」。

在 skill 生态失控之前，先把守门人放在最有效的位置——**每次 install 调用都过一道**。
...
如果你在写 skill 并打算发到社区，发布前必须过一道 SkillSpector 扫描。看 SARIF 报告，把所有 HIGH 和 CRITICAL finding 处理掉（改写代码、移除危险模式、固定依赖版本）。
...
**操作**：`pip install "skillspector[mcp]"` 跑静态扫描

**收益**：零成本，所有 22 个分析器里除 3 个 semantic 之外全部跑。几分钟就能在本地看到 SARIF 报告。

### 第二步：CI 里跑 baseline

**操作**：CI 里跑 `skillspector scan --baseline`

**收益**：把第一次扫描的 finding 写成 baseline，提交到 git。之后 CI 跑 `--baseline`，只在新 finding 出现时报警。这是「持续监测」的标准动作。

### 第三步：接入 LLM 语义分析

**操作**：配 `SKILLSPECTOR_PROVIDER` + 凭证

**注意**：开 LLM 后会发现 baseline 里有大量 SQP/SDI 类 finding（写作品质、声明一致性），这些都是 LLM 容易误报的——把它们逐条 triage 后写进 baseline。

### 第四步：MCP server runtime gating

**操作**：把 SkillSpector 装成 MCP server，注册到 Claude Code / Codex CLI

**收益**：从此 agent install skill 的流程里多一道自动 gate。这一步收益最大，但需要 agent runtime 支持 MCP（目前主流的几个都支持）。
...
不能。SkillSpector 覆盖的是已知漏洞模式（68 个），但新型攻击、业务逻辑漏洞、权限设计的合理性判断仍然需要人工审查。

**正确的用法**：把 SkillSpector 当成**第一道自动门**，过滤掉 80% 的低级问题，让人专注于剩下的 20%。

### 误报率高吗？如何处理？

静态分析器（特别是 prompt_injection、anti_refusal 类）有一定误报率。
...
**保守做法**：不确定时，把 finding 写进 baseline（标注 `reason`），而不是直接忽略。
