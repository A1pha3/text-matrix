...
作为一款 Public Beta 产品，Yazi 已经可以作为日常主力工具使用。核心设计：所有 I/O 操作均为异步执行，CPU 任务分散到多个线程。
...
Yazi 的差异化在于**全异步架构 + 内置图片预览**。相比 ranger（Python，同步 I/O）和 lf（Go，图片预览依赖外部工具），Yazi 在大目录浏览和图片预览场景下体验差异明显。Lua 插件系统降低了扩展门槛，但生态还在早期（60+ 官方插件 vs ranger 的 Python 生态）。

如果你日常在终端里管理文件，且终端支持 Kitty/Sixel 图片协议，Yazi 值得切换。如果只是偶尔用 `ls` + `cd`，切换的收益有限。
