...
Yazi 值得拆开看的地方在于它把三件原本互相牵制的工程目标压进了同一个二进制：全异步 I/O、内置图片预览、Lua 插件系统。ranger 想预览图片要靠 w3m、Überzug 这类外部进程，lf 也得借助 chafa 或 Überzug++；Yazi 直接在 Rust 里解码 PNG/JPEG/GIF/WebP，再通过 kitty、Sixel、iTerm2 等 10 余种终端协议把像素写回终端。终端支持就开箱预览，不必再装一堆辅助工具。
...
`IoWorker` 把文件读取和目录列表都走 `tokio::fs`，而不是 `std::fs`——后者会阻塞 Tokio 运行时，等于把整个 UI 卡住。这层抽象的代价是所有 I/O 调用都要写成 `.await`，代码可读性略降，但换来的是 UI 线程永远不会因为某个文件操作被挂起。
...
异步 I/O 解决了"不阻塞"，但文件管理器还需要决定"先做哪个"。Yazi 的任务调度器维护一个优先级队列：
...
调度优先级大致是：UI 任务 > 文件操作 > 后台任务。这个顺序的逻辑是——UI 卡顿用户立刻能感知，文件操作慢一点用户可以等状态栏更新，后台任务（如预加载缩略图）最不紧急。任务取消同样关键：快速滚动文件列表时，前一个文件的预览任务如果不取消，会堆积成大量无用的 I/O，反而拖慢当前可见文件的预览。
...
内置支持格式：PNG、JPEG、GIF、WebP、SVG、BMP、ICO 等。LRU 缓存的作用是——同一个文件来回滚动时不必重复解码，缓存命中直接取缩略图。

把图片预览做进核心，代价是 Yazi 必须自己维护一整套解码和协议适配代码，体积和复杂度都比"调外部工具"高。收益是用户不必再为图片预览装一堆辅助工具，配置成本接近零。原本散落在用户侧的协议适配工作被收进了工具本身——这是 Yazi 做的一个明确取舍，也是它和 ranger、lf 在工程取向上的分水岭。

## 为什么插件系统选 Lua

Yazi 的插件用 Lua 5.5 编写。这个选择背后有几层考量。

WASM 听起来更现代，但 WASM 运行时嵌入 Rust 的成本不低，且 WASM 模块需要工具链编译，对插件作者门槛高——写一个插件要先装 Rust 工具链、配 wasm-pack、编译成 `.wasm` 文件再放进插件目录。Python 嵌入成本更高，还要带一个解释器运行时，且 Python 的 GIL 会和 Yazi 的异步模型冲突。Lua 的优势在于：解释器小（几百 KB）、嵌入 Rust 的绑定成熟（mlua）、插件作者不需要编译步骤，写完 `.lua` 文件直接生效。这种"改完即用"的开发体验对插件生态的早期增长很关键。
...
异步 I/O 解决了本地文件操作的并发问题，但 Yazi 还想把远程文件也纳入同一个界面。虚拟文件系统（VFS）做的是这件事——把不同来源的文件统一成同一套目录操作接口：
...
这八步里，异步 I/O 负责不阻塞，任务调度负责优先级和取消，图片预览负责解码和协议适配，Pub/Sub 负责跨标签页状态同步。任何一条退回同步模型，体验都会塌——要么 UI 卡，要么预览慢，要么复制阻塞浏览。
...
Yazi 的取舍是：用 Rust 换性能和内存安全，用内置图片预览换零配置体验，用 Lua 换插件门槛和扩展能力的平衡。代价是 Rust 学习曲线陡，插件生态比 ranger 小，且 Lua 沙箱不如 WASM 严格。

这三者之间不是替代关系。如果你已经在 ranger 上有一套 Python 插件工作流，且不依赖图片预览，没有强理由切换。如果你日常需要预览图片、在大目录里频繁切换、且终端支持 kitty 或 Sixel，Yazi 的体验差异是可感知的。
