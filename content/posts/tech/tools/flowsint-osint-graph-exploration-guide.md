...
很多 enrichers 依赖外部 API、Docker 工具或公开服务；而且项目明确禁止未经授权的入侵、监视、骚扰、doxxing、政治操纵和违反隐私法规的使用方式。

## 三、从 0 到 1：15 分钟看懂用户工作流

第一次接触 Flowsint，不必先研究源码，先把“用户如何使用它”搞明白。
...
pivot 就是“调查中的一个可复用动作”。

### 4.4 Enricher：高层业务逻辑

Enricher 是一个 **知道输入类型、输出类型、图关系、参数和日志** 的高层工作流单元，不是简单的 API 调用器。
...
Tool 关心“怎么拿数据”，Enricher 关心“拿到数据后怎么落成调查结果”。

### 4.6 Flow：多个 enrichers 的链式编排

Flows 文档给出的定义非常直接：**Flow 就是多个 enrichers 的链式连接，前一个输出作为后一个输入**。

Flowsint 不仅做“一跳 enrich”，还支持把调查过程沉淀成可复用的 pipeline。
...
因为让 Flowsint 与普通 OSINT 工具拉开差距的是 **类型系统 + 图编排 + 可扩展的 enrichers**，界面只是入口。
...
Flowsint 的类型写完后要进入一整套发现与暴露机制，不是“写完类就结束”。
...
第一次看 Flowsint 容易问：“既然有 enricher，为什么还要 Tool？”
...
做一个“真正像 Flowsint 风格”的扩展，建议按下面顺序走：
...
Flowsint 天生适合“把碎片信息连成关系图”，这种能力既可以用于合法调查，也可能被滥用。项目把伦理边界写在最前面，是在提醒你：
...
检查下面 5 个问题，确认自己是否已经理解了 Flowsint：
...
**把多类查询统一纳入图模型，并支持持续扩展与复用**，不是“查询能力绝对更强”。
...
**它试图用类型系统、图模型和可扩展编排，把调查过程产品化，而不是简单地把 OSINT 能力堆进一个前端。**

它最有价值的地方是下面这套组合拳，而不是某个单独 enricher：
...
只“临时查一个域名”的话它可能偏重；但需要持续积累、反复验证、多人协作的调查工作，Flowsint 的设计非常值得认真研究。
