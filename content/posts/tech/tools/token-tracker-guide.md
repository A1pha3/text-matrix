...
source_key: "gh:vibe-cafe/vibe-usage"
aliases:
...
Vibe Usage（仓库名是 vibe-usage）已经把“从多个本地日志中抓 Token 数据，再同步到仪表板”这件事做成了现成产品。它的价值在“把多工具解析、同步、后台服务和展示面板都包好了”，不是“会不会读日志”。
...
因此，设计本地 Tracker 时，第一原则是“先确认这个工具到底把数据存成了什么”，不是“先写一个 JSONL parser”。

### 2.2 为什么本文选 OpenClaw 当教学样本

OpenClaw 适合作为第一站，是因为它具备三个教学优势：
...
对 OpenClaw 这类日志，最核心的判断是“这是不是带 usage 的 assistant message”，不是“这一行是不是 message”。一个足够说明问题的简化样本如下：
...
用户消息的成本通常已经体现在随后那条 assistant 消息的 `input` 字段里。

### 3.2 归一化字段是多工具统计的起点

不同工具、不同 provider、甚至同一工具的不同版本，字段名都可能不一样。要做的是把它们归一到你自己的 canonical schema，而不是记住某一种命名。
...
下面这段脚本假设文件名是 `token-tracker.mjs`。它是能直接运行的最小版本，不是伪代码。
...
该复用的是解析完成后的统一条目结构，不是某个工具的路径。一个够用的 canonical entry 可以长这样：
...
Claude Code 就是一个很好的反例。它不仅有 `projects` 目录，还会把会话转录写到 `transcripts`；前者更适合提 Token，后者更适合补会话元数据。所以多工具支持的关键是“搞清这个工具把什么信息拆到了哪里”，不是“多扫几个目录”。
...
本地 Tracker 的强项是观察趋势，不是给财务做最终对账。出现差异时，通常是统计对象本来就不同，不是代码错了。常见原因包括：
...
1. 纯本地 Token Tracker 最重要的是统计口径和事实锚点，不是界面。
