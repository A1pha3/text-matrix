...
### 7.5 退出码：脚本与 CI 里最该先确认的事实

`rg` 的退出码只有三档，脚本判断命中和失败都靠它：

| 退出码 | 含义 |
| ------ | ------ |
| `0` | 至少命中一处 |
| `1` | 未命中，且没有出错 |
| `2` | 发生错误（例如文件无法访问），除非配合 `--no-messages` |

Shell 里的 `set -e`、`if rg ...; then ...` 以及 CI 步骤判定都以这三档为准。`1` 表示"没找到"，不是"出错"；把 `rg foo || exit 1` 的语义想反是脚本里常见的坑。

配合脚本还有两个容易写反的命令，注意区分"行"与"文件"：

- `rg -v 'foo' file`：输出文件中**不含** `foo` 的行；
- `rg --files-without-match 'foo'`：列出**整个文件都没有** `foo` 的文件名。

前者是行级取反，后者是文件级取反。help 里专门提醒过：它的结果可能跟 `-l`（列出含命中文件）不一致，写脚本时别混用，也别拿 `-v` 当"列出未命中的文件"用。
...
### 12.8 误区八：`-L` 表示"列出未命中的文件"

错误。

- `-L` / `--follow` 的真实含义是**跟随符号链接**，默认不跟随；
- 要列出"没有任何命中的文件"，应该用 `--files-without-match`（它没有 `-l` 那种短别名）；
- 顺带分清两个取反：`-v` 取反的是**行**，`--files-without-match` 取反的是**文件**。
...
7. 退出码 `0/1/2` 对应命中 / 未命中 / 出错，脚本判断靠它；`-v` 取反行，`--files-without-match` 取反文件。
