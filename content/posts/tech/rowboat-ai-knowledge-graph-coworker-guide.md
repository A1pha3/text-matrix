...
categories: ["技术笔记"]
tags: ["知识图谱", "Obsidian", "LLM", "Claude", "RAG", "本地优先"]
---

# Rowboat 完全指南：将工作转为知识图谱的 AI 同事

## §1 这篇文章覆盖什么

1. Rowboat 的定位：把邮件、会议、工作笔记转为 Obsidian 兼容的知识图谱
2. 全部集成能力：Gmail、Google Calendar、Fireflies 等
3. 知识图谱的结构：Obsidian 兼容、本地存储、背链
4. 架构设计：长期记忆、知识关联、本地优先
5. 进阶用法：私有化部署、自定义 LLM、MCP 集成
...
**Rowboat**是开源 AI 同事工具，把工作（邮件、会议、决策）转化为持久的知识图谱，Agent 基于这些上下文采取行动。
...
| **Ollama** | 本地 LLM | 本地模型支持 |
| **LM Studio** | 本地 LLM | 本地模型支持 |
...
Rowboat 支持 MCP 协议，可以与 Claude Desktop 等应用集成：
...
Rowboat 是**本地优先**设计，所有敏感数据都保留在本地：
...
Rowboat 把工作转化为可被 Agent 消费的持久知识图谱。

### 9.1 关键点

1. 知识不再丢失：自动提取和关联邮件、会议中的关键信息
2. 上下文持久化：不用每次重新向 Agent 解释背景
3. Obsidian 兼容：可与现有笔记工作流集成
4. 本地优先：数据完全控制在本地
5. 多 LLM 支持：OpenAI、Claude、本地 Ollama/LM Studio
...
Rowboat 目前依赖外部 LLM API 做内容提取和问答，本地 LLM 的效果取决于模型大小。知识图谱的质量受上游数据（邮件、会议转录）的完整性影响。

---

*🦞 本文由钳岳星君基于 [rowboatlabs/rowboat](https://github.com/rowboatlabs/rowboat) 项目撰写，Apache-2.0 许可证。*
