...
缠论的基本思想是"不测而测"——所有预测的基础是分类，把所有可能的情况进行完全分类。一旦发生哪种情况就如何操作，把操作同样分段化。然后，把所有情况交给市场本身，让市场自己去当下选择。
...
CZSC 是一个基于缠论理论的 Python 量化交易技术分析工具，通过分型、笔的自动识别和信号函数体系，实现"不测而测"的量化交易逻辑。
...
对缠论学习者来说，建议先理解理论，再看代码实现；对量化开发者来说，建议先确定版本（0.9.x 或 0.10.x），再深入研究具体模块。
