...
source_key: "gh:OpenBB-finance/OpenBB"
aliases:
...
> **关键问题**：OpenBB 是什么？如何设计架构？如何集成到自己的应用？
> **难度**：⭐⭐⭐⭐（专家设计）
> **预计阅读时间**：40 分钟
...
3. **优势：开源免费、模块化架构、MCP 原生支持、Python/CLI/Terminal/Workspace 多端消费。**
...
- OpenBB 提供的是数据访问，不是交易决策
- 数据有来源和质量限制，不是所有数据都同等完整
- MCP 支持是原生集成，但不是所有 AI 框架都官方支持
...
OpenBB 是一个开源金融数据平台，通过"连接一次，到处使用"的设计思路，把散落的金融数据源统一成标准化接口，暴露给 Python/CLI/Terminal/Workspace/MCP/REST API 等多种消费端。
