...
**问题不在 AI 不够聪明，在它缺乏一套强制执行的流程约束。**

9arm-skills（GitHub: [thananon/9arm-skills](https://github.com/thananon/9arm-skills)，⭐ ~960，Fork ~137）就是为解决这个问题设计的。作者 Thananon Patinyasakdikul（Arm）把项目里最常用、最需要纪律保障的四件事——调试、复盘、代码审查、向上沟通——分别编码成四个技能（Skill）。每个技能是一份**规定了触发时机、执行顺序和退出条件的可执行合约**，不是「你可以参考的建议」。
...
四个技能在这个流程中各自在正确的时机给出了 AI 在没有约束时不会做出的行为：拒绝（不满足复现就不推进）、保留（代码标识符不删）、诚实标注（只测了一个配置就只说一个配置）。
...
9arm-skills 做的事用一个词概括：「可预期」。当你能预期 AI 在调试时先逼你拿到复现、在复盘时拒绝给未验证的 bug 写文档、在审查时先怀疑改动本身的必要性——这些行为就不再是「AI 碰巧做对了」，而是你提前设计好的流程在执行。
