...
读完你会理解：
- 为什么把工程流程写成 UI 界面的「提示词模板」不够用，而写成 Agent 的「可执行约束」是另一个思路
- 9arm-skills 里 4 个技能各自管什么、在什么时机触发、拒绝在什么条件下执行
- 这套设计的工程哲学——以及它不适用于哪些场景

**前置知识**：需要知道 Claude Code 的「自定义指令」功能，以及 AI 编程助手的基本工作方式。如果你用过 Claude Code 的 `CLAUDE.md` 或 `.claude/skills` 目录，理解起来会更顺畅。

**文章结构**：先从你熟悉的日常场景切入 → 给一张四技能的职责总览图 → 逐一拆解每个技能的设计 → 用一个完整案例串起来 → 再退一步看设计逻辑 → 最后给团队采用建议。如果你对某个技能已经了解，可以直接跳到[完整流转案例](#一个完整的流转案例)看它们怎么配合。
...
它说得没错。函数确实可以抽。但你的团队有自己的一套做法：哪些逻辑该抽、抽到哪个模块、命名前缀跟什么对齐——这些隐性知识，通用 AI 模型不知道。它给的是一句不会错但也不太有用的建议。

再把场景放大一点。你刚修了一个折磨两天的 bug，想让 AI 帮你写复盘文档。它洋洋洒洒给你三段话，看起来结构完整，但仔细一读：没有复现步骤，没有根因追溯链，没有验证方案——它写的是一篇「叙事散文」，不是一份工程师之间传递判断的工程记录。

**这不是 AI 不够聪明。是它缺乏一套强制执行的流程约束。**

9arm-skills（GitHub: [thananon/9arm-skills](https://github.com/thananon/9arm-skills)，⭐ ~960，Fork ~137）就是为解决这个问题设计的。作者 Thananon Patinyasakdikul（Arm）把项目里最常用、最需要纪律保障的四件事——调试、复盘、代码审查、向上沟通——分别编码成四个技能（Skill）。每个技能不是一个「你可以参考的建议」，而是一份**规定了触发时机、执行顺序和退出条件的可执行合约**。

---

## 一张图看懂四技能的分工

在深入每个技能之前，先看清它们各自的职责边界。四个技能覆盖了两条主线——一条面向工程执行（工程师对工程师），一条面向信息传递（工程师对组织）。
...
两条线有明确的交接点：`post-mortem` 产出的是面向工程师的工程真相，如果你需要给 VP 或 PM 看，把这份产出交给 `management-talk` ——它负责把函数名、文件路径、commit SHA 翻译成领导层能用来做决策的语言。两个技能各有自己的「不做什么」的边界。

---

## 四个技能的完整拆解

上面是地图。下面逐一展开每个技能「管什么、怎么执行、在什么条件下拒绝执行」。
...
1. **复现（Reproduce reliably）** — 在提出任何修复假设之前，必须拿到一个可运行的复现脚本。如果是偶然性 bug（flaky），先把复现率从 1% 提到 50% 以上——循环触发、加并发压力、注入 sleep 缩小时间窗口。50% 的 flaky 可以调试，1% 的不行。**完全没有复现 → 停下来，明确告知用户，不准跳到假设阶段。**
...
**关键约束**：除非用户明确说「跳过口诀」，AI 必须在第一条回复里逐字背诵四步口诀，然后按序执行。这不是建议——这是技能强行加在 AI 行为上的闸门。
...
上面是设计。下面用一个真实场景演示四个技能怎么配合——假设你接手了一个 GPU 训练挂起的 bug。

**1. debug-mantra 接管调试 session**

CI 报 `Tada` 通信库在 8-GPU LLM-7B 微调时 eval 阶段永久挂起，无报错、无超时——busy-spin 在 `tadaKernel_AllReduce_f32_RING`。
...
- 症状与根因的因果链：挂起在 ring waitloop（调用栈最后一帧），实际 bug 在 launch-prep（几帧之前）。跳过的同步是静默的，直到 dumbModel 的 reduce-scatter pattern 在每次 eval step 精确触发门控
...
四个技能在这个流程中不是「锦上添花」——它们各自在正确的时机给出了 AI 在没有约束时不会做出的事：拒绝（不满足复现就不推进）、保留（代码标识符不删）、诚实标注（只测了一个配置就只说一个配置）。

---

## 这套设计的工程逻辑

拆解完技能，再退一步看为什么这套设计管用。下面几条不是贴在项目 README 里的「设计哲学」，而是从实际 SKILL.md 文件中能读到的决策。

### 约束先于知识

通用 AI 编程助手的默认行为是「给出最可能有帮助的回答」。这在问答场景里没问题，但在工程流程场景里有问题——它会在证据不足时写复盘、在复现不完备时提修复、在没读全链路时给审查意见。

9arm-skills 的做法是把**拒绝条件**写进触发逻辑里。`debug-mantra` 的退出条件是「没有可靠复现就停下来」。`post-mortem` 的退出条件是「四个必要输入缺一不可」。`scrutinize` 的 Step 1 是「先问这个改动该不该存在」。
...
两份文档各自的版本就是各自的真相。复盘文档里的 `tadaLaunchPrepare` 是六个月后 `git log --grep` 的回溯点；管理层 Slack 里的「skipped synchronization in the comms fast-path」是 VP 在站会上向 PM 转述的一句判断。两个版本互不为对方妥协。
...
软链接安装脚本（`link-skills.sh`）只链接 shippable 目录下的技能，`deprecated/`、`in-progress/`、`personal/` 全部跳过。这意味着草稿不会污染可用技能列表，废弃有正式的退出通道——你可以放心在 `in-progress/` 里试探新流程，而不必担心它被 AI 意外触发。
...
回到最开始的问题：为什么 AI 给你的建议「听起来对但就是差点意思」？

答案是它缺一组约束。一组告诉它在什么时候执行哪个流程、在什么条件下停下来的约束。

9arm-skills 做的事如果用一个词概括，不是「更聪明」，是**可预期**。当你能预期 AI 在调试时先逼你拿到复现、在复盘时拒绝给未验证的 bug 写文档、在审查时先怀疑改动本身的必要性——这些行为就不再是「AI 碰巧做对了」，而是你提前设计好的流程在执行。

在一个团队每天跟 AI pair programming 的环境里，把流程写成约束而不是建议，可能是让 AI 从「通用工具」变成「团队工具」最朴素也最有效的一种方式。

*读完可以问自己两个问题：你的调试流程如果写成一份 AI 必须遵守的合约，第一行会是什么？你的团队现在最缺纪律保障的不是「写代码」，而是修完 bug 之后的哪一步？*
