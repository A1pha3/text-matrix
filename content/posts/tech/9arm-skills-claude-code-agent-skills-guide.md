...
下面三个练习从观察行为到动手改写，覆盖 9arm-skills 的核心使用路径。
...
用"能/不能"回答下面 8 题。答"不能"的条目就是该回去重读的章节。

| # | 自测项 | 答"不能"时回看 |
|---|--------|---------------|
| 1 | 能说出 debug-mantra 四步口诀的顺序和每一步的退出条件 | [debug-mantra](#debug-mantra调试四念处) |
| 2 | 能列出 post-mortem 的四个必要输入条件，以及缺任意一个时系统的行为 | [post-mortem](#post-mortem工程复盘的及格线) |
| 3 | 能解释 scrutinize 为什么把意图放在追踪之前——这一顺序要避免的是哪类浪费 | [scrutinize](#scrutinize外部视角的端到端审查) |
| 4 | 能说出 management-talk 保留什么、删除什么、翻译什么，以及这三类操作各自的原因 | [management-talk](#management-talk工程事实的语境转换) |
| 5 | 能描述四个技能在 GPU 挂起 bug 场景中的完整配合流程 | [一个完整的流转案例](#四一个完整的流转案例) |
| 6 | 能解释 9arm-skills 目录分层如何防止草稿技能被意外触发 | [实验空间与发布门槛分治](#实验空间与发布门槛分治) |
| 7 | 能根据团队情况判断是否适合采用 9arm-skills，并给出采用顺序 | [适用与不适用](#七适用与不适用) |
| 8 | 能基于现有技能文件自定义一个团队专属版本 | [动手练习](#十动手练习) |
...
2. **把 debug-mantra 的四步口诀改写成你团队的调试纪律**：9arm-skills 的四个技能是作者 Arm 的个人实践，不代表通用真理。如果你的团队靠 bisect 定位而不是调试器，把"追踪失败路径"里的三条路径顺序改成 bisect 优先。
