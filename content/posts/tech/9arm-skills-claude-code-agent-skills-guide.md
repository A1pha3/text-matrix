...
- [五、没有 9arm-skills 时 AI 会怎么表现](#五没有-9arm-skills-时-ai-会怎么表现)
- [六、这套设计的工程逻辑](#六这套设计的工程逻辑)
- [七、适用与不适用](#七适用与不适用)
- [八、局限性](#八局限性)
- [九、自测问题](#九自测问题)
- [十、收束](#十收束)
...
通用模型默认给「最可能有帮助的回答」，但工程流程需要的是「在证据不足时拒绝回答」。这两种倾向之间的差距，就是 9arm-skills 想填的。

9arm-skills（GitHub: [thananon/9arm-skills](https://github.com/thananon/9arm-skills)，⭐ ~960，Fork ~137（截至 2026-05 撰写时））就是为解决这个问题设计的。作者 Thananon Patinyasakdikul（Arm）把项目里最常用、最需要纪律保障的四件事——调试、复盘、代码审查、向上沟通——分别编码成四个技能（Skill）。每个技能是一份**规定了触发时机、执行顺序和退出条件的可执行合约**，不是「你可以参考的建议」。
...
上面是设计。下面用一个虚构场景演示四个技能怎么配合——假设你接手了一个 GPU 训练挂起的 bug（虚构）。
...
四个技能在这个流程中各自在正确的时机给出了 AI 在没有约束时不会做出的行为：debug-mantra 在没有可靠复现时拒绝推进，post-mortem 保留所有代码标识符不删。

---

## 五、没有 9arm-skills 时 AI 会怎么表现
...
上面这些差异来自约束的缺失——AI 做了它默认会做的事，但工程流程需要的是在证据不足时停下来。

---

## 六、这套设计的工程逻辑

拆解完技能，再退一步看这套设计为什么管用。下面几条直接来自 `SKILL.md` 文件里的工程决策，不是抽象原则。
...
1. 四个技能（debug-mantra、post-mortem、scrutinize、management-talk）分别约束了什么？各自的退出条件是什么？
2. bug-fix-flow 中 reproduce 的输出被谁消费？如果跳过复现直接进入修复假设，后续哪个技能会拒绝继续？
3. post-mortem 保留代码标识符（函数名、文件路径、commit SHA）的原因是什么？management-talk 又为什么要把这些标识符删掉？两份文档各自的受众是谁？
4. scrutinize 的四步 workflow 为什么把「意图」放在「追踪」之前？如果先做追踪再做意图判断，会漏掉哪类问题？
5. 9arm-skills 的目录分层（`engineering/`、`in-progress/`、`deprecated/`、`personal/`）如何防止草稿技能被 AI 意外触发？

---

## 十、收束

回到最开始的问题：AI 给你的建议「听起来对但就是差点意思」，缺的是一组约束——告诉它在什么时候执行哪个流程、在什么条件下停下来。

9arm-skills 把约束写成了可执行的合约。修完 bug 让 AI 起草复盘时，它会先列出缺哪些必要输入——这种拒绝行为比让它直接写一篇流畅但无法验证的文档有用得多。调试、复盘、审查三个环节都有类似的约束在起作用。

9arm-skills 的价值在于把团队隐性知识写成 AI 必须遵守的退出条件——这比让 AI 自由发挥更适合有既定流程的团队。

如果打算上手，建议先做一件事：把你团队现在最没纪律保障的那个环节（多半是修完 bug 到写复盘之间）写成一份 AI 必须遵守的合约，第一行写退出条件——「没有可靠复现就不起草」。
