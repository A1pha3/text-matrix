...
读完你会理解：
- 把工程流程写成 UI 界面的「提示词模板」和写成 Agent 的「可执行约束」——这两种思路的分界线在哪里
- 9arm-skills 里 4 个技能各自管什么、在什么时机触发、拒绝在什么条件下执行
- 这套设计的工程逻辑，以及它不适用于哪些场景
...
拆解完技能，再退一步看这套设计为什么管用。下面几条不是空泛的设计原则，而是从 `SKILL.md` 文件里直接读到的工程决策。
...
它缺的是约束。一组告诉它在什么时候执行哪个流程、在什么条件下停下来的约束。

9arm-skills 做的事用一个词概括，不是「更聪明」，而是「可预期」。当你能预期 AI 在调试时先逼你拿到复现、在复盘时拒绝给未验证的 bug 写文档、在审查时先怀疑改动本身的必要性——这些行为就不再是「AI 碰巧做对了」，而是你提前设计好的流程在执行。

一个团队每天跟 AI pair programming，把流程写成约束而不是建议，可能是让 AI 从通用工具变成团队工具最朴素也最有效的方式。

*合上这篇文章前，可以想两个问题：你的调试流程如果写成一份 AI 必须遵守的合约，第一行会是什么？你的团队现在最缺纪律保障的，不是「写代码」这一步，而是修完 bug 之后的哪一步？*
