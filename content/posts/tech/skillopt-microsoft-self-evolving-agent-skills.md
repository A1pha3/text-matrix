...
如果你正在做闭源大模型上的 Agent，这个思路很关键。很多时候你改不了权重，LoRA 或 SFT 也不现实，真正能持续迭代的适配层只剩下 prompt、tool policy、workflow instruction 这类文本工件。SkillOpt 的贡献，是把这层文本工件第一次当成训练对象，而不是一次性写完的说明书。
...
SkillOpt 要解决的正是这个缺口。它不是让模型“聪明一点”去写提示词，而是把 Skill 更新这件事变成了一个有训练集、验证集、步长、门控、记忆和导出工件的过程。
...
也正是因为这个 gate 存在，SkillOpt 才更像训练，而不是一套 fancy 的 prompt chaining。
...
论文实验里，SkillOpt 对比的并不只是“人工写得够不够好”，还包括几类已经在尝试把执行经验转回文本工件的方法：Human Skill、One-shot LLM Skill、Trace2Skill、TextGrad、GEPA、EvoSkill。

如果把这些方法按“文本如何更新”来放在一张表里，SkillOpt 的位置会更清楚：

| 方法类型 | 代表思路 | 常见短板 | SkillOpt 补上的东西 |
| --- | --- | --- | --- |
| 人工编写 / 一次生成 | 先写一版可用规则 | 起点可能不差，但后续很难系统迭代 | 把后续迭代变成可重复的训练回路 |
| 轨迹蒸馏 | 从执行经验里抽经验回写文本 | 容易偏重“总结了什么”，较少约束“怎么更新” | 加入 edit budget、gate 和 rejected buffer |
| 梯度风格文本优化 | 用语言形式近似“梯度”推动文本更新 | 更新方向有了，但步长和接受条件可能不稳 | 把步长控制、验证门和长期记忆一起补齐 |
| 演化式改写 | 多候选搜索、反思和筛选 | 容易把重点放在搜索候选，而不是稳定训练单一工件 | 围绕单份 Skill 文档做连续、受限的更新 |

这里最关键的区别不是“别人不会改文本，SkillOpt 会改”，而是**SkillOpt 把文本更新这件事做成了类似训练器的形状**：有 rollout 证据、有受限步长、有 held-out gate、有失败记忆，还有跨 epoch 的慢更新。

所以更准确地说，SkillOpt 的创新点不是“第一次有人想到改 Skill”，而是**第一次把 Skill 更新组织成一套可控、可验证、可导出的训练过程**。
