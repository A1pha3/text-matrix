...
**Heretic**（[GitHub](https://github.com/p-e-w/heretic)）把事情反过来：它把上述决策全部交给 Optuna 的多目标 TPE（Tree-structured Parzen Estimator）采样器，在"拒绝次数"和"KL 散度"两个维度上并行搜索 Pareto 前沿。用户只需要指定模型 ID，不需要理解 Transformer 内部机制。
