...
把它看成「AI 工头」比看成「AI 员工」更准确：它不亲自执行每个任务，而是把复杂问题拆解，分派给一群子智能体，用共享记忆和消息总线协调它们，最后汇总结果。看一座智能体框架的成色，关键是三条线索——它怎么在子智能体之间切分边界、怎么跨子智能体传递状态、又在哪里切断外部代码带来的风险。下文对 DeerFlow 的拆解就沿着这三条线走。
...
这组结论测的是**工具调用和多轮编排**这一路，不是模型在聊天、写作等通用任务上的强弱。DeerFlow 的瓶颈通常落在「子智能体能否按格式把参数塞进工具、长链路里意图会不会漂」，所以模型差异在这里比在单轮问答里更明显。它不能推出「某某模型在所有场景都更强」，也不能替代你在自己任务上跑一次小样本对比——尤其是公司内网或私有 API 这类来源差异很大的场景。
...
部署完成不等于配置正确。下面七项检查覆盖从服务健康、沙箱、模型连通到记忆持久化的完整链路，每项都给出预期输出，逐条跑一遍再验收。
