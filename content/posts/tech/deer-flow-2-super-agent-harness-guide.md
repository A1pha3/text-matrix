...
**场景**：每次 Pull Request 提交后，自动运行 DeerFlow 进行代码审查，检查安全漏洞、代码质量和最佳实践合规性。
...
可以。DeerFlow 的组件是松耦合的。你在 `deerflow.yaml` 中配置 `sandbox.mode: local` 就能跳过沙箱隔离。同样，不配置可观测性后端也不会影响核心功能。
