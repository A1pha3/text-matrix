...
三端共享同一套卷格式，但系统级能力并不完全等价：

| 平台 | 形态 | 主要能力 |
|------|------|---------|
| Windows | 安装器 + 驱动（64 位 `.sys` 已签名） | 系统盘加密 / 非系统盘加密 / 隐藏卷 / 隐藏 OS（系统级能力最全） |
| macOS | `.dmg` 安装器 | 非系统盘加密 / 隐藏卷；系统盘加密靠 FileVault，二者并存时需专门配置 |
| Linux | 命令行工具（CLI）+ 图形用户界面（GUI） | 非系统盘加密 / 隐藏卷；dm-crypt 集成 / 任意映射设备，系统盘加密走 LUKS |
...
- 所有读写都经过 AES、Serpent、Twofish（三种都是对称分组密码），或它们的级联组合。级联最多叠三种算法，这一点 TrueCrypt 本身就有；VeraCrypt 的增量是新增 Camellia、Kuznyechik 两种算法，以及它们参与的若干级联组合；
