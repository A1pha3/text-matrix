...
source_key: "gh:JoeanAmier/TikTokDownloader"
...
1. 理解 TikTokDownloader 的两条采集主线（抖音线 vs TikTok 线）的能力边界和差异
2. 独立配置 Cookie 并跑通「终端交互模式」的批量下载流程
3. 使用 Web API 模式将下载能力集成到自己的脚本或定时任务中
4. 根据实际场景（偶尔下载 / 定时采集 / 不想配 Python 环境）选对使用方式
5. 排查常见问题（Cookie 过期、代理配置、并发限制）

## 目录

1. [这篇文章要解决什么](#一这篇文章要解决什么)
2. [系统地图：抖音线 vs TikTok 线](#二系统地图抖音线-vs-tiktok-线)
3. [一次完整的批量下载怎么样流过系统](#三一次完整的批量下载怎么样流过系统)
4. [技术架构](#四技术架构)
5. [安装](#五安装)
6. [使用](#六使用)
7. [API 参考](#七api-参考)
...
1. **抖音线和 TikTok 线在评论采集、搜索/热榜能力上有什么差异？**
   ...
   抖音线支持评论采集、搜索和热榜；TikTok 线不支持评论采集、搜索和热榜。这是因为抖音和 TikTok 的 API 端点不同，TikTok 国际版对这些功能的接口访问有限制。
   </details>
...
2. **TikTok 下载一直超时，应该检查哪个配置项？**
   ...
   应该检查 `settings.json` 中的 `network.proxy` 配置项。国内访问 TikTok 必须配置代理，否则会超时。
   </details>
...
3. **Web API 模式下，批量下载一个账号的全部发布作品，大致流程是什么？**
   ...
   流程：
   1. 启动 API 服务（选择 Web API 模式）
   2. 调用 `/douyin/user` 获取账号作品列表
   3. 遍历作品列表
   4. 对每个作品调用 `/douyin/video` 下载
   5. 可选：将脚本挂到 cron 实现定时自动下载
   </details>
...
5. **三种使用方式（终端交互、Web API、Docker）分别适合什么场景？**

   <details>
   <summary>点击查看参考答案</summary>
   
   - **终端交互**：偶尔下载几个视频，不想写代码
   - **Web API**：需要集成到自己的脚本或定时任务，批量下载 + 后续自动化处理
   - **Docker**：不想装 Python 和 FFmpeg，快速体验
   </details>

## 练习

### 练习 1：配置 Cookie 并下载单个视频

**任务**：配置抖音 Cookie，然后用终端交互模式下载一个视频。

**步骤**：
1. 浏览器登录抖音网页版
2. 打开开发者工具，复制 Cookie
3. 启动 TikTokDownloader，选择"从剪贴板读取 Cookie"
4. 选择终端交互模式，下载一个视频
5. 验证下载的视频文件是否清晰

### 练习 2：使用 Web API 模式下载

**任务**：启动 Web API 服务，然后用 curl 调用 API 下载视频。

**步骤**：
1. 启动 TikTokDownloader，选择 Web API 模式
2. 访问 `http://127.0.0.1:5555/docs` 查看 API 文档
3. 用 curl 调用 `/douyin/video` 下载视频
4. 验证下载成功

### 练习 3：配置代理并下载 TikTok 视频

**任务**：配置代理，然后下载一个 TikTok 视频。

**步骤**：
1. 编辑 `settings.json`，配置 `network.proxy`
2. 配置 TikTok Cookie
3. 尝试下载 TikTok 视频
4. 验证代理配置正确
...
1. **入门**：配置 Cookie，用终端交互模式下载第一个视频，理解抖音线和 TikTok 线的差异。
2. **应用**：用 Web API 模式下载一个账号的全部作品，理解 API 调用流程。
3. **自动化**：编写脚本定时自动下载账号新作品，理解 Cookie 更新机制。
4. **扩展**：研究 TikTokDownloader 源码，理解插件架构和 httpx 异步请求机制。
5. **深入**：尝试为 TikTokDownloader 添加新功能（如支持新的社交媒体平台）。

## 资料口径说明

1. **信息来源**：本文基于 TikTokDownloader GitHub 仓库 README、官方文档和作者实际使用经验编写。
2. **技术细节**：Cookie 配置步骤和代理配置已在 macOS 环境验证，其他平台可能略有差异。
3. **局限性**：本文未覆盖 TikTokDownloader 的所有高级功能（如 SQLite 数据库、PyInstaller 打包）；未讨论 TikTokDownloader 在法律敏感场景（如版权内容下载）中的合规性。
4. **更新记录**：本文于 2026-06-30 使用 `cn-doc-writer` skill 优化，添加了学习目标、目录、自测题、练习、进阶路线、资料口径说明等教育元素。
