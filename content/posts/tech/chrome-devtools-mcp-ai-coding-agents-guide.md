...
1. 理解 MCP 协议如何与 Puppeteer 衔接，以及 CDP 在这一链路中的角色
2. 列出 `chrome-devtools-mcp` 提供的所有工具函数，并说明每个函数的输入输出语义
3. 在 Node.js v20.19+ 环境下安装、启动并验证 `chrome-devtools-mcp`
4. 将 MCP server 配置到 Claude Desktop（或其他 MCP 客户端），完成一次完整的浏览器自动化任务
5. 了解该项目的权限边界，如何在开发分支上添加自定义工具函数，以及如何控制 AI 的浏览器操作权限
...
1. **基础练习**：按照本文 `§5 使用说明` 的步骤，安装 `chrome-devtools-mcp` 并配置到 Claude Desktop，完成一次简单的浏览器截图任务。

2. **工具调用练习**：使用 Claude Desktop 驱动浏览器完成以下任务链：
   - 打开 `https://example.com`
   - 获取页面控制台消息（只获取 error 级别）
   - 截取页面全页截图
   - 检查 AI Agent 调用的工具函数顺序

3. **网络监听练习**：开启网络拦截，访问一个包含多个 API 请求的页面，获取所有网络请求列表，并分析哪些请求是 POST 方法。

4. **扩展开发练习**：在 `chrome-devtools-mcp` 源码中添加一个新的工具函数 `get_page_title`，该函数返回当前页面的 title。参考 `§6 开发扩展` 的示例完成。

5. **安全配置练习**：配置 Chrome 启动参数，限制 AI Agent 只能访问 `github.com` 和 `example.com` 两个域名，尝试访问其他域名并观察是否被拦截。

---

## 进阶路径

1. **深入 MCP 协议**：阅读 [MCP 协议规范](https://modelcontextprotocol.io)，理解 JSON-RPC 2.0 消息格式和工具函数注册机制，尝试为其他浏览器自动化工具（如 Playwright）编写 MCP Server。

2. **Puppeteer 高级用法**：学习 Puppeteer 的 CDP 会话管理，访问本文 `§6.3` 提到的未封装 CDP 能力（如 DOMStorage、ServiceWorker），扩展 `chrome-devtools-mcp` 的工具函数。

3. **多 Agent 编排**：研究如何在多 Agent 场景下使用 `chrome-devtools-mcp`，参考本文 `§3.3` 提到的 `--experimentalPageIdRouting` 和 `--isolated` 参数，实现多 Agent 并行浏览器自动化。
