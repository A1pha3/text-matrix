...
如果时间有限，可以先读 §一（核心判断 + 总览图）、§四（任务流案例）和 §七（决策建议），这三节加起来大约 10 分钟。
...
A2UI-Bench 是一个相对新的基准。75.6 分本身不提供绝对参照系，论文的关键比较点是"超越了输入完整 27 倍长度 Schema 的最强前沿模型"。这意味着 Macaron-A2UI 通过 LoRA 训练内化了对 UI Schema 的理解，而不是在推理时依赖超长 prompt 注入格式定义。
...
如果你对第 3 题没有明确答案，建议回看 §3.2 中 MinT 的"模型仓库与推理服务物理分离"的设计原则。基础模型更新时 MinT 相当于"换底座"——所有 Adapter 需要重新对齐，这是 MinT 尚未完全解决的工程难题。
