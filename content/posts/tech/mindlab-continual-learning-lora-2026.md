...
理解这个方向的关键，是把 LoRA 从"大模型全参数后训练的廉价平替"重新定位为"持续学习智能体的核心架构机制"。这两个定位之间的差距，是 Mind Lab 这一系列研究想填的工程鸿沟。
...
**关键洞察：** 仅仅 0.12% 的额外参数，就能让模型在重度记忆基准测试上获得 30% 以上的提升。这意味着"长上下文"的瓶颈不是注意力机制，而是参数层的容量。δ-mem 把"工作记忆"从上下文层下沉到参数层，绕过了 O(n²) 的算力墙。
...
**关键洞察：** MinT 把"模型仓库"和"推理服务"在物理上分离——基础模型是 CPU/GPU 的热工作集，LoRA Adapter 是持久化的策略目录。这种分离让"百万级 LoRA 同时在线"成为可能。
...
多数投票下准确率随模型数量 k 呈现经验上的对数增长关系——这是从三个扩展轴上涌现出来的、基于模型数量的 scaling law。

这一发现把"模型数量"从工程限制变成了 scaling 杠杆。生产部署上百万 LoRA 的难度，不只是工程问题，也是统计问题——多 LoRA 聚合可以提供与单一大模型相当甚至更好的鲁棒性。
...
**关键洞察：** 复杂的 UI 生成能力完全可以通过高效微调被内化到模型权重中。这意味着 UI 生成能力不是"原生能力"，而是"训练出来的小型能力"——这与生成式 UI 的传统思路（用巨型模型 + 复杂 prompt）形成鲜明对比。
...
如果你是 AI 基础设施创业者，2026 年下半年押注 PEFT 不会太早。如果你是大模型公司，把 LoRA 从"工程技巧"提到"基础设施战略"层是当务之急。如果你是投资人，从现在起把"持续学习智能体"和"AGI 路线"分开估值——前者是工程问题，后者是哲学问题，工程问题的回报曲线更可预测。
