...
8. [20 个 Triad：把"领域知识"预编译到成员组合里](#八20-个-triad把领域知识预编译到成员组合里)
9. [benchmark 解读：协议性指标 vs 决策性指标](#九benchmark-解读协议性指标-vs-决策性指标)
...
**Q: 我只有 Claude Code，没有装其他 provider，还能用吗？**
能用。Council 会以 Claude 为兜底，所有成员都跑在 Claude 上。但这样就失去了"跨 provider 强制分流"的意义，极性配对仍在同一家族内，对抗效果会打折扣。
...
1. **Socrates 和 Feynman 的极性配对逻辑是什么？为什么必须跨 provider？**
<details>
...
3. **`execution-lean` 只有 5 个人，为什么反而更适合技术决策？**
<details>
<summary>查看答案</summary>
...
**步骤**：
1. 克隆仓库并安装：`git clone https://github.com/0xNyk/council-of-high-intelligence.git && cd council-of-high-intelligence && ./install.sh`
2. 运行 `/council --dry-route` 查看路由表
3. 确认至少 2 个 provider 可用（例如 Claude + OpenAI）
4. 记录哪些 persona 被路由到了哪个 provider
...
**步骤**：
1. 启动 Claude Code 或 Codex
2. 输入 `/council --duo "我应该选哪个数据库？"`
3. 观察 2 位成员如何互相质疑
4. 查看最终的 `STANCE:` 输出
...
**任务**：
1. 从 18 个 persona 中选择 3 个最适合这个场景的角色
2. 解释为什么选这 3 个（考虑极性配对）
3. 用 `/council --triad <你的triad名称>` 测试你的配置
...
1. **结构优化**：
   - 删除重复的"十、采用顺序与适用边界"章节（原第329-351行）
   - 确保标题层级正确（一、二、三...十二）

2. **教学性增强**：
   - 将"自测检查"改为标准"自测题"格式（使用 `<details>` 标签）
...
| 教学性 | 20/20 | 有学习目标（本文导读）、解释"为什么"、学习元素自然融入、递进合理 |
