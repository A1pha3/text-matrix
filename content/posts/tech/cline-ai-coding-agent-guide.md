...
>
> **核心理念**：让 AI 助手真正成为你的"第二双手"，不仅仅是代码补全，而是能够自主完成整个编程任务。

## 项目概览

Cline 是一个基于 Claude Sonnet agentic coding capabilities 的 AI 编程助手。与传统的代码补全工具不同，Cline 可以帮助你完成整个软件开发任务，包括：
...
借助 VS Code v1.93 的新版 shell 集成，Cline 可以直接在终端中执行命令，接收输出，并实时响应。这意味着它可以：
...
Cline 是一个强大的 AI 编程助手，它超越了传统的代码补全，真正成为了一个可以自主完成编程任务的 agent。无论你是需要构建复杂的应用程序，还是想要自动化日常的开发工作流，Cline 都值得一试。

其对 MCP 协议的支持、多种 API 提供商的兼容性、以及人工审核的安全机制，使其成为一个可靠且灵活的 AI 编程工具。
