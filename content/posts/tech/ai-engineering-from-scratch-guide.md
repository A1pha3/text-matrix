...
这篇导读面向两类读者：想把"会调 API"补成"能自己搭模型"的开发者，以及想评估这套课程值不值得投入时间的学习者。读完你至少能判断两件事：这门课的知识结构是否覆盖你的目标，以及从哪个阶段切入最省时间。文章不替你做决定，只把结构、方法和取舍摆清楚。
...
为什么要把依赖关系写死？因为 AI 工程的知识是叠出来的：不懂反向传播，就调不动优化器；不懂 Attention，就解释不了幻觉。允许自由跳课等于把断层留给读者自己踩。这条硬约束是课程与"收藏夹式教程合集"的分水岭。
...
六步把"原理 → 框架 → 产品"三层都走一遍。多数教程只做 USE IT 这一层，学完只会调框架 API，遇到 bug 分不清是原理问题还是框架问题。BUILD IT 补的是原理层，SHIP IT 补的是产品层——这三层缺一层，调试时就会少一条定位线索。
...
想省时间，先跑这条命令再决定从哪切入。熟悉线性代数和概率论可以直接跳到 Phase 2 或 Phase 3；目标是 Agent 开发的话，建议从 Phase 13 开始倒查前置依赖。
...
这条串法带出一个反直觉的结论：**Phase 14 的 Agent 行为是否可靠，取决于 Phase 7 和 Phase 10 的理解深度**。如果对 Attention 和采样策略只有框架层认知，Agent 在长对话里出现的"幻觉"和"工具误调用"就无从定位。这条路径本身也从侧面检验了依赖硬约束的意义——单点知识看起来都能用，但只有当它们按顺序长在一起，才组得出一个能上线的系统。
...
1. **你的短板在原理还是框架？** 只差"会用某个库"，按教程刷 USE IT 更快；回答不了"为什么这样设计"，才需要走 BUILD IT。
2. **你能接受 3-4 个月的跨度吗？** 完整路径是场长跑，跳课省下的时间，最终会在断层处加倍还回来。
3. **你愿意手写实现吗？** 如果只想快速出 demo，这门课是绕路；如果想把 Agent 做到能上线，这条绕路恰恰是近道。

三个问题都指向"值得投入"，再按上面三条路径选一条；否则先用底部相关资源里的轻量教程把意图验证清楚，再决定要不要进来。
...
- 课程仍在持续更新，Phase 15-19 的内容可能会调整；仓库 Star/Fork 数字会随时间变化，本文数据截至 2026 年 8 月。
