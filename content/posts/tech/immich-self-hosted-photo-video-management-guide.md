...
Immich 是一个**自托管的照片和视频管理解决方案**，定位是 Google Photos 的开源替代品。
...
1. Immich 的技术栈分为哪几个核心模块？它们分别用什么语言/框架实现？
2. Immich 的搜索功能支持哪几种类型？CLIP 搜索的原理是什么？
3. 为什么 Immich 需要 PostgreSQL 而不是 SQLite？pgvecto-rs 的作用是什么？
4. 部署 Immich 时，你应该把 `UPLOAD_LOCATION` 挂载到什么地方？为什么？
5. 人脸识别流程分为哪几步？FaceNet 和聚类分析分别解决什么问题？
6. 如果你要给一个 3 人家庭部署 Immich，你会怎么规划存储、备份和访问权限？
7. Immich 支持哪些外部存储后端？如果你已经有 NAS，怎么集成？
...
- **性能调优**：如果你的照片库超过 100GB，关注 ML 模型的加载速度和搜索索引的重建频率。GPU 加速对人脸识别和影响搜索体验很大。
- **备份策略完善**：3-2-1 备份原则具体到 Immich 怎么落地？数据库备份用 `pg_dump` 还是文件系统快照？上传文件用 `rsync` 还是 `rclone`？
- **多用户权限规划**：如果你要给家人用，提前规划好管理员、普通用户、只读用户的权限边界。
- **外部集成**：Immich 提供了 REST API 和 OpenAPI 规范，你可以写脚本批量导入照片、自动打标签、或者集成到现有工作流里。

**想参与项目本身：**

- 仓库地址：<ADDRESS_REMOVED>
- 项目使用 NestJS + SvelteKit + Flutter，如果你熟悉其中任一技术栈，可以从 good first issue 入手。
- 机器学习模块用 Python + PyTorch，如果你熟悉人脸识别或 CLIP 模型，可以参与模型优化。
