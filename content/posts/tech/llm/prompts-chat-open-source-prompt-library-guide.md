...
Prompts.chat 的仓库主页明确写着：它**前身是 Awesome ChatGPT Prompts**。它不是从零起步的新产品，而是从最早一批“提示词集合”演化而来。
...
它已经不只是静态提示词集合，而是在把提示词做成一套可管理、可分发、可集成的内容系统。
...
系统处理的不是“文章段落”，而是**可检索、可过滤、可调用、可分享、可持续更新的内容实体**。
...
官方仓库主页明确提到，它适用于 ChatGPT、Claude、Gemini、Llama、Mistral 等现代 AI 助手。它提供的是**模型无关的提示词资产**，而不是只绑定某一家模型厂商。
