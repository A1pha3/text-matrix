...
> **关键问题**：如何用 NeMo Skills 构建从数据生成到评估的完整 LLM 技能提升流水线？
> **难度**：⭐⭐⭐⭐（高级）
...
NeMo Skills（[NVIDIA-NeMo/Skills](https://github.com/NVIDIA-NeMo/Skills)）是 NVIDIA 开源的 LLM 技能提升工具集，提供从合成数据生成（Synthetic Data Generation, SDG）、模型训练到基准评估的完整流水线。设计思路是：在本地工作站开发，一行配置切换到大规模 Slurm 集群。
