...
- [§4 MITRE ATT&CK 完整映射：13 个 TTP 的攻击工程化](#§4-mitre-attck-完整映射13-个-ttp-的攻击工程化)
...
完成以下检查，评估你的 AI Coding Agent 安全配置：

- [ ] **Agent 权限约束**：已开启 sandbox 模式或配置 `--permission-mode acceptEdits`，避免全开执行权限
- [ ] **域名白名单**：已配置 agent 允许访问的域名列表，阻止访问不常见域名
- [ ] **审计日志**：已启用 `~/.cursor/activity.log` 或 `~/.claude/sessions/` 备份，并定期 grep 高危命令
- [ ] **凭据隔离**：Apple Developer / 代码签名证书已迁移到硬件 token（YubiKey 等）
- [ ] **双因子认证**：Apple ID 已启用硬件双因子（非短信）

勾选 3 项以上，说明你的基础防护已到位；全部勾选，说明已达到较好的安全实践。
