...
策略复盘经常被跳过，但它是闭环中最容易被低估的一环。一次复盘不回答"赚没赚钱"，而是回答"这笔交易赚的是什么钱"。回测结果出来后，不做复盘直接换参数优化，等于在跑一条没有反馈回路的研究流程。仓库的 Analytics 和 Risk 分类覆盖了 quantstats、pyfolio、Riskfolio-Lib 等工具，正好补上这个缺口。
...
沿机器学习这条线，站内可以读 [Stefan Jansen《Machine Learning for Trading》2nd：量化金融 ML 工程化完全指南]({{< relref "stefan-jansen-machine-learning-for-trading-guide.md" >}})；偏中文量化研究与规则表达，可以转到 [CZSC：缠中说禅技术分析工具完全指南]({{< relref "quant/czsc-chanlun-technical-analysis-tool-guide.md" >}})。
...
- 常见错误五：在同一个数据集上反复调参数，直到曲线"完美"。排查方法是做样本外测试或交叉验证，把数据分成训练集和测试集，测试集在最终评估前不能碰。如果样本外表现远差于样本内，说明参数已经过拟合了历史噪声。
