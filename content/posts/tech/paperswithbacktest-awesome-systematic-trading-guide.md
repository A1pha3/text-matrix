...
`paperswithbacktest/awesome-systematic-trading` 把系统化交易拆成了几条可以分别学习的主线：数据、回测、分析、论文、书单和课程。初入量化的人，最大瓶颈是入口太散，不是资料太少——这份仓库做的就是第一轮收束。

但 2026 年再看它，需要多读一层上下文。GitHub 仓库是一个好用的入口，README 里列出 97 个库、40+ 篇策略论文、55 本书、23 个视频，以及博客和课程；与此同时，官方站点已经把重心转到产品化平台本身，首页主打 5,000+ 篇可运行论文、1.04 TB 清洗数据、60+ 课程内容，README 的策略区也直接写了"Strategies are now hosted here"。仓库是索引，站点是工作台。

这篇文章是目录地图：先分清仓库、官网、课程和博客各自负责什么，再看回测框架怎么选，最后用一条最小任务流把这些资源串起来。
...
仓库本身已经不再承担"完整策略目录"的全部职责。还按 2023 年那种"README 就是全部知识库"的方式阅读，后面的判断容易偏掉。

| 载体 | 它现在负责什么 | 该怎样使用 |
| ---- | ---- | ---- |
| GitHub README | 资源导航、项目入口、书单和课程索引 | 用来做第一轮筛选和建立知识地图 |
| paperswithbacktest.com | 5,000+ 可运行论文、数据、API、MCP、工作台 | 已经知道要查什么时，直接去站内搜策略和数据 |
| Course | 61 节内容、25 个 code notebook 的学习路径 | 适合按章节系统补课，零散查资料效率低 |
| Blog | 对量化、AI 与研究方法的持续输出 | 用来跟踪作者的判断，补充 README 的静态内容 |

顺着这张地图继续延伸，数据基础设施可以接 [OpenBB：开源金融数据平台专家级技术文档]({{< relref "quant/openbb-open-data-platform-guide.md" >}})，AI 量化研究流水线可以接 [Qlib：微软亚洲研究院 AI 量化投资平台从入门到精通]({{< relref "quant/qlib-ai-quantitative-investment-platform-guide.md" >}})。
...
这张图也是这份仓库的阅读顺序。初学者容易一上来就盯着框架名字，但系统化交易是一条闭环：先有研究假设，再决定数据需求，再进入回测与绩效分析，最后才谈部署。仓库的价值在于它把这条闭环上的主要入口都摆在了同一页里。
...
它把事件驱动回测、向量化原型、加密框架、数据源、组合优化和策略论文放在了同一个结构里，这件事散落在 20 个独立仓库里做不到。同时把论文和代码入口放在一起——README 的策略区同时给出 Sharpe、波动率、调仓频率、论文链接和 QuantConnect 实现入口。虽然这些数字不能直接拿来比实盘价值，但它大幅缩短了"看到论文标题"到"找到第一份代码"之间的距离。

用它排优先级也很方便：不需要把 97 个库都装一遍，先回答自己是研究、回测、部署、还是做组合管理，就能把大部分无关选项排除掉。
...
2. 再去 Data Sources 里挑一套拿得到、也能解释清楚的数据。学习阶段用 yfinance、AkShare、TuShare 没问题，但别把它们当成生产级行情源。
...
这个示例只回答一件事：一个最简单的双均线想法，在一套公开数据上能不能跑出可参考的历史结果。它还没有回答滑点、成交约束、交易时段差异和样本外稳定性，所以它是原型，不是实盘结论。

## 如何读仓库里的 Sharpe 比率

README 的策略区很吸引人，因为每条策略都给出 Sharpe、波动率、调仓频率和论文链接，但这些数字是研究线索，不是统一 benchmark。
...
3. 组合阶段：开始接触 PyPortfolioOpt、Riskfolio-Lib、quantstats，理解"多策略组合"的价值远高于寻找"单一神策略"。

4. 进阶阶段：等能稳定复现实验之后，再去碰 QLib、FinRL、MlFinLab、LLM for Trading 这类更容易过拟合的方向。

按资料类型来排优先级，顺序是：先读 README 建地图，再读一篇论文和一份实现，再补课程里的研究方法与统计内容，最后才是博客和 AI 量化扩展材料。

沿机器学习这条线，站内可以接着读 [Stefan Jansen《Machine Learning for Trading》2nd：量化金融 ML 工程化完全指南]({{< relref "stefan-jansen-machine-learning-for-trading-guide.md" >}})；偏中文量化研究与规则表达，可以转到 [CZSC：缠中说禅技术分析工具完全指南]({{< relref "quant/czsc-chanlun-technical-analysis-tool-guide.md" >}})。

## 常见错误与排查

- 常见错误一：把向量化回测结果当成真实成交结果。排查方法是回头检查策略有没有依赖 intraday 成交顺序、订单簿深度、滑点或手续费模型。
...
- [OpenBB：开源金融数据平台专家级技术文档]({{< relref "quant/openbb-open-data-platform-guide.md" >}})：把本文里的"数据层"继续展开，补齐数据获取与多端消费能力。
...
- [CZSC：缠中说禅技术分析工具完全指南]({{< relref "quant/czsc-chanlun-technical-analysis-tool-guide.md" >}})：关心规则型技术分析和中文量化社区语境，这篇更贴近实战表达。
