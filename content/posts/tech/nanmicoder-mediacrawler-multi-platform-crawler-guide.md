...
这个项目最值得关注的不是平台覆盖广度，而是它的"反直觉"技术选择：在一个大多数爬虫项目都会堆逆向代码的领域，它选择用浏览器登录态缓存 + JS 表达式取签名参数的方式绕过加密算法，把技术门槛从"逆向工程师"降到了"会用浏览器的人"。

下文先给七平台覆盖矩阵，再拆解 CDP 模式的工程取舍，最后给出最小上手步骤和适用边界。
...
所有平台能力对齐，意味着换平台时不需要重写业务逻辑，只需改 `--platform` 参数。
...
1. **JS 逆向**：用 AST（抽象语法树）解析、Hook（钩子，即拦截浏览器内部函数调用）还原加密算法，再纯 Python 重写。维护成本高，平台一改版本就废。
...
- `ENABLE_GET_COMMENTS`：是否爬评论，默认 `False`（第一次用容易漏掉这一项）。
...
作者在 README 顶部用 ⚠️⚠️⚠️ 强调"大家请以学习为目的使用本仓库"，这是 License 与司法案例双重支撑下的硬约束。
...
MediaCrawler 的价值在于把"多平台 + 免逆向 + 登录态复用"这三件事用工程化方式串起来，让个人开发者能在一个下午内跑通七平台爬虫。
