...
[RocketChat/Rocket.Chat](https://github.com/RocketChat/Rocket.Chat) 是 GitHub 上规模最大的自托管团队通信仓库之一：
...
要理解这个仓库的形状，先抛掉"Rocket.Chat 是一个 Meteor 应用"的旧印象。`apps/meteor` 仍是承担实时消息主链路的核心应用，但它已经不是孤岛——`apps/` 旁还有 `uikit-playground`，`packages/` 里有 50 多个 TS 包，`ee/` 下另有一套企业版二进制和微服务。下面把这张 monorepo 全景画清楚。
...
## Apps-Engine：被低估的扩展机制

Rocket.Chat 的 FEATURES.md 写得很长，但读仓库时真正值得单独拆开的是 `packages/apps-engine`。Apps-Engine 不只是"插件加载器"，它定义了一整套把第三方 App 当作一等公民的运行时：
...
工程上的关键判断：**Rocket.Chat 的"集成 vs 自研"边界就在 Apps-Engine**。如果一个功能能在 App 里用钩子 + UI 扩展实现，就不要去 fork `apps/meteor`。这是 50+ packages 之所以能稳定演化的核心约束——主应用边界稳定，第三方能力在引擎里横向扩张。
...
需要警惕的一个工程现实：**这些微服务的协议和数据格式都是 NATS/moleculer 私有事件**，不是公开 HTTP API。如果你在自部署里要接入自己写的服务，最稳的路径是写一个独立的"适配层"用 moleculer client 接入，不要尝试通过 REST 反向 hack，否则会失去 NATS 的事件回放与背压能力。

## 联邦、E2EE 与合规：CommsOS 的真正护城河
...
Rocket.Chat 用 Yarn 4 workspaces + Turborepo 把上百个 TypeScript 包组织成主应用 + 微服务 + UI 包 + Apps-Engine 的四层结构，数据走 MongoDB Oplog，服务间走 NATS/moleculer，扩展点全部下沉到 Apps-Engine。要不要选它，看的是"是否需要把通信数据从 SaaS 拽回自己的网络"，而不是"它能不能取代 Slack"。