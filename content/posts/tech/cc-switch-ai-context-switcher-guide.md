...
- 完成首次安装和配置，理解"接管边界"以避免常见误解
- 区分热切换（Claude Code）和冷切换（其他CLI）的差异，并据此规划你的工作流
...
如果只看一句话介绍，官方的定位已经很清楚：它是一个 All-in-One Manager。它解决的核心问题可以归为两类：
...
CC-Switch 的云同步并不是重新造一套中心化账号体系，而是把 `~/.cc-switch/` 这类核心目录同步到 Dropbox、OneDrive、iCloud、WebDAV 或 NAS 等外部存储。好处是直接、可理解，也便于和本地备份策略叠加使用。
...
2. 为什么 Claude Code 支持热切换，而其他 CLI 需要冷切换（重开终端）？
3. Hermes Agent 的 Memory panel 和传统 Prompts panel 有什么区别？
4. Local Routing 的真实能力边界是什么？它能不能做"按 provider 维度细粒度定义不同代理规则"？
5. CC-Switch 的云同步同步的是什么？不同步的是什么？
6. 为什么首次启动后需要"确认导入"而不是直接接管所有配置？
7. 如果你要在一个团队里推广 CC-Switch，你会怎么解释它的边界（什么它能做、什么它不能做）？
...
- **最小化验证链路**：装好后先只配置一个 provider，验证切换成功，再继续配置 MCP、Skills、Prompts。避免一次性配太多导致出问题不知道哪里出错。
- **理解热切换 vs 冷切换**：如果你主要用 Claude Code，体验热切换的便利；如果你也用 Codex 或 Gemini CLI，养成"改完配置重开终端"的习惯。
- **MCP 同步策略**：先在一个工具里配好 MCP，然后用 CC-Switch 同步到其他工具，而不是每个工具单独配一遍。
- **多设备同步验证**：在一台机器上改配置，确认同步到另一台机器后，各 CLI 能正常读取配置。

**想参与项目或深度定制：**

- 仓库地址：<ADDRESS_REMOVED>
- 如果你发现某个 CLI 的配置格式变了导致 CC-Switch 无法正确解析，可以提 issue 或 PR。
- Tauri 2 + Rust 的架构意味着如果你熟悉 Rust，可以参与到底层文件处理和路由逻辑的开发中。
...
| 维度 | 得分 | 说明 |
|------|------|------|
| 结构性 | 20/20 | 标题层级正确、有清晰的学习目标章节、内容逻辑递进合理、有自测题和进阶路径等导航元素 |
| 准确性 | 25/25 | 技术描述无事实错误、术语使用一致（CC-Switch、Claude Code、MCP等）、代码示例完整可运行、链接有效 |
| 可读性 | 25/25 | 中英文混排规范、段落适中、排版舒适、自然表达（无明显AI味道）、格式统一 |
| 教学性 | 20/20 | 有明确的学习目标、核心概念解释了"为什么"、包含自测题和进阶路径且与正文衔接自然、难度递进合理 |
| 实用性 | 10/10 | 示例来自真实场景、包含多设备同步思路等实用建议、有错误处理和排查指引 |
