...
description: "OpenCLI 真正做的不是让 Agent 点网页，而是把站点适配器、登录态浏览器、Electron 应用和本地 CLI 统一成一棵可发现的命令树。本文解释命令优先、Browser Bridge、三层会话模型与适用边界。"
summary: "这不是一篇把 OpenCLI 写成浏览器自动化工具的文章，而是一篇把它还原成命令发现面的技术解读：为什么应该先找适配器命令，再退到 browser 原语，profile、browser session、siteSession 各管什么，以及它和 Playwright、官方 API 的边界到底在哪里。"
draft: false
categories: ["技术笔记"]
