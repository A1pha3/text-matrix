...
Penpot 的主张是：**设计即代码（Design as Code）**。

设计即代码不只是口号，它直接体现在 Penpot 的每一个设计决策中：
...
对于开发者而言，**在 Penpot 中设计的网格系统，可以直接对应到生产环境中的 CSS Grid 代码**。不再有「设计稿上是 Grid，生产代码里用 Flexbox 勉强凑」的割裂感。
...
设计师完成设计 → 开发者打开 Inspect → 直接获取可用代码，整个流程无需任何第三方工具或插件。
