...
- **自定义表示法**：你可以定义自己的元素类型和样式，不限于 C4 的 Person/System/Container/Component 四层
...
| **LikeC4** | 代码生成图 + 实时预览 | 支持 | 支持 | 高（可自定义表示法） |
...
- 需要自由绘制非标准图形的场景（LikeC4 的 表示法 有结构约束）
- 团队对 DSL 学习成本敏感的场景
