...
v2.0.0-rc.1 修复了一个值得注意的 bug：`parse_instinct_file()` 在解析本能文件时会静默丢弃 `Action`、`Evidence`、`Examples` 部分的内容，导致导入后本能库不完整。
...
ECC 2.0 的核心控制平面使用 Rust 重写，位于 `ecc2/` 目录。已可在本地构建，暴露以下命令：
...
Claude Code、Codex、Cursor、OpenCode、Gemini、Zed、GitHub Copilot 等 7 种主流 Agent，覆盖 12 个语言生态。

### 232 个技能全装上会不会太重？

可以使用选择性安装（v1.9.0+）。通过 `install-plan.js` 生成安装计划，只装当前项目需要的 skills。

### ECC 和普通的 Claude Code 插件有什么区别？

插件市场里的插件通常是单个功能，而 ECC 是一整套系统，组件之间有联动关系（hook 触发后调用 skill，skill 执行结果写入 context 记忆）。

### ECC 是否支持团队共享配置？

支持。ECC 的配置（skills、rules、hooks 等）可以提交到版本控制系统，团队成员共享同一套配置。
