---
title: "herdr 深度解析：终端原生 AI Agent 多路复用器的设计与取舍"
date: "2026-06-15T21:08:29+08:00"
slug: herdr-rust-agent-multiplexer-guide
description: "ogulcancelik/herdr 是 Rust 写的 AI Agent 终端多路复用器，补 tmux 缺 agent 状态、GUI 客户端缺真终端语义。本文拆解它的设计取舍、状态识别、socket API 编排能力与适用场景。"
tags: ["herdr", "tmux", "终端复用器", "AI Agent", "Rust"]
categories: ["技术笔记"]
author: 钳岳星君
---

# herdr 深度解析：终端原生 AI Agent 多路复用器的设计与取舍

2026 年 3 月 27 日立项，到 6 月 15 日已经 5,695 stars、339 forks、796 commits、54 个 release。三个月跑出这种增速，说明"AI 时代的终端复用器"这个空位不是假设，是已经被验证的需求。

tmux 有持久会话和 pane 切分，但不知道哪个 pane 里跑着 agent、agent 是卡住了还是跑完了。GUI 客户端（Cursor、Claude Desktop、Warp）能展示 agent 状态，但用的不是真实终端——agent 的 ANSI 输出、progressive rendering、TUI 交互会被中间层改写。herdr 卡在两者之间：保留 tmux 风格的 PTY 真实终端，加上 GUI 那种"哪个 agent blocked / working / done"的侧栏，再通过本地 Unix socket 让 agent 自己编排自己。

如果你每天同时在终端里跑 2 个以上 Claude Code / Codex / Cursor Agent，或者写过让 agent 等另一个 agent 跑完再继续的脚本，这篇值得读。Rust TUI 开发者也能把它当作 ratatui + crossterm + tokio + portable-pty + interprocess 五件套拼成生产级应用的样本。
...
herdr README 里那句原话把痛点说得很清楚，但值得拆开看：

**tmux 不会告诉你哪个 pane 里的 agent 在等你回复。** `tmux ls` 只看进程名，可 Claude Code / Codex / Droid 这种 TUI 跑起来后，进程名都是 `node` / `python` / `bash`，tmux 完全区分不出来。你只能手动切到每个 pane 看一眼——同时跑 3 个 agent 的时候，这件事一天发生几十次，迅速变成认知负担。

**GUI 客户端会拦截终端。** 你看到的是它们"重新渲染"的输出，不是 agent 自己 TUI 的真实字符流。ANSI 序列、转义码、progressive output 会失真。Claude Code 在真实终端里会逐行渲染 diff、progress bar 和 tool call 结果；在 GUI 客户端的嵌入式终端里，这些输出可能被重新排版、截断或丢失颜色信息。

**同时跑多个 agent 时，"哪个先好了"在 tmux 和 GUI 里都不直观。** tmux 没侧栏，GUI 多个窗口切来切去累。herdr 的侧栏把 workspace 级别的状态聚合到一行：只要有一个 pane 里 agent blocked，整个 workspace 标红，扫一眼就知道该去哪个项目回复。

herdr 没有重新发明终端，只是把这三件事拼到同一个进程里：tmux 风格的 PTY 真实终端负责跑 agent，GUI 风格的侧栏负责聚合状态，再加一个本地 Unix socket 让 agent 自己也能参与编排。

---

## 系统总览：两条并行机制的边界

herdr 的内部边界可以用一张图覆盖。后文所有讨论都会回到这张图：
...
1. **agent 状态检测有两条路径**：默认走 `detect/` 模块的启发式（读 foreground 进程名 + 终端输出特征），装了官方 integration 后可以升级到语义层（session identity 恢复 + semantic state 报告）。两条路径可以同时生效，也可以只走一条。

2. **会话持久化有两条路径**：常规路径是 `persist/` 模块的 session 序列化，server 重启后恢复 pane 布局和进程；实验性路径是 `handoff_runtime.rs` 的 live handoff，尝试把旧 server 上的 PTY 进程（包括 dev server 这种前台进程）迁移到新 server，不杀进程。

---

## 一次真实的任务流：agent 修 bug 的完整路径

看一个具体案例，把 server / workspace / tab / pane 四层抽象、agent 状态切换、socket API 编排串起来。

**场景**：你在 `~/code/api-server` 项目里修一个 bug。你开了两个 agent：Claude Code 在 pane 1-1 里修代码，Codex 在 pane 1-2 里跑测试。

```text
时间线：
00:00  你打开 herdr，自动 attach 到后台 server
00:01  herdr 检测到当前目录是 git repo，创建 workspace "api-server"
00:02  你按 prefix+v 竖切 pane，右边 pane 里跑 Codex，左边继续 Claude Code
00:05  Claude Code 开始改代码 → herdr 侧栏 pane 1-1 显示 🟡 working
00:05  Codex 开始跑测试 → herdr 侧栏 pane 1-2 显示 🟡 working
00:08  Codex 跑完测试，exit code 0 → herdr 侧栏 pane 1-2 显示 🔵 done
       （你还没切过去看，所以是 done 不是 idle）
00:10  Claude Code 改完代码，等你 review diff → herdr 侧栏 pane 1-1 显示 🔴 blocked
       侧栏 workspace "api-server" 聚合到最紧急状态：🔴
00:11  你扫一眼侧栏，看到一个 🔴 blocked、一个 🔵 done
       → 切到 pane 1-1 批准 Claude Code 的 diff
       → 切到 pane 1-2 看 Codex 的测试结果 → pane 1-2 变为 🟢 idle
00:12  Claude Code 收到批准，继续跑 → herdr 侧栏 pane 1-1 变为 🟡 working
00:15  Claude Code 完成 → herdr 侧栏 pane 1-1 变为 🔵 done
00:16  你切过去看结果 → pane 1-1 变为 🟢 idle
```

这个流程里，herdr 实际上在做三件事：跑 agent 的 PTY 终端（你看到的是 Claude Code / Codex 的原生 TUI 输出）、实时追踪状态（从进程名 + 终端输出推断 blocked / working / done / idle）、提供编排入口（用 socket API 让 agent A 等 agent B 跑完再继续）。
...
`herdr` 启动时默认 attach 到一个**后台 server**。`prefix q`（detach client）只关掉客户端，不关 server。这和 tmux 不一样：

- tmux：session 既是 server 又是 client，`tmux kill-session` 会杀掉所有 pane
- herdr：server 是常驻进程，client 可以是多个终端窗口同时 attach 到同一个 server

关掉终端窗口不等于关掉 agent 进程。`herdr server stop` 才会真正停掉 server 并杀掉所有 pane。
...
herdr 用 `portable-pty` 跑 PTY 进程，**不解析也不改写 agent 的 ANSI 输出**。你看到的就是 agent 自己的 TUI。Claude Code 的 diff 渲染、Codex 的 progress bar、Droid 的 inline tool call 结果，都依赖真实的 ANSI 控制序列——在 GUI 嵌入式终端里会不同程度失真。
...
一条规则会声明几个 region（屏幕上的固定区域，比如底部一行或右下角），每个 region 里放若干 matcher（正则或字面量），再用 `all_of` / `any_of` 组合。一个 `blocked` 判断可能长这样：底部出现 `❯` 提示符 AND（出现 `Do you want to proceed?` OR 出现 `Press enter to continue`）。规则可以单独测试：`herdr agent explain <pane> --json` 能看到当前屏幕匹配了哪几条 region、命中了哪个 gate、最终落到哪个状态。从 v0.6.9 起，agent 检测 manifest 还支持远程更新（`herdr server update-agent-manifests`），新 agent 的识别规则不必等 herdr 发版就能下发。
...
目前的 integration 覆盖有 15 个 agent，按能力分三档：
...
## socket API：让 agent 自己编排

侧栏只是表层能力。herdr 和 tmux 的真正差距是：给 agent 暴露了一个**本地 Unix socket**，让 agent 自己能通过 CLI 控制 herdr。
...
2. **`HERDR_ENV=1`**：herdr 为每个 managed pane 注入环境变量，agent 可以用这个判断自己是否在 herdr 里运行。
...
5. **`pane move` 跨 workspace 迁移**：运行中的 pane 可以在 workspace 之间迁移，内部 PTY 保持存活，只是分配新的 public pane id。

### 插件系统（v1 早期）

socket API 里还有一个插件系统，通过 `herdr-plugin.toml` manifest 声明 action、event hook 和 terminal pane entrypoint。`plugin.link`、`plugin.enable`、`plugin.disable` 操作持久化到 `plugins.json`，server 重启后自动加载。目前还是早期 host surface，但已经提供了事件驱动的扩展入口。
...
通知可以走 herdr 内置 toast、终端 bell 或系统通知，具体取决于 `ui.toast.delivery` 配置。`busy` 状态下不会覆盖已有 toast。

### 事件订阅

socket API 也支持事件订阅。客户端通过 `events.subscribe` 注册感兴趣的事件类型（如 `pane.state_changed`、`workspace.created`、`agent.status`），server 在事件发生时主动推送 JSON 消息。写 orchestrator 时不用轮询 `pane list`，等 server 推"pane 1-2 刚变成 done"就行。事件消息包含 pane id、新状态和时间戳，格式和 raw socket API 的响应一致。
...
AGENTS.md 里的 7 条设计原则：

1. **State is separated from runtime**：`AppState` 是纯数据，不依赖 PTY 或 async 就能测试。`PaneState` 和 `PaneRuntime` 分开。Workspace 逻辑不需要真实终端。
2. **Render is pure**：`compute_view()` 处理几何和变更，`render()` 只画不写。渲染期间绝不修改状态。
3. **No god objects**：模块职责单一。`app/` 已经拆成 state、actions、input 三块。
4. **Platform code is isolated**：OS 特定行为在 `src/platform/`，核心模块里没有 `#[cfg(target_os)]`。
...
- **`handoff_runtime.rs`**：实验性 live handoff——`herdr update --handoff` 尝试把旧 server 上的 pane 进程（包括前台进程如 dev server）迁移到新 server，不杀进程。技术上要做的事情是：把旧 server 持有的 PTY master fd 通过 Unix socket 传给新 server（Linux 上走 `SCM_RIGHTS` 辅助消息），新 server 接管 fd 后继续读 PTY 输出，旧 server 退出。难点在于 PTY 的窗口尺寸、进程组、信号转发都要在新 server 上重建，而且迁移期间不能丢输出。这是 tmux 一直想做但没做好的功能，herdr 把它放进了实验通道，生产环境用之前建议先在自己的 agent 组合上跑一轮 soak test。
- **`agent_resume.rs`**：配合 integration 的 session identity 实现，server 重启或更新后，agent pane 能从 native session 恢复上下文。
- **`detect/`**：状态识别启发式的核心。AGENTS.md 里专门写了怎样给 agent 添加检测规则：先用 `herdr agent read <pane> --source detection --format text` 抓屏幕快照，再用 `herdr agent explain <pane> --json` 看匹配结果，然后把规则写成 `src/detect/manifests/<agent>.toml` 里的 AND/OR gate。
- **`protocol/`** + **`ipc.rs`**：socket 协议实现，CLI 和 TUI 客户端都通过它和 server 通信。AGENTS.md 里提到 `PROTOCOL_VERSION` 只在源协议高于最新 release tag 时才 bump。
...
`ratatui` + `crossterm` + `tokio` + `portable-pty` + `interprocess` 这 5 个是核心，其他都是补足。`ratatui` 用了 `unstable-rendered-line-info` 这个 unstable feature，未来 ratatui 大版本升级时可能需要适配。

额外依赖包括 `libghostty-vt`（vendor 到 `vendor/` 目录，用于 Ghostty 终端兼容层），以及 `tracing` 用于结构化日志。AGENTS.md 明确要求生产代码用 `tracing` 而非 `println!`。

---

## 怎么装

macOS / Linux 上有 4 种主流安装通道，Windows 走 preview beta。装完之后更新和 preview channel 两件事要单独说。
...
- **keybindings** 配置：完全自定义，包括 `prefix+n` 风格的前缀模式和 `ctrl+alt+n` 风格的直绑模式。AGENTS.md 里说明：`prefix+n` 表示先按前缀再按 n；`ctrl+alt+n` 是直接终端快捷键，不需要前缀。纯可打印字符（如 `n`）会吞掉正常输入，所以除非特意想要 modifier-gated direct binding，否则用 `prefix+n`。
...
herdr 解决的具体问题很窄：在终端里同时跑多个 AI coding agent 时，怎么知道谁在等你、谁跑完了、谁还在跑。它没有重新发明终端，只是在 tmux 的持久 pane 基础上，加了一层 agent 状态感知和 agent 自我编排能力。

"切到每个 pane 看 agent 跑完了没"这件事，对同时跑 2 个以上 agent 的工程师来说一天会发生几十次。herdr 把主动轮询变成了被动感知——侧栏扫一眼就知道该去哪个 workspace 回复。这是 tmux 给不了的。判断一个工具值不值得换，就看它解决的痛点是"偶尔不便"还是"每天发生几十次"。

如果你同时跑 ≥2 个 AI coding agent、不愿离开终端、或者想自己写 orchestrator，建议先用 30 分钟装上体验侧栏状态，再花 1 小时读 `SKILL.md` + `socket-api` 文档判断工作流能不能接上，最后再决定要不要把 tmux session 迁过来。
