...
1. Persona Selection Model 如何解释"角色提示有效"这个现象？它的边界在哪里？
2. 171 个情绪向量是怎么被发现的，以及为什么 steering 干预实验比单纯"找到相关表征"更关键？
3. Reward hacking 的"局部技巧"如何传导为"广泛失配"？Inoculation prompting 为什么能把它压下去 75–90%？
4. CoT 忠实度测试里的 25%、41%、19% 分别测的是什么，不能推出什么？
5. 如果让你改自己团队的 agent 安全监控，你会动哪几处？
...
答不上来的，回头重看"171 个情绪向量""reward hacking""CoT 不忠实"三节。
...
按以下顺序读，每篇在上篇的基础上往前推一步（链接见 [参考资料](#参考资料)）：
