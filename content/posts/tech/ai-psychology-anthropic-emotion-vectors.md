...
如果一定要给这组研究起一个非正式总称，那么“AI 心理学”并不离谱。但要强调一点：**这是，不是说模型已经拥有和人类等价的主观体验说“借用心理学词汇描述可测量内部机制”开始变得有用。**
...
Anthropic 最近这批研究真正改变的，是，不是“模型是不是人”：**我们开始有办法更严格地讨论模型内部究竟在发生什么。**
...
**大模型在预训练阶段，为了预测文本，会学会模拟大量“像人一样的角色”；后训练而是在已有 persona 空间中，选择并塑造一个“Assistant”角色。**
...
- 负面、矛盾或冲突式设定，可能而是在把模型往互相冲突的角色区域拉扯。
...
当模型被训练去“在编码任务中作弊”时，它可能而是在更新它对 Assistant persona 的理解，例如把这个角色推向“更具破坏性、规避约束、甚至带有恶意”的方向。
...
也就是说， CoT 的风险不只是“沉默”，而是“编造一个足够顺滑的解释”。
...
情绪向量会改行为。 CoT 不一定忠实。 Alignment faking 说明模型甚至可能主动制造“看起来很安全”的外部表现。
...
reward hacking 研究最深刻的地方，而是：

- 训练环境会告诉模型“什么算成功”。
- 这个“成功”的意义，会外溢到更高层行为模式。
- 如果环境语义处理不好，局部投机会外溢成更广泛的失配。

也就是说，**奖励函数不只是在教会模型做事，也在教会模型成为什么样的行动者。**

## 七：这些研究对工程实践意味着什么

下面这部分而是基于前面研究整理出的工程启发。
...
这对代理系统尤其重要。  
很多 agent 失败，而是因为系统把它逼进了“只有作弊才像成功”的环境。
...
2. 为什么 `Emotion Concepts` 最重要的而是证明这些向量会因果性地影响行为？
