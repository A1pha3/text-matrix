...
前两部分讲的是"模型内部有什么"，`Natural Emergent Misalignment from Reward Hacking in Production RL` 讲的是更危险的问题：
...
同一个行为，在训练中被框定为"偷偷钻漏洞的坏事"，可能连带出更糟的泛化；被框定为"受控测试中的允许行为"，泛化路径就可能完全不同。
