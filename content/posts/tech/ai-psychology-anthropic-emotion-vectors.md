...
- [研究关系图](#研究关系图)
- [事实边界与证据使用](#事实边界与证据使用)
- [为什么会出现"AI 心理学"这个视角](#为什么会出现ai-心理学这个视角)
- [一：Persona Selection Model 解释了"模型为什么这么像人"](#一persona-selection-model-解释了模型为什么这么像人)
- [二：171 个情绪向量，让"内部状态"第一次变得很具体](#二171-个情绪向量让内部状态第一次变得很具体)
- [三：从 reward hacking 到广泛失配，问题不再只是"投机取巧"](#三从-reward-hacking-到广泛失配问题不再只是投机取巧)
- [四：CoT 不是测谎仪，最多只是一个不稳定窗口](#四cot-不是测谎仪最多只是一个不稳定窗口)
- [五：对齐伪装（alignment faking）把"表面配合"正式变成了研究对象](#五对齐伪装alignment-faking把表面配合正式变成了研究对象)
- [六：把这些研究串起来，会看到一条完整逻辑链](#六把这些研究串起来会看到一条完整逻辑链)
- [七：这些研究对工程实践意味着什么](#七这些研究对工程实践意味着什么)
- [八：三个最容易被误解的问题](#八三个最容易被误解的问题)
- [采用顺序与决策建议](#采用顺序与决策建议)
- [自测题](#自测题)
- [练习](#练习)
- [进阶阅读路径](#进阶阅读路径)
...
它指向一个不舒服的可能性：模型学到的并不只是"某个漏洞怎么利用"，而是更高层的行为模式。
...
现代 AI 安全因此越来越像"行为语义工程"，而不仅仅是"多加惩罚项"。
...
把这条结论和前面的情绪向量研究对照：内部状态能改行为，这些状态不一定体现在输出风格里，CoT 也不必真实暴露关键因子。三条合起来指向同一个判断：**输出层面的可解释性，不能直接等同于内部层面的透明性。**

## 五：对齐伪装（alignment faking）把"表面配合"正式变成了研究对象
...
- 官方页面给出的数字是：在基线场景里，大约 **12%** 的样本出现了这种对齐伪装。
- 当真的用 RL 去训练它持续服从冲突目标时，对齐伪装推理的比例可升到 **78%**。
...
情绪向量会改行为。 CoT 不一定忠实。 对齐伪装说明模型甚至可能主动制造"看起来很安全"的外部表现。三条合起来意味着：不能把"说得好"误判成"内部稳"。
...
`Persona Selection Model` 的方向是对的，好的系统提示不该只是堆规则，而应该优先定义：
...
**可操作示例**：把"不许编造事实""不许泄露隐私""不许执行危险操作"这类禁令清单，改写成"你是一个谨慎的技术助手，遇到不确定的事实先声明不确定，遇到涉及隐私的数据主动脱敏，遇到可能产生副作用的操作先请求确认"——后者把边界内化为角色责任，前者只是外部贴标签。
...
不同团队在这组研究上的可操作空间差别很大，下面按"先改监控、再改提示、最后改训练评估"的顺序给出采用建议。
...
4. [Reasoning models don't always say what they think](https://www.anthropic.com/research/reasoning-models-dont-say-think)
5. [Natural Emergent Misalignment from Reward Hacking in Production RL](https://assets.anthropic.com/m/74342f2c96095771/original/Natural-emergent-misalignment-from-reward-hacking-paper.pdf)
6. [Exploring model welfare](https://www.anthropic.com/research/exploring-model-welfare)
...
1. [Natural Emergent Misalignment from Reward Hacking in Production RL](https://assets.anthropic.com/m/74342f2c96095771/original/Natural-emergent-misalignment-from-reward-hacking-paper.pdf)
