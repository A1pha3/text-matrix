...
在子域名发现这件事上，工具可以分成两类：一类向目标发包，一类不向目标发包。Findomain 属于后者，它解决的是"被动子域名发现"问题——通过查询公开的证书透明日志（Certificate Transparency Logs，CT）和第三方威胁情报 API，把已经存在过的子域名捞回来，整个过程不向目标发送任何探测包。它不解决"主动枚举"问题，也就是不能用字典去硬撞 DNS。

这个定位决定了它的适用场景：Bug Bounty 猎人在 recon 阶段需要快速摸清目标资产面，企业安全团队需要监控自有域名的新子域名出现，红队在打点前需要被动收集而不暴露指纹。在这些场景里，Findomain 的价值是"快、广、不留痕"。它的代价是召回率受限于数据源——一个从未申请过证书、也从未被任何威胁情报平台收录的内网子域名，Findomain 发现不了。

这篇文章会把 Findomain 的技术路线拆开讲清楚：CT 日志到底是怎么工作的、为什么它是首选数据源、Findomain 用 Rust 怎么把多个 API 的并发查询压到 15 秒内、以及它和 massdns、dnsgen 这类工具的组合使用模式。
...
理解这个边界很重要：CT 日志是首选数据源，因为它免费、广覆盖、无需授权，但它不是唯一数据源。Findomain 的价值在于把多个数据源的结果去重合并，给出一个尽可能完整的子域名清单。
...
Findomain 支持的数据源列表（来自项目 README）包括以下这些。理解它们的覆盖差异和限制，才能判断 Findomain 在不同场景下的召回率。
...
实际使用中，Crt.sh + Certspotter + Virustotal（免费 key）这三个免费数据源就能拿到大部分子域名。要追求极限召回，再加上 SecurityTrails 和付费 Virustotal key 会有明显提升。从经验上看，Crt.sh 通常能贡献 60%-70% 的子域名，其他数据源补充剩下的 30%-40%。
...
整个流程的关键设计点是：查询阶段全并行、去重用 HashSet、解析阶段有通配符检测。这三点决定了 Findomain 的性能特征。查询阶段的并行性让它能在 15 秒内完成所有数据源的查询；HashSet 去重让结果干净；通配符检测避免了误报。
...
这个组合的逻辑是：Findomain 解决"已知子域名"的发现问题，dnsgen + massdns 解决"未知但可能存在的子域名"的发现问题。前者快但召回有限，后者慢但能发现 Findomain 漏掉的子域名。两者互补，不替代。
...
这个流水线里，Findomain 的位置是"快速被动发现的第一步"。它的价值是速度——5 秒内拿到第一批种子，让后续步骤可以尽早开始。subfinder 和 amass 会补充更多种子，但它们更慢（amass 的被动枚举可能要几分钟）。
...
Findomain 不是唯一的被动子域名发现工具。在这个领域里，subfinder 和 amass 是另外两个主流选择。理解它们的差异，才能选对工具。

`subfinder` 是 ProjectDiscovery 出品的被动子域名发现工具，也是用 Go 写的。它的设计哲学和 Findomain 类似——多 API 聚合，不做主动枚举。subfinder 的数据源列表和 Findomain 高度重叠，但它的优势是和 ProjectDiscovery 生态（httpx、nuclei、subjs 等）无缝集成，管道操作很顺畅。subfinder 的性能比 Findomain 稍慢（Go 的 GC 开销），但差距不大。
...
三者的定位差异：Findomain 追求速度，适合 recon 流水线的第一步；subfinder 追求生态集成，适合和 ProjectDiscovery 工具链配合；amass 追求深度，适合需要完整资产测绘的场景。实际使用中，三者结果合并是常见做法——每个工具的数据源和查询策略有差异，合并能拿到最全的子域名清单。

从性能角度看，Findomain 的 Rust 实现让它在内存占用和启动时间上有优势。一个典型的对比：对同一个中等规模域名，Findomain 跑完需要 5-10 秒，subfinder 需要 10-20 秒，amass 被动模式需要 1-5 分钟。但召回率上，amass 通常能比 Findomain 多发现 5%-15% 的子域名（取决于目标域名的特征）。

## 采用边界：什么时候用 Findomain，什么时候用别的

Findomain 不是万能工具，它的适用边界需要明确。

**适合用 Findomain 的场景**：

- Bug Bounty recon 阶段，需要快速摸清目标资产面。Findomain 的 5 秒级响应让它适合作为 recon 流水线的第一步。
- 企业自有资产监控，配合 cron 定期跑，新子域名通过 Webhook 告警。
- 红队被动信息收集，不能向目标发包的场景。Findomain 全程不碰目标服务器，不会触发目标的入侵检测。
- 大规模域名清单的初步筛选，从几千个域名里快速过滤出有子域名的目标。Findomain 的 `--quiet` 模式适合脚本化批量处理。

**不适合用 Findomain 的场景**：

- 需要发现内网子域名或无证书子域名的场景。CT 日志和威胁情报 API 都覆盖不到这部分，需要用 massdns 暴力枚举。
- 需要实时监控新子域名出现的场景。Findomain 的监控是周期性的，CT 日志本身也有索引延迟，做不到分钟级告警。
- 需要发现子域名接管（subdomain takeover）的场景。Findomain 只发现子域名，不检测 CNAME 悬挂。要做 takeover 检测，需要把 Findomain 的结果喂给 subjack 或 nuclei 这类工具。
- 对召回率要求极高的场景。Findomain 的召回受限于数据源，如果目标域名很新或者很小众，CT 日志和威胁情报平台可能都没有记录。这种情况下需要 subfinder、amass 多工具组合，加上 dnsgen + massdns 的暴力变形。
- 需要发现非标准端口服务的场景。Findomain 只发现子域名，不做端口扫描。端口扫描需要 nmap 或 rustscan 这类工具。

**工具组合建议**：

- 被动 recon 第一步：Findomain（快）+ subfinder（广）+ amass（深），三者结果合并去重。
- 主动枚举第二步：dnsgen 变形 + massdns 解析，补充被动工具漏掉的子域名。
- 存活验证第三步：httpx 或 httprobe 探测 HTTP 服务，nuclei 跑漏洞模板。
- Takeover 检测第四步：subjack 或 nuclei 的 takeover 模板。
- 端口扫描第五步：rustscan 快速扫全端口，nmap 做服务识别。

Findomain 在这个流水线里的位置是"快速被动发现的第一步"，它的价值是速度和广度的平衡，不是召回率的极致。
...
要深入理解 Findomain 背后的技术，建议按以下路径学习。
