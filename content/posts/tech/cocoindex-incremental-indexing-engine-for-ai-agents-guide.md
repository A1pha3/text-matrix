...
Stars 8,192，Apache-2.0 License，2026-05-05 最新推送，支持 Python 3.10-3.13，核心引擎用 Rust 实现。
...
传统数据管道里，你写一次性脚本处理数据，每次运行都重新处理全部数据。CocoIndex 里，你声明目标状态 `Target = F(Source)`，引擎保证在源或函数变化时持续同步目标。类似于 React 组件的状态声明：当 `Source` 或 `F` 变化时，引擎自动重新计算目标，且只重新计算受影响的节点。

这个模型有四个属性：声明式（描述"要什么"而非"怎么做"）、增量更新（源文件变更时只处理变化的部分）、代码变化感知（函数从 v1 改为 v2 时只重新执行依赖该函数的节点）、端到端血缘（每个目标 dot 都可以追溯到源字节）。
...
向量索引通过 `declare_vector_index(column="embedding")` 声明，可对接 Pinecone、Weaviate、Chroma 等向量数据库。每个 target dot 都能追溯到源字节，调试时能回答"这个搜索结果来自哪段代码/哪个版本"，合规审计时也能定位数据来源。
...
3. **向量数据库需自备**：不内置向量数据库服务，需要连接外部服务（Pinecone、Weaviate 等）。这意味着额外的运维成本和成本核算。
4. **v1 阶段**：仍处于快速迭代期，API 可能有 breaking changes。生产环境使用前建议评估 API 稳定性和边界场景，并订阅 release notes 跟进变更。
...
5. **生产化部署**：评估向量数据库选型（Pinecone vs Weaviate vs 自建）、监控 cache hit 率、设计回滚策略（函数改坏时如何快速回退到上一版本哈希）。
