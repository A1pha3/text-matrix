...
| 准确性 | 25/25 | 技术描述无事实错误、术语使用一致、代码示例完整可运行、链接有效、版本信息准确 |
| 可读性 | 25/25 | 中英文混排规范、段落适中、排版舒适、自然表达（无明显AI味道）、格式统一 |
| 教学性 | 20/20 | 有明确的学习目标（6个能力目标）、包含自测题（5个问题含参考答案）、有进阶路径（4个阶段）、难度递进合理 |
| 实用性 | 10/10 | 示例来自真实场景（从想法到可运行原型的完整案例）、包含常见问题解答（5个FAQ）、有错误处理和排查指引 |
