...
- [内容结构特征](#内容结构特征)
- [三类典型读者](#三类典型读者)
- [自托管选型常踩的几条边界](#自托管选型常踩的几条边界)
- [与同类 awesome-list 的对比](#与同类-awesome-list-的对比)
- [推荐使用方式](#推荐使用方式)
- [适用边界与已知问题](#适用边界与已知问题)
- [总结](#总结)
- [自测题](#自测题)
...
检验对 Self-Hosting Guide 的理解程度：

1. **这份 Guide 的价值是什么？它不是什么？**
   - 参考答案：价值是"带场景感知的 awesome-list 入口"，给刚入自托管赛道的人一张完整地图，给已经在跑服务的人提供按技术栈横向索引。它不是教程合集，不能直接替代官方文档或上手教程。

2. **这份 Guide 的目录结构是什么？为什么要这样组织？**
   - 参考答案：双层组织（先通用工具，再按技术栈分主题）。这样组织的原因是：通用工具层覆盖自托管路径上的所有通用工具，按技术栈分主题层详细展开该方向下的工具链与上手路径。

3. **自托管选型时常踩的几条边界是什么？**
   - 参考答案：Docker vs Podman（守护进程 vs 无守护进程、systemd 集成、桌面端生态、Compose 兼容）、WireGuard vs Tailscale（控制平面、NAT 穿透、学习成本、数据落地）、Nextcloud vs 商业云盘、Home Assistant vs 通用智能家居网关、Grafana vs Uptime Kuma vs Promtail。

4. **如何正确使用这份 Guide？**
   - 参考答案：第一遍扫描目录、按需深挖、横向比较、不要照搬部署片段。

## 练习

### 练习一：为你的 Home Lab 选型

**目标**：根据你的 Home Lab 需求，从 Guide 中选出合适的工具。

**步骤**：
1. 列出你的 Home Lab 的 3 个主要需求（如 NAS、VPN、智能家居）
2. 在 Guide 的目录中找到对应的主题章节
3. 对比同一节内的 3 个同类项目，选出最适合你的 1 个
4. 跳到对应官方仓库或文档，验证活跃度和适用性

**通过标准**：选型合理，验证充分。

### 练习二：对比 WireGuard 与 Tailscale

**目标**：理解两种 VPN 方案的差异，能根据场景选出合适的方案。

**步骤**：
1. 在 Guide 中找到 WireGuard 章节
2. 列出 WireGuard 和 Tailscale 的各 3 个差异点
3. 给出一个具体场景（如"小型团队远程办公"），说明应该用 WireGuard 还是 Tailscale
4. 解释为什么在这个场景下选择这个方案

**通过标准**：对比清晰，场景选择合理，解释有说服力。

### 练习三：评估 Guide 的适用性

**目标**：理解 Guide 的适用边界，能判断它是否适合当前需求。

**步骤**：
1. 列出 Guide 的 3 个适用场景和 3 个不适用场景
2. 给出一个具体需求（如"需要教程逐条执行"），说明 Guide 是否适合
3. 如果不适合，给出替代方案
...
- [ ] **阶段一：扫描 Guide 的目录** — 通读 `## Tools for Self-Hosting` 的所有三级标题，记住每个分类下最有名的 2-3 个项目
- [ ] **阶段二：深挖一个主题** — 选一个你感兴趣的主题（如 WireGuard），读 Guide 中的对应章节，再跳到官方文档深入学习
- [ ] **阶段三：部署一个自托管服务** — 选一个适合你的自托管服务（如 Nextcloud 或 Home Assistant），按照官方文档完成部署

## 常见问题

### 1. 这份 Guide 可以商用吗？

仓库没有 LICENSE 文件，默认遵循 GitHub ToS。引用与重分发时建议保留原作者署名（Mike Royal）。

### 2. Guide 的内容是最新的吗？

更新频率趋于放缓。`pushed_at` 最近一次提交停留在 2025-06-27，README 显示 `2026-06-15 updated_at` 来自 GitHub 自动维护性更新（star/topic 同步），不是代码/内容变更。

### 3. Guide 有中文版吗？

没有。README 全英文，没有翻译分支。如果需要中文资源，可以自己翻译或找社区翻译版本。

### 4. 如何贡献到这份 Guide？

可以在 GitHub Issues 区提出"建议加入 XX"或"建议删除 YY"，维护者会审核。也可以直接提交 PR。

### 5. Guide 中的工具都是开源的吗？

大部分是开源的，但也有少量闭源或混合许可的工具。使用前建议检查每个工具的许可证。