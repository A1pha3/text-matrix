...
2025 年，微软 TypeScript 团队宣布将 tsc 从 TypeScript/JavaScript 重写为 Go 实现。项目 repo `microsoft/typescript-go` 已有 25k stars，目前以 `@typescript/native-preview` npm 包提供预览版。

Go 重写带来的变化很直接：本地机器码、无运行时依赖、极低启动延迟、原生多线程。对于小型项目可能感知不强，但面对百万行级别的 TypeScript 代码库，tsc 的增量构建和类型检查耗时一直是开发体验的痛点——这一版就是冲着这个去的。

---

## 一、为什么重写？性能瓶颈是直接动因

TypeScript 编译器自诞生以来就是用 TypeScript/JavaScript 写的，这意味着：

1. **运行时依赖**：需要 Node.js 或其他 JS 运行时
2. **启动开销**：JIT 编译带来的冷启动延迟
3. **内存占用**：JS 引擎的内存开销对长期运行的编译器进程并不友好
...
微软的目标是让 tsc 成为一个可以直接分发、本地执行的高效二进制工具，而不是需要 JS 运行时支撑的解释型工具。
...
VS Code 扩展也已在 [VS Code marketplace](https://marketplace.visualstudio.com/items?ItemName=TypeScriptTeam.native-preview) 上架，配置：
...
当前版本支持大多数标准的模块解析模式，包括 Classic、Node (Node.js 风格)、AMD / UMD / System、esModuleInterop 相关选项。一些边缘解析模式仍在完善中。
...
typescript-go 不是永久分叉，它最终的归宿是合并回 TypeScript 主仓库。当前的独立 repo 和 issue tracker 会在某个时间点关闭，所有开发成果会以对用户透明的方式进入 TypeScript 官方版本。这意味着：
...
## 五、与 TypeScript 6.0 的有意变更

项目维护了一份明确的 [CHANGES.md](https://github.com/microsoft/typescript-go/blob/main/CHANGES.md)，记录了与 TypeScript 6.0 的**有意变更**。这些变更经过了设计讨论，不是 bug。如果你在预览版中发现了与原版不同的行为，先去 CHANGES.md 查一下——很可能是有意为之。
...
## 八、Go 如何实现 TypeScript 的类型系统

TypeScript 的类型系统出了名的复杂——协变/逆变推导、泛型约束、模板字面量类型、分布式条件类型。用 Go 来实现意味着：

### 类型表示

Go 没有泛型模板（泛型只在运行时通过反射或代码生成实现），TypeScript 的泛型系统在 Go 中需要用接口加类型断言来模拟，或者通过代码生成在编译期展开。
