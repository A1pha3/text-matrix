...
source_key: "gh:EvoLinkAI/awesome-gpt-image-2-prompts"
...
| UI 截图、信息图、说明海报、拆解图 | freestylefly | 强项在把布局与文字层级写成结构 |
...
研究这四个仓库之后，一个非常清楚的结论是：**稳定的 prompt，约束越明确越好，而不是形容词越多越好。**
...
GPT Image 2 的价值不在“神奇”，而在**它终于足够适合做结构化控制**。这也是为什么 freestylefly 那种 Prompt-as-Code 路线会特别重要。

## 4. 研究四个仓库后，我建议你重点学这 5 种写法

### 4.1 写实人像：先锁镜头、光线和皮肤，再谈气质

EvoLinkAI 和 Anil-matcha 里，人像类 prompt 的共同点是会先把以下几件事写死：
...
这类 prompt 容易稳定，因为先解决了“像什么设备拍的”“像什么光线里拍的”，而不是一上来写“高级、唯美、电影感”。
...
这类 prompt 的核心是**保留清单**，不是修饰词。你列得越清楚，结果越稳。
...
很多人不是不会写，是还在用“愿望型 prompt”。下面这张表可以直接拿来改写自己的输入。
...
一条 prompt 难以复用，通常是因为它把“想要什么气质”写得太多，把“什么不能变”写得太少。
...
如果你照这个流程走，得到的是一套**能继续迭代的视觉协议**，不是“一次性 prompt”。
...
如果你想确认自己不只是“看懂了”、还能“真的会写”，建议立刻做下面 3 个练习。
...
不一定。对于镜头、材质、摄影风格、 UI 结构这类约束，英文通常更稳定；对于你明确希望输出中文标签、中文海报、中文界面的场景，则应显式写 `Simplified Chinese`，并限制文字数量与层级。更实用的做法是**结构约束用英文，文本要求单独写中文输出规则**。
...
非常有必要。高质量模板往往是从失败案例里知道哪些词不能乱加、哪些约束必须补，而不是从成功案例里一次提炼出来的。最值得保留的失败样本通常有 3 类：
...
## 10. 结语：这 4 个仓库教会你的不只是“更长的 prompt”

把四个仓库放在一起看，最重要的收获其实只有一句话：

**好 prompt 控制得越清楚越好，描述华丽反而是噪声。**

更应该带走的是下面这条工作流，而不是某一段别人已经写好的长 prompt：
