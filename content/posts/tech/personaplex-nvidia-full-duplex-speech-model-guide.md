...
- 说清 PersonaPlex 的定位、架构和工作原理
- 用 pip 或 Docker 装好 PersonaPlex
- 选对声音类型和角色提示词
- 解释全双工对话和语音角色控制的实现方式
- 启动实时交互服务器和跑离线评估
- 自己写角色提示词、创建特定人设的语音助手
- 理解 Moshi 架构在 PersonaPlex 里的角色
...
PersonaPlex 是 NVIDIA 开发的**实时全双工语音对话模型**，通过**文本角色提示词**和**音频声音条件**控制角色。

简单说：你用文字定义 AI 的"人设"（比如"你是一个热情的客户服务员"），AI 就会用自然流畅的语音和你对话，并且**随时打断、交替说话**——跟真人对话一样。
...
由于使用了 **Helium LLM** 作为骨干，PersonaPlex 有较强的**分布外泛化能力**——能处理训练数据中没有见过的场景：
...
PersonaPlex 是一个**全双工语音对话模型**，核心特点：
