...
[Dify](https://github.com/langgenius/dify) 就是干这件事的开源项目。它把 AI 工作流、RAG 管道、Agent、模型管理和 LLMOps 整合到一个可视化界面里，开发者从原型到生产都能在一个平台上完成。

它不是万能的——定制化上限受平台能力约束，追求极致灵活的团队更适合直接用 LangChain/LangGraph。但多数团队的需求落在「快速验证 + 生产可观测」这个区间，Dify 在这里的投入产出比很高。

这篇笔记拆 Dify 的内部结构、部署方式和扩展点，读完能跑通示例，也能判断它适不适合你。
...
Dify 的核心是 API Server，所有用户操作经过它；Worker 承担异步任务；Sandbox 隔离执行不可信代码；PostgreSQL 和 Redis 分别存元数据和缓存。后面几节逐层拆开讲。
...
Agentic Workflow 就是针对这两个问题的系统化方案。它把 AI 任务的执行单元从「一次调用」扩展到「多步循环」，每个步骤可以由 LLM、其他模型或传统代码共同完成，步骤之间通过状态传递形成有向图结构。
...
**提示词即资产。** Prompt、上下文和对话历史是 Dify 的第一等公民。每次对话、每个工作流节点都有版本记录，可以回滚和对比。这个设计对应 LLMOps 的一条经验：模型能力的差异根子在 Prompt 工程，而 Prompt 工程需要版本管理。

**BaaS 优先。** 所有功能都配有 REST API 和 Webhook，天然嵌入已有业务系统。平台本身是 Backend-as-a-Service，前端通过 API 调用所有能力，不依赖 Dify 的前端界面。

### 2.3 Dify 与其他开发方式的对比

有些场景直接调 API 或 LangChain 更合适。下表按维度对比，选型时按团队的实际约束对照判断，而不是按"哪个更先进"。
...
团队需要快速验证 AI 概念并进入生产时，Dify 是投入产出比很高的路径。追求极致定制化或已有成熟基础设施的团队，LangChain/LangGraph 仍然是更灵活的底层框架。两者并不互斥——Dify 的自定义工具机制可以接入 LangChain Chain。
...
**Sandbox** 是一个隔离执行环境，用于安全运行用户上传的自定义 Python 代码片段和部分工具逻辑，防止恶意代码影响主机系统。Dify v1.0+ 对 Sandbox 做了显著强化。
...
这条路径上有三个设计点：同步链路只做必须做的事（模型调用 + 流式返回），副作用全部异步化；Sandbox 把不可信代码隔离在独立容器，主进程不暴露文件系统；模型调用经过 Model Runtime 抽象层，切换提供商不需要改工作流定义。
...
点击「试运行」输入竞品名称，观察每个节点的执行状态和输出。想更进一步，可以改造为支持多个竞品批量输入的版本——用到数组变量和循环节点。
...
Dify 还支持在「日志」页面对同一条用户输入，用不同 Prompt 版本做 A/B 对比。团队探索更好的 Prompt 表达方式时，这个功能可以用真实用户 query 快速验证效果。
