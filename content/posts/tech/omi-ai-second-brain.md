...
**对话流程**：
```
用户问题 → 意图分析 → 向量检索（Milvus/Pinecone）→ 上下文组装 → LLM生成 → 回复
```
...
client = Omi(api_key="your-api-key")

# 搜索记忆
results = client.search("上周会议的要点")

# 获取对话历史
history = client.conversations.list(limit=10)

# 创建行动项
action_item = client.action_items.create(
    content="跟进客户反馈",
    due_date="2026-04-25"
)
```
...
```typescript
// React Native应用接入Omi
import { OmiRN } from '@based/omi-react-native';
...
// 监听新记忆
omi.on('new_memory', (memory) => {
  console.log('新记忆:', memory.content);
});

// 搜索知识库
const results = await omi.search('产品规格');
```
