...
2025 年 10 月 9 日，Anthropic 正式发布 Claude Code 插件系统。同一天，Jesse Vincent 发布了 Superpowers 的第一个版本。截至 2026 年 6 月，这个项目在 GitHub 上积累了超过 238,000 个 Star，成为 Claude Code 生态中安装量仅次于 Anthropic 官方插件的第三方项目。

Superpowers 给 Coding Agent 加了一套开发流程——6 个强制检查点，Agent 每次动手都走同一条航线，每一步都不能跳过。
...
**实战技巧：** 如果你发现 Agent 问得太泛，可以直接给上下文。比如「我们的项目用 TypeScript + Next.js，已经有 Prisma ORM，不要引入新的数据库」。Agent 会把这些写进设计文档的约束节。
...
**实战技巧：** 如果你发现 Agent 一直在写测试但跑不通，可以手动介入。在 brainstorming 阶段明确说「这个任务的测试可以先写骨架，实现过程中再补全」，Agent 会调整策略。
...
Superpowers 不是凭空出现的。它处于 2025-2026 年 AI 辅助开发方法论演进的一个关键节点上。

### Superpowers vs Harness Engineering

Harness Engineering 由 Mitchell Hashimoto（HashiCorp 联合创始人）在 2026 年初正式提出，OpenAI 随后发表的 Codex 实验文章使其成为行业焦点。其公式是：
...
| 关注点 | 开发流程的纪律性 | Agent 环境的可靠性 |

Superpowers 可以理解为 Harness Engineering 的一个具体实现——它把 Harness 理念中的 Context Engineering（AGENTS.md）、Sub-Agents（子 Agent 开发）、Self-Verification（TDD + Review）等组件打包成了一个可安装的插件。独立开发者想立刻获得工程纪律，Superpowers 是最快的路径；团队搭建 Agent 开发环境，Harness Engineering 提供了更底层的设计原则。

### Superpowers vs Compound Engineering

Compound Engineering 是 2026 年兴起的另一个范式，其思想是「每次完成的任务都让下一次变得更快」——通过 AI Agent、自动化测试和快速反馈循环实现指数级生产力提升（300-700%）。它强调知识积累、可复用模式和自我文档化代码库。
...
Compound Engineering 解决的是「如何让第 100 个任务比第 1 个任务快 10 倍」，Superpowers 解决的是「如何让第 1 个任务做对」。两者不冲突——Superpowers 的 TDD 产生的测试套件本身就是 Compound Engineering 所依赖的知识资产。用 Superpowers 做出来的代码库天然具备 Compound Engineering 所需的测试覆盖和可维护性。
...
直接写 prompt 是一次性的、线性的交互，流程控制完全靠手动管理。Superpowers 是结构化的、自动触发的流程——Agent 的行为是预设的，不需要每次提醒「先写测试」「先确认需求」。区别类似于让一个实习生「用敏捷开发方法论」和直接给他一个 Sprint 管理工具——前者靠自觉，后者靠系统。
...
Superpowers 给 Coding Agent 装上了一套工程纪律。它不能让 Agent 变得更聪明，但能让 Agent 在复杂任务中保持方向感——先理解需求，再设计方案，拆成小步，每步验证，持续重构。

这套流程的核心价值不是代码本身，而是**约束**：每个技能是强制规程，不满足条件，流程不会继续。靠更好的 prompt 解决不了"Agent 直接写代码"的问题，需要在系统层面强制执行流程。

适合用 Superpowers 的场景：持续时间超过 30 分钟、涉及多个文件、有明确验收标准的功能开发。不适合的场景：30 秒能完成的单文件修改、纯探索性任务、一次性脚本。

如果你刚开始用 Coding Agent，Superpowers 能帮你建立正确的协作习惯。如果你已经用 Agent 做复杂任务，Superpowers 能减少"方向偏离"的次数。

---

## 练习

### 练习一：用 Superpowers 完成一个小功能

**目标**：安装 Superpowers，用一个模糊需求触发 brainstorming，体验完整流程。

**步骤**：

1. 安装 Superpowers（根据你的 Harness 选择安装方式）
2. 打开 Claude Code，输入一个模糊需求："帮我加个搜索功能"
3. 不要直接写代码——观察 Agent 是否进入了 brainstorming 模式
4. 回答 Agent 的提问，逐段确认设计文档
5. 批准计划后，观察 Agent 是否创建了 Git worktree
6. 观察子 Agent 是否先写测试再写代码

**通过标准**：你完整经历了 brainstorming → worktree → 计划 → 子 Agent 开发 → TDD → 审查 的完整流程。

### 练习二：对比有/无 Superpowers 的 Agent 行为

**目标**：理解 Superpowers 的约束到底改变了什么。

**步骤**：

1. 找一个已安装 Superpowers 的项目
2. 临时禁用 Superpowers（重命名 `.superpowers/` 目录）
3. 给 Agent 一个模糊需求，观察它的行为
4. 重新启用 Superpowers，给 Agent 同样的需求，观察行为差异
5. 记录差异：Agent 是否多问了问题？是否先写了测试？是否拆了任务？

**通过标准**：你能列出至少 3 个明显的行为差异，并解释每个差异对应的 Superpowers 技能。
