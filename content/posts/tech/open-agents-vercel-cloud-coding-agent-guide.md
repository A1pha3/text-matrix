...
draft: false
categories: ["技术笔记"]
tags: ["AI Agent", "Vercel"]
---

## 本文覆盖内容

1. Open Agents 的三层架构设计（Web、Agent Workflow、Sandbox VM）及其设计动机。
2. 核心架构决策：Agent 不运行在 Sandbox 内部，而是通过工具与 Sandbox 交互。
3. 基于官方模板在 Vercel 上完成一键部署，并配置 GitHub 集成。
4. Sandbox 的生命周期管理（休眠与恢复）机制以及对 Agent 稳定性的影响。
5. Open Agents 适合哪些场景，以及如何基于该模板做二次开发。
...
它是一个架构参考实现，不是开箱即用的产品——展示了如何在云端可靠地运行一个 AI 编码 Agent，包括 Web UI、Agent 运行时、代码执行沙箱和 GitHub 集成。Vercel 官方鼓励 fork 并根据需求自行修改。
...
Open Agents 是一个架构参考实现而非开箱即用产品。它展示了**如何在云端可靠地运行需要持久状态的 AI 编码 Agent**，以及**如何让 Agent 通过工具模式与隔离的代码执行环境交互**。
