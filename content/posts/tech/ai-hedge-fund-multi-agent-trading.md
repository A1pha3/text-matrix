...
想快速判断项目边界，先看 `§2`、`§7`、`§14`；想理解架构设计，先看 `§3`、`§5`、`§6`；想尽快跑起来，先看 `§8`、`§9`；想迁移到自己的系统，先看 `§10`、`§12`、`§15`。
...
回测模块复用 `run_hedge_fund()` 作为决策引擎，这一点很关键——意味着研究态运行与回测态运行共享决策逻辑，修改 Agent 行为后能更直接观察策略变化。
...
按阅读目的选择路径。想理解架构，先读 `README.md` → `src/main.py` → `src/agents/risk_manager.py` → `src/agents/portfolio_manager.py`。想研究策略，看 `src/agents/valuation.py`、`src/agents/fundamentals.py` 和 `src/backtesting/`。想了解产品化，看 `app/` 目录下的后端和前端实现。
