...
- [§9 Web 应用：项目已经开始走向产品化](#§9-web-应用项目已经开始走向产品化)
- [§10 这个项目最值得迁移的 5 个设计模式](#§10-这个项目最值得迁移的-5-个设计模式)
  - [10.1 观点生产与风险约束解耦](#101-观点生产与风险约束解耦)
  - [10.2 先缩小动作空间再调用 LLM](#102-先缩小动作空间再调用-llm)
  - [10.3 Agent 注册中心](#103-agent-注册中心)
  - [10.4 数据访问层集中封装](#104-数据访问层集中封装)
  - [10.5 同一决策引擎复用于实盘模拟与回测](#105-同一决策引擎复用于实盘模拟与回测)
- [§11 适合你的阅读与实践路径](#§11-适合你的阅读与实践路径)
  - [11.1 如果你是多 Agent 初学者](#111-如果你是多-agent-初学者)
  - [11.2 如果你更关心策略研究](#112-如果你更关心策略研究)
  - [11.3 如果你更关心产品化](#113-如果你更关心产品化)
- [动手练习](#动手练习)
  - [练习一：跑通最小化流程](#练习一跑通最小化流程)
  - [练习二：对比全量和分析型运行](#练习二对比全量和分析型运行)
  - [练习三：迁移到你的领域](#练习三迁移到你的领域)
- [自测题](#自测题)
- [§15 进阶路径](#§15-进阶路径)
- [总结](#总结)
- [资料口径说明](#资料口径说明)
...
答：核心设计思想是"意见可以发散，落单必须收敛"。分析层负责生成多元化的投资观点，风控层负责约束这些观点，决策层负责最终决策。分离的原因是分析观点可以很多元，但实际动作必须受组合约束、现金约束和风险承受能力限制。
</details>

**问题 2**: 投资风格型 Agent 和功能分析型 Agent 的区别是什么？为什么要同时保留两类 Agent？
<details>
<summary>查看答案</summary>
答：投资风格型 Agent 模拟特定投资风格（如价值投资、成长投资），功能分析型 Agent 提供技术分析（如风险管理、投资组合管理）。同时保留两类 Agent 是因为投资需要既考虑风格多样性，又考虑功能约束。
</details>

**问题 3**: `risk_management_agent` 和 `portfolio_manager` 的职责分别是什么？它们如何协作？
<details>
<summary>查看答案</summary>
答：`risk_management_agent` 负责计算风险约束（如最大单票暴露、最大回撤），`portfolio_manager` 负责根据分析观点和风控约束做最终决策。协作方式：`risk_management_agent` 先计算约束，`portfolio_manager` 在约束内做决策。
</details>

**问题 4**: LangGraph 在这套系统中的作用是什么？它如何定义节点执行顺序？
<details>
<summary>查看答案</summary>
答：LangGraph 负责编排多个 Agent 的执行顺序。它定义哪些 Agent 先执行，哪些 Agent 后执行，以及 Agent 之间的数据传递方式。通过工作流定义，LangGraph 确保分析 Agent 先产出信号，风险管理 Agent 计算约束，投资组合管理 Agent 做最终决策。
</details>

**问题 5**: 为什么这套系统强调"意见可以发散，落单必须收敛"？
<details>
<summary>查看答案</summary>
答：因为分析观点可以很多元（不同 Agent 可能有不同观点），但实际动作必须受组合约束、现金约束和风险承受能力限制。没有风控节点，多 Agent 只会放大噪声，不会提升可用性。收敛设计确保最终决策在安全动作空间内发生。
</details>
