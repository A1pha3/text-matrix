...
内部工具开发是个尴尬的领域：需求琐碎但真实，认真写一套前后端太重，用 Excel 凑合又撑不住权限和协作。ToolJet（39.9k stars，5.3k forks）在这个位置做了多年积累——一个 AGPL v3 开源的低代码平台，主语言 JavaScript，2026 年 8 月中旬仍在以几乎每天一个版本的节奏发版（v3.20.212-lts 与 v3.21.60-beta 相隔数小时）。
...
ToolJet 自托管时是一组服务，以官方架构文档为准：

```
浏览器：ToolJet Client（React）
  │  拖拽建 UI / 绑定查询 / 触发事件
  ▼
ToolJet Server（Node.js API 服务）
  │  认证授权 / 持久化应用定义 / 代跑查询 / 加密存凭据
  │  邮件服务（SMTP / Sendgrid / Mailgun）负责邀请与密码重置
  ▼
PostgreSQL（主数据库）
  ▲
  │  PostgREST：把 PostgreSQL 转成 REST API，供 ToolJet Database 使用
ToolJet Database（内置数据库）
```

- **ToolJet Client**：React 应用，负责可视化编辑、数据绑定、渲染与事件触发。
- **ToolJet Server**：Node.js API 服务，管认证、应用定义持久化、查询执行和数据源凭据的加密存储。
- **PostgreSQL**：主数据库，存应用定义、用户和加密后的数据源凭据。
- **PostgREST**：独立 Web 服务，把 PostgreSQL 暴露成 RESTful API，是 ToolJet Database 的底层。
...
### 机制 2：ToolJet Database 是包了一层 PostgREST 的 PostgreSQL

内置数据库不需要先接一个外部 Postgres。它在主库之上用 PostgREST 生成 REST API，表、字段、关系在 UI 里点出来就能建，天然支持 CRUD。对"只想快速存点数据、不想维护独立数据库"的场景，这一步省掉了最大的入门成本。
...
- 密钥类变量（`LOCKBOX_MASTER_KEY`、`SECRET_KEY_BASE`、数据库密码）：官方脚本 `internal.sh` 会帮你生成。
- 要用 ToolJet AI 功能，需要在网络里放行 `https://api-gateway.tooljet.com` 和 `https://python-server.tooljet.com`。
...
"60+ 组件、80+ 数据源"衡量的是**连接器与组件的覆盖广度**。它说明大多数内部工具的高频需求有现成件可拼，不能推出"任何需求都能在平台内表达干净"。低代码平台的表达力有上限：超过某个复杂度阈值（复杂状态机、特殊渲染、深度定制交互），维护成本会反超传统开发。组件数量也不代表性能——大表要靠分页和虚拟滚动支撑，官方在营销材料里提到可处理十万行级数据，但这针对的是分页场景，不是一次全量渲染。
...
- 读[架构文档](https://docs.tooljet.com/docs/contributing-guide/setup/architecture/)和 docker-compose 文件，弄清 Server / Client / PostgREST 各自的职责，自托管排障会顺利很多。
