...
本文就是要做这件事：先给判断，再给系统地图，再用一个"域名 → IP → ASN → 钱包 → NFT"的真实调查路径把抽象机制串起来，最后给出和 Maltego / SpiderFoot / Maigret 的差异点与采用建议。
...
读这张图要抓住一个关键判断：**依赖是单向的、且最底层是 `flowsint-types`**。这意味着任何模块升级类型定义时，理论上不会反向打穿 API 或前端——它给了项目一个看起来"清楚"的工程边界，也意味着新增一种实体（比如新增 `Vehicle` 实体）只需要触碰 `flowsint-types` 和可能的新 enricher，不会牵连 API。
...
Registry 也不复杂——`flowsint-enrichers/src/flowsint_enrichers/registry.py` 的 `EnricherRegistry.register()` 接受一个 Enricher 类，存到 `_enrichers` 字典里，然后 `load_all_enrichers()` 通过遍历包自动 `importlib` 触发装饰器，**所有 `*Enricher` 类自动注册**。这意味着新增能力只要写一个文件、加上 `@flowsint_enricher` 装饰器，零样板代码就能进入调度池。
...
写到这里必须正面回答一个问题：已经有 Maltego 和 SpiderFoot，为什么要选 Flowsint？
...
差异的本质是：**Maltego 把图藏在商业格式里、SpiderFoot 把图摊平成表格、Maigret 不存图**。Flowsint 是少数把 Neo4j 当作一等公民、并且把"图"作为产品核心来对待的开源项目。如果你的工作方式是"先扫一遍，几天后再回到这张图继续挖"——这是它真正站得住的位置。
...
如果你的场景是"我有一个 OSINT 任务，需要 3 个人协同 2 周"，Flowsint 的架构值得你花一个周末跑通 demo；如果你的场景是"我需要一个图标给我老板"，它太重了。

最后一条建议：**把它当平台来评估，不要当工具来评估**。平台意味着你愿意为它写代码、愿意接你的内部数据源、愿意让团队成员都进同一个 Neo4j——只有进入这个心智模型，Flowsint 的模块化设计才会变成"加速器"而不是"多余抽象"。
