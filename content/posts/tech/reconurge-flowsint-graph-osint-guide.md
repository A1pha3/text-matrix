...
1. 画出 Flowsint 的五模块架构图，说清楚各模块的单一职责和依赖方向
2. 解释 Enricher 的注册机制（`@flowsint_enricher` 装饰器 + Registry）以及 `scan()` 和 `postprocess()` 的分工
3. 走通一条"域名 → IP → ASN → 钱包"的调查路径，理解前端拖拽如何映射为 Celery 任务
4. 判断 Flowsint 和 Maltego / SpiderFoot / Maigret 在定位上的本质差异
5. 评估自己的场景是否适合投入时间来部署和维护 Flowsint

## 目录

| → | [系统地图](#系统地图先把五个模块的边界拆开) | [数据模型](#数据模型调查图里的节点和边到底是什么) | [任务流案例](#任务流案例一次域名--ip--asn--钱包--nft会流过哪些模块) | [模块边界](#模块边界哪些东西看起来有其实没有) |
|---|---|---|---|---|
| → | [对比分析](#与-maltego--spiderfoot--maigret-的对比定位到底差在哪) | [安装与上手](#安装与上手先跑通再决定要不要深挖) | [适用场景与边界](#适用场景与边界谁该先用谁可以等等) | [FAQ](#faq) |
...
1. Flowsint 的五个模块各有什么单一职责？为什么 `flowsint-types` 放在最底层？
2. `scan()` 和 `postprocess()` 的分工是什么？为什么图写入放在 `postprocess()` 里而不是 `scan()` 里？
3. 如果你要新增一个"域名 → SSL 证书"的调查能力，需要改哪几个模块、写哪几行代码？
4. 在什么场景下 Flowsint 比 Maltego 更合适？什么场景下反过来？
5. 为什么 Flowsint 的 README 说测试套件是 "incomplete"，这对生产部署意味着什么？
