...
OAuth 2.0 scope 控制的是"Agent 能访问哪些服务"，不是"访问服务后能执行哪些动作"。持有 `query_database` 权限的 Agent 同样可以执行 `drop_table`，scope 在此处失效。

AGT 的策略引擎在工具调用真正发生之前拦截，用代码判断动作是否允许，不依赖模型配合。策略引擎的拒绝发生在代码层：被 AGT 拒绝的动作，Agent 在代码层面无法绕过。
...
需要启用 AGT 的身份层（零信任身份验证）。为每个 Agent 分配独立身份（SPIFFE/DID），出问题时审计日志里会记录 `agent: did:mesh:agent-7` 这样的身份标识，可以精确追溯。

身份层与 MCP 安全网关配合，可以检测工具投毒、名称抢注和隐藏指令注入。
...
隔离执行沙箱层负责限制影响范围。AGT 的 Agent Runtime 用四特权环隔离执行：
- Ring 0：内核级，最高权限，几乎不开放给 Agent
- Ring 1：系统级，可访问文件系统和网络
- Ring 2：应用级，受限文件和网络访问
- Ring 3：用户级，仅可计算，无 I/O

不可信代码应路由到 Ring 3 或 Ring 2 执行。即使代码逃逸了模型约束，也无法触达宿主系统的敏感资源。
...
应先上策略执行 + 审计日志。原因：
1. 这两层覆盖 LLM06（过度授权）和 LLM10（无界消耗）两类核心风险
2. 不依赖额外基础设施，投入产出比最高
3. 同时上所有层会增加运维复杂度和延迟开销（沙箱模式引入 10-30% 延迟开销）
...
AGT 在模型输出之后、工具执行之前插入代码层拦截：
- 注入可能成功（模型被误导），但注入成功后试图执行的危险动作会被策略引擎拦截
- 根因在模型层，AGT 解决的是"注入成功后的损害控制"，不是"注入预防"
- AGT 项目红队测试报告显示，提示词级安全的策略违规率为 26.67%，应用层强制执行的违规率为 0.00%

（对应章节：风险覆盖与 OWASP Top 10 的映射）
...
**主要优化点：**
1. 添加学习目标、目录、常见问题排查、练习题、进阶路径等完整教学元素
2. 将"自测检查"改为标准"自测题"格式（5道题，含`<details>`标签参考答案）
...
**优化历史：**
- 2026-06-28：补充了"练习题"章节、"资料口径说明"章节，拆分长段落
- 第47轮自动化任务：将"自测检查"改为标准"自测题"格式，清理并更新优化说明章节
