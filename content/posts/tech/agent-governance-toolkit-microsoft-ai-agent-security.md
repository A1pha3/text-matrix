...
AGT 的架构可以拆成四条独立主线，分别解决动作是否允许、谁发起的、危险代码如何隔离、失控后如何止损这四个问题。
...
提示词约束是对模型的概率性请求。OAuth 2.0（开放授权 2.0）的 scope 和 IAM 角色控制的是 Agent 能访问哪些服务，而不是 Agent 连上服务后会做什么。一个有 `query_database` 权限的 Agent 同样可以执行 `drop_table`，OAuth 2.0 scope 拦不住。策略引擎补的就是这一层：在工具调用真正发生之前，用确定性代码判断动作是否允许。
...
用一个具体任务把四层护盾串起来：用户让 Agent 清理数据库旧数据，Agent 决定调用 `drop_table` 工具。
...
策略引擎的拒绝是结构性的：被 AGT 拒绝的动作，Agent 在代码层面无法绕过。提示词级安全只是请求模型不要做，AGT 让越权动作在代码层不可执行。
...
不要捕获 `Exception` 后忽略 `GovernanceDenied`，这会让治理失效。捕获特定异常，记录上下文，走审批流程。
