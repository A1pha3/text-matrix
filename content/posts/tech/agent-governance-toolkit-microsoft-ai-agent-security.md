...
Agent Governance Toolkit（以下简称 AGT）把 Agent 的不可控行为从模型概率层下沉到确定性代码层。提示词级别的安全约束（“请遵守规则”）是对随机系统的礼貌请求，不构成控制面。AGT 在模型意图到达工具之前，用确定性代码拦截每一次工具调用、消息发送和 Agent 委托。

为什么需要这样做？OWASP 在 [OWASP Top 10 for LLM Applications 2025](https://genai.owasp.org/llm-top-10/) 的 LLM01:2025 Prompt Injection 条目中明确指出：“it is unclear if there are fool-proof methods of prevention for prompt injection”（提示词注入可能没有万无一失的预防方法）。学术研究也支持这一判断：Andriushchenko 等人在 ICLR 2025 论文中报告，对 GPT-4o、Claude 3、Llama-3 使用自适应攻击，越狱成功率达 100%。AGT 自己的红队测试显示，提示词级安全的策略违规率为 26.67%，而应用层强制执行的违规率为 0.00%。

**项目地址：** [github.com/microsoft/agent-governance-toolkit](https://github.com/microsoft/agent-governance-toolkit)
**当前版本：** v3.7.0（Public Preview，MIT 许可证）
...
AGT 的架构可以拆成四条独立主线，每条主线解决一个不同的问题。读者先搞清边界，再进入细节。
...
| 审计日志 | 能否证明发生了什么 | 防篡改决策记录 | 合规场景必需 |

每一层都是可选的。大多数团队从策略执行加审计日志起步，随着风险面扩大再加其他层。

## 核心能力

### 策略执行引擎

策略引擎是 AGT 的第一道关卡，每一次工具调用都会被拦截并求值。默认拒绝（default deny），策略不匹配时动作不会放行。
...
为什么不用提示词约束？因为提示词约束是对模型的概率性请求。OAuth 2.0（开放授权 2.0）的 scope 和 IAM 角色控制的是 Agent 能访问哪些服务，而不是 Agent 连上服务后会做什么。一个有 `query_database` 权限的 Agent 同样可以执行 `drop_table`，OAuth 2.0 scope 拦不住。策略引擎补的就是这一层：在工具调用真正发生之前，用确定性代码判断动作是否允许。

### 零信任身份验证

在多 Agent 系统里，五个 Agent 可能共享一个 API key。出问题时“某个 Agent 干的”不构成事件响应。AGT 的身份层为每个 Agent 分配独立身份：

- **SPIFFE**：工作负载身份框架，为 Agent 颁发可验证的身份凭证。
- **DID（去中心化标识符）**：跨组织 Agent 协作时的身份互信。
- **mTLS**：Agent 与工具之间的双向加密认证。

身份层与 MCP（Model Context Protocol，模型上下文协议）安全网关配合，可以检测工具投毒、名称抢注（typosquatting）和隐藏指令注入。MCP 是 Anthropic 提出的工具调用协议，AGT 的 MCP Security Gateway 在 MCP 工具注册和调用时做安全校验。
...
危险工具调用会被路由到低权限环执行。即使代码逃逸了模型约束，也无法触达宿主系统的敏感资源。

### 可靠性工程

Agent 在生产环境会失控：无限循环、资源耗尽、级联失败。AGT 的 Agent SRE 模块提供：
...
这一层直接对应 SOC 2、ISO 27001 等合规框架对“可追溯性”的要求。
...
抽象的架构图看不出四层护盾怎么配合。下面用一个具体任务串起来：用户让 Agent 清理数据库旧数据，Agent 决定调用 `drop_table` 工具。
...
关键点：策略引擎的拒绝是结构性的，不是“不太可能”。被 AGT 拒绝的动作，Agent 在代码层面无法绕过。这是 AGT 与提示词级安全的根本区别——前者让越权动作在代码层不可执行，后者只是请求模型不要做。
...
需要说明边界：AGT 主要解决 LLM06（过度授权）和 LLM10（无界消耗）这两类应用层风险。LLM01（提示词注入）的根因在模型层，AGT 无法消除注入本身，但可以保证注入成功后危险动作仍被拦截。LLM04（数据投毒）和 LLM08（向量弱点）的防护依赖训练数据和嵌入层治理，AGT 提供的是检测和审计能力，不是根治方案。
...
1. **先上策略执行 + 审计日志**。用 `govern()` 包装现有工具，编写 YAML 策略拦截破坏性动作。这两层覆盖 LLM06 和 LLM10，投入产出比最高。
...
哪类团队先上：金融、医疗、企业内部 IT 自动化等有合规要求且 Agent 持有高风险工具的团队。哪类团队可以等等：还在原型验证阶段、Agent 只做信息查询、没有不可逆副作用的团队。
...
不要捕获 `Exception` 后忽略 `GovernanceDenied`，这会让治理形同虚设。捕获特定异常，记录上下文，走审批流程。
...
沙箱模式会引入 10-30% 的延迟开销（取决于工具类型）。对延迟敏感的场景，可以只对高风险工具启用沙箱，低风险工具直接执行。
