...
1. **区分开源库与云服务的能力边界**，判断自己的场景该走哪条线
2. **用 uv 跑通最简示例**，定位环境问题
3. **画出 Agent / Browser / LLM Adapter / Tools 四个组件的数据流向图**
4. **给 Agent 注册自定义工具**，扩展查天气、调内部 API 这类能力
5. **在生产环境识别内存、反爬、CAPTCHA 三类常见坑**并选对应方案
6. **用 CLI 快速探索页面结构**，再决定是否写完整脚本
