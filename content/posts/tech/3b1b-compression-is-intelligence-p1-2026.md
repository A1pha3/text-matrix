...
源视频: [3Blue1Brown《Reinventing Entropy | Compression is Intelligence Part 1》](https://youtu.be/l6DKRf-fAAM)，片长约 31 分钟，2026-06-07 发布于 YouTube（中文版字幕：翻译贰鼠 / 校正畏狐之狐）。文中标注的「mm:ss」为原视频时间点，方便按段回看。
...
3Blue1Brown（下文简称 3B1B）从最朴素的问题切入：**ASCII 编码每字符 8 个 bit，太浪费了**。
...
- 猜中 z（P=0.0001）：-log₂(0.0001) ≈ 13.3 bits
...
这里有两个翻译，贯穿全片：把"压缩"翻译成"预测"，把"信息量"翻译成"惊讶度"。预测得越准 = 越不惊讶 = 信息越少 = 码字越短。两个方向在数学上是等价的，第七节会证明。
...
那"上下文感知"能到什么程度？3B1B 把镜头拉回香农本人。香农 1951 年发表在 Bell System Technical Journal 上的《Prediction and Entropy of Printed English》里，用的"语言模型"是他妻子 Betty。
...
结果：**当猜手至少有 100 个字符的上下文时，香农估计英语的熵在 0.6–1.3 bits/字符之间，通常引用为约 1 bit/字符**。这个数字乍看有点疯——意思是每个字符平均只需要一个"是/否"就能还原。对比第一节，ASCII 是 8 bits，独立预测约 4 bits，长上下文压到约 1 bit。而 3B1B 在结尾预告，Part 3 会用一个高质量语言模型做编解码，把文本压到**非常接近这个下限**（原话是 surprisingly close）。
...
如果你有一个完美的预测器 P(next char | context)，你可以用**算术编码**（arithmetic coding）把文本编码成平均 -log₂(P) bits/字符。视频只点出等价关系，把具体算法留给了 Part 3，这里把机制补齐：它不把每个符号单独映射成固定码，而是把整条消息看作 [0,1) 区间里的一个点——每读到一个字符，就把当前区间按预测概率切成几段，取对应字符那段，最后用区间里任意一个二进制小数代表整条消息。概率高的字符占的区间大，需要的小数位就少；整条消息的编码长度约等于所有字符 -log₂(P) 之和——这正是交叉熵的定义。预测越准，区间越"窄"，编码越短。
...
第三条是真正的革命。它意味着**任何编码的平均长度都压不破熵这条下界**，同时也保证下界不是空想——总能无限接近。
...
| 几字符（n-gram） | ~2–3 bits/字符 | 3–4x |
| 一句话（十几字符） | ~1–2 bits/字符 | 4–8x |
| 更长上下文（≥100 字符） | ≈1 bit/字符 | ≈8x |

说明：前几行是数量级示意，与香农 1951 年实测的条件熵递减曲线（知道前 2–3 个字符时约 2–3 bits/字符）同量级；最后一行对应香农在至少 100 字符上下文下的估计（0.6–1.3 bits/字符）。更长的文档级上下文条件熵还会更低，但视频没有给具体数字。
...
- 想看 3B1B 如何把同一套工具推到神经网络训练上。本文只覆盖 Part 1 的核心；把这套框架完整接到 Transformer 上，是 Part 2 的事。

**视频信息**：[3Blue1Brown《Reinventing Entropy | Compression is Intelligence Part 1》](https://youtu.be/l6DKRf-fAAM)，约 31 分钟，2026-06-07 发布。这是系列的第一集，Part 2[《But what is cross-entropy? | Compression is Intelligence Part 2》](https://youtu.be/GlYgs6v2YfU)于 2026-07-16 发布。本文按主题重组，不逐句转写；文中标注的「mm:ss」为原视频时间点，可按它回到具体某段。

## 延伸阅读

想往下挖，按这条线走：

- **Shannon 1951《Prediction and Entropy of Printed English》**（Bell System Technical Journal）：猜字母实验的原始出处，也是"英语熵约 1 bit/字符"这个估计的来源。
- **Cover & Thomas《Elements of Information Theory》**：熵、交叉熵、KL 散度的标准教材，第 5 章处理信源编码定理。
- **Delétang et al.《Language Modeling Is Compression》**（DeepMind，2023，[arXiv:2309.10668](https://arxiv.org/abs/2309.10668)）：把"预测 = 压缩"落成实测——用算术编码把 Chinchilla 系列模型变成无损压缩器，直接和 PNG、FLAC、gzip 这类压缩器对比。
...
- [ ] 能回答：一个训练好的 LLM 的 cross-entropy loss，理论下界的含义是什么，下界由什么决定
