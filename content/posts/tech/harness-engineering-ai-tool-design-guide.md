...
- 解释 Harness（缰绳）的核心定位：它是一套「给 AI 造缰绳」的结构化方法，而不是某个具体工具
- 对照五个组件（指令层、约束层、能力层、记忆层、编排层）判断你的 AI 协作流程缺哪一层
- 区分「建议」和「约束」：为什么 Must Not / Should / May 三层约束比一条「请」字更有效
- 在真实项目中从零建立一份可用的 Harness（INSTRUCTIONS.md + CONSTRAINTS.md + CAPABILITIES.md）
- 判断什么时候该「加规则」，什么时候该「做减法」——Harness 不是一次性工程产物，而是在每次 AI 犯错后生长出来的

## 目录

1. [核心问题](#1-核心问题)
2. [什么是 Harness](#2-什么是-harness)
3. [Harness 的五个组件](#3-harness-的五个组件)
4. [五个组件详解](#4-五个组件详解)
5. [实战案例](#5-实战案例)
6. [如何开始](#6-如何开始)
7. [自测题](#自测题)
...
Mistake → Rule 的循环是 Harness 进化的核心机制。
...
每一条指令、约束和能力定义都有维护成本。删掉一条不必要的规则，比新增一条规则更能提升 Harness 质量。
...
当前阶段的重心是把成功项目的 Harness 模板化、可移植化。随着模板积累，从手工设计过渡到半自动生成是可行方向；但经验工程的瓶颈不在生成 Harness，而在从散落项目里提取出经得起复用的模式。
...
1. Harness 的五个组件中，「指令层」和「约束层」的核心差异是什么？分别举一好一坏的例子。
2. 为什么「建议」（May）在 Harness 里要单独分层？如果你把所有建议都改成强制约束，会出现什么问题？
3. 记忆层存哪三类信息？如果你要让 AI 在复盘中「记住」上次的架构决策，应该写入哪种记忆？
4. 编排层解决什么场景下的问题？举一个单 Agent 搞不定、需要多 Agent 协作的具体例子。
5. Mitchell Hashimoto 说「每次犯错加一条规则」。如果你的 Harness 文件已经膨胀到 500 行，该怎么办？

## 练习

### 练习 1：为你的主力项目建立第一份 Harness

1. 选一个你最近在做的项目
2. 创建 `.harness/` 目录，写第一版 `INSTRUCTIONS.md`（指令层）和 `CONSTRAINTS.md`（约束层）
3. 用这份 Harness 完成 3 个真实任务，记录：哪些指令不够明确？哪些约束被 AI 绕过了？
4. 根据记录迭代 Harness，目标是：同样的任务，第二次比第一次少改 50% 的 AI 输出

### 练习 2：把项目中的重复模式抽成能力层

1. 找出你项目中 3 个反复出现的模式（如错误处理、API 响应格式、测试结构）
2. 为每个模式写一个 `CAPABILITIES.md` 条目，包含：模式名称、适用场景、代码示例、必须遵守的规则
3. 让 AI 在后续任务中主动引用这些能力，观察输出质量的变化

### 练习 3：建立一个「AI 犯错 → 加规则」的复盘循环

1. 每次 AI 输出不符合预期时，记录：预期是什么、实际输出是什么、根因是什么
2. 如果根因是「指令不明确」，加到指令层；如果是「边界没定清楚」，加到约束层；如果是「经验没沉淀」，加到能力层
3. 每月回顾一次 Harness 文件，删掉已经内化到习惯里、不需要再写的规则
...
- 从指令层和约束层开始，不要一次铺满五层
- 先写 `INSTRUCTIONS.md`（给 AI 一张地图）和 `CONSTRAINTS.md`（明确边界）
- 迭代 3-5 个任务后再看效果，不要写完就放着

### 进阶（Harness 已经在用，但想提质量）

- 把项目中反复出现的模式抽成能力层（CAPABILITIES.md）
- 建立记忆层：项目记忆（架构决策）+ 任务记忆（进度）+ 偏好记忆（代码风格）
- 引入编排层：如果任务已经复杂到单 Agent 搞不定，定义 Agent 角色分工

### 专业（Harness 成为团队基础设施）

- 建立 Harness 模板库：不同项目类型（Web API、CLI 工具、数据分析）各有一套基线 Harness
- 定期做「减法回路」：删掉已经被能力层覆盖的冗余指令、合并同类约束
- 把 Harness 的迭代记录变成团队的集体知识（每次犯错加一条规则 → 团队共享）
