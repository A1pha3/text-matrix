...
> **核心判断**：这个仓库交付的不是"金融版聊天机器人"，而是一套把投行、行研、私募、财富管理和基金运营的工作流拆成 agent、skill、command 和 connector 的参考实现——同一份 prompt 和 skill，可以在 Cowork 里交互式用，也可以通过 Managed Agents API 挂到自家编排层后面
...
`vertical-plugins/` 里的 skills 是"源"，`agent-plugins/<slug>/skills/` 是"副本"。每个命名 agent 把要用的 skills 打包了一份，装 agent 时不需要额外补装 vertical plugin。但如果你只想拿 `/comps`、`/dcf`、`/earnings` 这样的单条命令，直接从 vertical plugin 装会更干净。仓库里还配了 `scripts/sync-agent-skills.py`，用来在修改 vertical 源文件后把更新推到所有打包了该 skill 的 agent。
...
**这是参考模板，不是即插即用的生产系统。**仓库内容几乎都是 Markdown、JSON 和 YAML——没有构建系统，没有二进制分发，没有 docker-compose。你可以直接装起来试，但只要牵涉真实金融数据、内部术语、PPT 模板、Excel 模板、审批链路或监管留痕，几乎都要做二次定制。Anthropic 给的不是一个封闭产品，而是一套"你们公司往里塞自己流程和约束"的骨架。
...
1. 仓库提供的不是数据本体，而是"把数据接进工作流的接口形状"。用这个仓库不等于免费用 Bloomberg 或 CapIQ 的数据。
...
如果你的公司让 Claude 跑在 Excel、PowerPoint、Word 和 Outlook 里，通过 Microsoft 365 加载项来用，这个模块就是给 IT 管理员用的部署工具。它不是 Cowork 插件，而是 Claude Code 插件，走的是企业自有云——Vertex AI、Bedrock 或内网 LLM gateway——而不是 Anthropic 的 API。
...
这个仓库不是为了把金融专业人员从流程里拿掉，而是为了把他们从大量第一版底稿、整理、校核、拼装和标准化输出里解放出来。
