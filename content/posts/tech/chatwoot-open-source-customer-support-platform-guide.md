...
### 练习一：部署 Chatwoot 到 Docker Compose 并完成基础配置

**目标**：完成 Chatwoot 的自部署，接入一个邮件渠道，并验证基本会话流转。

**步骤**：

1. 准备一台 4 核 8G 的服务器（或本地 Docker 环境）。
2. 按照 `docker-compose.production.yaml` 启动所有服务（PostgreSQL + Redis + Rails + Sidekiq）。
3. 完成首次配置向导：创建管理员账号、配置邮件渠道（用 Mailgun 或 SendGrid 的 SMTP）。
4. 在网站上嵌入 Widget，从访客端发一条测试消息，验证坐席端能收到并回复。
5. 检查 Sidekiq Web UI（`/sidekiq`），确认异步任务正常运行。
...
**目标**：为团队制定一个可复用的渠道接入流程，避免遗漏关键配置。

**步骤**：

1. 列出你们团队需要接入的渠道（网站 Widget、邮件、WhatsApp、Facebook 等）。
2. 为每个渠道整理需要的凭证、Webhook 配置、消息格式要求。
3. 设计一个测试矩阵：每个渠道至少测试"发送消息""接收回复""附件传输""表情符号"四个场景。
4. 输出一份检查清单文档，包含每个渠道的接入步骤、常见错误、排查方法。

**通过标准**：检查清单能被一个新成员拿来独立完成渠道接入，且一次通过率 > 80%。
