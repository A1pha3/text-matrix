...
| Forks | 3,454+ |
| License | MPL-2.0 |
| 主要语言 | Clojure (后端) + ClojureScript (前端) + Rust (WASM渲染层) |
| 创建时间 | 2015-12-29 |
| 最新稳定版 | 2.16.1 (2026-06-24) |
| 核心定位 | 开源设计协作平台，支持自托管和设计即代码 |

## 学习目标

读完本文后，你应该能够：

- 理解 Penpot 的核心价值：开源、自托管、设计即代码
- 掌握 Penpot 的架构：Clojure/ClojureScript 全栈 + Rust/WASM 渲染层
- 了解 Penpot 与 Figma 的关键差异：开放标准、数据所有权、MCP Server
- 部署 Penpot 到自托管环境（Docker Compose / Kubernetes）
- 判断 Penpot 是否适合你的团队（适用边界）

## 目录

- [快速信息卡](#快速信息卡)
- [学习目标](#学习目标)
- [一、核心判断：Penpot 的"开放性"不是营销话术](#一核心判断penpot-的开放性不是营销话术)
- [二、系统地图：Clojure 后端 + ClojureScript 前端的全栈统一](#二系统地图clojure-后端--clojurescript-前端的全栈统一)
- [三、L4 前端：ClojureScript + Rust/WASM 的混合渲染](#三l4-前端clojurescript--rustwasm-的混合渲染)
- [四、L3 后端：Clojure + Ring + PostgreSQL 的"老派稳健"](#四l3-后端clojure--ring--postgresql-的老派稳健)
- [五、设计的"代码化"：Design Tokens + Inspect 模式](#五设计的代码化design-tokens--inspect-模式)
- [六、MCP Server：2026-06 的关键 GA](#六mcp-server2026-06-的关键-ga)
- [七、自托管：Docker / Kubernetes / Elestio 三条路](#七自托管docker--kubernetes--elestio-三条路)
- [八、2026 关键版本演进](#八2026-关键版本演进)
- [FAQ](#faq)
- [自测题](#自测题)
- [进阶路径](#进阶路径)
...
**Q1：Penpot 和 Figma 的核心区别是什么？**

Penpot 是开源（MPL-2.0）且支持完全自托管，设计文件使用开放格式（SVG/CSS/HTML/JSON）；Figma 是闭源 SaaS 服务，使用私有 .fig 格式。Penpot 的 MCP Server 已 GA，支持 AI Coding 工具直接读写设计。

**Q2：Penpot 的 MCP Server 是什么？有什么用？**

MCP Server 是 Penpot 2.16 系列（2026-06）的关键功能，允许 AI Coding 工具（Cursor/Claude Desktop/Claude Code）直接读写 Penpot 设计。你可以在 IDE 里说"把 Penpot 上这个组件导出成 React 代码"，实现设计到代码的自动化。

**Q3：Penpot 适合生产环境吗？**

适合。Penpot 已发展到 2.16.1 版本，MCP Server 已 GA，Docker Compose/Kubernetes/Elestio 都提供部署方案。特别适合需要数据自托管的金融/政府/国企场景。

**Q4：如何从 Figma 迁移到 Penpot？**

可以通过 SVG 导出 + 组件映射（手动或半自动）迁移。新项目建议先用 Penpot 建立 Design System 工作流；存量项目可以双轨运行（Penpot 跑新组件、Figma 跑存量）。

**Q5：Penpot 的插件生态如何？**

相比 Figma 还很年轻，但核心功能（Design Tokens、组件/变体、Inspect 模式）已成熟。插件开发 SDK 已开放，社区正在成长。

---

## 自测题

**Q1：Penpot 的许可证是什么？它允许商用吗？**

A1：MPL-2.0。允许商用，但修改后的 MPL 代码段必须公开（和 MIT/Apache 比更严格）。

**Q2：Penpot 的前端和后端分别用什么语言？**

A2：后端用 Clojure（JVM），前端用 ClojureScript，高强度渲染模块用 Rust/WASM。

**Q3：Penpot 的 MCP Server 在哪个版本 GA？**

A3：2.16.0（2026-06-11）完整 GA，2.16.1（2026-06-24）有多项生产级修复。

**Q4：Penpot 的设计令牌（Design Tokens）可以导出哪些格式？**

A4：CSS Variables、JSON、Style Dictionary、Tokens Studio 兼容格式。

**Q5：Penpot 自托管需要哪些基础设施？**

A5：PostgreSQL（主存储）、Redis（实时协作广播）、文件存储（本地/S3 兼容对象存储）、Nginx（反向代理，需配置 WebSocket 透传）。
...
**阶段1：基础使用（1-2周）**
- 部署 Penpot（Docker Compose 一键启动）
- 创建第一个设计项目
- 学习 Design Tokens 和组件/变体
- 使用 Inspect 模式导出代码

**阶段2：团队协作（1个月）**
- 设置 Design System 工作流
- 配置实时多人协作
- 建立设计审查流程
- 集成到开发工作流（Inspect 模式）

**阶段3：MCP Server 集成（1-2个月）**
- 配置 MCP Server（WebSocket 代理）
- 在 Cursor/Claude Code 里调用 Penpot
- 实现设计到代码的自动化
- 建立"设计即代码"工作流

**阶段4：企业级部署（2-3个月）**
- Kubernetes 生产级部署
- PostgreSQL/Redis 高可用配置
- 备份和灾难恢复策略
- 权限管理和 SSO 集成
