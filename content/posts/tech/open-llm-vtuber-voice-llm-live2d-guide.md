...
- 它如何用一个 Silero VAD 状态机（IDLE / ACTIVE / INACTIVE）+ `asyncio.Task.cancel()` 实现"AI 正在说话也能被插嘴"
- LLM 流式输出是如何被 `sentence_divider → actions_extractor → tts_filter → display_processor` 一串装饰器逐步改写成"句子 + 表情 + 嘴型 + 字幕"四种信号
- 在本地部署时，Ollama / sherpa-onnx / CosyVoice / Edge TTS 各自落到管线的哪一节
...
为什么这很重要：因为 AI 自己 TTS 出来的声音会通过扬声器回灌麦克风。`db_threshold=60` 的音量门限把"AI 在远处说话"这种低分贝声音直接过滤掉，而 `required_misses=24`（约 0.8s）又让 AI 的句子之间不会因为短暂停顿被误判成"用户开口了"。
...
**这意味着表情不是写死在 prompt 模板里"if 包含'开心'→joy"的字符串匹配，而是 LLM 自己在流式输出中插入结构化标签**。这就是为什么换模型、换人设都不影响表情驱动——只要模型支持中文/英文标签输出。
...
这意味着 Open-LLM-VTuber 不打算和你抢 agent 设计的活——它只做"把 agent 输出接上 Live2D 嘴型 + TTS 音频 + 打断通道"这一段，**前端、外观、流式管线三件套**。
...
读完代码你会发现 Open-LLM-VTuber 的最大价值不是"给你一个完美 AI 女友"，而是它**完整暴露了一套"语音 + LLM + Live2D + 打断"的开源参考架构**：
...
每个模块都被设计成"**实现 + 工厂 + 接口**"三层，留好扩展点。这种代码组织方式，比任何"教程"都更值得读。

如果你的目标是在生产里跑一个"AI 数字员工"或"AI 直播间主播"，可以把它当作起点；如果你只是好奇"桌面 AI 伴侣怎么搭"，把它 clone 下来、按推荐配置跑一遍、再顺着本文这几条路径去翻源码，1~2 小时就能理解这条管线的全貌。
...
- Release Notes v1.2.0: <https://github.com/Open-LLM-VTuber/Open-LLM-VTuber/releases/tag/1.2.0>
- v1.2.0 与 v1.1.0 release 页: <https://github.com/Open-LLM-VTuber/Open-LLM-VTuber/releases>
- 项目文档: <https://open-llm-vtuber.github.io/docs/quick-start>
- 关键源码：
  - `src/open_llm_vtuber/websocket_handler.py`
