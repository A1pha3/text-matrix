...
- 能 / 不能：说出 Ace-Step-UI 与 Suno 在数据隐私、使用限制、自托管、UI 可定制性上的四点差异。（→ 1.3）
- 能 / 不能：复述 Prompt 输入、Zustand 状态管理、AudioPlayer 三者的职责边界和数据流向。（→ 3.2、6.1、6.4）
- 能 / 不能：把一次生成任务从点击"生成"到音频播放拆成 6 个步骤并指出每步发生在前端还是后端。（→ 4）
- 能 / 不能：解释"前端只负责交互、模型推理在后端"这一边界对排查性能问题的意义。（→ 1.4、9）
- 能 / 不能：在远程后端场景下正确配置 `VITE_API_BASE_URL` 并说明它在哪里被读取。（→ 5.2、6.3）
- 能 / 不能：说出 `pollStatus` 的超时时间、最大重试次数和退避策略。（→ 6.3）
- 能 / 不能：解释 Zustand `persist` 中间件把历史记录写到了哪里，以及清浏览器缓存会发生什么。（→ 6.4）
- 能 / 不能：根据自己的 GPU 显存判断是否适合跑 ACE-Step 1.5，并说出 CPU 模式的妥协方案。（→ 8.2、9.2）
- 能 / 不能：复述采用顺序的 5 个步骤，并解释为什么后端要先于 UI 跑通。（→ 10.1）
- 能 / 不能：对照"先用 / 等等 / 暂不建议"三类用户，给自己或团队归类并说明理由。（→ 10.2）
