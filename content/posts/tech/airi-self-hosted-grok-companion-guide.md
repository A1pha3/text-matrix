...
1. **AIRI 的三条并行链路是什么？各自负责什么？** 渲染链路（角色如何被看见）、AI 链路（角色如何思考与说话）、游戏链路（角色如何对外部世界产生作用）
2. **xsAI 的作用是什么？** 把 20+ 模型提供商抽象成统一接口，让换模型不需要改核心代码
3. **AIRI 的游戏代理能力基于什么实现？** Minecraft 通过 Mineflayer（Node.js 端 Minecraft bot 协议实现），Factorio 通过 RCON API 和 `autorio`
4. **语音链路和本地数据层如何协同支撑"陪伴感"？** 语音负责实时交互，数据层负责跨会话记忆——两者结合让 AIRI 具备持续积累上下文的能力
5. **AIRI 的适用边界是什么？** 适合有一定技术背景、想本地部署 AI 虚拟角色的用户；不适合完全没有技术背景、想要开箱即用的普通用户
6. **三个 Stage（Web/Tamagotchi/Pocket）的区别是什么？** Stage Web 负责零安装体验，Stage Tamagotchi 负责桌面端低延迟推理，Stage Pocket 负责移动端随身入口
7. **DuckDB WASM 在 AIRI 中扮演什么角色？** 让浏览器端也能跑嵌入式 SQL，用于存储记忆系统和 RAG 模块的向量数据

如果以上 7 项你都能确认，说明你已经抓住了 AIRI 的核心设计要点。
...
*如需了解 AIRI 的记忆系统实现原理，可以阅读 `@proj-airi` 组织的 RAG 和 memory 相关模块源码。*
