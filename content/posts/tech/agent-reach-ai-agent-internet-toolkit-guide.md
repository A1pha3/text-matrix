...
CLI Agent 写代码、改文档、管项目都没问题，但让它去网上找点东西就卡住了——推特 API 要付费（Basic 档 $100/月），Reddit 的公开接口逐年收紧、未认证请求常被 403，小红书必须登录，YouTube 字幕要单独抓，B 站海外 IP 被封。每个平台都有各自的门槛，让 Agent 读一条推文就要先解决认证、抓取、解析三件事。
...
- **Reddit 不用 PRAW**（Python 官方库，平台逐年收紧后大量请求被 403），用 rdt-cli + Cookie 认证。Reddit 对未认证的公开 JSON 请求逐步设限，Cookie 是目前最稳定的绕行方式。
...
Reddit 的公开 JSON 接口已逐步收紧，未认证请求常返回 403。安装 `rdt-cli` 后运行 `rdt login`（自动从浏览器提取 Cookie），之后 Agent 可以用 `rdt search "关键词"` 搜索、`rdt read POST_ID` 读帖子全文和评论。
...
1. Agent Reach 自身只做哪三件事？为什么不在 Agent 和上游工具之间插入中间层？
   ...
   Agent Reach 自身只做三件事：`install`（装工具 + 配环境 + 注册 SKILL.md）、`doctor`（检测每个渠道通不通）、`uninstall`（清理）。
   不在 Agent 和上游工具之间插入中间层，因为这样不增加调用延迟，且架构可插拔——任何渠道不满意就替换对应 channel 文件。

   </details>

2. 零配置渠道和 Cookie 渠道的判断依据是什么？各举两个例子。
...
3. Twitter 选型为什么不用官方 API？代价是什么？
   ...
   Twitter 官方 API Basic 档 $100/月，1 万次读取。用 twitter-cli + Cookie 可以免费使用。
   代价是 Cookie 会过期（7-14 天），需要定期重新导出；且存在被平台检测并封号的风险。
...
4. `agent-reach doctor` 报告某渠道 `❌` 时，你应该按什么顺序排查？
   ...
   排查顺序：
   1. 检查上游工具是否安装（`shutil.which("tool")`）
   2. 检查 Cookie/Token 是否配置
   3. 检查 Cookie 是否过期（重新用 Cookie-Editor 导出）
   4. 检查网络连通性（如 B 站海外 IP 被封，需要代理）
...
5. 什么场景下应该用 MCP 模式而不是 CLI 模式？
   ...
   MCP 模式适合 Claude Desktop、VS Code 等 MCP-aware 的 Agent，因为 Agent 自动发现能力，不需要读 SKILL.md。
   CLI 模式适合 Claude Code、Cursor、OpenClaw 等支持 SKILL.md 的 Agent，更直接灵活。
...
2. **费用信息**：Twitter 官方 API Basic 档 $100/月为 2023 年起公开的费率，平台此后多次调整（并逐渐转向按量计费），具体以 X 开发者文档为准。Cookie-based 方案目前免费，但平台可能随时改变策略。
