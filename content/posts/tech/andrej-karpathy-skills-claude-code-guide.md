...
这不是幻觉，是行为缺失——模型缺少优秀工程师的思考纪律。Karpathy 把这种缺失归纳为四类，并且——更重要的是——给出了一套可操作的纠正指南。这套指南被社区封装成了 **Andrej Karpathy Skills**，一个可以直接安装到 Claude Code 中的规则集。
...
针对这四类行为缺失，Karpathy Skills 定义了四条强制规则。它们是硬约束，违反这些约束的行为会被视为执行错误。
...
**第二步：解释原则，而非宣读规则。** 而是用一两个本团队的真实案例说明每条原则解决的问题。用团队自己的代码做例子，原则会自己说话。
...
不会。它们压制的是没有方向的"创作冲动"——LLM 在没有充分理解需求时的自发填充行为。真正的创造力需要约束才能聚焦。一个类比：好的建筑设计而是在承重墙和预算框架内的最优解。Karpathy Skills 就是给 LLM 划定承重墙。
...
Andrej Karpathy Skills 做了一件事：把一位世界级 AI 研究者对 LLM 行为的理解，翻译成了一组可执行的约束规则。它教 LLM 做一个更好的协作者——写更好的代码只是附带效果。
...
说到底这个 Skills 在回答一个问题：**在人类和 AI 协作的编程环境中，谁是主导者？** Karpathy 的回答是：给出目标的人类。AI 是目标驱动引擎，人类是目标定义者。约束让 AI 把算力花在真正需要的地方。
