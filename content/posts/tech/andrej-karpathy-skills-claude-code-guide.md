...
**Q1**：Think Before Coding 原则中的 "Stop when confused" 为什么是反直觉的？

> **答案**：我们给 AI 下指令时默认期待它"无论如何都要产出结果"。这条规则明确告诉它：不产出比瞎产出更有价值。这让 AI 学会在不确定时主动澄清，而不是默默猜。

**Q2**：Simplicity First 原则中"No abstractions for single-use code"这一条为什么关键？

> **答案**：LLM 有一个顽固的倾向——把"代码复用"等同于"抽象"。它不理解有时候重复比抽象更清晰。一个 30 行的逻辑被提取成一个 15 行的函数加上 5 个参数和一段 docstring——净损失。单次使用的代码不需要抽象。

**Q3**：Surgical Changes 原则要求 LLM "Match existing style, even if you'd do it differently"，为什么这一条最难执行？

> **答案**：因为它要求 LLM 压抑自己的审美偏好——这恰恰是 LLM 最不擅长的事。你项目用的是 snake_case，它想改成 camelCase——不行。你习惯把类型定义放在文件顶部，它想挪到末尾——不行。

**Q4**：Goal-Driven Execution 原则建议把"步骤指令"转化为"成功标准"，为什么这样更有效？

> **答案**：传统的人机交互模式是"指令 → 执行 → 交付"。Karpathy 提出的模式是"目标 → 循环验证 → 达标交付"。告诉 AI "添加输入校验"，LLM 可能加个 `if` 就收工；换成"为非法输入编写测试，然后让测试通过"，它要经历写测试 → 跑测试 → 发现边界遗漏 → 补充逻辑 → 重新测试 → 通过的完整闭环。

**Q5**：多条原则冲突时，Karpathy 建议的优先级顺序是什么？

> **答案**：实际运行中四个原则几乎不会冲突——它们在同一个工作流的不同阶段起作用。如果要放弃一条原则，先放弃效率（Simplicity），保留安全（Surgical）和正确性（Think、Goal-Driven）。
...
1. **个人试用**：在非关键分支上安装 Skills，观察 diff 模式变化，记录哪些原则触发了行为调整。
2. **改造提示词**：把高频任务的"步骤指令"改写为"成功标准"，让 Goal-Driven Execution 自动生效。
3. **团队试点**：2-3 人的小团队先统一安装，运行两周后评估返工率和代码审查通过率的变化。
4. **读 Karpathy 原帖**：找到 Karpathy 2026 年初的博客和社交媒体帖子，读原文中对 LLM 编程陷阱的完整观察。
5. **组合其他 Skills**：尝试把 Karpathy Skills 与 wiki skill、domain-specific skills 组合使用，观察协同效果。
6. **贡献社区**：如果团队有特定的编码规范，把它写成 CLAUDE.md 的补充规则，提交到 forrestchang/andrej-karpathy-skills 仓库的 PR。
