...
| → | [系统分层](#系统分层) | [从 shortform 开始](#从-shortform-开始) | [安装与适配](#安装与适配) | [v2.0.0-rc.1 详解](#v200-rc1-详解) | [任务流案例](#任务流案例) | [安全](#安全) | [FAQ](#faq) | [自测](#自测) |

## 学习目标

读完这篇文章，你应该能够：

1. 说清楚 ECC 的四层结构（Operator / Control Plane / 核心资产 / Harness 适配）各自负责什么，以及为什么要把共享逻辑放在 core 层而不是 harness 层
2. 解释 Skills 和 Commands 的区别——为什么逻辑应该写在 skill 里，command 只作为 trigger
3. 描述六种 hook 类型（PreToolUse / PostToolUse / UserPromptSubmit / Stop / PreCompact / Notification）的触发时机和典型用途，能写出可运行的 hook JSON 配置
4. 完成 ECC 的基础安装和选择性安装，知道如何用 `install-plan.js` + `install-apply.js` 只装当前项目需要的 skills
5. 判断自己的场景是否值得装 ECC，包括单 harness 用户、多 harness 用户、context window 紧张等不同情况下的决策依据
...
1. **session 恢复**：agent 启动时，ECC 的 context 持久化机制从 SQLite state store 恢复上一个 session 的结构化摘要——上次改了哪些文件、做了什么决策、踩过什么坑。agent 不需要你重新解释项目背景。
2. **UserPromptSubmit hook 触发**：你输入"review 这个 PR"时，UserPromptSubmit hook 注入上下文约束（来自 `rules/` 里的 `code-review.md`），比如"必须检查测试覆盖率变化、必须检查新增依赖的安全性"。

### 执行阶段

3. **skill 加载**：agent 识别到这是 PR review 任务，加载 `pr-review` skill（如果存在）或 `code-reviewer` subagent 定义。skill 内容只在被调用时才进入 context，不会预加载。
4. **工具调用循环**：agent 用 Read / Grep / Glob 读 PR 改动的 8 个 `.ts` 文件。每次工具调用前后：
   - **PreToolUse hook**：检查工具调用是否合规（如 `rules/security.md` 里的"禁止读取 `.env` 文件"）
   - **PostToolUse hook**：如果 agent 用 Edit 修改了文件，自动跑 `prettier --write` 和 `tsc --noEmit`；如果只是 Read，跳过格式化
5. **subagent 委派**：agent 发现 PR 新增了一个 MCP 调用，把"审查这个 MCP 调用的安全性"委派给 `security-reviewer` subagent。subagent 有独立的 tool 白名单（只允许 Read / Grep / Glob，不允许 Write / Bash），审查完成后把结果返回给主 agent。主 agent 不需要把 MCP 相关代码塞进自己的 context。
6. **并行化判断**：如果 PR 改动量大，`parallel-execution-optimizer` skill 会判断是否需要横向扩展。判断依据是单个 agent 的 context 水位——如果逼近 80%，建议用 git worktree 隔离出独立 agent 实例分头审查不同模块。

### 收尾阶段

7. **Stop hook 触发**：agent 输出 review 结果后，Stop hook 扫描所有修改文件，检查残留的 `console.log`、未通过的 `tsc` 错误、未格式化的代码。如果发现问题，hook 退出码非 0，agent 会继续修复。
8. **记忆持久化**：session 结束时，hooks 把本次 session 的结构化摘要写入 SQLite state store——review 了哪个 PR、发现了哪些问题、建议了哪些改动、哪些规则被触发。下一个 session 启动时恢复。
9. **模式提取（可选）**：如果你连续三次用同样的方式做 PR review（同样的检查项、同样的输出格式），ECC 的持续学习机制会建议把这个流程固化成新的 `/pr-review` skill，下次直接调用。
...
1. **读 shortform guide（第 1 天）**：通读 `the-shortform-guide.md`，理解 hooks、skills、subagents、MCP 这四块怎么联动。不要跳过 shortform 直接读 longform——没有操作直觉，longform 的 token 优化和验证循环细节很难落地。
2. **基础安装 + 验证（第 1 天）**：跑 `./install.sh`，然后 `node tests/run-all.js` 确认零失败。在 Claude Code 里新建一个 session，执行 `/refactor-clean` 看 hooks 是否触发。
3. **选择性安装（第 2 天）**：用 `install-plan.js --target claude-code --langs typescript,python` 只装当前项目需要的 skills，避免 249 个 skills 全装带来的管理负担。
4. **配置 hooks（第 3-4 天）**：从 PostToolUse 格式化 hook 开始（`prettier --write` + `tsc --noEmit`），跑两天稳定后再加 Stop hook 的 console.log 扫描。用 `hookify` 插件对话式创建，不用手写 JSON。
5. **配置 subagents（第 1 周）**：从 `code-reviewer` 和 `security-reviewer` 开始，把审查类任务委派给 subagent，释放主 agent 的 context。
6. **跨 session 记忆验证（第 1 周）**：连续跑两三个 session，确认第二个 session 能恢复第一个 session 的文件变更和决策摘要。
7. **ecc2/ Rust control-plane（可选）**：如果你对 session 状态追踪和 dashboard 感兴趣，本地跑 `cargo run -- dashboard` 试用。alpha 阶段，不要用于生产。
