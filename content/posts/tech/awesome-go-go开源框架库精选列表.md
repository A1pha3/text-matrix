...
awesome-go 的分类覆盖了从基础设施到上层应用的完整范围，按八个维度组织：基础工具（CLI、配置、日志、测试）、Web 开发（框架、路由、ORM）、数据库与存储（驱动、缓存、迁移）、网络与通信（HTTP、消息队列、RPC）、数据处理（算法、ML、流处理）、安全与加密（认证、密码学）、DevOps 工具链（构建、CI、代码分析）、特殊领域（区块链、游戏、IoT、WASM）。前四个维度覆盖了 80% 的日常后端需求，后四个按业务场景按需查阅。下面逐栏给出说明和代表项目，遇到具体需求时先在这里定位"该去哪一栏找"。
