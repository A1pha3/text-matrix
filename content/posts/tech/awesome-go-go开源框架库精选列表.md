...
[awesome-go](https://github.com/avelino/awesome-go) 是由 [Avelino](https://github.com/avelino) 维护的一个精选列表（Curated List），专门收录 Go 语言生态中优质的**开源框架、库和软件**。它脱胎于 [awesome-python](https://github.com/vinta/awesome-python) 的思路，目前是 GitHub 上订阅量最高的 Go 语言资源列表。

截至本文写作时，该项目已获得 **171,395 颗星标**，收录了数千个经过社区筛选的开源项目，常见使用场景有三类：
...
awesome-go 的分类覆盖了从基础设施到上层应用的全链条。下面按领域划分主要类别，每个分类给出说明和代表项目，方便你快速判断"这个需求该去哪一栏找"。
...
sarama 是 Shopify 维护的 Kafka Go 客户端，功能完整但 API 偏底层；nats.go 对应 NATS 这条轻量消息总线，适合做微服务间的事件分发。
...
- **AI 一栏**的 ollama 不是 Go 写的推理引擎本体，而是可以通过 Go 客户端（如 ollama-go）嵌入到自家服务里调用的本地大模型运行时；langchaingo 则是 LangChain 的 Go 移植，适合做 RAG、Agent 编排。
...
Cryptography 一栏补几个具体项目：age 是 Filippo Valsorda 设计的现代文件加密工具，API 简洁、默认安全；nacl 是 NaCl（Networking and Cryptography Library）的 Go 绑定，适合做对称加密和公钥加密；argon2 是密码哈希竞赛冠军的 Go 实现，用于存储用户口令；protonmail/bcrypt 则是 bcrypt 的常见 Go 实现。
...
Continuous Integration 一栏的 drone 是用 Go 写的 CI 引擎，可以用 Docker 容器作为构建步骤；goreleaser-action 把 Go 项目的发版流程封装成 GitHub Action；go-toolchain 是 Go 官方在 1.21 之后引入的工具链管理机制，CI 里指定 `go 1.x.x` 即可自动下载对应版本。
...
| Web 性能 | **Fasthttp** | 比标准库 net/http 快约 10 倍的 HTTP 框架 |
| 微服务 | **gRPC** | Google 出品，ProtoBuf 驱动 |

注意"最常用"不等于"一定适合你"。Fasthttp 在追求极限吞吐时确实快，但它不兼容 net/http 的 Handler 签名，所有中间件都得重写——如果团队已经在 Gin 上沉淀了大量中间件，迁移成本要算进选型里。

---

## 任务流案例：用 awesome-go 完成一次技术选型

光看分类表还不够具体。下面用一个真实场景把流程串起来，演示"接到需求 → 落到库"的完整路径。
...
- asynq：Redis 驱动的任务队列，作者也是 go-redis 的维护者
...
整个流程从"打开 awesome-go"到"选定库并落地"，大约耗时半天。如果跳过 Step 1 直接 Google，很容易被各种博客带偏到 RabbitMQ 或 Kafka 这种重型方案上——这就是 awesome-go 的价值：它把"候选集"先收敛到了 Go 原生生态里。
...
维护者会定期清理失效链接和长期不更新的项目，所以列表里的"活跃度"是有保障的——这也是 awesome-go 比一般博客合集更可信的原因。

---

## 采用顺序

如果你是第一次接触 awesome-go，建议按下面的顺序使用：

1. **先读 README**：把整个 README 浏览一遍，建立分类心智地图，知道"Go 生态里有哪些领域被收录了"。
...
5. **反向贡献**：如果你维护的库已经达到收录标准，按贡献指南提交 PR，把列表反哺给社区。

---

## 进阶路径

把 awesome-go 用熟之后，可以往这几个方向延伸：
...
- **参与维护**：在 awesome-go 上认领一个分类的"巡检"工作，定期检查链接有效性、项目活跃度，这是进入 Go 核心社区的低门槛路径。
- **横向对比 Awesome 家族**：对照 [awesome-rust](https://github.com/rust-unofficial/awesome-rust)、[awesome-python](https://github.com/vinta/awesome-python)，看不同语言生态的分类设计差异，能反过来加深对 Go 生态的理解。
...
没有官方关系。它由社区维护，维护者 Avelino 是商业公司的高管，列表本身是个人/社区项目。但因为它维护质量高，Go 团队成员也会在演讲里引用它。
...
awesome-go 是 Go 语言生态的地图。对于刚入门 Go 的开发者，它是寻找"这个需求该用哪个库"的最佳起点；对于有经验的开发者，它是定期巡检生态演进的参考读物。
