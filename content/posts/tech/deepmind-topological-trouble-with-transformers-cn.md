...
为了不把两条线搅在一起，先分清两组容易混淆的机制。一组是"状态追踪"，指模型要长期维护一份随输入更新的内部状态；另一组是"工作记忆"，指模型靠反复重读上下文窗口来"临时想起"信息。Transformer 靠后者绕开了早期 RNN 记不住远距离信息的老问题，但前者恰恰是它的软肋。带着这对界限往下读，你会明白为什么深度方向的循环救不了状态追踪，而步方向的循环才是要害。
