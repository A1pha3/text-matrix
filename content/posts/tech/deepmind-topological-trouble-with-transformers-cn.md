...
本文按论文的论证顺序走：从"为什么 Transformer 会失败"的形式化分析，到 Patchscopes 给出的实证证据，再看 CoT 为什么只是补丁，最后落到论文的核心贡献——一套把当代所有"循环 Transformer"按两个维度分类的分类法。
...
这篇论文最让人警醒的不是它给出了什么答案，而是它揭示了一个被忽视的事实——过去几年我们在"用 CoT 续命"的同时，把 Transformer 的"状态追踪缺陷"用工程手段遮住了。账单越涨越长，本质问题没人动。Mozer 在 1990 年代研究 RNN 的时候，没有赶上算力的时代；三十年后他回到同一个问题面前，算力终于追上了。这也许是一个隐喻：循环架构不是"历史的回潮"，而是"被推迟的必然"。
...
**Q：CoT 真的没用吗？我看到很多任务上 CoT 效果很好。**
A：论文承认 CoT 的效果是真实的。但论文认为 CoT 是一种回避：它把深层的状态"打印"出来，变成可见的文字输出，再让模型在新一轮处理时重新读入。这能用，但代价是算力、内存和概念层面的。核心论点是：下一代基础模型应该超越 CoT，而不是依赖 CoT。

**Q：Mamba、RWKV-7 这些架构真的能替代 Transformer 吗？**
A：目前还不能。这些架构在状态追踪任务上确实突破了 Transformer 的上界，但在大规模语言建模任务上，它们的竞争力只是"相当"，还没有"超越"。混合架构（如 Olmo Hybrid）可能是近期的更务实选择。

**Q：论文说"深度循环治不了本"，但 Universal Transformer 好像效果不错？**
A：Universal Transformer 的深度循环确实能在某些任务上提升效果，但论文的论点是：它没有改变"状态表示必须往深层推"这个几何约束。深度循环增加了"中段"的层数，但本质上还是在耗尽深度——只是推得慢了一点。

**Q：这篇文章对普通 LLM 用户有什么意义？**
A：理解为什么 CoT 模型会"思考"很久、为什么账单很贵、为什么模型偶尔会"忘记"前面做出的判断。这些现象不是 bug，是架构性缺陷的必然结果。未来如果循环架构真正成熟，推理成本和幻觉问题都有可能显著改善。