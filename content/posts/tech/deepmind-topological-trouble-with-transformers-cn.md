---
title: "Transformer 的拓扑缺陷：DeepMind 为什么说 CoT 是补丁"
date: "2026-06-18T08:50:00+08:00"
slug: "deepmind-topological-trouble-with-transformers-cn"
description: "DeepMind 2026 年的论文证明：Transformer 的纯前馈架构天然不擅长追踪状态，CoT 只是给结构性缺陷打补丁。真正的出路在循环架构。"
draft: false
categories: ["技术笔记"]
...
文章按论文的论证顺序展开：先做形式化分析，回答"Transformer 为什么失败"；再用 Patchscopes 给出实证；然后论证 CoT 只是补丁；最后交出论文的核心贡献——把当代所有循环 Transformer 按两个维度分类的分类法。

先分清两个容易混淆的概念。**状态追踪**指模型长期维护一份随输入更新的内部状态；**工作记忆**指模型反复重读上下文窗口、临时"想起"信息。Transformer 靠后者绕开了早期 RNN 记不住远距离信息的老问题，但前者恰恰是它的软肋。理解这对区别，就能明白：深度方向的循环救不了状态追踪，步方向的循环才是要害。

---

## 一、Transformer 为什么不擅长"状态追踪"

要理解这篇论文，先得拆开 Transformer 在做什么。可以把它想象成一座图书馆：每次有人提问，图书馆员不会"记住"之前说过什么，而是把所有对话记录摆在桌上，重新翻阅一遍，然后作答。
...
论文在第 4 节讨论了这种"作弊"的合理性，引用了 Merrill & Sabharwal 2025 的形式化结果：在 Transformer 中，识别长度 $n$ 的正则语言只需要 $O(\log n)$ 层。这个下界意味着 Transformer 在理论上可以用 log n 层解决很多看似需要线性记忆的问题。
...
```
User: 60         Model: lower      ← 隐含答案 < 60，区间缩为 [1, 59]
User: 41         Model: lower      ← 隐含答案 < 41，区间缩为 [1, 40]
User: 70         Model: higher     ← 隐含答案 > 70；与前两轮推出的"答案 < 41"直接矛盾
```

即便让 Gemini 3 Thinking 显式写出"我选定的数字是 42"，当用户猜 42 时，模型依然回答"lower"——它在思考阶段做出的判断，已经被后续处理遗忘了。

**例子二：bank 歧义。** 这是论文借助 Lepori et al. 2025 工作的核心实证，使用 Patchscopes 工具（Ghandeharioun et al. 2024, ICML 2024）来观察模型内部的语义消歧过程：
...
用 Patchscopes 工具观察 Gemma2-9B 内部，可以看到一个非常清晰的模式：
...
CoT 是在给一个有结构性问题的大楼加装电梯——能用，但每次上楼下楼都要花时间花电费。真正解决的办法是**重新设计大楼**。
...
把两个维度交叉，得到 9 个格子。论文用一张表列了 6 个有工作的格子（其余 3 个是空白，值得探索）：
...
### 关键观察 3：Depth + Step + Latent Thought 是新前沿
...
既然架构有根本缺陷，为什么 Transformer 还能在这么多任务上表现得这么好？
...
论文在第 5 节列出了 5 个"有前景的研究方向"。

### 7.1 增强状态空间模型

这是论文最看好的方向。前文提到的 DeltaNet 负特征值扩展（Grazzi et al. 2025）、RWKV-7、PaTH Attention 都属于这一类。它们的共同点是：保留并行训练、在状态更新规则上引入非线性或非平凡动力学、在大规模语言建模任务上展现出与标准 Transformer 相当的竞争力。

论文还提到了 **gated linear attention**（Yang et al. 2024b）和 **gated DeltaNet**（Yang et al. 2025a）——这些"门控 + Delta 规则"的混合架构被 Merrill et al. 2026 的 OLMo Hybrid 报告证明：在理论和实践上，混合架构都比"纯 Transformer"和"纯线性注意力"更强。
...
- **多阶段训练**：先用标准前馈 Transformer 预训练，再引入循环机制做微调。
...
翻译成工程师的语言：**Transformer 让我们摆脱了"难以训练"的 RNN，但也让我们陷入"状态追踪困难"的泥潭；CoT 是当下唯一可用的补丁，但它的代价是账单；真正的出路在循环架构——具体来说，是步方向的循环、配合现代并行训练技术的状态空间模型与线性注意力变体。**
...
落到实践上，论文的判断很直接。如果你的场景本质是"长期维护状态"——多轮对话的一致性、跨长步数的推理、状态机式的执行——步方向的循环（Mamba、DeltaNet、RWKV-7、PaTH 这类状态空间模型与线性注意力变体）比继续堆 CoT 更接近问题的根。如果场景只是"重读上下文就能答"——检索、摘要、单轮问答——纯前馈 Transformer 配合工作记忆已经够用，为循环付出自回归训练的额外成本未必划算。判断的标准就是那张 9 宫格：先想清楚任务卡在哪个轴上，再决定要不要上循环。

## 九、常见误解

**这是不是说明 Transformer 过时了？**

不是。论文的批评有明确边界：它针对的是"需要长期维护状态"的一类任务。很多任务靠工作记忆——反复重读上下文——就能解决（见第六节因素 1），纯前馈 Transformer 在这些场景仍然高效。

**那我该放弃 CoT 吗？**

不必急。CoT 仍是当下唯一可用的补丁，它让模型解决了不少原本解不了的串行问题（见第四节）。论文点出的是它的代价——算力、上下文占用、把自动认知变成外显思考——而不是主张立刻弃用。

**深度方向的循环是不是没有价值？**

有价值，但解决不了状态追踪的根（见第五节关键观察 1）。深度循环只是把同一组层反复用，状态表示仍要一层层往上推，最终照样耗尽深度。要长期维护状态，得靠步方向循环。

**用了线性注意力就一定能追踪状态吗？**

不一定。论文把 DeltaNet 的负特征值扩展称为"突破了纯前馈架构的能力上界"（见第五节关键观察 2），但这是特定机制上的突破。能否真正长期追踪状态，还取决于具体的循环结构和训练目标——论文在评价 latent thought 模型时就点明：有的能跟踪状态，有的不能（见第五节关键观察 3）。
...
- **背景报道**：[36kr 报道](https://www.36kr.com/p/3857099719791876)（2026-06-18）