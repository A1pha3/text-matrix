...
Multica 的核心思路：当团队里同时跑了五六个 coding agent 时，谁给它们分配任务、谁跟踪进度、谁在它们跑偏时知道、谁把某次成功的修复变成团队里所有 agent 都能复用的技能？
...
Multica 把「agent as teammate」这个思路落地了：看板、issue assign、comment、blocker report——这些不只是给 human 用的协作工具，也是 agent 的工作界面。从这个角度看，它是一个开源的 AI-native 团队协作基础设施，而不是一个单纯的 agent 调度器。
