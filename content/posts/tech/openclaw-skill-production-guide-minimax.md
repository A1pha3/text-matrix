...
模板化不是什么新鲜概念，但它指向一个容易被忽略的区分：**你追求的是 AI 的执行确定性，而非创造性**。这两个目标需要完全不同的工程策略。
...
模板化不是限制 AI 的能力，而是在不该自由发挥的地方锁死，在该自由发挥的地方放开。一个产品爆炸图网页的 HTML 骨架应该被固定，但你传给模板的文案、图片路径、颜色变量可以交给 AI 去生成。
...
拆分完之后，每一步都可以独立调试。如果网页样式有问题，你不需要重新生成视频。如果视频首尾帧对不上，你不需要动 HTML。
