...
如果你正在挑 LLM 应用框架，或者想看"生产级 Agent 长什么样"，Awesome AI Apps 是一个值得花两小时翻一遍的样本库。它把 80 多个可运行项目按难度从 Starter 排到 Advanced，覆盖 Agno、CrewAI、LangChain、PydanticAI、AWS Strands、DSPy 等主流框架，并集中展示了 RAG、Memory、MCP、Voice 四类横向能力的工程写法。

它的价值是让你对照同一类问题在不同框架下的解法。教写 Agent 这件事交给框架官方文档，合集负责提供样本。本文按这个角度重组合集内容：先给系统地图，再按难度逐层拆解，最后用一个研究任务的完整流水线把各类机制串起来。

本文代码示例为示意性写法，展示主要调用结构，具体 API 以仓库内可运行代码为准。
...
合集里的项目看起来杂，实际上在解四类独立问题。把它们拆开看，选型时不会混。
...
这四类机制可以叠加，但各自独立演进。一个生产级 Agent 通常同时用 Agent 框架 + RAG + Memory + MCP，但每类都有自己的选型空间和失败模式。Voice 是第五类，但它更偏实时基础设施（LiveKit、Pipecat），与前四类不在同一维度。
...
下面按难度逐层拆解。每类先说"为什么单独成类"，再给项目列表和典型写法。
...
每个框架都有自己的 Agent 抽象、工具注册方式和运行入口。Starter 类项目把每个框架压缩到一个最小可运行文件，让你在 50 行代码内看清"这个框架怎么定义 Agent、怎么挂工具、怎么触发执行"。这是选型前的快速对照样本，生产代码另写。
...
Agno 的特点是 Agent 即对象，工具通过参数挂载，运行入口统一为 `run()`。对照 CrewAI 的 `Crew` + `Task` 抽象、LangGraph 的状态图抽象，可以看出不同框架对"Agent 边界"的理解差异。
...
Starter 只展示框架骨架，Simple 层补上真实业务场景：金融数据查询、日历调度、网页自动化、模型路由、自然语言查库。每个项目都带一个可复现的业务问题，适合在选完框架后用来验证"这个框架能不能撑住我的场景"。
...
`llm_router` 解决的是成本问题：把简单查询路由到小模型，复杂查询才调用大模型。生产环境里这一层通常独立部署，与具体 Agent 框架解耦。
...
语音 Agent 的瓶颈集中在实时性。VAD（语音活动检测）、STT、TTS、流式打断、WebRTC 传输构成独立的技术栈，与文本 Agent 的"调一次 API 拿结果"模式完全不同。合集只收了两个项目，但覆盖了两种主流路径：LiveKit 的实时房间模型和 Pipecat 的流水线模型。
...
`VoicePipelineAgent` 把 VAD → LLM → TTS 串成流水线，每一段都可替换。Gemini Realtime 的优势在于原生支持流式输入输出，省去中间 STT 步骤。
...
Agent 要调用外部工具（GitHub、数据库、搜索引擎）时，传统做法是每个工具写一套适配代码。MCP（Model Context Protocol）把"工具如何被发现、如何被调用、如何返回结果"标准化成协议，Agent 端按 MCP 客户端规范实现，工具端按 MCP 服务器规范实现。这层抽象让工具复用成为可能：一个 GitHub MCP 服务器可以同时被 Claude Code、Cursor、任意 MCP 兼容 Agent 调用。
...
MCP 的边界在于：它只覆盖"工具调用"这一段，工具内部逻辑仍由 Agent 决定。GitHub MCP 服务器封装的是 GitHub API 调用，至于拿到仓库数据后怎么分析，留给 Agent 自己处理。
...
LLM 本身是无状态的，每次调用都是独立请求。要让 Agent 记住"用户上次说预算 5 万""用户偏好简洁风格"，必须在外部存储里维护用户档案和会话历史。这听起来像数据库问题，但难点在于：记忆该存什么、何时召回、如何避免污染上下文。Memori 用 SQL 原生存储，GibsonAI 提供托管层，合集里 12 个项目展示了不同记忆策略的写法。
...
第二次 `run` 时，Memori 会从 SQL 存储里召回第一次会话的关键信息（电商、AI 方案），注入到当前上下文。这层召回逻辑对 Agent 透明，框架端只看到 `memory` 参数。
...
模型训练数据有截止日期，且无法访问企业内部文档。RAG（Retrieval-Augmented Generation）的思路是：先把私有文档切块向量化存入向量库，用户提问时检索相关片段，再把片段作为上下文喂给模型。合集里 12 个项目覆盖了从基础 RAG 到 Agentic RAG（让 Agent 自己决定是否检索、检索什么）的演进路径。
...
Advanced 层项目都是多 Agent 流水线，单个项目通常涉及 3 个以上 Agent 协作，并集成外部服务（Twilio、MongoDB、Temporal）。这些项目更接近真实生产代码，复杂度也最高，适合在掌握前几层后用来研究编排模式。
...
流水线模式把"研究"拆成收集 → 分析 → 综合三个阶段，每个阶段由专门 Agent 负责。这种拆分让每段都可独立调试和替换，但代价是阶段间数据格式必须严格约定，否则下游 Agent 无法解析上游输出。

---

## 任务流案例：一个研究任务如何流过系统

把前面几类机制串起来看。假设任务是"研究 AI 在医疗领域的最新进展，生成报告并发到邮箱"，一个完整流水线会这样流转：
...
这个例子里，Agent 框架负责编排，RAG 负责私有数据检索，Memory 负责用户偏好，MCP 负责外部工具调用。四类机制各司其职，边界清晰。生产环境里每一层都可能成为瓶颈：Agent 框架的编排延迟、RAG 的召回率、Memory 的污染问题、MCP 的工具故障，需要分别监控。
