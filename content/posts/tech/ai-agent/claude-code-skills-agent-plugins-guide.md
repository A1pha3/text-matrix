...
AI 编程工具缺的是，不是知识，是工作流。Claude Code Skills 把 205 个领域的专家工作流写成了 AI 能直接执行的指令文件——每个技能本质上是一份写死的检查清单、决策框架和工具链的组合。本文不逐个介绍技能功能（官方文档已经做了这件事）解析这套系统的设计思路、三层架构分工，以及你在自己项目里应该按什么顺序引入。
...
Skills 解决的就是这个"知道概念 vs 知道怎么干"的落差。它是，不是给 AI 补充背景知识直接给出工作流。每个技能包含：
