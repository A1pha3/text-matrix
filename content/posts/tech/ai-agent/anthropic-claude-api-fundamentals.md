...
1. **理解 API 认证机制**：掌握 API Key 与 Bearer Token 的使用方法
2. **安全管理密钥**：学会环境变量、.env 文件、云密钥服务等安全管理方式
3. **构建 API 请求**：使用 Python SDK 发送请求并解析响应
4. **实现多轮对话**：维护会话历史，实现上下文连续的对话
5. **使用系统提示词**：控制模型行为，实现角色扮演和输出格式化
6. **实现结构化输出**：使用 JSON 模式或工具调用实现结构化数据提取
...
本文对应课程第 1 章「Fundamentals」，涵盖以下内容：
...
`max_tokens` 控制单次请求最多生成的 token 数。

**重要概念**：
- Token 是文本处理的最小单位（1 个 token 约等于 0.75 个英文单词或 1-2 个中文字符）
- 如果 `max_tokens` 太小，响应会被截断
- 如果 `max_tokens` 太大，会浪费 token（因为按使用量收费）
...
多轮对话是指多次请求之间保持上下文，让 Claude 能够理解对话历史。
...
有时候你需要 AI 返回特定格式的数据（如 JSON），而不是自由文本。
...
| 分类任务 | 带标签的 JSON | {"category": "positive", "confidence": 0.95} |
| 表格数据 | JSON 数组 | [{"name": "...", "age": 25}, ...] |
| API 响应 | 标准化 JSON | 统一的错误码和消息格式 |
...
### 问题 1：为什么不能硬编码 API 密钥？

<details>
<summary>查看答案</summary>

硬编码 API 密钥有三个严重问题：
1. **代码泄露**：密钥进入 Git 仓库，被他人获取
2. **历史记录**：Git 历史会永久保存密钥
3. **生产事故**：密钥在日志中打印，被监控系统捕获
...
推荐做法：
1. **开发环境**：使用 `.env` 文件 + `python-dotenv` 加载
2. **生产环境**：使用 AWS Secrets Manager / 云密钥服务
3. **永远不要**：硬编码、提交到 Git、在日志中打印
...
维护一个 `conversation_history` 列表，每次请求时传递完整的消息历史。
...
当对话历史过长时，有两种策略：
1. **Trim（裁剪）**：移除最早的消息，只保留最近的 N 条
2. **摘要（Summary）**：用 Claude 将早期对话压缩成一段摘要
...
推荐两种方式：
1. **工具调用（最可靠）**：定义 JSON Schema，让 Claude 调用工具返回结构化数据
2. **JSON 模式**：在系统提示词中指定 JSON 格式要求
...
**任务**：创建一个 Python 脚本，从环境变量读取 API 密钥，实现安全的 Claude API 客户端封装。

**参考答案**：
```python
# 参考 §1.1 的 SDK 初始化推荐做法
```

### 练习 2：实现多轮对话聊天机器人

**任务**：创建一个命令行聊天机器人，维护对话历史，实现多轮对话。
...
**任务**：使用 Claude API 从一段文本中提取结构化信息。
...
1. **学习提示词工程**：阅读 Claude API 基础专题（二）：提示词工程
2. **学习工具调用**：阅读 Claude API 基础专题（三）：工具调用
3. **实现流式响应**：在实际项目中实现流式响应
4. **集成到 Web 应用**：使用 Flask/Django 构建 Web 应用
5. **实现上下文管理**：研究如何高效管理长对话历史
6. **优化成本和性能**：研究如何选择合适的模型、控制 token 使用量
...
1. **信息来源**：本文基于 Anthropic 官方文档和 API 参考
2. **版本时效性**：本文描述的是 Claude API 的当前版本，建议对照官方文档验证
3. **代码示例**：本文的代码示例基于 `anthropic` Python SDK，未经完整运行验证
4. **模型名称**：本文提到的模型名称可能会随版本更新而变化
5. **成本数据**：本文提到的成本数据是基于 Sonnet 模型的估算
6. **更新记录**：本文最后更新于 2026-03-25
...
**文档元信息**
难度：⭐⭐⭐⭐ | 类型：专家设计 | 更新日期：2026-03-25 | 预计阅读时间：45 分钟 | 字数：约 6000 字
