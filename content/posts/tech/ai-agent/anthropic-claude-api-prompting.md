...
系统提示词（System Prompt）设置了 Claude 的"默认行为"，影响整个对话过程。
...
Chain of Thought（CoT）提示词是让 Claude 在给出最终答案之前，先展示推理过程。
...
- 继续阅读：工具调用专题（三）- 深入了解 function calling（待发布）
- 实践项目：用 Claude API 构建一个智能客服系统
