...
AI Scientist-v2 的核心创新是把"跑实验"从一条直线变成一棵可控的探索树。本文拆解它的树搜索机制，说清 v2 相比 v1 到底解决了什么、又牺牲了什么，最后给出这套系统在当前阶段的适用边界。
...
v2 相比 v1 最根本的变化在于：它把实验设计从人类手写的模板交还给了智能体自己。由此衍生出两个直接结果——实验路径从单线执行变成了树搜索，以及产生了首个被 ICLR Workshop 接收的全 AI 生成论文。
...
这里需要说清楚：v2 并不一定比 v1 产生更好的论文。当有强起始模板可用时，v1 的成功率更高、产出更稳定。v2 的"更广泛探索"是用成功率换来的。
...
三阶段流水线中，第二阶段是核心。实验管理器在这里不是"执行一个实验脚本"，而是在多棵实验分支树之间调度资源：每个 Worker 独立探索一条路径，失败时自动触发调试重试，管理器按"最佳优先"策略分配资源给更有希望的分支。
...
取决于场景。有明确目标和成熟模板时，v1 成功率更高，产出更稳定。需要开放性探索、假设空间不清晰时，v2 的树搜索更有价值。

**一次完整运行需要多久？**

数小时，具体取决于并行度和实验复杂度。假设生成阶段最快（几分钟），树搜索实验阶段最耗时（几小时）。

**没有 GPU 能跑吗？**

不能。深度学习实验需要 NVIDIA GPU 和 CUDA 支持。如果只是想看论文生成效果，可以跳过实验阶段直接运行论文撰写。

**Semantic Scholar API 有频率限制怎么办？**

可以跳过引用阶段，或者设置 `S2_API_KEY` 提高限额。

## 九、总结与建议

AI Scientist-v2 真正的价值不在于"能生成论文"，而在于把实验设计本身变成了一个可被智能体探索的问题。论文只是这个探索过程的副产品。
...
下面 8 条用来检查对核心内容的理解。不看原文能答出 6 条以上说明抓住了；3 条以下建议回看对应章节。
...
三个练习从理解到分析递进，覆盖"读输出、跑流程、做决策"。建议按顺序完成。
