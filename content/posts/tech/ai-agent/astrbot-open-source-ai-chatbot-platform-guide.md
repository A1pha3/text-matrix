...
拆开之后，接一个新平台只需要写对应的渠道适配；加一个能力只需要在引擎侧扩展，不需要迁就某个 IM 的接口。RAG（Retrieval-Augmented Generation，检索增强生成）、MCP（Model Context Protocol，模型上下文协议）、Agent Sandbox 这些能力对每个平台是一致的，因为它们都住在引擎层。
...
换模型不换代码，这正是"模型无关"的实际含义。顺带一提，Dify、阿里云百炼应用、Coze 这类 LLMOps 平台也能作为接入源，适合已经在这些平台编排好 Agent 流程的团队。
...
Stars 38,750、Forks 2,778 说明关注度很高，但推不出"生产就绪"。Stars 涨得快可能只是踩中了 IM 机器人这个热点，不代表它在你的流量规模下稳定。插件数 1000+ 来自官方市场统计，它描述的是"有多少插件可用"，不是"每个插件都维护良好"——社区插件质量参差，上生产前要各自评估。
