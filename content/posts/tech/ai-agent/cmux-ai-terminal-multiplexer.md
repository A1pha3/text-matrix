...
1. **说出 cmux 的核心定位**：它和普通终端、Warp 有什么区别
2. **判断自己是否需要 cmux**：列出 3 个适合用 cmux 替代现有终端的场景
3. **完成一次基础配置**：安装 cmux、配置 Claude API Key、运行第一个 AI 命令
4. **设计一条技能扩展路径**：基于 cmux 的 Skills 框架，规划一个自定义工作流
...
用这份清单检验你对 cmux 的掌握程度：

### 基础概念

- [ ] 能说清 cmux 和传统终端（如 iTerm2、Terminal.app）的核心区别
- [ ] 能解释为什么 cmux 选择 Swift + libghostty 而不是 Electron
- [ ] 能说出 cmux 的 Skills 系统和 Claude Code 的 Skills 有什么异同

### 配置与使用

- [ ] 能为 cmux 配置 Claude API Key 并设置环境变量
- [ ] 能创建一个自定义 Skill 并实现基本功能
- [ ] 能配置远程 daemon 并实现跨设备开发

### 决策与对比

- [ ] 能为"个人开发"和"团队协作"两种场景分别判断是否需要 cmux
- [ ] 能对比 cmux、Warp、tmux 三者的适用边界
- [ ] 能判断一个现有工作流是否值得迁移到 cmux
...
1. 安装 cmux 并配置 Claude API
2. 尝试内置的 Claude 命令（`cmux claude "..."`)
3. 阅读一个现有 Skill 的源码（如 `skills/git-helper/SKILL.md`）
4. 理解 Skill 的触发条件和执行逻辑

### 第二步：自定义扩展（3-7 天）

1. 创建一个自定义 Skill，解决一个真实痛点（如自动格式化代码、自动提交消息）
2. 学习 cmux 的 Hooks 系统，实现事件驱动的工作流
3. 尝试远程 daemon，体验跨设备 AI 终端

### 第三步：深度集成（长期）

1. 将 cmux 集成到现有开发流（如 Git hooks、CI/CD）
2. 探索 cmux 的源码，理解其架构设计
3. 为 cmux 贡献 Skill 或提交 PR

### 相关资源

| 资源 | 链接 | 用途 |
|------|------|------|
| 官方文档 | https://cmux.dev/docs | 完整配置指南 |
| Skills 示例 | https://github.com/manaflow-ai/cmux/tree/main/skills | 学习最佳实践 |
| Discord 社区 | https://discord.gg/cmux | 获取帮助和交流 |
