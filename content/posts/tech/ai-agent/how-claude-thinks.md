...
- 了解什么是 AI 可解释性（Interpretability），以及它为什么重要
- 知道 Anthropic 的"电路追踪"方法大致在做什么
- 看懂 Claude 为何看起来像是在"用概念"而不是"用某种具体语言"思考
- 理解 Claude 在写诗、做心算、回答事实问题时暴露出的内部规律
...
下面依次展开：研究者用什么方法观察模型内部 → 模型是否用“概念”而非语言思考 → 提前规划的证据 → 心算的真实路径 → 什么时候推理不可靠 → 幻觉的误触发机制 → 越狱中的安全竞争。每个发现都附带干预实验的验证条件和相应的使用启示。
...
于是，他们尝试造一台“AI 显微镜”——是，不是只和模型对话尽量去做内部链路追踪（Tracing），观察模型在生成输出时，哪些概念被激活、如何相互影响、以及哪些路径真正改变了结果。
...
为了研究这些特征之间的连接关系，团队构建了一个**替代模型（Replacement Model）**。可以理解为一个为解释任务而构造的近似模型：它尽量保留原模型相关行为，但把内部表示改写成更可分析的“特征网络”。需要注意两点：
...
> 🦞 **作者后记**：理解 AI 如何工作，是让 AI 更可靠、更安全的重要前提。Anthropic 的这组研究让我们第一次较清晰地看到，大模型内部并不是一团不可触碰的迷雾；但它也提醒我们，当前看到的仍只是局部。对 AI 最务实的态度，或许是在持续验证中不断校准判断。
