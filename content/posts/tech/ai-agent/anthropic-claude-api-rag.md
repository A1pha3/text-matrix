...
description: "RAG（检索增强生成）系统的构建方法：文档分块、嵌入向量生成、向量数据库选择、语义搜索策略、重排序技术，附带完整的 Python 代码实现示例。"
draft: false
categories: ["技术笔记"]
...
| 功能 | 模板 |
|------|------|
| RAG 提示词 | `build_rag_prompt(question, context_docs)` |
| 检索流程 | `retriever.retrieve(query, top_k)` |
| 问答 | `rag.query(question)` |

### 下一步

- 继续阅读：MCP 协议专题（五）- 深入了解 MCP 架构
- 实践项目：用向量数据库搭建本地知识库
