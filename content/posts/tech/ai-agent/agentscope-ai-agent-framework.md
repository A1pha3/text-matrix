...
AgentScope 最值得关注的地方不是“功能很多”，而是 **功能背后的抽象一致**。消息、状态、工具、协议、工作流、实时交互，最终都被收敛到一套清晰的模型里。
...
与“先拼功能再回头补抽象”的框架不同，AgentScope 是先建模再扩展。这也是为什么它的架构能承载更复杂的 Agent 系统演进。
...
AgentScope 是在认真回答"Agent 应用如何工程化"这个问题，而不是在做又一个 Agent Demo 框架。
