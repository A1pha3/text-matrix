...
github_repo: "tw93/Mole"
source_key: "gh:tw93/Mole"
...
**Mole**是由[twa](https://github.com/tw93)（也是 Pake 和 Kaku 的作者）开发的 macOS 全能维护工具，核心定位是**Deep clean and optimize your Mac**。它将 CleanMyMac、AppCleaner、DaisyDisk 和 iStat Menus 等多个知名工具的功能整合到一个二进制文件中，提供命令行界面的深度清理和系统优化体验。
...
1. **理解 Mole 定位**：明白 Mole 作为 macOS 全能维护工具的核心价值
2. **掌握核心特性**：了解深度清理、智能卸载、磁盘分析、实时监控等核心特性
3. **动手实践**：掌握 Mole 的安装、基础命令、进阶用法
4. **对比同类工具**：理解 Mole 与 CleanMyMac、AppCleaner 等工具的异同
5. **评估适用性**：判断 Mole 是否适合你的 macOS 维护需求

## 目录

1. [项目概述](#项目概述)
2. [核心特性](#核心特性)
   - [All-in-one 工具箱](#all-in-one-工具箱)
   - [Deep Cleaning 深度清理](#deep-cleaning-深度清理)
   - [Smart Uninstaller 智能卸载](#smart-uninstaller-智能卸载)
   - [Disk Insights 磁盘洞察](#disk-insights-磁盘洞察)
   - [Live Monitoring 实时监控](#live-monitoring-实时监控)
3. [快速上手](#快速上手)
   - [安装](#安装)
   - [基础命令](#基础命令)
4. [进阶用法](#进阶用法)
...
Mole 是 macOS 维护领域的创新工具，它将多个知名工具的功能整合到一个命令行工具中，提供高效、可脚本化的系统维护体验。
...
1. **Mole 的核心定位是什么？**
   ...
   Deep clean and optimize your Mac. 将 CleanMyMac、AppCleaner、DaisyDisk 和 iStat Menus 等多个知名工具的功能整合到一个二进制文件中，提供命令行界面的深度清理和系统优化体验。
   </details>

2. **Mole 的"安全优先"设计原则体现在哪些方面？**
   <details>
   <summary>查看答案</summary>
   路径验证、受保护目录规则、保守清理边界、明确确认、操作前预览（--dry-run）、操作记录日志。
   </details>

3. **Mole 支持哪些安装方式？**
   <details>
   <summary>查看答案</summary>
   Homebrew（推荐）、安装脚本、Windows 分支（实验性）。
   </details>

4. **Mole 与 CleanMyMac 的主要区别是什么？**
   <details>
   <summary>查看答案</summary>
   Mole 完全免费开源、单一二进制、高度可脚本化、适合终端用户；CleanMyMac 是 GUI 工具、付费、功能完整。
   </details>

5. **如何使用 Mole 的安全预览模式？**
   <details>
   <summary>查看答案</summary>
   使用 `mo clean --dry-run` 命令可以预览清理内容，不实际删除。也可以使用 `mo clean --dry-run --debug` 查看详细日志。
   </details>

## 进阶路径

1. **安装 Mole**：通过 Homebrew 或安装脚本安装 Mole
2. **基础使用**：使用 `mo` 命令打开交互式菜单，熟悉基础命令
3. **深度清理**：使用 `mo clean` 深度清理系统，先用 `--dry-run` 预览
4. **配置白名单**：配置清理和白名单，保护重要文件
5. **自动化维护**：将 Mole 命令集成到自动化脚本中，实现定期维护

## 资料口径说明

本文基于 Mole 项目的 GitHub 仓库信息（最后更新：2026-04-07）。由于 Mole 是活跃维护的开源项目，以下内容可能随时间变化：

1. **核心数据**：Stars、Forks、版本号等数据来自 GitHub 仓库页面，可能已更新
2. **功能特性**：Mole 的功能特性持续演进，本文描述的功能可能需要与新版本对照
3. **命令示例**：Mole 的命令和输出示例可能随版本更新而变化，请以官方文档为准
4. **安全设计**：Mole 的安全机制和策略可能随版本演进而调整
5. **跨平台支持**：Windows 版本的支持情况可能已变化，请查看最新分支



---

*本文基于 Mole V1.33.0 版本编写，发布时间：2026-04-07*
