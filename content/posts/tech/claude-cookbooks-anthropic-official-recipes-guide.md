...
如果你正在构建一个基于 Claude 的应用——无论是智能客服、文档分析工具还是多模态内容理解系统——Anthropic 维护的 [Claude Cookbooks](https://github.com/anthropics/claude-cookbooks) 仓库可能是你绕过坑、快速上手的最高效路径。这个仓库在 GitHub 上积累了超过 40,000 个 Star，540 余次提交，73 位贡献者，代码主体是 Jupyter Notebook（约 95.7%）加上少量 Python 脚本。
...
从这个结构可以看出，仓库的设计遵循一条清晰的进阶路径：先从 `capabilities` 入手掌握单次 API 调用的基础能力，再进入 `tool_use` 学会让 Claude 调用外部工具，接着用 `multimodal` 解锁视觉输入，最后通过 `patterns/agents` 和 `extended_thinking` 把整个系统串联成复杂的 Agent 工作流。
...
Cookbooks 的价值在于把上述模式拓展到了十几个不同的应用领域。下面按使用频率逐一介绍。
...
完成本文学习后，用以下检查项验证你的理解程度：

1. 你能独立写出一个包含两个工具的 Tool Use 对话循环吗？要求支持工具间数据传递和多轮调用。
2. 对于一段 3000 字的文章，你能用 RAG 模式（嵌入 -> 检索 -> 注入提示 -> Claude 生成）从外部知识库中检索相关内容并生成答案吗？
3. 你能在不查阅文档的情况下，写出 Prompt Caching 的 Python 代码并说出缓存的计费规则吗？
4. 给定一段包含图表的 PPT 截图，你能写出多模态请求让 Claude 提取其中数据并转换为结构化表格吗？
5. 你能准确说出 Haiku / Sonnet / Opus 的适用场景和成本差异，并为三个不同难度的任务选择合适模型吗？
6. 你能实现一个子代理模式：用 Haiku 预处理 5 篇长文档提取关键信息，再交给 Sonnet 综合生成最终报告吗？

如果以上 6 项中你能完成 5 项以上，说明你已经掌握了 Cookbooks 的核心模式，可以在实际项目中应用。如果还有缺口，建议回到对应的 Cookbooks Notebook 中运行一遍完整代码，亲手调试比阅读十遍更有效。
