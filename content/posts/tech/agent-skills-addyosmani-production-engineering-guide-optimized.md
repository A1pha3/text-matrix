---
title: "Agent Skills：addyosmani 的生产级 AI编程工程技能框架"
...
Agent Skills 要解决的问题就是这个。它由 Google 前工程师 addyosmani 开源，把资深工程师在真实项目中的工作流封装成 23 个结构化技能和 7 条命令，强制 AI 按 Define → Plan → Build → Test → Review → Ship 的完整生命周期工作。

用 Claude Code 或 Cursor 的团队大多遇到过同一件事：AI 产出的代码能跑，但缺测试、没审查、没有 Spec。Agent Skills 把这些环节写成 AI 可执行的步骤，让每次产出都走同一套流程。如果 AI 想跳过测试或审查，内置的 anti-rationalization 表会把它拉回来。
