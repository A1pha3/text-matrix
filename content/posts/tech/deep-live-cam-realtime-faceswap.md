...
把实操里最容易踩的几个点先摆出来，省得你到运行时报错了才回头翻：

- **一定要 GPU 吗？** 不是。预构建有 CPU 版，能跑但慢：首次加载模型要等，实时预览的帧率也会掉。想把它当日常工具用，一块 NVIDIA 卡或 Apple Silicon 的体验会明显更好。
- **换脸结果「露怯」从哪排查？** 先看源脸照片是否清晰、人脸是否正对镜头。再看目标画面，光源和角度越接近源脸越稳。开着 Face Enhancer 时先关掉看原图，能分清楚是「交换本身就差」还是「增强在后处理拖后腿」。
- **画面里不止一张脸？** 默认只换主脸。要替换所有人，开 Many Faces；要给不同人贴不同源脸，用 Face Mapping，先把源和目标对应好再 Live。
- **视频导出总中断？** 2.7 把导出挪进独立进程，就是为了缓解这类尾段崩溃。若在手动安装的 2.1.6 老链路上复现，多半是这条旧线的编码或音频参数问题，先核对命令行参数是否齐全，再考虑换预构建。
- **标注要求到底多严格？** 官方只给了边界——使用真人的脸必须征得同意、线上分享要如实标注。落到某个平台该写多清楚、要不要加水印，README 不给标准答案，得按你所发布平台的规则来。
...
到这里，主线应该已经清楚了。用它之前，把这四问过一遍，答案基本就决定了路径——其中后两问也顺便检验你前面几节有没有读串：

- 我要的是**即时反馈的创作**，还是**一张能投大屏的漂亮单帧**？前者这台给得起，后者要回到逐帧精修流程。
- 我有没有 GPU？没有也能跑，但「实时」体验会明显缩水，先拿 CPU 版试一件真活再决定值不值得上硬件。
- 我用的脸是不是真人、有没有授权和标注义务？这决定的是「能不能合法用」，比「能不能跑」更靠前。
- 我要开箱即用，还是能改源码？前者直接下预构建包，后者才值得走 README 那条 2.1.6 老路径。
