...
`ml-explore/mlx`（下文简称 MLX）是 Apple 机器学习研究团队自研、面向 Apple silicon 的**数组框架**（array framework），核心区别点不是 Python 写起来像 NumPy 这么简单，而是三件事同时成立：
...
**关键事实**：MLX 的 `array` 本身**没有 buffer**。`ArrayDesc` 里 `std::shared_ptr<Data> data` 那一栏，只有在 `eval()` 真正把图落地、或者这个数组是用户手动 `array(数据, shape)` 构造的时候才存在。换句话说：
...
对单机多 GPU 不友好（这部分仍走 backend 自己的 stream），主要解决**多机多卡 Apple silicon**（Mac Studio 集群）或**多机 Linux+CUDA** 的场景。`MLX_ENABLE_X64_MAC=OFF` 显式禁掉 Intel Mac build，因为 Apple silicon 才是首选目标。
...
`mlx-examples/lora/` 给出的对照（来源：仓库 README + issues）：

| 平台 | 设备 | seq_len | tokens/sec | 备注 |
|---|---|---|---|---|
| MLX | M2 Ultra (192GB) | 2048 | ~14k | LoRA r=8, qlora |
| MLX | M2 Max (96GB) | 2048 | ~8k | LoRA r=8 |
| PyTorch + MPS | M2 Ultra | 2048 | ~6k | 不开 compile |
| PyTorch + CUDA | A100 80GB | 2048 | ~120k | 4-bit weight, FSDP |

**测的是什么**：纯 forward + backward，**不包含**数据加载、tokenizer、checkpoint 落盘；**不测** batch>1 的吞吐（MPS 路径上 batch>1 的 matmul 仍跑 accelerate，没有充分利用 Apple GPU 的 SIMD 宽度）。

**能推出什么**：

- Apple silicon 的 M2 Ultra 在 7B 模型上**对个人研究者**是可用平台，**不是**生产训练平台。
- MLX vs PyTorch+MPS **同平台**有约 2× 提升，主要来自 fused kernel（`MLX_METAL_JIT` 路径）和 unified memory 零拷贝。

**不能推出什么**：

- "MLX 比 A100 快 10×"（绝对吞吐差 1 个数量级以上，差的不是框架）。
- "Apple silicon 可以取代 CUDA 训练"（小模型 + 小 batch + 大显存的边缘场景下 yes，主流 LLM 训练仍必须 CUDA）。

### 5.2 推理延迟：Whisper / Stable Diffusion

`mlx-examples/stable_diffusion/` 给的 512×512 SDXL 推理：

- M2 Max：~17s / 步（fp16 + reduce-overhead）
- M2 Ultra：~9s / 步
- 同等模型在 A100 80GB：~0.5s / 步

**测的是什么**：单 batch、fp16、50 步采样；**不包含** encoder / decoder 文本侧。

**能推出什么**：

- 创意工具做本地预览（30 张图/h 量级）可行；批量生产（>1000 张/小时）必须上 A100 / H100。

**不能推出什么**：

- "Apple silicon 适合 AIGC 服务"——SLA < 1s 的服务仍要 GPU 服务器。
...
- **不支持 x86_64 Mac**：`MLX_ENABLE_X64_MAC=OFF` 是默认。Intel Mac 用户要自己打开这个选项，但 Metal kernel 在 Intel iGPU 上没有可用测试矩阵。
...
MLX 的价值不在于"再造一个 PyTorch"，而在于**把 Apple silicon 的 unified memory 真正暴露成 array framework 的一等公民**，让 CPU/GPU 的差别从 API 表面消失。在 Apple silicon 上做研究、跑中小模型推理、做 C++ 嵌入，这三个场景下 MLX 是 2026 年这个时间点最省心的选项；其他场景，仍是 PyTorch + CUDA 的天下。
