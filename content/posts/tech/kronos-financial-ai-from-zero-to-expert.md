...
Kronos 的核心思想，它借用的是语言模型里最成功的一条路线，而不是把价格序列硬套成自然语言：
...
如果你真想达到专家水平，第一步是会区分事实层、推断层和想象层，而不是会复述宣传语。
...
`DependencyAwareLayer` 的直观理解：**它保证细粒度 Token 的生成始终受粗粒度结构约束，而不是让 `s1` 和 `s2` 各自独立猜。**公开代码展示出的设计思路是“先有骨架，再在骨架内补细节”，不是两层并列、互不相干地各自输出。
