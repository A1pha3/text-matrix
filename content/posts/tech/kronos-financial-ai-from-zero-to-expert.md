...
categories: ["技术笔记"]
tags: ["Kronos", "金融基础模型", "金融时间序列", "K线预测", "时间序列基础模型", "量化研究", "论文精读"]
draft: false
---
...
如果你更习惯先看总图，再拆模块，这张图可以先把 Kronos 当前公开主链路一次串起来。
...
3. **它既能编码也能解码。**这意味着 Tokenizer 并不只是为“压缩输入”服务，它还决定了模型最终能恢复出怎样的连续数值。
...
assert all(col in x_df.columns for col in required_cols), "缺少 OHLC 必需列"
assert not x_df[required_cols].isna().any().any(), "OHLC 存在 NaN"
assert len(x_df) == len(x_timestamp), "历史数据和历史时间戳长度不一致"
assert len(y_timestamp) == pred_len, "未来时间戳长度和 pred_len 不一致"
assert x_timestamp.is_monotonic_increasing, "x_timestamp 不是递增时间序列"
assert y_timestamp.is_monotonic_increasing, "y_timestamp 不是递增时间序列"
assert lookback <= max_context, "lookback 超过 max_context"
...
assert pred_df.notna().all().all(), "预测结果含 NaN"
assert (pred_df[["open", "high", "low", "close"]] > 0).all().all(), "预测价格出现非正值"
...
assert not df[feature_cols].isna().any().any(), "输入数据存在 NaN"
assert lookback <= max_context, "lookback 超过 max_context"
assert len(df) >= lookback + pred_len, "样本长度不足以切出历史段和预测段"
...
print("saved to ./kronos_prediction_output.csv")
```
...
如果你更喜欢先看决策树，再读分支细节，这张图可以直接把第 7 节的选择逻辑串起来。
...
图 4：Kronos 预测到执行的评估闭环图

如果你想把“模型效果”真正翻译成“系统效果”，这张图就是第 10.6 节最应该记住的骨架。
...
1. **会用一个模型，只是起点；会判断哪些说法可信，才算真正入门。**
2. **理解 Tokenizer、时间特征和推理接口，比背模型宣传指标更重要。**
3. **金融模型的价值，永远要放在完整研究与执行链路里判断，而不是只看一段预测曲线。**
4. **真正的专家能力，是在公开版本的边界上，提出下一步该怎么改，并把它设计成可以被验证的实验。**
