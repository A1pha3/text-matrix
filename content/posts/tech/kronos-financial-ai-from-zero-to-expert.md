...
5. 本文只写能从公开论文摘要、README 与代码交叉核实的内容。比如"两级检索系统""Kronos-Chat""500B+ Tokens""92% 基准 SOTA"等说法当前无法直接证实，不写入结论层。
...
### 3.2 离散化编码器（Tokenizer）是核心组件

`KronosTokenizer` 不是一个简单归一化器，而是一个带有 Transformer 编码器、解码器与量化器的独立模块，负责把连续的多维输入压缩成适合语言建模的离散表示，再从离散表示重建回连续空间。
...
## §9 专家视角：如何读 Kronos 论文

使用 Kronos 只需知道 API 怎么调。要到达能改进的层次，需要换一套问题意识。
