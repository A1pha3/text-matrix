...
大部分程序员的学习路径是：先学会用某个工具，然后读文档，看教程，遇到问题去 Stack Overflow 搜答案。这套方法对付日常开发够用，但天花板很低——你永远在「用」技术，而不是「理解」技术。
...
传统学习路径的典型问题是：你学了一堆「怎么做」，但不知道为什么这么做。
...
每个项目给你「做什么」（需求规范）和「怎么验」（测试用例），你自己解决「怎么做」。Problem-Based Learning 的效果已被大量研究证实：主动构建知识比被动接收知识的留存率高出数倍。
...
这个仓库的价值和局限都来自同一个事实：它让你「从零构建」，但构建的是简化版本。
