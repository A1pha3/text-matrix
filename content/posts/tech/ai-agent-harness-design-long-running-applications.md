...
**关键发现**：在 Anthropic 早期实验里，Claude Sonnet 4.5 表现出明显的上下文焦虑，仅靠压缩难以稳定支撑长任务，因此 Context Reset 在那个阶段成为关键设计。

但这不是一个永久结论。随着模型能力提升与 Harness 迭代，Anthropic 在后续版本中已经展示过：对于某些任务，可以去掉 Context Reset，改用单次连续会话配合自动 Compaction。也就是说，**是否需要 Reset，取决于模型特性、任务长度和交接成本，而不是固定教条**。
...
这一组数字对应的是早期 Harness：Initializer/Coding Agent、Feature List、Progress File、`init.sh`、Git 交接与 Context Reset 的组合。
...
这一组数字对应的是后续简化版本：Planner-Generator-Evaluator 仍然保留，但不再沿用早期那套 Context Reset + 每轮独立交接的重编排方式，而是更多依赖连续会话与自动 Compaction。
