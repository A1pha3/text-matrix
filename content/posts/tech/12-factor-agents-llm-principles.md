...
上下文窗口是 Agent 的核心工程对象，容量数字只是它的一个属性。
...
XML 标签直接标记了信息边界，模型更容易定位相关上下文，降低"中间丢失"（lost in the middle）效应。对工程师来说，事件结构本身就是调试信息——你能直接看到每一步的类型和数据，不需要在 JSON 嵌套里翻。上下文工程还包括 RAG（检索增强生成）、跨会话记忆、Schema 对齐解析（BAML 等工具）。这些技术各管一段数据供给，但**上下文的构建方式必须由你完全控制**——否则你永远没法系统性地优化 token 效率和注意力分配。
...
把"一个大 Agent 做 10 件事"和"5 个小 Agent 各做 2 件事"放在一起对比，差异是具体的：上下文长度上，前者要把 10 个场景的说明塞进同一个 system prompt，后者每个 Agent 只装载自己那 2 个场景的说明；工具数量上，前者挂 20+ 个工具，LLM 每轮要在 20 个候选里做选择，后者每个 Agent 5 个以内；测试难度上，前者改一处 prompt 可能影响 10 个场景的回归，后者改一个 Agent 只跑它自己的测试套件；故障半径上，前者一个幻觉可能把支付逻辑带偏，后者支付 Agent 出问题不会污染报告 Agent。这四条加起来，决定了大 Agent 在 70% 之后每提升 1% 都要付出更高的边际成本——每多一个场景，回归测试套件就要多覆盖一条路径，prompt 调整的影响面也随之扩大。
...
Factor 8 把 while loop 的终止条件、重试次数、人工升级写在 `agent_loop` 函数体里；Factor 12 把每一步的状态变更建模为 `agent_step(state, event) -> new_state` 的纯函数调用。两者落地后，Agent 的执行轨迹变成一串可序列化的 State 对象，每一步的输入输出都能在日志里查到——LLM 只决定单步选哪个工具、填什么参数，不再接管整个应用的控制权。
...
Heroku 那套原则把"应用该怎么部署"从手工操作拉进了工程化轨道，配置外置、依赖显式、进程无状态这些做法后来成了云原生时代的默认前提。12-Factor Agents 在 LLM 应用上做的是同一件事：把 prompt 构建位置、上下文格式、控制流归属、状态持久化这些原本被框架隐藏的决策，逐条暴露出来交给团队自己掌控。
