...
12-Factor Agents 在 GitHub 上已获 20k+ stars（截至 2026-05，参见 [GitHub 仓库](https://github.com/humanlayer/12-factor-agents)）之后，作者 Dex 持续在更新补充材料——视频讲解、脚手架 `npx/uvx create-12-factor-agent`、Discord 社区和开源参考实现 `got-agents/agents`（链接有效性以发布时为准）。团队既拿它审视现有实现里哪些环节失控，也拿它指导新项目从第一行代码开始的结构。
...
- [好 Agent 的本质是软件](#好-agent-的本质是软件)
  - [软件本质上是有向图](#软件本质上是有向图)
  - [Agent Loop 的承诺与现实](#agent-loop-的承诺与现实)
- [系统地图](#系统地图)
...
- [常见翻车现场](#常见翻车现场)
  - [从 70% 到生产级的典型翻车路径](#从-70-到生产级的典型翻车路径)
- [常见问题](#常见问题)
- [自检清单](#自检清单)
- [练习题](#练习题)
- [进阶路径](#进阶路径)
- [项目资源（含 Factor 13 荣誉提及）](#项目资源)
...
提示词要进 Git、要能 grep、要在 code review 里被讨论——把它当成和业务代码同等地位的工程资产来管理。
...
一个电商团队的客服 Agent 踩过这个坑：执行状态存在内存里，业务状态存在订单表里。某次部署重启后，Agent 丢了执行状态，但订单表显示"已退款"——Agent 重复执行了退款操作。修复方案是把 `agent_tasks` 表和 `orders` 表放在同一个事务里，Agent 每完成一步就更新两张表的状态字段。
...
控制流——"什么时候做什么、什么条件下跳转、什么情况下终止"——写在确定性代码里，LLM 只负责单步决策（选哪个工具、填什么参数）。
...
**边界判断**：错误该不该压进上下文，看的是"LLM 重新决策能不能改变结果"。如果是系统性的——API key 失效、数据库连接断了——重试不会改变结果，应该走传统异常处理。Factor 9 最适合的是**可恢复的、瞬时的、LLM 有能力做出有意义决策的错误**：API 限流、临时超时、数据格式不匹配。区分这两类错误的规则不妨写成一段确定性代码：凡是 `error_type` 在 `RECOVERABLE_ERRORS` 集合里的进上下文，其余走传统异常抛出。
...
把"一个大 Agent 做 10 件事"和"5 个小 Agent 各做 2 件事"放在一起对比，差异是具体的：上下文长度上，前者要把 10 个场景的说明塞进同一个 system prompt，后者每个 Agent 只需装载自己那 2 个场景的说明；工具数量上，前者挂 20+ 个工具，LLM 每轮要在 20 个候选里做选择，后者每个 Agent 5 个以内；测试难度上，前者改一处 prompt 可能影响 10 个场景的回归，后者改一个 Agent 只跑它自己的测试套件；故障半径上，前者一个幻觉可能把支付逻辑带偏，后者支付 Agent 出问题不会污染报告 Agent。这四条加起来，决定了大 Agent 在 70% 之后每提升 1% 都要付出指数级成本。
...
Agent Loop 解决的是"模型怎么调工具"这一层的问题，业务流程该怎么编排还得另外设计——把它当应用架构用，等于把 if/else 全交给概率模型。
...
- [ ] 需要人工介入时，是否通过统一的工具调用机制而非硬编码的异常分支？（Factor 7，答否→回读 [Factor 7](#factor-7用工具调用联系人类)）
...
4. **对照 12-Factor App 原始方法论**：读 [12factor.net](https://12factor.net) 原文，理解 Agent 版本在哪些地方做了类比、哪些地方做了调整，加深对工程动机的把握
