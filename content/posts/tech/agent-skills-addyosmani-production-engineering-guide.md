...
description: "16.8k Stars 的生产级工程技能框架，为 AI 编程 Agent 提供 Define→Plan→Build→Test→Review→Ship 的完整生命周期管理。20 个结构化技能，7 条 slash 命令，让 AI 编码遵循专业工程师的工作方式。"
draft: false
categories: ["技术笔记"]
...
AI 编码 Agent 默认走最短路径——拿到需求直接写代码，跳过规格说明、省略测试、忽略安全审查。这种模式做原型够用，放到生产环境就会暴露问题：没有测试的代码不敢重构，没有审查的代码藏着边界缺陷，没有 Spec 的项目在需求变更时失控。

Agent Skills 解决的正是这个问题。这套由 Google 前工程师 addyosmani 开源的技能框架，把资深工程师在真实项目中使用的工作流、质量门控和实践建议封装成 23 个结构化技能和 7 条 slash 命令，强制 AI 按定义、规划、构建、验证、审查、部署的完整生命周期工作。

如果你正在用 Claude Code、Gemini CLI 或 Cursor 等 AI 编码工具，并且发现 AI 产出的代码"能跑但不敢上生产"，这篇文章会拆解 Agent Skills 如何把工程纪律注入 AI 的工作流。读完本文，你会了解：

- Agent Skills 解决的核心问题和它的三层组件结构
- 6 阶段、7 命令、23 技能如何协同工作
- 如何用一个完整任务流（用户认证系统）把这套框架跑通
- 哪类团队该先用、哪类可以等等、从哪个阶段切入最稳
...
AI 编码 Agent 的默认行为是走最短路径：拿到需求直接写代码，跳过 Spec（规格说明）、测试、安全审查和代码审查。这种模式适合做原型，但放到生产环境就会暴露问题——没有测试的代码不敢重构，没有审查的代码藏着边界缺陷，没有 Spec 的项目在需求变更时失控。

Agent Skills 的做法是把资深工程师的工程实践写成 AI 可执行的工作流。每个技能不是一段参考文档，而是一组带步骤、检查点和退出条件的流程，AI 必须按流程走完才能产出代码。
...
> 数据采集于 2026-04-12。仓库技能数量会随版本迭代变化，最新数量以仓库 README 为准。

---

## 系统总览：三层组件如何配合

Agent Skills 由三层组件构成，理解这三层的关系是使用这套框架的前提。
...
Slash 命令是用户接触框架的入口，每条命令激活一组技能，技能在执行过程中按需加载参考清单。三层之间的关系是"调度 → 执行 → 补充"，不是平级并列。
...
每条命令不是简单的提示词，而是一个工作流调度器。以 `/build` 为例：
...
这一节演示如何用 6 阶段、7 命令、23 技能完成一个真实任务：开发一个支持邮箱注册、登录、密码重置的用户认证系统。系统不含 OAuth 和第三方登录，技术栈为 Node.js + Express + PostgreSQL。
...
| One slice at a time | 一次实现一个垂直切片 | `/build` 每个切片含实现+测试+提交，不跨切片 |
| Tests are proof | 测试是代码正确的证据 | `/test` 要求覆盖率 ≥ 80%，未覆盖代码视为不存在 |
| Improve code health | 持续提升代码健康度 | `/review` 五轴审查，变更规模控制在约 100 行 |
...
Agent Skills 的一个关键设计是 anti-rationalization 表。AI 在执行流程时倾向于找借口跳步，每个技能都内置了常见借口和反驳：
...
AI Agent 在压力下（长任务、复杂上下文）会复现人类工程师的偷懒模式，anti-rationalization 表把"借口 → 反驳"的逻辑写进流程，让 AI 无法绕过。
...
| 框架 | 定位 | 与 Agent Skills 的差异 |
|------|------|----------------------|
| LangChain Agents | LLM 调用链编排 | 专注 LLM 调用逻辑，不涉及工程纪律 |
| AutoGen | 多 Agent 对话协作 | 专注 Agent 间通信，不涉及代码质量门控 |
| CrewAI | 角色化 Agent 编排 | 专注角色分工，不涉及开发生命周期 |
| Agent Skills | 生产级工程技能 | 专注把工程纪律注入 AI 编码流程 |

这四套框架不是互斥关系。LangChain、AutoGen、CrewAI 解决的是"AI 如何协作和调用工具"，Agent Skills 解决的是"AI 写代码时如何遵循工程纪律"。如果项目同时需要多 Agent 协作和生产级代码质量，可以组合使用。
...
不冲突。`/review` 命令产出的是 AI 审查报告，不替代人工审查。建议的流程是：AI 先跑 `/review` 修复明显问题，再提交人工审查，减少人工审查的低级问题负担。
...
2026 年大多数 AI 编码工具已经能让 AI 写出能跑的代码。Agent Skills 补齐的是"代码能跑"和"代码敢上生产"之间的工程纪律——Spec、测试、审查、灰度发布，这些环节在 AI 默认行为里是被跳过的。

如果你正在用 AI 编码工具做生产级项目，最稳的切入点是 `/review` 和 `/test`——这两个命令不改变现有流程，只在代码完成后增加质量门控，团队适应成本最低。等团队习惯这套节奏后，再逐步引入 `/spec` 和 `/plan`，把工程纪律前移到需求阶段。

这套框架不适合所有场景。探索性研究、一次性脚本、教学演示代码不需要 23 个技能的完整流程，强行使用反而拖慢节奏。判断标准是：代码需要上线、需要维护、需要多人协作时，Agent Skills 值得引入；代码写完就扔时，手动控制更直接。
