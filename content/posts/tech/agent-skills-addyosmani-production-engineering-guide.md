...
AI 编码 Agent 默认走最短路径：拿到需求直接写代码，跳过规格说明、省略测试、忽略安全审查。做原型够用，放到生产环境就暴露问题——没有测试的代码不敢重构，没有审查的代码藏着边界缺陷，没有 Spec 的项目在需求变更时失控。

Agent Skills 解决的就是这套默认行为。它由 Google 前工程师 addyosmani 开源，把资深工程师在真实项目中的工作流封装成 23 个结构化技能和 7 条命令，强制 AI 按 Define → Plan → Build → Test → Review → Ship 的完整生命周期工作。
