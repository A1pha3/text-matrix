...
description: "16.8k Stars 的生产级工程技能框架——23 个结构化技能和 7 条 slash 命令，强制 AI 按 Define→Plan→Build→Test→Review→Ship 的完整生命周期工作。"
draft: false
categories: ["技术笔记"]
...
Agent Skills 解决的就是这套默认行为。它由 Google 前工程师 addyosmani 开源，把资深工程师在真实项目中使用的工作流、质量门控和实践建议封装成 23 个结构化技能和 7 条命令，强制 AI 按 Define → Plan → Build → Test → Review → Ship 的完整生命周期工作——不是在建议 AI 这么做，而是让 AI 无法跳过这些步骤。

用 Claude Code、Gemini CLI 或 Cursor 的团队最常遇到一个困境：AI 产出的代码能跑，但缺测试、没审查、没有 Spec，放到生产环境就是在赌。Agent Skills 把这种赌注变成了可重复的流程。

往下读之前，值得先摸清这几点：

- 三层组件（命令 → 技能 → 参考清单）之间是调度关系，不是平级堆叠
- 6 个阶段有严格的推进条件，Refine 循环允许任何阶段回退
- 完整任务流案例会展示一个用户认证系统如何从头到尾穿过这套框架
...
Agent Skills 的做法是把这套工程实践写成 AI 可执行的流程：每个技能不是一段参考文档，而是一组带步骤、检查点和退出条件的指令，AI 必须按流程走完才能产出代码。
...
Agent Skills 由三层组件构成。理解这三层的关系，才能判断什么时候该手动输入命令、什么时候 AI 会自己激活技能。
...
这一节的目标：把 6 个阶段的名字和推进条件记住，然后能判断你手头的项目处于哪个阶段、缺少哪个阶段的产物。
...
Agent Skills 的一个关键设计是 anti-rationalization 表——AI 在执行流程时会本能地找借口跳步。每个技能都内置了常见借口和反驳，把"借口 → 反驳"的逻辑写进流程，让 AI 无法绕过。
...
这些借口不是虚构的——AI 在长任务、复杂上下文下确实会复现人类工程师的偷懒模式。所以 anti-rationalization 表不是"建议"，是硬条件。
...
LangChain、AutoGen、CrewAI 经常和 Agent Skills 放在一起讨论。但它们的定位完全不同：前三者解决的是"AI 如何协作和调用工具"，Agent Skills 解决的是"AI 写代码时如何遵循工程纪律"——两者不是互斥关系，可以组合使用。
...
2026 年，让 AI 写出能跑的代码已经不是问题。Agent Skills 在做的是另一件事——把 Spec、测试、审查、灰度发布这些 AI 默认会跳过的环节，硬编码到它的工作流里。不是提醒，是强制。
...
但 23 个技能不是越多越好。探索性研究、一次性脚本、教学演示代码不需要完整的 Define → Ship 流程——强行走完只会拖慢节奏。反过来，代码需要上线、需要长期维护、需要多人协作时，这套流程值得引入。
...
把以上 4 步走完一轮，你对 Agent Skills 的理解就不会停在纸面上。
