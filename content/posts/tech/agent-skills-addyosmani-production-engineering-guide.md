...
Agent Skills 解决的就是这套默认行为。它由 Google 前工程师 addyosmani 开源，把资深工程师在真实项目中使用的工作流、质量门控和实践建议封装成 23 个结构化技能和 7 条命令，强制 AI 按 Define → Plan → Build → Test → Review → Ship 的完整生命周期工作。AI 在执行过程中如果想跳过测试或审查，anti-rationalization 表会把它拉回流程内。

用 Claude Code、Gemini CLI 或 Cursor 的团队最常遇到一个困境：AI 产出的代码能跑，但缺测试、没审查、没有 Spec，放到生产环境就是在赌。Agent Skills 把测试、审查和 Spec 这些环节写成 AI 可执行的步骤，让每次产出都走同一套流程。

读下去之前，先摸清这几点：

- 三层组件（命令 → 技能 → 参考清单）之间是调度链，上一层驱动下一层
...
Agent Skills 的一个关键设计是 anti-rationalization 表。AI 在执行流程时会本能地找借口跳步，每个技能因此内置了常见借口和反驳，把"借口 → 反驳"的逻辑写进流程。
...
这些借口在长任务、复杂上下文下确实会出现——AI 会复现人类工程师的偷懒模式。anti-rationalization 表因此被设计成硬条件，技能执行时会逐条对照检查。
