...
github_repo: "VectifyAI/OpenKB"
source_key: "gh:VectifyAI/OpenKB"
...
draft: false
categories: ["技术笔记"]
tags: ["LLM", "知识库", "RAG", "Python", "Obsidian", "向量检索"]
---
...
向量检索的局限在于它是一种"浅层匹配"。Embedding 模型将文本映射为向量时，丢失了语义层级、逻辑关系和领域知识结构。同样一段关于"注意力机制"的文字，在一篇论文里是作为核心贡献出现的，在另一篇里是作为背景知识一笔带过的，向量相似度无法区分这种差异。
...
OpenKB 的思路来自 Karpathy 的一个观点：LLM 有能力理解文档的语义层级（哪些是核心概念、哪些是子主题、概念之间如何关联），我们应该让 LLM 做这件事，而不是简单地把文档切成块然后做最近邻搜索。
...
PageIndex 是 OpenKB 最关键的技术组件，也是它能做到"No Vector DB"的原因。
...
这解决了传统 RAG 系统的一个痛点：当知识库中某篇文档更新时，必须重新对整个语料库做 Embedding——这在文档量大的场景下成本很高。OpenKB 的增量机制使得维护成本与变更规模成正比，而非与总文档量成正比。
...
OpenKB 代表了一种新兴的知识管理思路：不再依赖向量相似度做"大海捞针"式的检索，而是让 LLM 理解文档的结构和语义，将原始资料编译成一张活的知识网络。这条路是否会成为主流还需要时间来验证，但它解决的是真实存在的痛点——传统 RAG 在长文档、多文档、跨文档场景下的局限性。

作为一个 4 月刚发布就登上 Trending 的年轻项目，OpenKB 的社区正在快速成长。想试的话，从 `pip install openkb` 开始。
