...
读完这篇文章，你应该能回答三个问题：工具调用的 Token 到底烧在哪、三层降级每一层解决什么不同的故障模式、以及你自己的场景是否适合在工具链里加这一层代理。
...
读完这篇文章，试着回答下面这几个问题来自检理解程度：你的 CLI 工具一次改 bug 的完整链路里，Token 消耗最大的环节是 prompt 还是工具输出？三层降级中，配额耗尽触发的降级和 HTTP 5xx 触发的降级，在行为上有什么不同？你当前的工作流，放 9Router 和不放 9Router，月度账单差距大概在哪个量级？如果能回答这三个问题，说明你已经抓住了 9Router 系统设计的核心逻辑。
