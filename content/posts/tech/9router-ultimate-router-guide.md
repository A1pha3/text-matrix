...
市面上已经有 OpenRouter、One API、LiteLLM 等代理方案，但 9Router 走了一条不同的路——它在代理层之上叠了三件事：RTK 工具输出压缩、三层自动降级、订阅额度最大化利用。对于重度依赖 Claude Code / Cursor / Codex 等 CLI 编程工具的开发者，这套组合直接对应账单上最痛的几个点。

下面从系统设计视角拆 9Router 的架构，重点讲 RTK 压缩和自动降级两条主线的实现原理，最后给出不同场景的采用建议。
