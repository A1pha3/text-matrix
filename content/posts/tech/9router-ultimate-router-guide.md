...
**Q: 9Router 一定要四个 CLI 全装吗？**
不用。最少只需要一个 CLI（Claude Code 或 Codex）。其他层（廉价层、免费层）会自动回退。

**Q: RTK 压缩会影响模型回答质量吗？**
只压缩 `tool_result`（工具返回结果），不压缩对话历史和系统提示词。实测对回答质量影响极小，但能减少 20-40% 的 Token 消耗。

**Q: 多账号轮询会不会被服务商封号？**
只要每个账号都遵守服务商的速率限制（RPM/TPM），多账号轮询是合法使用方式。不要在短时间内用同一个账号发送大量请求。

**Q: Dashboard 的"预算上限"和"配额追踪"有什么区别？**
配额追踪是服务商返回的真实剩余额度；预算上限是你自己设置的使用上限。两者取更严格的：即使配额还剩 50%，但预算上限已到 80%，也会触发降级。

**Q: 如果三层都耗尽了怎么办？**
9Router 会返回 529 状态码（Too Many Requests），并在 JSON 响应中说明所有层都已耗尽。建议提前设置告警，及时续费或调整预算上限。
...
1. **结构优化**：
   - 将"自测题"从"学习目标"章节内提取为独立章节
   - 添加"常见问题（FAQ）"章节（5 个问题）
   - 确认标题层级正确（学习目标 → 目录 → 一、... → 十二、）
