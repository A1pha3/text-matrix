...
Claude Code 每月 $20 的订阅（价格为文章撰写时官方定价，可能已调整）赠送大量 API（应用程序接口）调用额度。问题在于：大多数开发者根本用不完——月底额度自动清零。同时，他们可能又在另一条线上单独购买了 OpenAI 或 Anthropic 的按量付费 API。
...
同一提供商的多个账号可以在 Dashboard 里分别添加。请求进来时，Router 在所有可用账号之间做轮询（round-robin，循环分配）。这样做有两个好处：
