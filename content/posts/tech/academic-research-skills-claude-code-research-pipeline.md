...
这几道检查点背后连着一套引用审计系统——每条引用带三层定位符，开一个开关就能逐条拉取原始文献做 claim（主张）级别的核验——以及一条写死了最多两轮修订、每轮都等人拍板才放行的流水线。下面从三条防线讲起，再沿一条完整流水线走一遍，看每个阶段机器出什么、人决定什么。
...
- **跨学科团队**：Material Passport（材料护照）的数据轨迹和 Sprint Contract（冲刺合约）的盲审协议适合多作者协同工作流。
...
- **对 LLM 输出有强确定性要求的研究**：ARS 文档反复强调 LLM 输出不是 byte-reproducible 的。Material Passport（材料护照）记录了配置但不等同于可重放保证。
