...
1. 说出 ARS 流水线 10 个阶段的顺序，并指出三道硬性阻断点（Stage 2.5、Stage 4→5 Claim Audit、Stage 4.5）的位置和触发条件。
2. 复述 Lu et al. (2026) 提出的 7 类 AI 研究失败模式（M1–M7），并对应到 ARS 流水线中的具体检查机制。
3. 配置 `ARS_CLAIM_AUDIT=1` 开关，解释三层引用定位符（DOI/URL、被引段落、页码或段落号）和 5 类 HIGH-WARN 结果的含义。
4. 根据自身研究场景（论文类型、团队规模、写作流程成熟度）判断是否适合采用 ARS，并给出推荐的上手顺序。

**自测问题：**

1. Stage 2.5 做 30% 抽样而 Stage 4.5 做 100% 全量，为什么不全用全量？抽样检查的代价和收益如何权衡？
2. Integrity Gate 在 Stage 2.5 和 4.5 都会运行，但 observer agent 在这两个阶段被显式跳过——设计者为什么担心 observer 的报告会稀释门控检查的严肃性？
3. Claim Audit 默认 OFF（`ARS_CLAIM_AUDIT=1` 才开启），而不是默认 ON——这个默认值背后的取舍是什么？

## 目录

- [这套工具把每一类已知的失败模式做成了一道硬性阻断](#这套工具把每一类已知的失败模式做成了一道硬性阻断)
- [管线总览](#管线总览)
...
ARS 最值得看的地方不在智能体数量（32 个），而在它把 Lu et al. (2026, *Nature*) 在 Limitations 章节逐条列出的 7 类 AI 研究失败模式，做成了流水线上三道绕不开的检查点。引用幻觉？Stage 2.5 抽样核验。实验结果凭空出现？Stage 4.5 全量复检。机器试图用编造的数据补缺口？插 `[MATERIAL GAP]` 标记，倒逼人类填。

这几道检查点背后连着一条引用审计系统——每条引用带三层定位符，开一个开关就能逐条拉取原始文献做 claim 级别的核验——以及一条写死了最多两轮修订、每轮都等人拍板才放行的流水线。
...
三个阶段出现硬性阻断：**Stage 2.5**（写完初稿后）、**Stage 4→5 Claim Audit**（可选开关 `ARS_CLAIM_AUDIT=1`，逐条拉取引用原文核验）、**Stage 4.5**（最终出版前）。前两道是 Integrity Gate（完整性门控）——机器先跑完 7 类失败模式检查，出报告，然后**必须等人确认**才能继续。
...
ARS 设计逻辑的起点是 Lu et al. (2026, *Nature* 651:914-919) 的一项工作（注：该论文卷期页码需进一步核实，下文 Zhao et al. 引用数据同理）。该团队构建了 The AI Scientist——第一个通过顶级 ML 会议（ICLR 2025 workshop，盲审得分 6.33/10，workshop 均分 4.87）盲审的完全自主 AI 研究系统。但论文的 Limitations 章节直接把系统暴露的失败模式逐一列出，构成了 ARS 的检查清单。
...
Stage 2.5 做**抽样检查**——对 30% 的 claim（主张，最少 10 条）逐条核验。Stage 4.5 做**全量检查**——100% claim 覆盖，零容忍。任何模式在 2.5 被标记为 SUSPECTED，到 4.5 必须是 CLEAR 或用户手动 Override，否则流水线卡住。
...
另外一篇直接影响 ARS 设计的论文来自 Zhao et al. (2026-05)（注：扫描规模与幻觉引用数需进一步核实）。他们扫描了 arXiv、bioRxiv、SSRN 和 PMC 上 250 万篇论文的 1.11 亿条引用，保守估计 **2025 年一年就有 146,932 条幻觉引用**，并在 2024 年中观察到一个引用幻觉率的拐点。

更麻烦的是，这项研究还发现了「真实引用但错误归因」：引用本身指向真实存在的论文，但论文里写的主张和引用源实际说的不是一回事。Zhao et al. 把这个问题描述为 open challenge（开放挑战）。ARS 的回应是 v3.7.3 开始在每条引用上附加**三层定位符**（locator anchor，定位锚点），v3.8 补上了 opt-in 的逐条审计——下面防线二专题讲这个。
...
项目 showcase 里有一份真实案例：Stage 2.5 的 Integrity Report 抓出了 **15 条伪造引用和 3 处统计错误**。而同一个项目后来的 Post-Publication Audit Report 显示，即使过了 3 轮完整性检查，仍然有 21/68 个问题被漏掉——这恰恰说明了 Integrity Gate 的设计逻辑：它无法保证零漏检，但每一处遗漏都会被记录在案，配置参数和检查报告都对人可见。
...
1. 先装，跑 `/ars-plan` 熟悉苏格拉底引导模式——不花钱，不产生正式产出，纯熟悉交互范式。
2. 跑一次 `/ars-lit-review` 做文献综述——验证搜索质量是否符合你的领域期待。这一步的质量直接决定后续写作阶段的引用基础。
3. 开 `ARS_CLAIM_AUDIT=1` 跑一次完整流水线——确认引用审计在你的领域里能不能识别出问题。如果你的领域文献主要来自非英文期刊或预印本，审计模块的检索覆盖可能有限。
4. 根据实际体验决定是否关掉 Claim Audit（默认 OFF——需要主动开的开关不是 bug，是设计选择。全量审计每条引用都会拉取原文，token 和时间成本比标准流水线高 50-100%）。
...
第三问没有标准答案——它让你思考 Integrity Gate 的能力边界。Lu et al. 论文里提到的那 7 类失败模式，有些（M7 帧锁定）在单次流水线里几乎不可能被机器检测到。
...
这个练习帮你建立对 Claim Audit 的校准直觉——审计模块有假阳性（把合理的推论标成 unsupported），也有假阴性（漏掉真正的引用不匹配）。了解它的错误模式，你才能决定在投稿时要不要把 Claim Audit 报告作为补充材料。
...
| # | 自测项 | 答"不能"时回看 |
|---|--------|---------------|
| 1 | 能说出 ARS 流水线 10 个阶段的顺序和三个硬性阻断点的位置 | [管线总览](#管线总览) |
| 2 | 能复述 M1-M7 七类失败模式，并指出哪两类在一个 Integrity Gate 内几乎不可能被机器检测到 | [防线一](#防线一integrity-gate-与-7-类-ai-研究失败模式) |
| 3 | 能解释三层引用定位符分别解决什么问题，以及 5 类 HIGH-WARN 的触发条件 | [防线二](#防线二三层引用定位与-claim-audit) |
| 4 | 能解释为什么 Claim Audit 默认 OFF——这个默认值的取舍逻辑 | [防线二](#防线二三层引用定位与-claim-audit) |
| 5 | 能描述一次完整流水线中，人类在哪些节点需要做决策 | [一条完整流水线的运转实例](#一条完整流水线的运转实例) |
| 6 | 能估算一笔 15K 词论文跑 ARS 的 API 费用，并说出这个数字包含了什么、不包含什么 | [成本分析](#成本450k750k-tokens-换一篇-15k-词论文值不值) |
| 7 | 能在 Claim Audit 报告中区分假阳性和真问题 | [动手练习](#动手练习) |

## 进阶路径

1. **读 Lu et al. (2026) 原文**：[Nature 651 期](https://www.nature.com/)——这篇论文不仅提供了 ARS 的 7 类失败模式框架，它的 Limitations 章节本身就是一套"AI 研究系统该怎么写局限性"的示范。
...
academic-research-skills 最值得看的地方不在智能体数量或流水线长度，而在于：**它把 AI 辅助学术写作的已知失败模式写成了一道道可以观察、可以配置、可以在流水线里阻断的检查点，让每一处可能出错的位置都留下可复查的记录。**

Integrity Gate 抓到过 15 条伪造引用和 3 处统计错误。同一条流水线的 Post-Publication Audit 又发现了 21 个漏网的问题。这说明 Integrity Gate 不是摆设，也说明它不是银弹。ARS 没有承诺「用了这个工具论文就不会出问题」，而是在每个可能出问题的环节上开了一扇窗——让你能看到、能判断、能阻断。
