...
### 两篇奠基论文：Lu et al. 和 Zhao et al.

除了 Lu et al. 提供的失败模式分类，ARS 的设计还直接受 Zhao et al. (2026-05) 一项大规模审计的驱动。他们扫描了 arXiv、bioRxiv、SSRN 和 PMC 上 250 万篇论文的 1.11 亿条引用，保守估计 **2025 年一年就有 146,932 条幻觉引用**，并在 2024 年中观察到一个引用幻觉率的拐点。

更麻烦的是，这篇论文发现了一种叫「真实引用但错误归因」的现象：引用本身指向真实存在的论文，但论文里写的主张和引用源实际说的不是一回事。Zhao et al. 把这种情况描述为一个 open challenge，而 ARS v3.7.3 开始在每条引用上附加**三层定位符**（locator anchor），v3.8 补上了 opt-in 的逐条审计。
...
前面两道防线——Integrity Gate 和 Claim Audit——解决的是诚信问题。第三道防线解决的是写作质量。
...
光讲机制容易让人觉得这是一堆概念的排列。下面用一个具体研究问题走一遍，看流水线的每个阶段实际产出什么、人类在什么节点介入。假设研究问题是：
...
如果开了 `ARS_CLAIM_AUDIT=1`，逐条拉取原始文献，逐条判定 claim 是否被事实支持。这是一个 LLM-as-judge 的流程，附带金标准校准。产出 5 类 HIGH-WARN 的审计结果。
...
`scripts/check_data_access_level.py` 在 CI/CD 中强制执行这套标注，设计灵感来自 Anthropic 的 automated-w2s-researcher（2026）。理解这套机制的价值不需要太费事：它确保 Integrity Gate 做检查时看到的是「已经被人确认过的数据」，而不是原始草稿——这道隔离层如果不存在，完整性检查就失去了基准面。
...
这恰好是 Lu et al. (2026) 揭示的 7 类失败模式所对应的解法：全自动系统因为无法察觉自己在 M1-M7 上的失效而一路狂奔，ARS 通过在每个节点插入人类确认，把这 7 个盲区变成了可见的检查清单。代价是时间、注意力和 \$4–6 的 API 费用。回报是每一条引用都能追踪到原文，每一处统计错误都有被机器抓到过的可能。
