...
Claude 能读网页、跑脚本、浏览代码库，唯独开箱看不了视频。你丢给它一个 YouTube 链接，它要么猜标题，要么拉一段丢了九成画面信息的字幕。问题不在模型不聪明，而在输入通道——Claude 只认文本和图片，而视频是"连续画面 + 音频"。Claude Video 的切入点很朴素：把"看视频"这个问题翻译成 Claude 能处理的输入——帧（图片）加字幕（文本），再让模型通读，这层转换才是它真正解决的事。
...
选哪档取决于你要"快跑一遍"还是"精看一段"。四档其实是同一件事的四种资源取舍：`transcript` 最省、看得最粗，`efficient` 追求快，`balanced` 在成本与信息量之间找平衡，`token-burner` 不设上限换细节。
...
参数不多，围绕录像、帧、转录三处调。都来自命令本身的选项，不涉及运行时额外状态：

| 参数 | 作用 | 场景 |
|---|---|---|
| `--detail transcript` / `efficient` / `balanced` / `token-burner` | 选抽帧策略 | 见前文四种模式 |
| `--start` / `--end` | 圈定视频片段 | 长视频只要某段，片段内帧预算更密 |
| `--resolution 1024` | 提高抽帧宽度 | 想看细节，代价是图片 token 约翻 4 倍 |
| `--no-whisper` | 关掉 Whisper 兜底 | 无字幕但不想配 key 时，只留帧 |
...
一个通用的张力：帧是信息量，也是成本。`balanced` 兜底日常，`token-burner` 只对锁定片段用——把预算花在真正值得看的地方，是这套工具最值得记住的用法。