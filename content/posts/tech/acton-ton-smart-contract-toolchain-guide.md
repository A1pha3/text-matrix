...
export async function compileJettonMinter(): Promise<Cell> {
  ...
  console.log('func-js version:', compilerVersion());
...
验证通过后，tonscan.org 上该合约页面会显示"Verified"标记并展示源码。这是合约审计和社区信任的基础，dApp（去中心化应用）前端团队通常要求合约先验证再集成。
...
5 题都能答出，说明已掌握 TON 合约开发工具链的核心。答不出的，回到对应章节重读。
