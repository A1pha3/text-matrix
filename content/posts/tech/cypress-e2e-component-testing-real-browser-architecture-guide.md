...
> **目标读者**：在前端 / 全栈工程里负责回归测试的工程师，技术负责人，以及正在为 Playwright / Selenium / Cypress 做选型的架构师。
> **核心问题**：在 Selenium WebDriver、Playwright、TestCafe 已经各占一席的 2026 年，Cypress 还剩什么不可替代的设计？它“跑在浏览器内部”的架构到底解决了什么真问题，又带来了什么真限制？
...
**Cypress 最大的护城河不是“又一个 E2E 框架”，而是它在浏览器内部同进程跑测试代码这个根本性的架构选择——这个选择让它拿到了真实 DOM、真实网络请求、真实计时器的同步访问能力，也让它失去了跨域、跨多 tab、部分桌面浏览器的覆盖能力。** 选 Cypress，本质上是选 “在真浏览器里拿调试体验”，还是选 “WebDriver 协议跨浏览器覆盖率”，二者在 2026 年依然是有真实取舍的。
...
| 上层能力 | E2E 测试、组件测试、Cypress Studio AI、Cypress Cloud、Accessibility 测试、Code Coverage |
...
|------|-------------------|------------|---------|
| 通信协议 | W3C WebDriver（HTTP+JSON） | 自有 CDP（Chrome）/ WebDriver BiDi（Firefox/Safari） | 自有协议 + Node.js IPC |
| 测试代码执行位置 | 测试进程（用户机器或 CI） | 测试进程 | **被测浏览器内部** |
| 浏览器自动化方式 | 浏览器驱动（chromedriver / geckodriver） | 浏览器内置调试端口（CDP/WebDriver BiDi） | Cypress 后台服务代理 |
| 同步访问应用状态 | 否（需显式等待 / polling） | 有限（auto-wait，但跨 tab 异步） | **是**（同一运行时同步访问 window / document / DOM） |
| 多浏览器覆盖 | 全主流浏览器 | Chromium / Firefox / WebKit | Chromium / Firefox / WebKit / Edge / Electron |
...
| 录制与回放 | 需第三方 | Trace Viewer | Cypress Studio（录制回放 + AI 建议断言） |
| 商业化 | 无 | 无 | Cypress Cloud（并行化、Spec Prioritization、Flake Detection） |
| 协议/集成浏览器网络栈 | 走 WebDriver 抽象层 | 直接接 CDP | 自有 Node 守护进程转发 |

> 关键差异不是 “Cypress 更快”，而是 “Cypress 测试代码和被测应用跑在同一个浏览器实例的同一个事件循环里”。这让“自动等待”变成自然行为，让“拿到 DOM”变成同步操作，让“时间旅行”成为可能——但也决定了它不能像 Selenium 那样跨多浏览器远程驱动。
...
- **① 是 Electron 桌面应用**：Test Runner 的 GUI 层（Cypress 14+ 起已支持 Headless 模式与 Web 模式）。
- **② 是 Node.js 守护进程**：负责启动浏览器、转发 `cy.*` 命令、采集视频 / 截图、记录网络日志。
- **③ 是真正的“跑测试的地方”**：被测应用 + Cypress 注入脚本在**同一 JavaScript 运行时、同一事件循环**里执行。

### 4.2 同运行时的三个工程后果

1. **测试代码可以直接 `window.x` / `document.y` / `app.$.state` 同步访问**：不需要 `await page.evaluate(...)`，Cypress 把这些访问序列化在测试上下文中。
2. **自动等待 (Automatic Waiting) 是“自然行为”而不是“特殊机制”**：当 `cy.get('.btn').click()` 被调用时，Cypress 重试链 (retry-ability) 会反复 query DOM 直到元素存在、可点击、稳定（动画结束）；这背后是 Mocha 的 `done` 回调 + 内部 polling 状态机。
3. **跨域 / Same-Origin Policy 不再是测试的硬约束**：因为测试代码和被测应用在同一个 origin 的同一个进程里。

### 4.3 代价

- Cypress 不能像 Playwright 那样精细控制“多个独立的浏览器上下文”——它的“浏览器”概念更接近“一个被测实例”。
...
> `cypress` 是“自带浏览器下载”的 meta 包：首次运行时会下载与系统匹配的 Chromium（以及可选的 Firefox / Edge / WebKit）。这一步在 CI 上常常被缓存到 `~/.cache/Cypress/`。
...
Cypress 14+ 起，组件测试（Component Testing）成为推荐用法之一，**直接在真浏览器里挂载前端组件**（React / Vue / Angular / Svelte 都支持）：
...
- 同时 Cypress Studio / 时间旅行 / 网络拦截等机制都可用
...
就只能用真浏览器。Cypress Component Testing 提供的正是“真浏览器 + 一站式挂载 + 完整 E2E 调试能力”的组合，这是它和 Jest + React Testing Library / Vitest + JSDOM 的最大差异。

### 7.3 Cypress Studio AI

- 在 Test Runner 里点击 “Enable Studio”，可以录制浏览器操作生成测试代码
- 录制结束后 Cypress 用 LLM 推荐断言（如 `should('be.visible')` / `should('have.text', ...)`）
- 生成的代码可直接编辑、可入仓
- 适合“快速铺回归用例”的场景，但断言准确度需要人 review
...
**Cypress 用“测试代码跑在被测浏览器内部”这个反直觉的架构换来了自动等待、时间旅行、零跨域的丝滑体验；代价是移动真机覆盖、多 tab 多上下文、跨语言绑定的能力弱。** 在 2026 年的前端测试栈里，它依然是“JS / TS 技术栈 + 真浏览器回归 + 团队愿意为体验付费”场景下的最稳选择。
