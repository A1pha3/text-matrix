...
Codex Skills 是 Awesome Codex Skills 项目（以下简称 ACS）提出的核心理念：**Codex 能替你执行完整的任务流程**——发邮件、创建 GitHub Issue、发 Slack 消息、操作 Notion、跑自动化脚本，1000+ 应用全覆盖，远不止生成文本。
...
. 生产力与协作（Productivity & Collaboration）
...
`connect` 是 ACS 中最核心的技能之一。通过 Composio CLI，它让 Codex 能够操作真实应用——**真正替你执行**，不是"告诉你怎么做"。
...
. meeting-notes-and-actions：从会议记录到可执行任务
...
Awesome Codex Skills 是一个**让 AI 代理从"会说"到"会做"的完整生态**，远超单纯的技能列表。通过模块化的 Skill 设计，Codex 可以在保持上下文精简的同时，按需调用任意复杂度的任务执行能力。
