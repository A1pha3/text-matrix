...
lastmod: "2026-05-28T00:09:08+08:00"
...
边界先划清，后面的技巧才不会被用歪。多数人会卡在这里，是因为一开始就把目标设成了“还原原文”。
...
先把这个预期放稳：**很多不同的 prompt，本来就可能生成非常相似的视频。**
...
演示里这么写够用，真放进工作流就太松了。把流程拆成两轮，会稳很多。
...
这一轮先求拆干净，别急着写顺。先把 `subject / action / camera / audio / unknowns` 五层拆开。
...
做到这一步时，基本就能把“整条视频的感觉”拆成若干条真正可执行的镜头任务，而不是把所有信息塞回一个超长 prompt。

## 13. 最后一页执行清单

真开始做时，就照下面这个顺序跑一遍。前面的步骤顺着这条线排下来，不容易走偏。
...
把它当成“视频版 prompt diff”就够了。Gemini 先把成片里可见、可听、可追问的证据摊开；后面要做的，是把这些证据重新压回生成模型吃得下的字段和镜头指令。

这套流程能不能留下来，主要看返工时有没有抓手。先观察，再标 `unknowns`，再重组成生成语言，最后回看第二次生成结果。哪一层错，就回哪一层修。这样沉淀下来的，才是一套能复查、能复用的工作记录。
