...
draft = false
title = 'Gemini Reverse Engineer Ai Video Prompt Guide'
+++
...
截至 `2026-05`，社区教程还喜欢把某个 Preview 型号挂在标题里。做反推时，先看任务类型，再看型号。视频理解文档里更有用的信息是：**所有 Gemini 模型都支持视频剪辑和自定义 `FPS`，不过 `2.5` 系列在这类视频处理上的质量通常更稳。**

先把任务类型分清：这次是手工诊断、批量分析，还是高动作细节追问。型号选择放在后面，判断会清楚得多。

做一次手工试验，用 [Google AI Studio](https://aistudio.google.com/) 就够了。要围着同一条视频反复追问，或者要控制 `FPS`、缓存和文件复用，直接走 API + Files API。要是你此刻卡在免费额度、模型档位和 API 入口选择上，可以先看 [free-llm-api-resources：免费 LLM API 资源汇总清单](./free-llm-api-resources-guide.md)。那篇把 Google AI Studio 的免费层和常见替代渠道整理得更集中。
...
### Step 0：先决定输入通道，不要一上来就上传

入口先定。入口选错了，后面再怎么调 prompt 都是在补锅。
...
同一条长视频要问很多轮时，再叠缓存。显式缓存默认 `TTL` 是 `1` 小时，适合先把大视频和长系统提示词压进缓存；隐式缓存命中时，账单里也能看到命中的 token。
...
### Step 3.5：把社区大 prompt 压成你自己的工作模板

直接整段粘贴社区原版系统提示词，短期内能跑，长期却不利于调参。你很难知道到底是哪一句在起作用，也很难针对不同平台收缩输出。

更省事的办法，是保留它的观察框架，再把模板拆成两层：第一层写证据，第二层做生成改写。以后要换平台、缩输出、补字段，改的都是局部。
...
最后一条别省。把该留白的地方写出来，往往比多塞几个看起来专业的参数更有用。

### Step 3.8：如果你要长期复用，直接让模型吐出 JSON 中间稿
...
差别就在这里：前者已经把镜头和动作压回生成语言，后者只是把词排在一起。
...
如果你更关心正向写 prompt、拆镜头表和建立稳定出片流程，而不是先做反推，可以继续读 [Seedance 2.0 视频制作实战指南：从提示词到分镜的全流程教程](../video/seedance-2-video-production-guide.md)。那篇更适合从“怎么拍”这一侧建立手感。
...
## 8. 进阶：把复现率拉高的 8 个操作细节

前面 `1` 到 `7` 节已经够你跑完第一轮。下面这 `8` 条更像提复现率时会用到的细调项。
...
如果你想看这些字段最后怎样真正落到出片、转场和后期拼接里，而不只停在 prompt 层，可以继续读 [AI 广告制作实验：6 小时 vs 30 万美元，广告行业会被颠覆吗？](../video/ai-advertising-production-6-hours-vs-300k.md)。那篇更接近真实视频制作链路里的取舍。
...
如果你不想让这些字段继续散在聊天记录和临时文档里，可以继续读 [Prompts.chat：开源提示词平台、自托管方案与 MCP 集成完全指南](./llm/prompts-chat-open-source-prompt-library-guide.md)。那篇更适合把“好用的 prompt 片段”收束成真正可搜索、可复用的资产层。
...
这一轮先别追求行文顺不顺，先把 `subject / action / camera / audio / unknowns` 五层拆开。
...
做到这里时，你最好已经能分清到底是平台口味不同，还是你的字段提纯本身还有缺口。

### 练习 4：多镜头视频做镜头表

找一段 `20` 到 `30` 秒、至少有 `3` 个镜头变化的视频，不要直接求一条总 prompt。先把它拆成镜头表，再决定哪些镜头值得单独反推，哪些只需要做情绪和节奏参考。

练到这里时，你应该已经能把“整条视频的感觉”拆成若干条真正可执行的镜头任务，而不是把所有信息塞回一个超长 prompt。

## 13. 最后一页执行清单

真要上手，就按下面这张操作单跑一次。它只是把前面的流程压成一个不容易走偏的顺序。
...
把这件事看成“视频版 prompt diff”就行。Gemini 做的是把成片里可见、可听、可追问的证据重新摊开；后面的工作，是把这些证据压回生成模型吃得下的字段和镜头指令。

差距通常不在那条神秘的 system prompt 上，而在流程有没有拆开：先观察、再标 `unknowns`、再重组成生成语言，最后回看第二次生成结果。流程拆开后，返工才有抓手，字段也能留下来复用。
