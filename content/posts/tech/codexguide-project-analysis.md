...
| → | [这份指南解决的是什么问题](#1-这份指南解决的是什么问题) | [内容架构：四阶段路径](#2-内容架构四阶段路径) | [入口地图：先选入口再学用法](#3-入口地图先选入口再学用法) | [配置体系：四层架构](#4-配置体系四层架构) | [任务设计方法论](#5-任务设计方法论) | [实战案例库：14 个场景](#6-实战案例库14-个场景) | [自测](#自测) | [进阶路径](#进阶路径) |
...
装好 Codex 桌面 App 或 CLI，找一个**确定能跑通**的小任务（比如改一个按钮文案、补一条单元测试），按「六要素」写任务描述，亲自看 Codex 的完整输出。目标：感受「任务描述质量」对输出质量的影响。

**阶段二：把 AGENTS.md 用进真实项目（本周）**

在你的主项目里建 `AGENTS.md`，最少包含：项目一句话说明、常用命令、代码规范、禁止事项。然后给 Codex 派一个**中等复杂度**的任务（比如加一个 API 端点、改一个组件的 props），对比「有 AGENTS.md」和「没有 AGENTS.md」的输出差异。
...
如果你在带团队，把 CodexGuide 的「团队化」章节过一遍，挑 2-3 个最适合你团队当前阶段的 playbook 条目，先落地。别一次全上，选**最痛的那个问题**先解决。
