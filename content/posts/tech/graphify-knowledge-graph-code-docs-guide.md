...
source_key: "gh:Graphify-Labs/graphify"
...
Graphify 的灵感来自 Andrej Karpathy 的工作方式：他在一个 `/raw` 文件夹中存放论文、推文截图和笔记。Graphify 是对这一工作流的系统性回答——通过构建可查询的知识图谱，让 AI 能够在索引后的结构化知识中快速找到跨文件的隐含联系。
...
- **Claude Code**（主要平台）、Claude Desktop
