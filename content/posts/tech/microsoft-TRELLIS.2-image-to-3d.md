...
`microsoft/TRELLIS.2` 把"图像到 3D 生成"从传统 iso-surface 场的限制里拉出来：核心贡献是 **O-Voxel**——一种"field-free"稀疏体素结构，配合 4B 参数的 vanilla DiT 和 Sparse 3D VAE（16× 下采样），让模型在 H100 上用约 3 秒生成 512³ 全纹理资产、17 秒生成 1024³、60 秒生成 1536³，并能原生表达开放表面、非流形几何和内含封闭结构。
...
2. **图像编码**：用图像编码器（仓库未明示，但通常是 DINOv2/SigLIP 类视觉 encoder）拿到条件向量
3. **DiT 采样**：4B 参数的 vanilla DiT 在 O-Voxel 潜空间里采样，条件由图像特征注入
4. **Sparse 3D VAE 解码**：把潜空间解码回完整 O-Voxel（含 PBR 通道）
5. **O-Voxel → 纹理 mesh**：CUDA 加速的 marching cubes（因 O-Voxel 是 field-free，这里其实更接近体素到 mesh 的直接转换）
...
- 已经有 NVIDIA H100 / A100 工作站，想批量做 image-to-3D 的团队
...
- 大场景重建（建筑级、城市级）：模型按单资产训练，跨资产的拼接不是它的目标
