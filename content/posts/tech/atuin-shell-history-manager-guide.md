...
Shell 自带的历史是一行一行的纯文本，没有退出码、没有目录、没有主机信息，更没有跨机器同步。Atuin 用 SQLite 替换这套机制，把每条命令连同它的上下文一起存进数据库，再通过端到端加密在多台机器间同步。一旦存储变成结构化数据，按退出码、目录、时间筛选就不再是 grep 和管道的组合题，而是一条 SQL 的事。

> 文中提到的 29.1K Stars 为 2026 年 4 月初的统计时点数据，实际数值请以 [GitHub 仓库](https://github.com/atuinsh/atuin) 当前显示为准。

下文按"安装配置 → 日常使用 → 自建服务器 → 选型判断"的顺序展开，最后给出采用顺序和适用边界。
...
先通过一张表看清 Atuin 与 Shell 自带历史在存储格式、记录字段、跨机器同步、搜索方式等维度上的差异，帮助快速判断升级能带来什么，后续章节会逐项展开：
...
Atuin 的搜索有两条入口：交互式全屏搜索（`Ctrl+R` 呼出）和命令行搜索（`atuin search`）。前者用于实时翻找，后者适合写进脚本或精确筛选：
...
下面跟踪一条命令从敲下到在另一台机器上被搜索到的全过程，用来串起 Atuin 的本地记录、加密同步和召回三条主线：
...
`atuin stats` 基于数据库里的命令记录，统计最常用的命令和按时间段分布的使用频率，适合用来回顾自己的工作习惯：
...
全屏搜索界面里的常用按键如下，熟悉这些按键能让你在翻找历史时少切回鼠标，直接用键盘完成导航、修改和执行：
...
Atuin 选择 SQLite 而不是继续用文本文件，原因在于 Shell 历史天然带结构——退出码、目录、主机、会话、时长都是命令的属性，按这些维度筛选时，SQL 一句就能完成；文本文件要做到同样效果，得靠 `awk`、`grep` 和临时脚本的组合，且无法保证一致性。
...
官方同步服务器对个人使用足够，但企业团队或对数据主权有要求的场景更适合自建。Atuin 的服务器组件用 Rust 写成，可以单二进制运行，也支持容器部署。
...
fzf 是 Shell 的模糊查找工具，Atuin 可以与 fzf 配合使用，也可以完全替代它。两者的分工差异在于：Atuin 背后是带上下文的 SQLite 数据库，能按退出码、目录、主机筛选，并跨机器同步；fzf 是通用的模糊匹配器，输入流是什么就匹配什么，轻量但不存储上下文。如果只需要单机搜索且已习惯 fzf 的交互，两者可以并存，Atuin 接管 `Ctrl+R`，fzf 留给其他场景。
...
1. **存储动机**：根本原因是 Shell 历史天然带结构（退出码、目录、主机、会话、时长），文本文件无法把这些字段对齐存储。SQL 一句能搞定的场景示例：查找"过去 7 天在 `/srv/app` 目录下退出码非 0 的 `make` 命令"——`SELECT command FROM history WHERE cwd='/srv/app' AND exit_status<>0 AND timestamp>... AND command LIKE 'make%'`，纯文本历史需要 `grep` + `awk` + 时间过滤的组合，且无法可靠关联退出码。
...
在本文覆盖的安装、同步、自建服务器之外，还有几个方向值得继续深入：
