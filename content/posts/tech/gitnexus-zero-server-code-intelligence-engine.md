...
[GitNexus](https://github.com/abhigyanpatwari/GitNexus) 把代码仓库索引成交互式知识图谱，供 AI 编程工具查询调用关系、依赖结构和影响范围。
...
Claude Code 或 Cursor 处理中型代码库时，常见三类问题：

1. 只能看到当前对话窗口内的代码片段，无法追踪完整调用链
2. 改一个函数时不知道谁在调它，改一处可能引发连锁故障
3. 重构时不了解模块间依赖关系，容易破坏隐含的接口契约

传统 RAG 只能检索"相关文本"，回答不了"这个函数被谁调用、调用深度如何、属于哪个功能模块"这类结构性问题。

GitNexus 把代码仓库转成知识图谱——节点是函数、类、模块，边是调用关系、依赖关系、继承关系、数据流向。AI 工具通过 MCP 协议查询图谱，拿到调用链和影响范围。
...
GitNexus 把代码从文本变成结构化的图谱，AI 工具可以据此查到改动的影响范围、完整调用链和模块间依赖关系，而不是只看当前文件和局部引用。
