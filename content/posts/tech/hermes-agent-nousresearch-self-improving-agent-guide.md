...
Hermes 的做法是让 Agent 自己判断：**这段对话里有没有值得长期保留的信息？** 判断标准来自 Agent 在推理时自己做的决策，不是硬编码的规则。Agent 会跳过闲聊、一次性查询和已过时的信息，只把「对后续任务有用的内容」写进 memory。

代价是 Agent 可能漏记。但从工程角度看，**漏记比误记好**——误记会在检索时污染上下文，让 Agent 做出错误决策；漏记只是多问一次。
...
每个 skill 是一个目录，核心文件是 `SKILL.md`，使用 YAML frontmatter 声明元数据：
...
在 Telegram 上说到一半的话题，可以在 Slack 上继续——Agent 看到的是同一个 `session_id` 和同一份对话历史。每个平台都支持：
...
Hermes 的差异化来自「学习-记忆-网关-后端」四者拼在一起之后的组合效应——拆开看每个单项都有替代品，拼起来没有。只需要「在终端里写代码」的话，Claude Code 或 Aider 更轻；需要「一个会成长、能从手机里指挥、能在云上干活的 Agent」的话，Hermes 是当前唯一同时覆盖这几点且开源的选项。
...
- 每天跟 Agent 交互超过 10 轮，且任务类型重复度高 → 学习循环能直接生效
