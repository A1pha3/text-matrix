...
> | 💻 主要语言 | Python |
> | 📅 最后更新 | 2026-06-26 |
> | 🔗 在线预览 | [GitHub README](https://github.com/calesthio/OpenMontage#readme) |
...
- [快速信息卡](#快速信息卡)
- [学习目标](#学习目标)
- [核心判断](#一核心判断这不是一个视频工具而是一个无代码编排器)
- [系统地图](#二系统地图三层知识架构)
- [12 条流水线](#三12-条流水线从动画解说片到口播播客)
- [Provider 评分选择器](#四provider-评分选择器避免prompt-spaghetti)
- [质控门禁](#五质控门禁把看起来像视频挡在门外)
- [零 API Key 路径](#六零-api-key-路径完全离线的入门方式)
- [任务流案例](#七任务流案例一句-prompt-走过-openmontage)
- [采用顺序与适用边界](#八采用顺序与适用边界)
- [小结](#九小结)
- [自测题](#自测题)
- [进阶路径](#进阶路径)
...
5. **如果你想把 OpenMontage 用于生产环境，推荐的采用顺序是什么？**
   - 参考答案：1) `git clone` + `make setup`，先跑 `make demo` 跑两条零密钥样片；2) 拿一条现有内容，让 Agent 给出 2-3 个差异化方案；3) 拿到 Provider Key 后，按样例 prompt gallery 逐档升级；4) 在 `config.yaml` 里把预算模式调到 `warn` 或 `cap`，再开始跑生产流量。
...
- [OpenMontage 样例 Prompt Gallery](https://github.com/calesthio/OpenMontage/blob/main/PROMPT_GALLERY.md)
- [Remotion 官方文档](https://www.remotion.com/docs)
- [HyperFrames 项目](https://github.com/calesthio/HyperFrames)
