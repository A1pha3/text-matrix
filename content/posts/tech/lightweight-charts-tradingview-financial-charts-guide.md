...
该库特别适合以下场景：需要展示金融数据的交互式图表但又不想影响网页加载速度和性能；希望用交互式图表替代静态图片图表；需要在网页上展示大量图表但又想控制页面大小。与同等规模的静态图片相比，Lightweight Charts 能够显著减少网页的总体积。
...
Lightweight Charts 的设计理念是"轻量而不简陋"。虽然包体积接近静态图片，但它提供了丰富的图表功能，包括多种图表类型、技术指标支持、实时数据更新等。核心使用 HTML5 Canvas 进行渲染，确保在各种设备上都能获得流畅的图表体验。
