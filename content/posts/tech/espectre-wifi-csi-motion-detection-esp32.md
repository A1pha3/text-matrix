...
> **核心问题**：能不能用一个 10 欧元的 ESP32、**完全不需要摄像头和麦克风**、**完全本地推理**、**直接对接 Home Assistant** 地检测房间里有没有人在动？穿墙、零云端、零训练数据采集。
...
1. **它不是“又一个智能家居传感器”，它是“信号处理 + 边缘 AI + 智能家居”的完整栈**。从 CSI 原始 I/Q 数据采集、增益锁定、子载波选择、移动方差分段 / 神经网络判别，到 ESPHome 原生组件、Home Assistant 自动发现——一条龙，没有断层。
...
3. **它对家庭隐私是真友好**。无摄像头、无麦克风、无云端、无路由器配置。CSI 本身只反映“无线电信道物理特征”，不含语音、图像、人脸——这是它和摄像头 / 毫米波雷达方案的根本差异。
...
`espectre` 的核心是这条 5 阶段处理管道（来自 README）：
...
这意味着即使 AGC 在跑、信号强度在变，**CV 反映的信道“湍流程度”依然稳定**。
...
这种**“启动时自校准”的设计哲学**贯穿整个项目——NBVI 在做、Hampel 滤波在过滤异常、自适应阈值在算 P95——所有需要手工调的超参都被算法在启动 / 运行时自动算出来。
...
`espectre` 这个项目的真正价值不在于它有多复杂，而在于它把**“Wi-Fi CSI + 信号处理 + 边缘 AI + ESPHome + Home Assistant”**这条跨越无线通信、信号处理、嵌入式 C++、Python ML、智能家居自动化的完整链路，**用 GPL-3.0 全部开源了出来**。

在 2026 年这个“智能家居摄像头越来越多、家庭隐私越来越被侵蚀”的时间点上，**espectre 用一台 10 欧元的 ESP32 做出了“看不见、听不见、穿墙检测、完全本地”的动检方案**——这本身就是对行业的一种回答。

如果你正在做智能家居、IoT、信号处理、或者只是想给 Home Assistant 加一个**真·隐私友好**的人体传感器，**花 10 欧元买一块 ESP32-S3 跑一遍它的 10 分钟 setup**，是 2026 年最值得的开源项目体验之一。
