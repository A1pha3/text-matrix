...
- 社区推荐频道 [GDQuest](https://www.youtube.com/@gdquest)、[Heartbeast](https://www.youtube.com/@Heartbeast)（非官方，社区口碑最佳）
...
学习曲线方面，GDScript 对有 Python 经验的开发者几乎零门槛，但 C# 与 Unity 开发者迁移时需要重新理解节点树范式——Unity 的 Component 思维在 Godot 里会写出冗余代码。Unreal 的 Blueprint 与 C++ 学习曲线都更陡，但 Blueprint 在策划与美术协作上的优势是 Godot Visual Script（已在 4.0 移除）无法替代的。
...
- **C# 项目导出失败**：确认 `.NET SDK` 版本与 Godot 4.x 要求的 .NET 版本匹配（4.3+ 推荐 .NET 8），且 `csproj` 的 `TargetFramework` 配置正确。
- **`@onready` 节点为 null**：`@onready var label = $Label` 在 `_ready()` 前解析节点路径，如果路径写错或节点不存在会得到 null。用 `print(label)` 排查，或检查节点路径拼写。
- **`move_and_slide` 不生效**：`CharacterBody2D` 的 `move_and_slide()` 必须在 `_physics_process` 里调用，放在 `_process` 里会导致物理插值异常。本文示例为简化放在 `_process`，生产代码建议迁移到 `_physics_process`。
...
1. Unity 的 GameObject + Component 与 Godot 的 Node + Scene，在组合颗粒度上的实质差异是什么？写出一条 Unity Component 思维在 Godot 里会写出的冗余代码模式。
2. 一个玩家场景需要在 HUD、存档系统、音频管理器三处同步 HP 变化。用 Godot 的信号机制画出数据流，并说明为什么不需要 EventBus。
3. GDScript、C#、GDExtension 三者各适合哪类模块？如果一个项目同时有 UI 逻辑、网络协议解析、大规模粒子模拟，怎么分配语言？
4. Forward+、Mobile、Compatibility 三条管线分别测的是什么？为什么不能直接看 spec 表选管线？
5. `res://` 与 `user://` 在导出后的行为差异是什么？存档文件应该放哪个路径？
6. `set_target_position` 每帧调用为什么会造成 CPU 压力？生产环境怎么优化？
...
1. **新项目（2026 之后启动）**：规模在独立或中小团队，**Godot 4.7+ 应该是默认候选**。Unity 2024 的按安装量收费政策 + Unreal 5% 营收分成让 Godot 的 MIT 协议优势空前清晰。
...
*本文基于 Godot 4.7-stable（截至 2026-06-19，最新稳定版）。所有示例代码在 Godot 4.7.x + GDScript 2.0 上验证。*
