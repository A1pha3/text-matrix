...
4. 根据目标设备的 GPU 档位选择 Forward+、Mobile、Compatibility 渲染管线，并说明为什么不能只看 spec 表。
...
通过 GDExtension 接口，可以用 C++、Rust、Zig 等编译型语言写高性能节点，作为原生动态库加载。在计算密集型模块（物理求解、图像处理、ML 推理）上显著快于 GDScript，差距通常在一个数量级以上。GDExtension 不需要重新编译引擎本体，热重载友好，是替代旧 GDNative 的官方推荐方案。
...
Godot 的定位是覆盖独立游戏开发的主流需求，不追求与 Unreal 在 3A 画面上正面竞争，但性能数字必须配合测量条件看——下面这些数字只能帮你判断 Godot 是否进入候选，能不能跑你的项目，得在目标硬件上跑你自己的场景原型。
...
GDScript 语法贴近 Python，有 Python 经验的开发者可直接上手语法部分，节点树与信号范式需要单独学；C# 与 Unity 开发者迁移时需要重新理解节点树范式——Unity 的 Component 思维在 Godot 里会写出冗余代码。Unreal 的 Blueprint 与 C++ 学习曲线都更陡，但 Blueprint 在策划与美术协作上的优势是 Godot Visual Script（4.0 标记弃用，4.1 移除）无法替代的。
...
- 大型多人在线（Godot 网络模块偏弱，4.x 重写 .NET 集成后，网络模块仍不及 Unreal Replication）
...
4. Forward+、Mobile、Compatibility 三条管线分别测的是什么？为什么不能直接看 spec 表选管线？（参考「渲染管线」与「性能与上限」）
5. `res://` 与 `user://` 在导出后的行为差异是什么？存档文件应该放哪个路径？（参考「常见问题与排查」第 3 条）
6. `set_target_position` 每帧调用为什么会造成 CPU 压力？生产环境怎么优化？（参考「物理与导航」章节末段）
