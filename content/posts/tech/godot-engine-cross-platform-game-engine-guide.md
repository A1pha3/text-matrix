...
> 一句话定位：**MIT 协议、111K+ Stars、统一节点式架构、2D 与 3D 原生一等公民、覆盖桌面/移动/Web/主机全平台**——这是开源世界目前唯一能在同一份编辑器里完成 2D 与 3D 严肃作品、且对独立开发者零成本开放的游戏引擎。

如果你正在为下一个项目选型——从 2D Roguelike、3D 独立游戏到严肃的横版动作或像素模拟器——Godot 已经是绕不开的选项。本文从架构、脚本系统、节点范式、渲染管线、导出能力、生态与商业化六个角度，给出一份**从入门到生产决策**的完整参考。
...
| Stars | 111,573 |
| Forks | 25,503 |
| 主要语言 | C++（引擎核心，~70%）+ 自有脚本与 GDScript 运行时 |
| 协议 | MIT（极致宽松） |
...
Godot 的全部世界观都建立在 **Scene → Node → Signal** 三件套之上，这是它与 Unity（GameObject + Component）和 Unreal（Actor + Component）的根本差异。

### 场景（Scene）即数据

场景是一棵节点树，可以序列化到 `.tscn` 文本文件里。它既可以作为单个实体（比如玩家角色），也可以被嵌套复用（比如"敌人"场景被多个关卡引用）。**一个 Godot 项目本质上是若干 `.tscn` 文件 + 全局脚本 + 资源的集合**。
...
| **Compatibility** | Web (WebGL2)、老旧设备 | 纯 OpenGL 兼容、保留核心功能、不支持高级光照 |
...
4. **风险预案**：核心代码写在 GDScript 业务层而非场景里（场景不可热重载全部改动），复杂模块用 GDExtension 隔离。
