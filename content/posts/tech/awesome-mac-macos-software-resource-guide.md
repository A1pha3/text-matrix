...
[awesome-mac](https://github.com/jaywcjlove/awesome-mac) 是 jaywcjlove 维护的 macOS 软件推荐清单，截至 2026 年 6 月已积累 106K Stars。清单覆盖开发者工具、设计工具、媒体工具、效率工具等分类，但收录数量大并不意味着每款都适合你。下面按分类整理，每个分类给出取舍依据。
...
开发者工具是 awesome-mac 收录最多的分类。配置原则是：IDE 选一个主力加一个轻量编辑器；终端选一个顺手模拟器加一个 shell；版本控制必装 Git，GUI 客户端按需；数据库工具跟着技术栈走，不要装一堆用不上的。
...
挑终端模拟器时关注两件事：是否需要 GPU 加速（Alacritty、kitty），以及是否要内置 AI 补全（Warp）。
...
Git 是必装项。GUI 客户端的选择取决于协作场景：单人开发用 Lazygit 足够，团队协作考虑 Tower 或 GitKraken。
...
数据库工具跟着技术栈走：PostgreSQL 用 Postico，MySQL 用 TablePlus，MongoDB 用 Compass。TablePlus 支持多数据库，适合技术栈不固定的场景。
...
设计工具的取舍主要看协作场景：团队协作选 Figma，独立设计选 Sketch 或 Affinity 全家桶。Adobe 全家桶适合已经在用且依赖特定功能的用户，新用户建议先试 Affinity。
...
图像处理工具按使用频率和复杂度区分：日常修图用 Pixelmator Pro，开源需求用 GIMP，批量压缩用 ImageOptim。
...
颜色工具围绕工作流配置：取色用 ColorSnapper 或 Sip，配色方案管理用 Palette Master。
...
系统工具决定 macOS 的日常使用效率。配置思路是：Homebrew 是入口必装；窗口管理选一个顺手的（Rectangle 免费够用，BetterTouchTool 功能最全）；截图工具看是否需要录屏。
...
窗口管理工具的差异主要体现在交互方式上：鼠标拖拽边缘吸附用 Rectangle，触控板手势用 BetterTouchTool，平铺式用 Amethyst 或 yabai。
...
截图工具的差异在于是否集成录屏和标注：CleanShot X 功能最全，Kap 适合录屏 GIF，OBS 适合长录屏。
...
媒体工具按专业程度分层：日常用 IINA 播放、HandBrake 转码就够；专业音频用 Logic Pro，专业视频用 DaVinci Resolve 或 Final Cut Pro。
...
效率工具的配置围绕个人工作流展开：笔记工具选一个主力（Obsidian 或 Notion），不要同时维护多个；下载工具看场景，yt-dlp 几乎覆盖所有视频站点。
...
笔记工具的核心差异在于数据归属：本地优先选 Obsidian 或 Bear，云端协作选 Notion，开源大纲选 Logseq。
...
RSS 阅读器的选择取决于同步范围：本地用 NetNewsWire，跨平台用 Reeder，自建用 FreshRSS 或 Miniflux。
...
安全工具的配置取决于威胁模型：普通用户装一个密码管理器加一个防火墙就够；高安全需求加恶意软件检测和勒索防护。

### 8.1 密码管理

密码管理器的差异在于生态绑定：跨平台选 1Password 或 Bitwarden，开源优先选 Bitwarden 或 KeePassXC。
...
虚拟化方案按用途区分：Docker 开发用 Docker Desktop 或 OrbStack，跑其他系统用 UTM 或 Parallels。OrbStack 比 Docker Desktop 轻量，M 系列 Mac 推荐。
...
云存储的取舍围绕协作和隐私展开：个人同步用 iCloud Drive，跨平台协作用 Dropbox 或 Google Drive，隐私优先用 pCloud 或 Syncthing。
...
4. **按需装效率工具**：Raycast（启动器）加 Rectangle（窗口管理）加 1Password（密码）。这三个工具每天使用频率最高，投入产出比最好。
5. **按项目装专业工具**：数据库、设计、媒体工具按当前工作需要装。不要预装"以后可能用到"的工具，安装包会占用磁盘空间且需要定期更新。
...
4. **Docker Desktop 和 OrbStack 怎么选？为什么 M 系列 Mac 推荐 OrbStack？**
   答：M 系列 Mac 上 OrbStack 启动更快、内存占用更低，因为 OrbStack 用原生虚拟化框架而非 Docker Desktop 的 Linux VM 套娃方案。Intel Mac 或需要官方支持选 Docker Desktop。
...
[awesome-mac](https://github.com/jaywcjlove/awesome-mac) 收录了 106K Stars 的 macOS 软件资源，从开发者工具到日常效率工具。清单覆盖面广，但安装前先问自己是否真的需要。按需安装，不要盲目堆砌。
