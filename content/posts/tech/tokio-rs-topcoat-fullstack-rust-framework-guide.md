...
# Tokio-rs Topcoat：服务端渲染即客户端响应性——Rust 全栈框架的范式突破

> **一句话判断**：Topcoat 不解决"怎么在浏览器里跑 Rust"（那是 wasm 的活），它解决"怎么让服务端渲染的页面自己会动"。`$()` 表达式是一段**编译两次**的 Rust——服务端求值一次生成 HTML，翻译成 JS 后在浏览器再跑一次；`#[shard]` 组件则在参数变化时回到服务端重渲染。三种执行模型共用同一份类型，没有 wasm bundle，没有客户端构建步骤。代价是：表达式只支持一个很小的 Rust 子集，整个 runtime 官方明示"高度实验性，预期破坏性变更"。

## 目录

- [一、问题的本质：为什么"全栈"总是意味着"双份代码"](#一问题的本质为什么全栈总是意味着双份代码)
- [二、`$()` 表达式：一份 Rust，两端执行](#二-表达式一份-rust两端执行)
- [三、shard：跨越"什么时候该回服务端"的边界](#三shard跨越什么时候该回服务端的边界)
- [四、`view!` 宏：HTML 即 Rust](#四view-宏html-即-rust)
- [五、模块化路由：从文件系统推断 URL 树](#五模块化路由从文件系统推断-url-树)
- [六、资产与样式：Tailwind 的零配置集成](#六资产与样式tailwind-的零配置集成)
- [七、适用场景与早期阶段警告](#七适用场景与早期阶段警告)
- [八、小结](#八小结)
- [五道自测题](#五道自测题)
- [参考资料](#参考资料)
...
需要说清边界：官方文档明确，"runtime expression 必须是类型检查过的 Rust，但它会被编译两次"，并且**只支持一个很小的类型与方法词汇表**。`$(...)` 是 `expr!` 宏的语法糖，那个宏记录了支持哪些语法、捕获变量如何取用，以及一个叫 `raw!` 的逃生舱——实在表达不了时，允许手写一段 JavaScript 混进去。所以"一份 Rust，两端执行"在现阶段要打个折扣：两端都认的是一份**受约束**的 Rust。
...
更微妙的是：**signal（信号）这种状态原语也是跨端统一的**。`signal open = false` 的初始值是一个普通 Rust 表达式，在服务端渲染时**求值一次并序列化进页面**；浏览器加载后把它作为响应式状态接住。`open.get()` 和 `open.set(v)` 是同一套 API，只不过前者跑在服务端 tokio runtime，后者跑在浏览器 V8。
...
顺带一提，`#[shard]` 不是唯一的"回服务端"通道。runtime 还有一套 **procedure（过程）** 机制：注册一个服务端函数，浏览器用与调用 `$()` 表达式相同的方式调用它，拿回 JSON。区别在于，shard 返回的是"一段要替换进 DOM 的 HTML"，procedure 返回的是"一份要交给 JS 处理的数据"。前者适合"整块视图刷新"，后者适合"取完数据自己拼交互"。`discover()` 会把这两类端点一并注册进路由。
...
需要诚实标注的是：**Topcoat 仍是早期实验项目，官方明确写了"Expect breaking changes"**。crates.io 上版本号已迭代到 0.8.x，说明 API 仍在快速变动；官方文档把 runtime 描述为"高度实验性，目前相当受限"，表达式的词汇表很小，很多模式还没有顺手的写法。tokio-rs 团队把它放在 GitHub 上，以收集反馈为主，而不是生产就绪的稳定版本。
...
1. **`$()` 表达式能覆盖多复杂的 Rust 子集？** 闭包、模式匹配、trait 方法——这些都能一一对应翻译吗？目前的展示样本偏向简单表达式；`raw!` 逃生舱的存在说明边界还很近。
2. **shard 重渲染的请求合并与缓存**——高频输入（如搜索框）如何避免请求风暴？是否有 dedup/debounce 策略？
3. **状态共享边界**——signal 的初始值由服务端求值并序列化进页面，但跨 shard 之间的共享状态如何协调？目前官方文档还没给出完整答案。
...
1. `$()` 表达式"编译两次"分别在哪两个地方执行？为什么说它避免了"双份类型"问题？
2. `$()` 表达式现阶段不是完整的 Rust——它受什么约束？实在表达不了时靠什么逃生？
3. `#[shard]` 组件和普通组件的区别是什么？它和 procedure 机制（返回 JSON 的服务端函数）各自适合什么场景？
4. signal 的初始值是怎么从服务端到达浏览器的？为什么说 `open.get()` 在两端是"同一套 API"？
5. `discover()` 路由推断的"编译期完成"和 Next.js 的"文件系统路由"本质区别是什么？

答案都在正文里，这里只给定位：一是在服务端求值生成初始 HTML、翻译成 JS 后在浏览器重跑，类型只写一次、编译期检查覆盖两端；二是只支持很小的类型与方法词汇表，`raw!` 是手写 JS 的逃生舱；三是 shard 在参数变化时回服务端重渲染整块 HTML 并原地替换，procedure 返回数据交给 JS 处理；四是初始值在服务端求值一次并序列化进页面，浏览器接住后作为响应式状态；五是 `discover()` 在宏展开期就已知整棵路由树，跳转与静态检查建立在这棵树上，而非运行时扫描。

## 参考资料

- 仓库：[tokio-rs/topcoat](https://github.com/tokio-rs/topcoat) — MIT 许可，早期实验，预期破坏性变更
- crates.io：[topcoat](https://crates.io/crates/topcoat) — 版本号、依赖与 README
- API 文档：[docs.rs/topcoat](https://docs.rs/topcoat) — `runtime` 模块详述表达式、signal、shard 与 procedure
- 官方文档：[Learn Topcoat](https://crates.io/crates/topcoat#learn-topcoat) — 入门与路线图
