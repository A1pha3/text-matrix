...
1. 理解 Multica 的真正定位，以及它和直接使用编码 Agent 的差别。
2. 说清 Workspace（工作空间）、Runtime（运行时）、Daemon（守护进程）、Agent（智能体）、Issue（任务单）、Skill（技能包）之间的关系。
3. 用官方推荐路径完成云端体验或自托管启动。
4. 理解 daemon 的执行模型、观察方式和排障路径。
5. 判断自己的团队到底需不需要 Multica，而不是被“多 Agent 协作”这个概念带偏。
...
换句话说，Multica 管理的不是“模型能力”，而是“Agent 的组织能力”。
...
这意味着 Agent 并不是在“完全无上下文”的终端里裸奔，而是在一个带平台上下文、工作区上下文和可用命令说明的隔离环境里执行。这也是 Multica 更像“Managed Agents 基础设施”，而不是“CLI 外面包一层 Web UI”的原因。
...
Multica 支持工作空间级别隔离。对于个人用户，这意味着不同项目不会混在一起；对于团队用户，这意味着不同团队、不同客户或不同项目可以有独立 Agent、Issue 和技能集合。
...
这意味着 Multica 不是“点一下按钮，后面全靠运气”，而是有明确运行模型和控制参数的。
...
这里要注意一个细节：CLI 创建 Agent 时，选择的是 Runtime ID。换句话说，provider 的实际能力来自 Runtime，而不是一个独立的“模型名字输入框”。
