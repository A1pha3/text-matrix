...
### 练习 1：部署 Headunit Revived 并完成首次连接

找一台闲置 Android 平板和一台 Android 手机，按照 README 的指引完成首次 USB 连接。记录过程中遇到的所有问题。

<details>
<summary>参考思路</summary>

**步骤**：
1. 在平板上下载并安装 Headunit Revived APK
2. 在手机上开启"开发者选项"和"USB 调试"
3. 用 USB 线连接手机和平板
4. 在手机上选择"Android Auto"模式（不是"仅充电"）
5. 在平板上打开 Headunit Revived，点击"USB"按钮
6. 在手机上点击"允许"授权对话框

**可能遇到的问题**：
- 手机没有弹出授权对话框：检查 USB 调试是否开启，USB 模式是否选对
- 平板没有反应：检查 Headunit Revived 是否点击了"USB"按钮
- 连接后马上断开：检查 USB 线是否支持数据传输（有些线只能充电）

</details>

### 练习 2：配置无线连接（Helper 模式）

在练习 1 的基础上，配置无线连接。安装 Helper App，实现"上车自动连"。

<details>
<summary>参考思路</summary>

**步骤**：
1. 在手机上安装 Headunit Revived Helper App（从 Google Play 或 GitHub Releases）
2. 确保手机和平板在同一个 WiFi 网络（或平板开热点）
3. 在平板上点击"WiFi"按钮，等待手机连接
4. 在手机上打开 Helper App，它会自动搜索并连接平板
5. 下次上车时，只要手机和平板在同一个网络，Helper App 会自动连接

**排查**：
- 如果无线连接频繁掉线，检查手机的"WiFi 助理"是否干扰
- 如果 Helper App 找不到平板，检查防火墙设置，确保 UDP 广播未被阻止

</details>

### 练习 3：用 Tasker 实现"上车自动启动"

如果你熟悉 Tasker，写一个 Profile：当手机连接车载 Bluetooth 时，自动启动 Headunit Revived Helper。

<details>
<summary>参考思路</summary>

**Tasker Profile**：
- 触发条件：State → Net → Bluetooth Connected → 选择车载 Bluetooth
- 任务：
  1. Launch App → Headunit Revived Helper
  2. Wait 5 seconds
  3. Send Intent → action: `com.github.andreknieriem.headunit.REQUEST_CONNECT`

**注意**：
- 需要 Headunit Revived Helper 在后台运行
- Intent action 可能需要根据 App 版本调整，查看 GitHub Wiki 获取最新信息
...
**优化内容**：
1. 添加"动手练习"章节（3 个实践练习，含参考答案）
2. 将"自测题"改为标准格式（4 道题，含`<details>`标签参考答案）
3. 确认所有必需章节齐全：学习目标、常见问题、自测题、练习、进阶路径、进阶阅读
