...
[awesome-systematic-trading](https://github.com/paperswithbacktest/awesome-systematic-trading) 是一个按策略分类的论文导航 + 工具链清单（2026 年 4 月观测约 8.4k Stars）。它要解决的问题很具体：量化交易的论文和工具数量庞大，新手很难判断从哪里开始，老手容易漏掉某个方向的奠基工作。仓库本身不提供交易信号或回测平台，定位是知识地图——帮你找到每个策略方向的奠基论文、可用的回测库和能拿到行情的数据源。
...
**研究阶段**：读论文，理解策略的经济学直觉和统计假设。产出是策略的逻辑描述和参数范围。这个阶段主要读论文和书籍。

**回测阶段**：用历史数据验证策略，检查收益、夏普比率（Sharpe Ratio）、最大回撤（Maximum Drawdown）、换手率。产出是回测报告和参数选择。这个阶段主要用回测框架（backtrader、zipline）和历史数据（yfinance、Polygon）。

**实盘阶段**：接入实时行情和券商接口，处理滑点、延迟、资金成本。产出是实盘交易记录。这个阶段主要接券商 API（Alpaca）和实时数据源。
...
论文给出策略假设，回测框架验证假设，数据源提供原料——awesome-systematic-trading 帮你快速定位每个环节该用什么。
...
def momentum_strategy(prices: pd.DataFrame, lookback: int = 12, holding: int = 1) -> pd.DataFrame:
    ...
    return signal.applymap(lambda x: 1 if x > 0 else -1)
```
...
class StatisticalArbitrage:
    ...
    def compute_weights(self) -> pd.Series:
        ...
        cov = returns.rolling(self.lookback).cov().iloc[-1]
...
backtrader 文档完整、社区活跃，适合新手；zipline 的 API 设计接近 Quantopian 的生产环境，但 Quantopian 已于 2020 年关闭，zipline 的维护由社区接手（[zipline-reloaded](https://github.com/stefan-j/zipline-reloaded)）。

### R 量化生态

R 在学术量化社区中仍然活跃，quantstrat 是 R 生态中最成熟的回测框架。下面用 TTR + quantmod 计算双均线信号，比 quantstrat 的完整流程更轻量：
...
**通用建议**：仓库帮你定位资源，论文得自己读、代码得自己跑、回测得自己验证。它缩短的是找资源的时间，剩下的工作仍要自己完成。
