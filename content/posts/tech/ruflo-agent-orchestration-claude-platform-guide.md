...
draft: false
categories: ["技术笔记"]
tags: ["Claude", "AI Agent", "MCP", "多智能体", "RAG", "自动化"]
---
...
- 判断你的团队是否需要多 Agent 编排，以及 Ruflo 和单 Agent 工具的边界在哪。
- 独立完成 Ruflo 的安装、MCP 配置和第一个 Swarm 任务的启动。
- 说出 Ruflo 的四条核心主线（群智、记忆、路由、联邦）各解决什么问题，以及它们如何配合。
- 根据任务类型选择合适的 Swarm 拓扑，配置 Anti-Drift 防止目标漂移。
...
哪类团队先上？如果你的项目已用 Claude Code 且存在多步骤编码任务（需要设计→实现→测试→审查），Ruflo 的 Swarm 能直接覆盖这个流程。如果你只在 Code 里做单轮问答，可以先观望。离线场景或已有 Ollama 环境的团队，可以通过 `ruflo-ruvllm` 插件接入本地模型，但学习记忆回路仍需云端模型驱动。

## 10. 常见问题

**Q：Ruflo 和 Claude Code 是什么关系？我的项目已经用 Claude Code 了，还需要 Ruflo 吗？**

Ruflo 构建在 Claude Code 之上，提供多 Agent 编排层。Claude Code 处理单次对话交互，Ruflo 负责多 Agent 协调、记忆持久化和学习。

具体判断：如果你的日常是单轮问答（"这段代码是什么意思？"），Claude Code 足够。但如果你每次做功能开发都需要手动走设计→实现→测试→审查四步，每次遇到相似 bug 都要重新描述上下文——Ruflo 的 Swarm 可以自动串起这个流程，记忆回路让你第二次处理同类问题时不再从零开始。

不安装任何插件也可以通过 CLI 使用 Ruflo，但作为 Claude Code 插件集成体验最完整。

**Q：需要自己管理 LLM API 费用吗？一个中等规模的 Swarm 任务大概花多少钱？**

是的。Ruflo 本身免费（MIT 许可证），但调用 Anthropic、OpenAI 等模型的费用由用户自行承担。

以一个 4 Agent Swarm（architect + coder + tester + reviewer）开发一个小型 REST API 为例：Architect 和 Reviewer 走 Sonnet（~$3/百万 token），Coder 和 Tester 部分子任务走 Haiku（~$0.25/百万 token），加上 WASM Booster 处理简单转换——单次任务总 token 消耗通常在 50K-200K 之间，折合 $0.05-$0.50。如果你的任务没有 Swarm 协调深度，只是简单代码生成，直接走 Claude Code 单 Agent 更省钱。
...
**Q：`ruflo doctor` 报 `MCP connection refused`，怎么排查？**

这种情况通常意味着 MCP Server 没有正确启动。按顺序检查：

1. 确认 Claude Code 已安装且登录：`claude --version`
2. 重新注册 MCP Server：`claude mcp add ruflo -- npx -y @claude-flow/cli@latest`
3. 检查是否端口冲突：`lsof -i :PORT`（MCP 默认端口取决于配置）
4. 如果上述都无效，用 debug 模式启动看完整报错：`RUFLO_LOG_LEVEL=debug ruflo doctor`

**Q：最多可以同时运行多少个 Agent？**

技术上没有硬性限制，但推荐每个 Swarm 最多 8 个 Agent 以防止协调开销和目标漂移。可以启动多个 Swarm 覆盖不同的任务域。一个典型的开发场景：Swarm A 负责后端微服务（4 Agent），Swarm B 同时做前端组件（3 Agent），两个 Swarm 互不干扰。

## 11. 自测：你是否理解了 Ruflo 的核心机制

以下 4 道题覆盖了文章的四条核心主线，每题只需一两句话回答：

1. **群智 & 拓扑** — 一个团队在用 Swarm 开发微服务时，Coder Agent 擅自改动了 API 签名，但 Reviewer Agent 没发现。打开 `hierarchical` 拓扑 + Raft 共识后，这个失误还能通过吗？Queen 在哪个环节拦截它？

2. **记忆 & 学习** — 你上周用 Swarm 修了一个 JWT token 刷新的 bug，这周又遇到类似的认证问题。RuVector 的学习闭环（RETRIEVE → JUDGE → DISTILL → CONSOLIDATE → ROUTE）中，哪一步负责判断上次的成功方案这次是否适用？如果判断为不适用，后续步骤还会执行吗？

3. **智能路由** — CLI 中输出了 `[AGENT_BOOSTER_AVAILABLE]`，但你发现当前任务涉及跨文件的类型推导，WASM 正则匹配可能不够。路由器是否会强行使用 WASM Booster？为什么？

4. **联邦安全** — 团队 A 和团队 B 通过 Federation 共享威胁情报。团队 B 的某台机器被攻破，开始向联邦发送虚假分析结果。双方都配置了 Byzantine 容错（f < n/3），共 7 个联邦节点。攻击者最多控制几个节点才能阻止联邦达成正确共识？

（答案参见本文第 2.2、2.3、4.4、4.6 节。）
