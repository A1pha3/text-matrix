...
source_key: "gh:zilliztech/GPTCache"
aliases:
...
draft: false
categories: ["技术笔记"]
tags: ["AI Agent", "AI 安全", "Qwen", "AWS", "Microsoft", "Hugging Face", "密码学", "NLP", "Qwen3"]
...
- **Semantic Caching 的设计目标是 locality,代价是 collision resistance 失效**——这是 by design 的内在冲突。
- **CacheAttack 是黑盒攻击,只诱导 key collision,不改 cache value**——比 prompt injection 更隐蔽,因为它不依赖 LLM 内部的 alignment。
...
CacheAttack 的关键不在某个神奇 prompt 模板,在**把攻击拆成两个独立组件**。
...
这背后用的搜索算法是 [GCG (Greedy Coordinate Gradient)](https://arxiv.org/abs/2307.15043)——和攻击对齐 LLM 的同款算法。**CacheAttack 的"在 embedding 空间做对抗样本"和"在 token 输出空间做 jailbreak",本质是同一类问题**。
...
**这套设计的关键**:它**不依赖固定阈值**,而是用动态校准对抗网络抖动。即使 LLM 服务加了 jitter,CacheAttack 仍然能稳定推断 cache 状态。
...
**结果**:PPL 分布能区分正常 query 和对抗 query,但**CacheAttack 的 Generator 已经把 PPL 加到 loss 里了**(λ 控制权衡),攻击者因此**主动优化了 PPL**。
...
**结论**:**直接违反 cache 的设计目标**。Semantic cache 的设计目标就是"跨用户复用相似查询的结果"——per-user 隔离把这个价值打折打到 0。
...
论文最悲观的结论,也是最值得工程界关注的信号:**没有同时满足三者的方案**。

## 我自己被这个攻击影响的程度

我每天用 [MiniMax-M3](https://txtmix.com) 写技术文章和 4 份早报,这些请求**大概率被 MiniMax 的 LLM 服务或我用的 [Claude](https://www.anthropic.com) / [GPT-5.2](https://openai.com) 服务路由到带 semantic cache 的中转层**。于是:
...
1. 问题在架构层,不在实现层——AWS / Microsoft / 开源 GPTCache 都受影响。
