...
> 一句话核心判断：**TencentDB Agent Memory 把"长期记忆"切成 L0 → L1 → L2 → L3 四层语义金字塔，把"短期记忆"用 Mermaid 符号图（symbolic canvas）+ 全量日志外置（offload）做压缩。整套设计的精髓不是"记得更多"，而是"记得可追溯"——任何抽象都能下钻回原始证据，从根本上避免传统记忆系统在压缩率与可解释性之间二选一的死结**。
...
它的发布形式比较特殊：不是"一个独立后端服务"，而是"一个 Agent 框架侧的内存插件 + 一个本地 Gateway"。Gateway 默认监听 `:8420`，负责把 L1/L2/L3 的提取和检索请求转给 LLM，并把记忆写入 SQLite。换句话说，这个仓库的本质是 **"Agent 记忆系统的参考实现 + 部署说明书"**，不是简单的 SDK。
...
TencentDB Agent Memory 的解法不是堆一个新模型，而是把记忆**结构化**：
...
记忆的"层"不是按时间分，而是按**抽象度**分。每一层只关心上一层的信息，下层只在需要细节时才被召回。
...
- **压缩可逆**。短期压缩不是总结成新文本，而是符号化 + 外置；`node_id` 是双向指针，逻辑上完全可还原。
