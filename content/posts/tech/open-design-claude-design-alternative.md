...
Open Design（简称 OD）是一个本地优先（Local-First）的开源设计系统，旨在成为 Anthropic Claude Design 的开源替代方案。该项目于 2026 年 4 月发布（与 Claude Design 同期），目前已获得 189 Stars 和 22 Forks，采用 Apache-2.0 开源许可证。
...
Claude Design 展示了当大语言模型停止生成文本、转而输出设计产物时会发生什么——它迅速走红，但很快暴露出一系列限制：闭源、付费、仅限云端、锁定在 Anthropic 模型和 Skills 体系内。没有检出场、没有自托管、无法部署到 Vercel、也无法替换为自己偏好的代码助手。

Open Design 试图解决的就是这个根本矛盾：如何让 AI 设计工具既保持 Claude Design 的核心理念（Artifact-First 心理模型、实时 Todo 计划流、沙箱 iframe 预览、五维自检），又不引入这些锁定限制？
...
项目采用 VoltAgent/awesome-design-md 定义的 9 段式 DESIGN.md 规范（颜色、字体、排版、间距、布局、组件、动效、品牌、反模式）。每个 Artifact 输出时会读取当前激活的设计系统文件。切换系统后，下一次渲染就会使用新的设计令牌。这使得设计系统天然具备可移植性。

**4. 交互式问答表单将 80% 的返工拦截在发生之前**

每一轮全新设计简报（Brief）都以一个 `<question-form id="discovery">` 开始，而非直接生成代码。这个表单强制在生成设计之前先锁定：界面类型、目标受众、视觉基调、品牌上下文、规模和约束条件。30 秒的单选题比 30 分钟的来回返工要高效得多。

**5. 守护进程让 Agent 感觉像在本地工作，实际上它就是**

守护进程启动代码助手时，会将工作目录（cwd）设置为项目的 artifact 文件夹（.od/projects/<id>/）。这意味着 Agent 拥有真正的 Read、Write、Bash、WebFetch 能力，可以读取 Skill 的 assets/template.html、grep 项目中的 CSS 变量、写入品牌规范文件，或者生成 PPTX/ZIP/PDF 文件——这些文件会在回合结束时出现在文件工作区作为下载标签。
...
**src/artifacts/parser.ts** — 负责从 Agent 输出中提取 `<artifact>` 标签。这是 OD 的核心机制：Agent 生成一个包含 HTML/CSS/JS 的 `<artifact>` 标签，parser 提取后渲染到沙箱 iframe 中供实时预览。
...
Open Design 是一个雄心勃勃的开源项目，它试图证明：即使不捆绑模型和不依赖云服务，AI 设计工具也能提供接近 Claude Design 的体验。其"本地优先 + 代码助手作为引擎 + Skill 驱动"的设计哲学，值得所有关注 AI 工程化落地的团队认真研究。
