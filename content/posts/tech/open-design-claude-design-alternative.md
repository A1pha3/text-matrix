...
守护进程启动代码助手时，会将工作目录（cwd）设置为项目的 artifact 文件夹（`.od/projects/<id>/`）。这意味着 Agent 拥有真正的 Read、Write、Bash、WebFetch 能力，可以读取 Skill 的 assets/template.html、grep 项目中的 CSS 变量、写入品牌规范文件，或者生成 PPTX/ZIP/PDF 文件。这些文件会在回合结束时出现在文件工作区，作为可下载的产物。
