...
GitHub 仓库 [chrislgarry/Apollo-11](https://github.com/chrislgarry/Apollo-11) 保存了 1969 年人类登月时阿波罗制导计算机（AGC）的源码，截至 2026 年 6 月累计 68,147 Stars。这份代码以 AGC 汇编写成，包含 Comanche055（指令舱）和 Luminary099（月球舱）两套程序，共约 110,000 行。
...
- [Margaret Hamilton 与登月软件工程](#margaret-hamilton-与登月软件工程) — 人物与 1202 报警
...
理解 AGC 的关键在于三件事：它的硬件约束如何塑造了指令集和内存模型；Comanche055 与 Luminary099 两套程序如何分工；以及 1202 报警那几分钟里，软件到底做了什么让登月没有中止。
...
1. 克隆仓库：`git clone https://github.com/chrislgarry/Apollo-11.git`
2. 浏览目录结构：`ls -la Apollo-11/`
3. 查看 Comanche055 和 Luminary099 目录，对比两个目录的文件数量和主要模块名称

### 练习二：阅读关键模块源码

选一个关键模块（自检、报警、姿态控制三选一），阅读源码后写一段总结：这个模块做了什么？如何适应 AGC 的硬件约束？

### 练习三：运行 Virtual AGC 模拟器

1. 按 [Virtual AGC 安装说明](https://www.ibiblio.org/apollo/install.html) 安装模拟器
2. 加载 Comanche055 或 Luminary099，观察 AGC 的启动过程和自检流程
3. 记录安装耗时、启动过程的关键输出
...
1. **AGC 的 4KB RAM 和 85K IPS 意味着什么？如何用现代设备类比？**
   - 4KB RAM 约为 iPhone 15 的 150 万分之一；85K IPS 约为现代 CPU 的数十亿分之一。每条指令都要算成本，任何冗余都是奢侈。

2. **为什么 AGC 需要 banking 机制？它是如何工作的？**
   - ROM 有 36,864 字（约 72KB），但 AGC 的地址空间有限，无法一次性寻址所有 ROM。Banking 把 ROM 分成多个 bank，通过切换 bank 来选择要访问的代码段。

3. **1202 报警时，AGC 的软件做了什么让登月没有中止？**
   - AGC 的调度器检测到过载（所有 core set 都被占用），自动丢弃低优先级任务，保留高优先级任务（如姿态控制、导航）继续执行。宇航员看到报警但系统继续工作，最终成功登月。

4. **如果你要设计一个现代嵌入式系统，会从 AGC 借鉴哪些设计思想？**
   - 优先级调度 + 过载降级、故障检测与恢复、自检与报警机制、资源约束下的算法优化。这些思想至今适用于 IoT 设备、卫星控制等场景。

5. **为什么 AGC 的源码对今天的程序员还有价值？**
   - 它不是怀旧，而是"资源约束下的实时系统"教科书案例。在 4KB RAM 和 85K IPS 的约束下实现优先级调度、故障恢复和实时中断，这些取舍今天依然适用于嵌入式系统。
...
### 阶段一：建立概念（1-2 周）
- 阅读 `ASSEMBLY_AND_OPERATION_INFORMATION.agc` 和 `AGC_BLOCK_TWO_SELF-CHECK.agc`，理解指令集和启动流程
- 能解释 AGC 的 banking 机制、中断处理和优先级调度

### 阶段二：深入模块（2-4 周）
- 阅读 `ALARM_AND_ABORT.agc`（异常模型）、`PINBALL_GAME_BUTTONS_AND_LIGHTS.agc`（DSKY 交互）、`P63-P67` 程序（登月下降）
- 能解释 1202 报警的完整含义，并跟踪一次登月下降如何流过 AGC

### 阶段三：运行与实验（1 个月）
- 安装 Virtual AGC 模拟器，加载 Comanche055 或 Luminary099，模拟一次登月任务
- 能在模拟器上触发 1202 报警，并观察调度器如何降级

### 阶段四：现代对比与启发（长期）
- 阅读 Hamilton 的论文和 NASA 历史报告，对比 AGC 与现代 RTOS（如 FreeRTOS、VxWorks）的设计思想
- 能写出一篇技术分析文章，对比 AGC 与现代嵌入式系统的可靠性设计
