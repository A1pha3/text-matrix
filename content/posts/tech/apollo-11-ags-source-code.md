...
- [Margaret Hamilton：登月代码女王](#margaret-hamilton 登月代码女王) — 人物与 1202 报警
...
3. 查看 Comanche055 和 Luminary099 目录
4. 记录：两个目录的文件数量、主要模块名称

### 练习二：阅读关键模块源码

1. 选择一个关键模块（如自检、报警、姿态控制）
2. 阅读源码，理解其实现逻辑
3. 写一段总结：这个模块做了什么？如何适应 AGC 的硬件约束？
4. 记录：阅读耗时、理解难点、收获

### 练习三：运行 Virtual AGC 模拟器

1. 安装 Virtual AGC：按照 `https://www.ibiblio.org/apollo/install.html` 的说明
2. 运行模拟器，加载 Comanche055 或 Luminary099
3. 观察 AGC 的启动过程、自检流程
4. 记录：安装耗时、启动过程、关键输出
...
本文基于 Apollo-11 仓库（[chrislgarry/Apollo-11](https://github.com/chrislgarry/Apollo-11)）公开源码整理，需要说明的边界：
