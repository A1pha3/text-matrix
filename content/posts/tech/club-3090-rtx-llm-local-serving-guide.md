...
`noonghunna/club-3090` 是一个刚创建的社区项目（2026-04-28），聚焦于在 RTX 3090（24GB 显存）上本地部署 LLM 的完整配置方案。它不是一个模型，不是一个框架，而是把市面上最优秀的推理引擎和量化方法整合成一套可复现配置的技术手册。项目支持 vLLM 和 llama.cpp 两条技术路线，当前主推 Qwen3.6-27B（INT4 AutoRound 量化版），实测性能达到 127 TPS code（vLLM dual 模式）。

这篇文章从硬件选型、引擎对比、架构设计、核心补丁解析到完整安装流程，系统性地介绍如何在 RTX 3090 上把 LLM 跑出生产级别的体验。
...
dual 模式指的是在双卡上各跑一个 vLLM 实例，通过负载均衡实现更高的总吞吐。这不是张量并行，而是两个独立实例的并行处理。
...
本地 LLM 部署面临的真正难题不是"能不能跑起来"，而是"环境能不能复现"。PyTorch 版本、CUDA 版本、transformers 版本、vLLM/llama.cpp 的 git commit——任何一处不匹配都可能导致推理结果不一样或直接跑不起来。把所有依赖打包进 Docker 容器，解决了环境一致性问题。
...
vLLM 和 llama.cpp 都支持 OpenAI API 格式，这意味着你现有的 LLM 应用几乎不需要修改代码就能切换推理引擎。
...
这不是一个 hack，而是一个认真的工程补丁——它在保持量化精度损失最小化的前提下，解决了跨架构部署的兼容性问题。
