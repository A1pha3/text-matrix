...
github_repo: "awslabs/aidlc-workflows"
source_key: "gh:awslabs/aidlc-workflows"
aliases:
...
draft: false
categories: ["技术笔记"]
tags: ["AWS", "工作流", "Claude Code", "Cursor"]
---
...
规则是纯文本文件，加载后不会报错，需要主动验证。最直接的办法是新建一个不带任何配置残留的会话，丢一个改动边界很小的小任务（比如"给 `User` 模型加一个字段"），看 AI 是否先产出分析报告再动手。如果 AI 上来就改代码，说明规则没被读进上下文，多半是文件路径没对。
