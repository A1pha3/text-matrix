...
说到底它是 IDE 里的一组"方向盘规则"——不改变 AI 的能力，只约束 AI 做事的顺序。目前支持的平台包括 Kiro、Amazon Q Developer、Cursor、Cline、Claude Code、GitHub Copilot 和 OpenAI Codex。
...
Phase ：理解与分析（Understand & Analyze）
...
AI-DLC 不替代任何 AI 编码工具，它工作在工具之上——约束工具的行为，不提供编码能力。
...
AI-DLC 的价值取决于你的日常开发模式与三阶段流程的重合度。按团队规模来看：
...
想快速体验的话，从 Kiro 或 Cursor 入手最简单——这两个平台对 Steering Files 的支持最原生。Claude Code 用户需要通过环境变量或配置文件加载，GitHub Copilot 和 OpenAI Codex 需要额外配置，起步成本稍高。

常见问题

**Q：我的 AI 工具已经能自动读项目文件了，Phase 1 还有必要吗？**

即使 AI 能自动读取项目文件，它也未必会有意识地做两件事——「标记可能受影响的模块」和「评估复杂度」。Phase 1 的价值不在「读文件」，而在强迫 AI 在动手前给出书面分析，让你有机会在编码开始前纠正方向。实际使用中，不少返工恰恰是因为 AI 自动读了文件但理解错了上下文。

**Q：三阶段流程会不会让简单任务变得很慢？**

会的。改一行日志也走完整三阶段确实不划算。AI-DLC 的设计逻辑不是「所有任务都要走全流程」——你可以按任务复杂度手动跳过某个阶段。更现实的团队策略是：重大功能走全流程，Bug 修复只走 Phase 1 + Phase 3，单行修改直接跳过。核心规则本身很轻量，加载但不触发条件规则时上下文开销很小。

**Q：我用 Cursor，队友用 Claude Code，AI-DLC 能统一吗？**

能。AI-DLC 是一组纯文本规则文件，不依赖任何特定工具的能力——它通过规则文本告诉 AI「按这个顺序做事」。Cursor 通过 `.kiro/` 目录加载，Claude Code 通过环境变量加载，读到的是同一套规则，产出的分析报告格式也一致。这也解释了为什么 AI-DLC 特别适合多人多工具协作的项目。
