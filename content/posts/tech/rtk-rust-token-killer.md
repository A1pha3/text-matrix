...
一个 Claude Code 会话（30分钟）里，开发者的 shell 命令会消耗约 118,000 tokens。用了 RTK，同样的操作只消耗约 23,900 tokens，节省 **80%**。
...
## 一、核心问题：LLM 在命令行场景里 Token 浪费在哪里？
...
## 三、Token 节省实测（30分钟 Claude Code 会话）
...
| **GitHub Copilot CLI** | `rtk init -g --copilot` | deny-with-suggestion（CLI限制）|
