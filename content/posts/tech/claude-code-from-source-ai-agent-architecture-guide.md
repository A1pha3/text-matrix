...
**AsyncGenerator** 是整个系统的心脏。它是一个生成器函数，不是传统的请求-响应循环——模型每产出一个 token，就 `yield` 一条 Message。这使得系统天然支持背压（下游消费慢了就自动暂停生成）和取消（外部可以随时终止生成器）。
...
这个设计的微妙之处在于：调用方拿到的是一个迭代器，可以逐条消费消息，也可以在任意时刻 `break` 终止循环。对于需要与用户界面交互的场景，UI 层可以随时中断 Agent 的执行，不会留下僵尸进程。
