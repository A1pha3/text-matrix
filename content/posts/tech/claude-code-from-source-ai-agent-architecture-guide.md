...
这本书的诞生逻辑很有意思。Claude Code 发布到 npm 时，源码地图（source maps）一起打包了进去。Balderas 读了每一个 `.js.map` 文件中的 `sourcesContent` 字段，拿到了完整的原始 TypeScript 源码——接近 2000 个文件。然后他用 36 个 Agent 协作，把这堆原始代码变成了一本 494KB 的叙事化技术书籍。
...
Agent 分工也值得一说。6 个并行 Agent 负责通读整个源码树，随后 12 个 Agent 产出约 494KB 的原始技术分析文档。写作阶段出动了 15 个 Agent，从零开始重写为叙事化章节，最后 3 个 Reviewer Agent 产出了 900 行反馈，由另外 3 个 Agent 逐一应用修复。关键的合规约束是：所有代码块都改写为伪代码，使用不同的变量名，仅用于说明架构模式，杜绝直接复制源码。

这篇文章不打算复述原书全部 18 章内容，而是提取其中最具迁移价值的架构设计——那些你在构建自己的 Agent 系统时可以直接借鉴的模式。
...
**AsyncGenerator** 是整个系统的心脏。它不是传统的请求-响应循环，而是一个生成器函数——模型每产出一个 token，就 `yield` 一条 Message。这使得系统天然支持背压（下游消费慢了就自动暂停生成）和取消（外部可以随时终止生成器）。

**Message** 是系统中流通的唯一数据类型。文本 token、工具调用结果、错误信息、系统事件，全部封装为统一的消息结构。这种归一化设计消除了类型转换的开销，也让 Hook 系统可以在任意节点插入处理逻辑。
...
**PermissionSystem** 按 Read / Write / Dangerous 三级分类所有工具操作。默认拒绝，明确允许。更关键的是，它在启动时冻结所有配置，运行时无法修改，杜绝了 prompt injection 通过配置文件提权的可能性。
...
这个设计的微妙之处在于：调用方拿到的是一个迭代器，可以逐条消费消息，也可以在任意时刻 `break` 终止循环。对于需要与用户界面交互的场景，这意味着 UI 层可以随时中断 Agent 的执行，而不会留下僵尸进程。

### 四层上下文压缩：渐进式遗忘

长对话是所有 Agent 系统的共同难题。Claude Code 的解法是渐进式压缩——不是一次性压缩，而是根据 Token 使用率触发不同强度的压缩策略。
...
为什么是四层而不是一个统一压缩函数？因为在 50% 用量时做深度摘要是不划算的——既浪费计算资源，又增加了等待延迟。Snip 几乎是零成本的字符串操作，Microcompact 也不过是合并相邻消息。只有当用量逼近极限时，才值得调用一次 LLM 做 Collapse 或 Autocompact。

这个渐进式策略带来的结果是：绝大多数短对话完全不会触发压缩，中等长度的对话只需 O(n) 的字符串合并，只有真正的超长对话才激活 LLM 级别的摘要压缩。
...
为什么只预启动只读工具？因为只读操作不产生副作用——读文件、搜索代码、查看 git log，这些操作即使预判错了也只是浪费一点计算，不会破坏任何状态。写操作则必须等待模型完整确认，因为错误的写入可能造成不可逆的破坏。
...
主 Agent 和 Fork Agent 的前三部分是逐字节相同的。在 Anthropic 的 Prompt Caching 机制下，缓存命中部分完全免费——这意味着 Fork Agent 只需为其独有的 `forkedTask` 部分付费。如果系统提示词和项目上下文占了总 prompt 的 90% 以上，Fork 的成本就只有主 Agent 的 5% 到 10%。
...
从 18 章的源码分析中，可以提炼出 10 个不依赖 Claude 或 Anthropic 的具体架构模式：

**AsyncGenerator 驱动**。核心循环是一个生成器函数而非死循环。天然支持背压和外部取消。适用于所有需要流式输出和可中断性的 Agent 系统。

**推测执行**。在模型流式输出时预启动只读工具。关键约束是只允许无副作用的操作参与推测。适用于任何需要降低首字节延迟的工具调用场景。

**并发安全分组**。按 Read / Write / Dangerous 分类工具，读并行写串行。这个分组逻辑与具体的 LLM 或框架无关，可以直接应用于任何多工具系统。

**Fork 缓存共享**。父子 Agent 共享字节相同的前缀以利用 Prompt Cache。如果你的 LLM 提供商支持前缀缓存（OpenAI 的 Prompt Caching、Google 的 Context Caching），同样适用。

**四层上下文压缩**。Snip → Microcompact → Collapse → Autocompact，从零成本到高成本的渐进式压缩。不需要四层，两层也能显著降低压缩开销。

**LLM 召回内存**。用 LLM side-query 选择相关记忆，而非向量搜索。如果你的系统不需要处理海量记忆（万条以上），这个方案比嵌入搜索更简单且效果更好。

**两阶段技能加载**。启动时只加载元数据，触发时才加载完整内容。适用于任何需要动态加载模块的插件系统。

**粘性门闩**。Beta 头一旦发送就永不撤销。如果你的缓存系统按字节匹配，这个模式可以最大化缓存命中率。

**槽位预留**。默认分配小输出槽，仅在溢出时扩容。适用于任何按输出 token 计费的场景。

**钩子配置快照**。启动时冻结所有可配置项，运行时任何修改尝试都被拒绝。这是防御 prompt injection 通过配置注入的基本策略。
...
8 种传输协议被抽象为统一的接口，上层的工具包装逻辑不感知底层传输的差异。这意味着同一个 MCP 工具可以在本地 stdio 和远程 WebSocket 之间无缝切换，而工具的使用者无需修改任何代码。

## 常见问题

**问：源码地图到底是什么？为什么通过 npm 就能拿到 Claude Code 的源码？**

源码地图（source map）是 JavaScript/TypeScript 生态中的标准机制，用于将压缩、转译后的代码映射回原始源码。`.js.map` 文件中的 `sourcesContent` 字段直接包含了原始源码的完整内容。Claude Code 发布到 npm 时包含了这些 map 文件，因此任何人都可以通过 `npm install` 后读取它们。这不是安全漏洞，而是 npm 生态中源映射的常规做法——大多数 TypeScript 项目发布时都会附带 source map。
...
Function Calling 是 OpenAI 专有的 API 约定，工具定义和调用结果都在 HTTP 请求体中传递，绑定在特定的 API 格式上。MCP 是一个独立于模型提供商的开放协议，定义了工具发现、调用和结果返回的标准化流程，支持多种传输层。简单来说，Function Calling 是 API 层面的约定，MCP 是架构层面的协议——它允许你在完全不修改 Agent 代码的情况下切换工具的实现位置（从本地进程到远程服务器再到 Docker 容器）。
...
提示：Prompt Cache 按字节匹配。任何一个字节的变化都会导致整个前缀缓存失效，需要重新缓存。这意味着 system prompt 的稳定性直接决定了缓存策略的有效性。
...
提示：OpenAI 在 2024 年也推出了 Prompt Caching，但行为和计费方式与 Anthropic 不同。OpenAI 的缓存是自动的（无法手动控制缓存哪些部分），且缓存的过期时间更短。这意味着 Fork Agent 的设计需要在 prompt 结构上做出调整——比如将可缓存前缀固定为前 N 个 token，并确保它在缓存过期时间内被复用。