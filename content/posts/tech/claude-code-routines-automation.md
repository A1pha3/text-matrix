...
这篇文章不逐项罗列配置字段。下面先拆三种触发器的边界和适用场景，用一个告警分级的完整案例把执行链路串起来，最后给出不同团队的采用顺序。
...
- Routine 和本地 Claude Code 的本质区别在哪里？什么时候该用 Routine，什么时候不该用？
- 三种触发器各自适合什么场景？边界在哪里？
- 一个 Routine 从触发到产出结果，中间经历了哪些环节？
- Routine 以谁的身份操作 GitHub、Slack、Linear？权限边界画在哪里？
- 为自己的团队设计第一个 Routine，从哪个场景切入最划算？
