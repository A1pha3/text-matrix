...
github_repo: "datawhalechina/easy-langent"
source_key: "gh:datawhalechina/easy-langent"
...
1. [学习目标](#学习目标)
2. [项目概述](#项目概述)
3. [第一章：LangChain 与 LangGraph 框架认知](#第一章langchain-与-langgraph-框架认知)
   - 1.1 为什么需要这两个框架
   - 1.2 LangChain：快速搭建简单到中等复杂度的应用
   - 1.3 LangGraph：复杂应用的结构化设计框架
   - 1.4 两个框架的关系
4. [第二章：LangChain 核心组件实操](#第二章langchain-核心组件实操)
   - 2.1 模型调用（ChatModel）
   - 2.2 提示词模板（PromptTemplate）
   - 2.3 输出解析器（OutputParser）
   - 2.4 记忆组件（Memory）
   - 2.5 工具调用（Tools）
5. [第三章：LangGraph 核心概念详解](#第三章langgraph-核心概念详解)
   - 3.1 状态（State）：工作流的共享数据载体
   - 3.2 节点（Nodes）：工作流的执行单元
   - 3.3 边（Edges）：工作流的执行路径
   - 3.4 完整工作流示例
6. [第四章：实战项目架构分析](#第四章实战项目架构分析)
   - 4.1 狼人杀 AI 游戏
   - 4.2 Agentic RAG 系统
   - 4.3 Data Agent 数据分析助手
7. [第五章：设计原则与实践建议](#第五章设计原则与实践建议)
8. [第六章：进阶学习路径](#第六章进阶学习路径)
9. [采用顺序与决策建议](#采用顺序与决策建议)
...
```mermaid
graph LR
    A["第一章：框架基础认知"] --> B["第二章：LangChain核心组件实操"]
    B --> C["第三章：LangGraph核心概念详解"]
    C --> D["第四章：实战项目架构分析"]
    D --> E["第五章：设计原则与实践建议"]
    E --> F["第六章：进阶学习路径"]
...
**A**: LangGraph 的状态是**显式的、中心化的、不可变更新**的。每个节点只返回要更新的字段，框架自动合并到全局状态。这跟 React 的 state 更新模式类似，便于追溯和调试。对比：LangChain 的 Chain 没有统一状态，数据通过 `|` 管道传递，复杂流程里状态容易散落。
