...
传统的 RAG（检索增强生成）只能检索知识库文档，而 Supermemory 将 RAG 和个性化记忆融合在同一个查询中。这意味着搜索结果既包含知识库中的通用信息，也包含与该用户相关的个性化上下文。
...
Supermemory 提供了一个在 AI 应用中快速集成记忆能力的完整方案。对于 Agent 开发者而言，它省去了从零构建记忆系统的工程成本；对于终端用户而言，它让 AI 真正开始"认识"使用它的人。随着 AI 工具在日常工作流中的深入，跨会话记忆将成为决定 AI 助手实用价值的关键能力，Supermemory 正是这个方向的先行者之一。
