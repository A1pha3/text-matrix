...
**DeepScientist 的思路**：把这些碎片化的、重复的、容易丢失的研究工作，转化为一个本地 AI 工作空间——能持续运转、持续积累、持续变强。

---

## 二、DeepScientist 是什么？

DeepScientist 是一个长期运行的 AI 研究伙伴，区别于一次性对话的摘要工具：它把任务、文件、分支、工件、记忆全部持久化，每次运行结束后保留成功和失败的路径供下一轮使用。
...
1. **从论文或研究问题启动真实项目**：输入主要论文、GitHub 仓库或自然语言研究目标 → 转化为可执行的 Quest
...
DeepScientist 认为，一个真正适合科研的系统至少要做到：
...
系统随着使用时间变长，积累的 Quest、分支、失败路径和经验会越来越多，后续任务的启动成本越来越低。
...
- 对于未发表的 ideas、敏感的实验历史、更长期的研究循环尤其有价值
...
**Q：数据安全吗？**
A：DeepScientist 默认本地运行，代码、实验、草稿都保存在你自己的机器上。敏感的研究 ideas 不需要发送到云端。
...
DeepScientist 把 AI 辅助科研做成了一个本地运行、长期积累的工作空间——Quest 和分支随时间增长，失败路径和成功路径都保留，下一轮研究的启动成本持续降低。
...
论文复现、实验管理、结果整理这些重复性工作占用了大量研究时间，DeepScientist 针对的正是这一块。
