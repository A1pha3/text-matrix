...
1. 理解 CubeSandbox 与传统 Docker 容器、传统 VM 的本质差异
2. 描述整体架构中各组件的职责和分工
3. 解释 eBPF 驱动的 CubeVS 网络虚拟化层如何实现 SNAT/DNAT、会话跟踪和网络策略
4. 理解模板（Template）生命周期：Init → Boot & Snapshot → Deploy
5. 在 WSL 2 或裸金属上安装 CubeSandbox 并跑通第一个 Agent 代码
6. 从源码构建和多机集群部署

<!-- truncate -->

- CubeSandbox 和 Docker 容器、传统 VM 的本质差异在哪
- 整体架构里各组件怎么分工
- eBPF 驱动的 CubeVS 网络虚拟化层怎么实现 SNAT/DNAT、会话跟踪和网络策略
- 模板（Template）生命周期：Init → Boot & Snapshot → Deploy
- 在 WSL 2 或裸金属上装好 CubeSandbox 并跑通第一个 Agent 代码
- 从源码构建和多机集群部署怎么做
...
**CubeSandbox 就是冲着这个矛盾来的**：在 60ms 内创建一个具备完整服务能力的硬件隔离沙箱，同时保持小于 5MB 的单机实例内存开销，从而在一台普通服务器上跑起数千个相互隔离的 Agent 环境。

> CubeSandbox 之所以能做到 60ms 冷启动，靠的不是"加快开机"，而是**资源池化预置 + 快照克隆**——沙箱模板预先以 MicroVM 形式冷启动并打下内存快照，后续每次创建沙箱实例时直接从快照热启动，跳过完整的 OS 引导过程。
...
> **为什么用 eBPF 而不是 iptables/brctl？** 传统方案在每个报文上都会引入额外开销，且随租户数量增长而加剧。eBPF 程序挂载在内核数据路径关键位置，数据面逻辑完全在内核态执行，无需用户态上下文切换，CPU 开销极低。
...
1. **基础练习**：按照本文 `§5 快速开始` 的步骤，在 WSL 2 或裸金属服务器上安装 CubeSandbox，并运行第一个 Agent 代码示例。

2. **模板制作练习**：使用 `cubemastercli tpl create-from-image` 命令从自定义镜像制作模板，并设置 `--expose-port` 和 `--probe` 参数。

3. **网络策略练习**：创建一个沙箱实例，配置网络策略只允许访问特定 IP 段，然后尝试访问被阻止的 IP 并观察是否被拦截。

4. **多机集群练习**：如果有条件，按照本文 `§6 多机集群部署` 的步骤，新增一个节点到集群中，并验证 CubeMaster 是否自动发现并纳入新节点。

5. **性能测试练习**：使用 E2B SDK 创建多个沙箱实例，测试单机可以运行多少个沙箱实例，观察内存开销是否如文档所说低于 5MB。

---

## 自测

完成以下自测题，检查你对 CubeSandbox 的理解：

1. CubeSandbox 的冷启动时间是多少？为什么能做到这么快？
2. CubeVS 网络虚拟化层使用了哪三个 eBPF 程序？它们分别挂载在哪个位置？
3. 出站流量和入站流量的报文路径分别是怎样的？
4. 模板生命周期分为哪三步？快照克隆技术的核心原理是什么？
5. CubeSandbox 如何兼容 E2B SDK？需要修改多少业务代码？
