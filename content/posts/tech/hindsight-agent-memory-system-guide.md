...
抽象机制讲完，用一个具体例子把它们串起来。假设有一个客户支持 Agent，用户发来消息说“我上周升级到 v2.0 之后，导入功能就一直报错”。
...
这个例子里，Retain、Recall、Reflect 三条主线在一次完整对话里各司其职：Retain 把新经验沉淀下来，Recall 把相关记忆取出来，Reflect 把零散经验提炼成可复用的判断。三条线共享同一个记忆库，但触发时机和产出物完全不同。
...
- Slack 社区：https://join.slack.com/t/hindsight-space/shared_invite

如果想深入理解 Hindsight 的设计动机，建议先读论文，再看 Cookbook 里的完整示例，最后对照 API 参考实现自己的集成。

---

_本文基于 Hindsight v0.5.0（2026-04-08），8.9k Stars，MIT 许可证。_
