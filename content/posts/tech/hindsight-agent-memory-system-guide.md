...
| [这套系统里有哪些并行机制](#这套系统里有哪些并行机制) | 三条主线如何拆开 | 想理解底层 |
| [为什么 RAG 和知识图谱不够用](#为什么-rag-和知识图谱不够用) | Hindsight 的设计取舍 | 选型阶段 |
| [三种记忆类型](#三种记忆类型worldexperiencemental-model) | World/Experience/Mental Model 各存什么 | 设计记忆结构 |
| [三大操作：Retain、Recall、Reflect](#三大操作retainrecallreflect) | 三个 API 怎么用 | 上手使用 |
| [一次对话如何流过系统](#一次对话如何流过系统) | 完整任务流案例 | 想看完整路径 |
| [Python SDK 实战](#python-sdk-实战) | 具体代码怎么写 | 集成开发 |
| [Node.js / TypeScript SDK](#nodejs--typescript-sdk) | Node 侧怎么用 | 集成开发 |
| [部署选项](#部署选项) | 四种部署方式怎么选 | 运维部署 |
| [多 LLM Provider 支持](#多-llm-provider-支持) | 如何切换模型 | 配置调整 |
| [LongMemEval 测的是什么](#longmemeval-测的是什么) | 基准测试能推出什么、不能推出什么 | 判断性能宣称 |
| [采用顺序与决策建议](#采用顺序与决策建议) | 什么场景值得引入 | 采用决策 |
...
- **结构性 (20/20)**：标题层级无跳跃，目录采用"章节—回答—读者"三维表格，总览地图（并行机制流程图）位于前 20% 位置
- **准确性 (25/25)**：Retain/Recall/Reflect 三条主线的触发时机和产出物描述可对账代码和论文，LongMemEval 基准解读注明了测量范围与不能推出的结论
- **可读性 (25/25)**：中英文混排规范，段落密度适中，无模板腔和生成式转场，采用顺序与决策建议直接给出取舍而非泛化总结
- **教学性 (20/20)**：学习目标按能力层级排列，6 道自测题覆盖记忆类型判断、检索路径选择、RRF 必要性、bank 划分策略等关键决策点
- **实用性 (10/10)**：4 种部署方式的命令可直接复制执行，落地顺序建议分 5 步推进，常见问题覆盖性能、成本和多语言场景
