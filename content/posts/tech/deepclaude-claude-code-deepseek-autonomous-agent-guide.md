...
deepclaude 的两条运行路径边界很明确，一开始分清就省掉大量排障时间：

| 维度 | CLI 直连 | Remote Control + 本地 Proxy |
|------|----------|---------------------------|
| 启动命令 | `deepclaude` | `deepclaude --remote` |
| 模型请求走向 | 直接打到 provider | 经 `127.0.0.1:3200` proxy 分流 |
| 需要的认证 | Provider API key | Anthropic OAuth + `claude.ai` 订阅 |
| 额外依赖 | 无 | Node.js 18+, `claude auth login` |
| 热切换 (`--switch`) | ❌ 不可用 | ✅ 可用 |
| bridge 链路 | 不涉及 | 直连 `wss://bridge.claudeusercontent.com` |

后面展开的 proxy、热切换、兼容层，都在这个框架里。
...
从这个例子能看出两条路径的核心差异：CLI 直连省了一层跳转，适合批量和自动化；Remote Control + Proxy 多了一层控制平面，适合需要热切换和成本监控的交互式场景。
...
便宜不意味着行为完全一致。适合多数场景的做法是高频日常任务走 DeepSeek，复杂推理、关键审查或高风险修改切回 Anthropic。
