...
Agent Skills 的应对方式是把安全审查前置到发布流程——100% 开源、CI 静态分析、lockfile（锁文件）内容哈希、人工 prompt 审核、Snyk Agent Scan（Snyk 智能体扫描），五层全部通过才进 catalog。CLI 安装侧另有纵深防御：输入清理、路径隔离、符号链接防护、原子锁文件、审计日志。截至 2026 年 4 月，仓库到达 skills-catalog-v0.14.3，1,029 次提交，56 个 release，支持 19 个 AI 编码 agent。
...
链式符号链接会被递归解析，最终目标必须落在允许目录内。`ELOOP` 错误（循环符号链接）会被捕获并强制删除链接。Windows 上用 directory junction（目录连接点）替代 symlink，获得更好的 OS 级隔离（见 [SECURITY.md](https://github.com/tech-leads-club/agent-skills/blob/main/SECURITY.md) Symlink Guard 章节）。
