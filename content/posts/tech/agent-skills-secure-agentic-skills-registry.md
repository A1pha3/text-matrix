...
5. MCP server 和 CLI 安装应该怎么选？两者的安全覆盖有什么差异？
   ...
   两者解决不同问题。CLI 安装把技能文件写到本地 agent 的技能目录，agent 启动时直接读取，适合需要长期使用、离线可用的场景。MCP server 在运行时按需查询 catalog，agent 通过 `list_skills`、`search_skills`、`read_skill`、`fetch_skill_files` 四个工具渐进式加载，适合探索新技能、按任务临时加载参考文档的场景。安全敏感场景优先用 CLI 安装，因为 lockfile 和审计日志只覆盖 CLI 路径。
