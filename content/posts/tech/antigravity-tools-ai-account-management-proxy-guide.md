...
| 指标 | 数值 | 说明 |
|------|------|------|
| GitHub Stars | 29.9k+ | 截至 2026-06-27 公开数据 |
| Forks | 3.2k+ | 同上 |
...
### 练习 1：部署 Antigravity Tools 到本地

根据本文的安装指南，完成以下任务：

1. 使用 Docker 部署 Antigravity Tools 到本地环境
2. 配置至少 2 个 Gemini API 账号
3. 测试协议转换功能（OpenAI ↔ Anthropic ↔ Gemini）
4. 观察账号轮换和 429 自动切换的效果

**目标**：掌握 Antigravity Tools 的部署和配置流程。
...
假设你需要在团队环境中部署 Antigravity Tools：

1. 设计高可用部署方案（多实例、负载均衡）
2. 配置账号配额管理策略
3. 评估合规风险（Web Session 转 API 的服务条款风险）
4. 制定监控和告警方案
...
1. **来源标注**：本文以 [sickn33/antigravity-tools](https://github.com/sickn33/antigravity-tools) 仓库的 README、配置示例、协议转换规则为准，并比对仓库最新主干内容做了事实校验。
2. **时效性**：仓库内容会随版本更新，文中涉及的版本特性、命令参数、配置选项以本文写作时（2026 年 5 月）的主干为准，后续可能变化。
3. **示例数据**：文中涉及的账号、密码、API Key、域名等示例数据均为说明性内容，非真实凭证。
4. **功能边界**：本文描述的是仓库当前状态，作者可能在不通知的情况下调整功能、增加或下线某些特性。
5. **适用场景**：本文的采用路径和建议基于开源文档和常见部署实践，你的团队可能需要根据实际情况调整。
6. **合规要求**：Antigravity Tools 通过 Web Session 转 API，可能违反某些服务的服务条款。生产环境请使用官方 API，并遵守相关服务条款。
