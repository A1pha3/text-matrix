...
github_repo: "yaojingang/yao-open-skills"
source_key: "gh:yaojingang/yao-open-skills"
...
- 理解贝叶斯决策的核心思想，以及它如何解决传统决策的困境
- 掌握先验概率、似然比、后验概率的计算方法
- 理解证据分级体系，能够评估不同来源证据的质量
- 使用多轮对话机制，通过迭代提问不断更新判断
- 生成完整的贝叶斯决策报告，包括执行摘要、自然频率解释和敏感性分析
- 应用贝叶斯决策框架到实际场景（产品功能决策、职业选择、创业方向选择等）
- 识别高风险领域，并正确添加免责声明
...
**练习 1：计算后验概率**  
假设你要判断是否应该投资一个创业项目：
1. 先验概率 P(H) = 0.2（基于类似项目的成功率）
2. 新证据：团队有成功创业经验 → LR ≈ 2.0
3. 新证据：市场规模大 → LR ≈ 1.5
4. 计算更新后的后验概率

**练习 2：证据分级**  
以下证据分别属于哪个等级？
1. 麦肯锡关于 SaaS 留存率的行业报告
2. 创始人的个人博客文章，解释为什么他们的产品会成功
3. 类似产品的用户调研数据（样本量 500）
4. LLM 建议：这个产品会成功

**练习 3：多轮对话模拟**  
模拟一个关于"是否应该接受这个 job offer"的多轮对话：
1. 第一轮：初始先验 P(接受) = 0.4
2. 第二轮：补充薪资信息，LR = 1.8
3. 第三轮：补充团队信息，LR = 1.3
4. 计算每轮的后验概率和决策就绪度
...
**练习 4：敏感性分析**  
对于练习 1 的投资决策：
1. 如果先验概率变化 ±10%，后验概率如何变化？
2. 如果证据等级降低一级（从 A 到 B），结论是否改变？
3. 如果阈值设定为 P(H) > 0.6 才投资，行动建议是否改变？

**练习 5：高风险领域识别**  
以下决策分别属于哪个风险等级？应该添加什么免责声明？
1. 是否应该做基因检测？
2. 是否应该投资加密货币？
3. 是否应该更换律师？
...
**Q1：贝叶斯决策适合所有决策吗？**  
不是。贝叶斯决策适合信息不完整、不确定性高的决策，但不适合：
- 紧急情况（需要快速决策，没有时间收集证据）
- 低风险决策（不值得投入时间进行贝叶斯分析）
- 纯主观偏好决策（如选择喜欢的颜色）

**Q2：我先验概率设置错了怎么办？**  
先验不必完美，但必须可辩护。如果你怀疑先验设置错了：
1. 进行敏感性分析，看看先验变化对结论的影响
2. 如果结论不稳定，收集更多证据或做低成本测试
3. 在报告中明确标注先验的来源和假设

**Q3：我如何判断证据质量是 A 级还是 B 级？**  
主要看三个因素：
1. 来源是否权威（同行评审 > 内部数据 > 个人经验）
2. 样本量是否足够（元分析 > 大样本研究 > 小样本研究）
3. 是否有利益冲突（独立研究 > 厂商赞助研究）
...
没有固定答案，但可以参考决策就绪度：
- 就绪度 < 0.45：继续收集信息
- 就绪度 0.45-0.75：接近可决策，识别剩余关键缺口
- 就绪度 > 0.75：已具备决策条件，可以输出结论
...
**Q6：贝叶斯决策能保证决策正确吗？**  
不能。贝叶斯决策只是一种更理性的决策框架，不能保证结果。它的价值在于：
1. 强制你明确表达假设和证据
2. 提供一种系统化的更新判断的方法
3. 让你知道什么时候应该做决定，什么时候应该继续收集信息

### 高风险问题

**Q7：我可以在医疗决策中使用贝叶斯框架吗？**  
可以，但必须明确标注"决策支持"，不能替代专业医生诊断。在做出高风险医疗决策前，请咨询医生。

**Q8：贝叶斯决策可以用于法律案件吗？**  
可以，但必须明确标注"参考用途"，不能替代律师意见。在重要的法律决策中，请咨询律师。
