...
- 掌握 Langfuse 的整体架构设计：Monorepo 组织方式、核心模块划分及数据流向
- 理解 **PostgreSQL + ClickHouse 双数据库架构**的设计哲学与各自职责分工
- 深入理解 Langfuse 的 **OpenTelemetry 追踪体系**：Trace → Span → Observation 的层级模型
- 掌握事件摄取流水线（Ingestion Pipeline）的工作机制：批处理、采样、评分校验
- 理解 Langfuse 与 **LangChain、LlamaIndex、OpenAI SDK** 等主流框架的集成原理
- 学会使用 Python / JS/TS SDK 进行应用插桩，并完成自托管部署
- 了解 Langfuse 的企业版功能与多租户隔离设计
...
Langfuse 的追踪模型与 OpenTelemetry 语义完全兼容，这意味着：
