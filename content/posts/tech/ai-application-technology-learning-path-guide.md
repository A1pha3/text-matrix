...
|---------|-----------|-------------|---------|
| 全参数微调 | 700 亿 | 140GB+ | 数天到数周 |
| LoRA (Rank=16) | 几亿 | ~24GB | 数小时到一天 |
| QLoRA (4bit) | 几亿 | 12-20GB | 数小时 |
...
# 输出：trainable params: 4,194,304 || all params: 8,072,282,112 || trainable%: 0.052%
```
...
**OpenClaw** 是当前活跃的开源 AI 智能体框架之一，由独立基金会维护。截至 2026 年 4 月，其 GitHub 仓库已获得社区广泛关注，Skill 生态持续增长。

> ⚠️ **事实性声明**：OpenClaw 生态数据（如平台支持数量、Skill 数量等）基于官方文档和社区公开信息，可能随版本更新而变化。建议访问 [OpenClaw GitHub](https://github.com/openclaw/openclaw) 获取最新数据。
...
| 特性 | 说明 |
|------|------|
| 多平台支持 | 支持 22+ 消息平台（Telegram、Discord、WhatsApp、iMessage 等） |
| 本地优先 | 数据本地存储，强调隐私 |
...
```
定义评估集 → 运行评估 → 分析失败案例 → 修改提示词/配置 → 重新评估 → ...
```
...
| LangChain 文档 | 框架文档 | [docs.langchain.com](https://docs.langchain.com/) |
