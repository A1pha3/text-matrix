...
根据本文的"学习路线总览"和你的现有基础，为自己制定一个 3-6 个月的学习计划：

1. 评估自己的当前水平（零基础、有编程基础、有量化基础）
2. 选择适合的学习路径（入门路径、进阶路径、专家路径）
3. 为每个阶段设定具体的学习目标和不标
4. 列出需要的学习资源（官方文档、在线课程、开源项目）

**目标**：掌握如何为自己或团队制定 AI 应用技术学习路线。

### 练习 2：动手实现一个 Mini RAG 系统

从本文的"§4 RAG：检索增强生成"章节出发，动手实现一个最小化的 RAG 系统：

1. 选择一个小型知识库（如 10-20 篇技术文档）
2. 使用 LlamaIndex 或 LangChain 搭建 RAG 管道
3. 实现向量检索和重排序
4. 测试不同检索策略的效果差异

**目标**：理解 RAG 的核心机制和实际效果。

### 练习 3：设计一个智能体系统

从本文的"§6 Agent：智能体架构"章节出发，设计一个解决实际问题的智能体系统：

1. 定义一个具体的使用场景（如代码审查助手、技术文档问答）
2. 设计智能体的工具集和决策逻辑
3. 实现基础版本（使用 LangGraph 或 CrewAI）
4. 测试和评估系统表现
...
1. LLM 的核心机制是什么？它是如何实现对语言的理解和生成的？
2. Prompt Engineering 的核心目标是什么？有哪些常见的提示词技巧？
3. Fine-tuning 和 RAG 分别解决什么问题？如何判断应该使用哪一种？
4. Function Calling 和 MCP 的关系是什么？为什么需要 MCP 这样的标准？
5. 智能体（Agent）的核心特征是什么？与普通 LLM 应用有什么区别？
6. Multi-Agent 系统的核心优势是什么？有哪些常见的协作模式？
7. Workflow Engineering 关注什么问题？为什么智能体系统需要工作流编排？
8. Context Engineering 的核心挑战是什么？有哪些常见的上下文管理策略？
9. Agent Skill 的设计目标是什么？一个好的 Skill 应该具备哪些特征？
10. Harness Engineering 的核心价值是什么？如何为 AI 系统建立评估体系？

### 实践设计（5 题）

1. 你要设计一个企业知识库问答系统，会选择哪些技术栈？为什么？
2. 在智能体系统中，如何平衡自主性和可控性？有哪些常见的约束策略？
3. 你要评估一个 RAG 系统的质量，会关注哪些指标？如何设计评估集？
4. 在 Multi-Agent 系统中，如何避免智能体之间的冲突和重复？
5. 你要把一个智能体系统部署到生产环境，会关注哪些工程问题？
...
1. 添加"练习"章节（3 个实践练习，含参考答案）
2. 添加"自测题"章节（5 道题，含`<details>`标签参考答案）
3. 添加"进阶路径"章节（3 个深入步骤）
