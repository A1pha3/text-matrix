...
- [Transformers 快速入门（中文）](https://transformers.run/) — 中文 Transformer 与 Hugging Face 生态教程
...
| 模型 | 上下文窗口 | 备注 |
| ---- | ---- | ---- |
| OpenAI GPT-4.1 | 1,047,576 Token | 适合长文档问答、代码上下文和工具调用任务 |
| Claude 4 系列 | Opus / Sonnet 为 1M Token，Haiku 为 200K Token | 适合长文档阅读、写作、代码与 Agent 任务 |
| Gemini 2.5 / 3.x 系列 | 多个模型提供 1M Token 或更高上下文 | 适合超长文档、多模态和批量上下文任务 |
...
- [OpenAI Prompt Guidance](https://developers.openai.com/api/docs/guides/prompt-guidance) — OpenAI 官方提示词建议
- [Claude Prompt Engineering](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering) — Claude 官方提示词指南
...
| ---- | ---- | ---- | ---- |
| 全参数微调 | 700 亿 | 通常需要多卡并配合 ZeRO / FSDP 等策略，显存与优化器状态开销很高 | 数天到数周 |
| LoRA (Rank=16) | 数千万量级 | 通常需要高显存 GPU 或多卡，取决于基座模型和训练策略 | 数小时到一天 |
| QLoRA (4bit) | 数千万量级 | 可显著降低显存，70B 级模型仍需认真核算 batch、序列长度和优化器状态 | 数小时到一天 |
...
>
> - **Agentic RAG**：不再是单一的“把问题转成向量并去查”，而是让 Agent 根据用户提问自主决定**查哪些库**、**生成什么查询关键字**，甚至发现信息不够时主动发起多次查询。
...
| ---- | ---- | ---- |
| 标准化程度 | 多由供应商或框架定义 | 开放协议，围绕工具、资源和提示能力建模 |
| 跨应用复用 | 往往需要为不同应用重复适配 | MCP Server 可以被多个兼容客户端复用 |
| 工具发现 | 多数场景依赖静态配置 | 支持客户端发现 Server 暴露的能力 |
| 生态支持 | 绑定具体模型或 SDK 的情况较多 | 更适合作为跨工具、跨客户端的连接层 |
...
- [OpenAI Function Calling 指南](https://developers.openai.com/api/docs/guides/function-calling) — OpenAI 官方函数调用文档
- [MCP 规范文档](https://modelcontextprotocol.io/specification/) — Model Context Protocol 完整规范
...
⬆️ [返回目录](#目录) | ⬆️ [上一节：Agent](#6-agent智能体架构-) | ⬇️ [下一节：Workflow Engineering](#8-workflow-engineering工作流编排-)
...
**如果已经有了多智能体（Multi-Agent），为什么还需要 Workflow？**
在真实的生成式 AI 生产应用中，智能体的“自主任性”是一把双刃剑。让智能体完全自主规划（如完全的 ReAct）可能导致失控、死循环或者极度不可控的 API 成本。Workflow 引入了**“确定性控制”（Deterministic Control）**：把系统的大方向（即业务流程）写死，只在具体的节点内让 LLM 发挥灵活性。
...
| 模式 | 说明 | 适用场景 |
| ---- | ---- | ---- |
| **提示词链（Chain）** | 任务 A 输出 成为任务 B 的输入 | 机器翻译审校、长文拆分润色 |
| **路由（Routing）** | 根据输入意图，分支走向不同的处理流 | 智能客服分发（如售后退款走 A 链路，技术咨询走 B 链路） |
| **并行（Parallel）** | 同时触发多个子任务执行，最后合并 | 生成产品报告时，并行收集财报、竞品资讯、新闻 |
| **状态机/图（State Graph）** | 支持循环与人工确认（Human-in-the-loop） | 长文写作（写提纲 -> 修改提纲 -> 写正文 -> 审核），代码自动生成 |

### 8.3 Workflow 实战框架对比

| 框架 | 特点 | 适用人群 |
| ---- | ---- | ---- |
| **Dify / Coze (界面级)** | 零代码可视化拖拽，即看即得 | 产品经理、AI 业务开发者 |
| **LangGraph (代码级)** | 基于状态图（StateGraph），精细控制循环与记忆 | 后端开发、需要极致定制的工程师 |
| **Flowise** | 开源低代码，支持灵活部署 | 中小企业团队内部 AI 中台 |

### 8.4 Workflow 常见问题与排查

| 问题 | 可能原因 | 解决方案 |
| ---- | ---- | ---- |
| 工作流经常卡在无限循环 | 图中缺乏确定的退出条件或最大递归限制 | 引入基于计数器的限制（如最多重试 3 次），强制跳转到结束节点 |
| 节点之间数据丢失 | 没用正确维护 State（状态池） | 在 LangGraph 这类框架中，检查 `state` 字典是否执行了正确的 `update/append` 操作 |
| 分支路由错误 | 路由节点的 LLM 判断边界模糊 | 强化路由节点的分类 Prompt，将 Few-Shot 给够 |
...
⬆️ [返回目录](#目录) | ⬆️ [上一节：Workflow Engineering](#8-workflow-engineering工作流编排-) | ⬇️ [下一节：Context Engineering](#9-context-engineering上下文工程-)
...
[OpenClaw](https://github.com/openclaw/openclaw) 是当前活跃的开源智能体框架之一。它支持通过 workspace skills 扩展能力，并在文档中把 [ClawHub](https://clawhub.ai/) 作为 Skill registry 入口。Skill 生态通常覆盖下面几类能力：
...
⬆️ [返回目录](#目录) | ⬆️ [上一节：Context Engineering](#9-context-engineering上下文工程-) | ⬇️ [下一节：OpenClaw](#11-openclaw开源智能体框架-)
...
> ⚠️ **事实性声明**：OpenClaw 的渠道支持、Skill registry、CLI 命令和推荐 Node.js 版本都在快速变化。本文只保留架构层面的学习价值，具体安装参数请以 [OpenClaw 文档](https://docs.openclaw.ai/) 和 [GitHub README](https://github.com/openclaw/openclaw) 为准。
...
| 特性 | 说明 |
| ---- | ---- |
| 多渠道接入 | 支持 Telegram、Discord、Slack、WhatsApp、iMessage 等常见消息入口，具体列表以文档为准 |
| 自托管 Gateway | 会话、路由、渠道连接和工具调用由用户运行的 Gateway 管理 |
| Agent 工作区 | 支持按 workspace 管理提示文件、工具和 Skills |
| 安全治理 | 对公开群聊、私信、沙箱和工具权限需要显式配置 |
...
| 问题 | 可能原因 | 解决方案 |
| ---- | ---- | ---- |
| `npm install` 失败 | Node.js 版本过低 | 确认 Node.js 24 或 22.14+：`node -v` |
...
⬆️ [返回目录](#目录) | ⬆️ [上一节：Agent Skill](#10-agent-skill智能体技能-) | ⬇️ [下一节：Harness Engineering](#12-harness-engineering评估工程-)
...
⬆️ [返回目录](#目录) | ⬆️ [上一节：OpenClaw](#11-openclaw开源智能体框架-)
...
| 13-14 周 | Multi-Agent + Workflow Engineering | 能够设计基于有向图的多智能体团队 | 搭建一个能执行审核-退回循环的 Coze/Dify/LangGraph 编排流 |
| 15-16 周 | Context Engineering | 能够为智能体系统设计长时记忆 | 实现增量摘要与结构化历史管理 |
