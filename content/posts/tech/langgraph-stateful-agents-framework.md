...
地图清楚了，接下来把每个抽象放进真实场景，重点看用错会出什么问题。
...
三堵墙的根子是同一处：链式模型把「执行」「状态」「控制流」耦合在一起，图形模型把它们拆开。Node 算，Edge 路由，State 承载数据，Checkpoint 持久化，四者互不依赖。这种正交分解（orthogonal decomposition，把系统拆成相互独立的维度）的好处在于：任何一部分出问题都能独立定位和修复，不用牵一发动全身。
