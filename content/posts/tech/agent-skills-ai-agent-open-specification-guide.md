...
Agent Skills 解决的是工具数量增长后上下文被挤占、同一能力跨平台重写两个问题。它把"启动时全量加载工具描述"改成"按需发现并激活"，让 Agent 启动时只承担元数据开销，未命中的 Skill 始终只占约 100 token。
...
Agent Skills（智能体技能，下文简称 Skill）是 Anthropic 主导的开放规范，由规范文档、参考实现、官方示例库和兼容 Agent 实现四部分组成。它针对两个场景：工具数量一多，启动时塞进上下文的工具描述就挤占任务可用空间；同一个能力要在 Claude Code、Copilot Chat、Codex 等不同实现里各写一遍。
...
启动时只承担元数据开销，未激活的 Skill 不会把指令和资源塞进上下文。工具数量增长时，启动开销只与 Skill 总数的元数据线性相关，激活开销只与命中 Skill 数量相关，两条增长曲线被拆开。
...
下表数据采集于 2026-03-28，下文引用的 Stars 数均以此为准：
...
`SKILL.md` 采用 YAML frontmatter（前置元数据）加 Markdown body（正文）的混合格式。YAML frontmatter 可被标准解析器（PyYAML、rufo 等）直接读取，Agent 在 discovery 阶段只解析 frontmatter 就能拿到元数据，不必扫整个 body；Markdown body 可以直接作为指令喂给 LLM，不需要二次转换。
...
`name` 限 64 字符，是为了在 discovery 阶段做索引时控制单条记录大小；`description` 限 1024 字符，因为它会在启动时全量加载进上下文，过长会直接抵消渐进式披露的收益；`compatibility` 限 500 字符，它只在激活后才会被读取，但仍要避免被当作指令注入。
...
三个阶段的边界是"什么时候读、读多少"。阶段 1 是固定开销，与任务无关；阶段 2 是命中开销，只与激活的 Skill 数相关；阶段 3 是执行开销，只与实际用到的资源相关。token 预算可以分别估算，某一层失败时也能单独排查。
...
对于大型 Skill，建议将详细参考文档分离到 `references/` 目录。`SKILL.md` body 在激活时会被全量读入上下文，把 API 全文写进 body 会直接吃掉几千 token；`references/` 只在指令显式引用时才加载，未引用的部分不占上下文。
