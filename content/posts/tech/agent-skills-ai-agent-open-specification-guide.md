...
Agent Skills（智能体技能，下文简称 Skill）是 Anthropic 主导的开放规范，把 AI Agent（智能体）的能力扩展从"启动时全部加载工具"改造成"按需发现并加载"。它针对的是两个场景：工具数量一多，启动时塞进上下文的工具描述就挤占任务可用空间；同一个能力要在 Claude Code、Copilot Chat、Codex 等不同实现里各写一遍。
...
这样 Agent 启动时只承担元数据开销，未激活的 Skill 不会把指令和资源塞进上下文。
...
Agent Skills 用 progressive disclosure（渐进式披露）来控制上下文开销：把内容分成三个阶段按需加载，避免一次性把所有 Skill 的完整指令塞进上下文。
