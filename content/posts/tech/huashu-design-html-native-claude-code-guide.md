...
source_key: "gh:alchaincyf/huashu-design"
...
Huashu Design 的目标不是造一个更好的图形工具——它要**让图形工具这层消失**。

## 二、原理分析：HTML 原生设计理念

### 2.1 为什么是 HTML？

大多数设计工具的输出格式是专有的（如 Figma 的 `.fig`、Sketch 的 `.sketch`），需要专用软件才能打开和编辑。Huashu Design 选 HTML 作为设计媒介，背后有三层考量：
...
### 9.1 Huashu Design 解决了什么

**1. 设计执行从 GUI 操作变成对话指令**
传统设计工具（Figma、Sketch）依赖鼠标操作，设计师的手和眼无法离开屏幕。Huashu Design 让设计师（和 AI Agent）通过自然语言描述完成设计，设计决策和执行分属两个环节。

**2. 从模糊需求到交付的完整工作流**
Huashu Design 定义了完整的设计工作流（品牌资产协议 → 设计方向顾问 → Junior Designer 模式 → 5 维评审），而不是只给一个模板让用户套。

**3. 不绑定特定 Agent，skill 可跨平台迁移**
以 skill 为单位输出，任何支持 skills.sh 协议的平台都可以使用，设计能力可以在不同 Agent 之间迁移。
...
**这是一个 80 分的 skill，离 100 分的产品还有距离**。对不愿意打开图形界面的人，80 分的 skill 比 100 分的产品好用。
