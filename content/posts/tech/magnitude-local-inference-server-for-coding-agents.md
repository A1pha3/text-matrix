...
Magnitude 的定位是**"先懂你的硬件，再谈模型"的本地推理服务器**：它剖析你的芯片、内存、带宽，从模型目录里挑出这台机器真正跑得动的模型，估算 tok/s，下载后按机器调好投机解码（speculative decoding）与并发，再写进你所用智能体的配置里。与"让智能体自己装 Ollama"的本质区别在于：智能体装 Ollama 时在猜——不知道哪个量化版本合适、跑多快；Magnitude 把"猜"换成了"查表"：为这台机器算好的推荐目录 + 生成 harness（智能体经其接模型的运维框架）配置的 onboarding 流程 + 面向智能体负载的推理调度（按需加载、空闲或内存吃紧时卸载）。
...
把上面的闭环落到一次实际操作里会更直观。假设你在用 OpenCode，想切到本地模型：

1. 跑 `magnitude setup`，或把启动提示词丢给智能体。CLI 先做硬件画像，输出这台机器的芯片、内存与带宽，并据此给出推荐模型和预估 tok/s。
2. 你从推荐列表里选一个（例如一个 Qwen 系的中型量化版），CLI 下载模型文件，落在本机缓存里。
3. setup 阶段把该模型的端点写进 OpenCode 的 harness 配置，让它指向本机推理服务。
4. 你照常写代码。发起请求时，Magnitude 按需把模型加载进内存处理；请求结束空闲，或内存被其他常驻进程吃紧，就把它卸下。
