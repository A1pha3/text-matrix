...
source_key: "gh:kyegomez/OpenMythos"
...
- **Recurrent-Depth Transformer（RDT）** 与传统深度 Transformer 的区别，以及它为何能让模型在单次前向传播中完成多步推理
- OpenMythos 的 **Prelude-Recurrent-Coda** 三阶段架构设计，以及循环块的数学更新规则
- 可切换注意力机制 **MLA**（Multi-Latent Attention）与 **GQA**（Grouped Query Attention）的设计权衡
- 稀疏 **MoE**（Mixture of Experts）如何实现参数规模扩展
- 循环模型训练中两大不稳定问题（残差爆炸与 Loss spike）背后的动力学根源，以及 **Parcae 架构**如何通过谱半径约束从构造上保证稳定性
- OpenMythos 的使用方法（安装、配置、预置模型变体、训练），以及从 1B 到 1T 参数各规模的配置差异
...
理解 Mythos 的架构设计，对 AI 研究者和工程师有三层意义：

1. **深度推理能力的来源**：Mythos 在复杂多步推理任务上的质变式表现（"突然就会了"而非"逐渐学会"），来自循环架构的内在特性，而非简单堆叠更多层
2. **推理时计算扩展的新范式**：增加循环次数而非模型规模来获得更强推理能力，给实际部署留出了全新的效率优化空间
3. **训练稳定性的工程突破**：循环模型训练的稳定性问题如何解决，对未来循环架构的研究有直接借鉴价值
...
**Loss Spike**：训练在某个时刻突然发散，之前一切正常的训练 run 突然崩溃。
...
实验观察：**所有失败的训练 run 学到的 `ρ(A) ≥ 1`，所有成功的 run 都维持 `ρ(A) < 1`**。这提供了诊断依据。
...
这直接解释了 Claude Mythos 能处理极长链路的数学证明、多步规划、分层论证等任务：靠更多循环，而非更多层。
...
OpenMythos 揭示了 Claude Mythos 可能采用的核心架构范式——**Recurrent-Depth Transformer**。这一范式的影响：

- **推理时计算扩展**：固定参数实现动态推理深度，"小模型大智慧"有了新路径
- **系统性泛化**：循环架构的相变特性，让模型在分布外任务上"突然就会了"，而非渐进式失败
- **内存效率**：隐藏状态不随推理深度增长，边缘端部署深度推理能力成为可能
- **稳定性突破**：Parcae 架构的谱半径约束方法，为后续循环模型训练提供了可复用的工程方案
