...
上线不到一个月，GitHub 50,544 星。

**读完你会：**

- 说出 caveman 压缩 LLM 输出的 7 条规则，并能判断一段文本被压缩时哪些部分会被删除、哪些部分会被保留
- 区分 lite / full / ultra 三种强度级别，根据任务类型（调试、教学、代码审查）选择合适的模式
- 理解 caveman 的 hook 架构如何通过 SessionStart 和 UserPromptSubmit 在多轮对话中维持压缩状态
- 在自己的 Claude Code 或 Cursor 中安装并配置 caveman，完成一次从安装到 `/caveman-review` 的完整流程
