...
**成熟度提醒**：README 明确标注这是 **pre-alpha 软件，正在快速演进，不用于生产**。API 与算法在 v1.0 前预计会有显著变化。写这篇文章时把它定位为"值得评估的方向"，而非可直接上生产的方案。
...
`switchyard-libsy` 把路由算法嵌入你自己的 Rust 应用。它自己从不调用模型——由算法决定用哪个 target，再把模型调用交还给你，因此可以塞进现有 proxy、gateway 或 agent runtime，而不必拥有 HTTP 栈。
