...
2. [核心判断：这不是软件，是数据策展](#2-核心判断这不是软件是数据策展)
3. [系统地图：仓库结构与生成管线](#3-系统地图仓库结构与生成管线)
4. [策展哲学：为什么只有几百个频道](#4-策展哲学为什么只有几百个频道)
5. [硬约束与适用边界](#5-硬约束与适用边界)
6. [使用方式：3 步上手](#6-使用方式 3-步上手)
7. [怎么参与贡献](#7-怎么参与贡献)
8. [适用人群与不适用人群](#8-适用人群与不适用人群)
...
### playlist.m3u8 里的频道突然不能看了，怎么办？

先确认是这个频道失效了，还是你的网络问题。换一个频道试试；如果多个频道都不能看，可能是播放器缓存或网络问题。单个频道失效的话，可以去仓库提 issue 告知维护者。

### 为什么有些频道显示 GeoIP 限制（Ⓖ）？

这些频道只允许特定国家访问。比如美国本地频道只在美国境内能看。Free-TV 用 `Ⓖ` 标记这些频道，PR 时会要求贡献者说明流在哪些国家能用。如果你在不允许的国家，这个频道就打不开。

### 我可以商业使用这个 playlist 吗？

仓库没有 LICENSE 文件，默认是"All rights reserved"。商业使用前需要联系维护者确认。个人观看没有问题，因为播放的是公开流地址。

### 为什么不用 iptv-org/iptv？

两个项目定位不同。Free-TV 走"几百个稳定频道"路线，人工策展、PR 评审质量；iptv-org 走"几十万频道 + 自动失效检测"路线，规模更大但稳定性较低。如果你想要稳定能看的清单，选 Free-TV；如果你想要尽可能多的频道，选 iptv-org。

### 怎么知道一个频道确实是免费的？

在添加频道的 PR 描述里需要证明。例如贴出电视台官网的"Free to air"声明、Wikipedia 页面说明、或政府公开广播服务的说明。不能只贴一个流地址就提 PR。
...
1. **Free-TV/IPTV 仓库的最终产物是什么？**
   <details>
   <summary>点击查看答案</summary>
   最终产物是 <code>playlist.m3u8</code> 文件，可以通过 URL <code>https://raw.githubusercontent.com/Free-TV/IPTV/master/playlist.m3u8</code> 直接在支持 M3U 的播放器中使用。
   </details>

2. **Free-TV/IPTV 与 iptv-org/iptv 的根本区别是什么？**
   <details>
   <summary>点击查看答案</summary>
   Free-TV 走"几百个稳定频道"路线，人工策展、PR 评审质量；iptv-org 走"几十万频道 + 自动失效检测"路线，规模更大但稳定性较低。
   </details>

3. **为什么 PR 不能直接修改 `playlist.m3u8` 文件？**
   <details>
   <summary>点击查看答案</summary>
   因为 <code>playlist.m3u8</code> 是生成产物，由 <code>make_playlist.py</code> 脚本从 <code>lists/*.md</code> 生成。直接修改生成产物会导致与源数据不同步。
   </details>

4. **Free-TV/IPTV 的三条策展哲学是什么？**
   <details>
   <summary>点击查看答案</summary>
   1. Quality over quantity（质量优先）；2. Only free channels（只收录免费频道）；3. Only mainstream channels（只收录主流频道）。
   </details>

5. **Free-TV/IPTV 仓库的 License 状态是什么？这对使用有什么影响？**
   <details>
   <summary>点击查看答案</summary>
   仓库没有 LICENSE 文件，默认是"All rights reserved"。个人观看没有问题，但做衍生作品需要联系维护者确认。
   </details>
...
### 练习 1：添加一个新的免费频道
1. Fork Free-TV/IPTV 仓库
2. 在 `lists/` 下找到对应国家的 `.md` 文件
3. 添加一个新的频道条目，使用 `[>]` 前缀表示有效流
4. 在 PR 描述中证明该频道确实是免费的（提供电视台官网链接）
5. 提交 PR 并等待评审

### 练习 2：运行生成脚本
1. Clone 仓库到本地
2. 运行 `python3 make_playlist.py`
3. 检查生成的 `playlist.m3u8` 文件
4. 尝试用 VLC 打开生成的 playlist
...
1. **了解 M3U 格式规范**：阅读 M3U 格式文档，理解 `#EXTINF` 标签的含义
2. **研究 iptv-org 项目**：对比 iptv-org/iptv 的架构，理解自动失效检测的实现方式
3. **搭建自己的 IPTV 播放器**：基于开源播放器（如 VLC 或 Kodi）定制自己的 IPTV 解决方案
4. **贡献其他数据源**：如果你有其他免费频道数据源，可以贡献给 Free-TV 或创建类似的数据策展项目
5. **研究 EPG（电子节目单）**：了解 EPG 格式（XMLTV），尝试为 Free-TV 添加 EPG 支持

---

## 资料口径说明

1. **信息来源**：本文基于 Free-TV/IPTV 仓库的 README（2026-06-15 版本）和仓库代码结构分析
2. **版本时效性**：仓库仍在活跃维护中，本文描述的功能和策展哲学可能在未来版本中变化
3. **数据准确性**：仓库的频道数据由社区贡献，本文未逐一验证每个频道的有效性
4. **License 状态**：仓库无明确 LICENSE 文件，本文的解释基于 GitHub 的默认规则，不代表法律建议
5. **使用风险**：使用本 playlist 观看电视频道需遵守当地法律法规，某些频道可能有地域限制
