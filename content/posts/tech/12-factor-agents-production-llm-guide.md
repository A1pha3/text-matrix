...
12-Factor Agents 是 HumanLayer 创始人 Dex（Dexter Horthy）在 2025 年 3 月底开源的一组工程原则，向经典的十二要素应用（12 Factor Apps）致敬。官方给它的定位是「构建可靠 LLM 应用的原则」（Principles for building reliable LLM applications），回答的问题只有一个：怎样做出足以交到生产用户手里的 LLM 软件。截至 2026 年 9 月 7 日，仓库获得 25,727 星标、1,957 次复刻；代码按 Apache 2.0 许可发布，文档内容按 CC BY-SA 4.0 许可发布。

这组原则和 Agent（智能体）框架的区别在于：它不提供运行时，只提供约束。每条原则都在回答同一个问题——什么东西该由代码管，什么东西可以交给模型。

官方仓库给出 12 条正式原则加 1 条附录（Factor 13：预取上下文）。本文按「输入层 → 执行层 → 控制层 → 架构层 → 附录」的顺序拆开，每条原则说清楚解决什么问题、边界在哪，最后给一个代码审查 Agent 的完整任务流和落地顺序。
...
- [附录：Factor 13 — 预取上下文](#附录factor-13--预取上下文)
...
这张表就是全文的地图——从数据进入模型到结果返回调用方的完整流水线。Factor 13（附录）不归入任何一层，因为它是一种横切上下文组装的优化策略，任何一层都可能用到。
...
作者 Dex 在 AI Engineer World's Fair 的 17 分钟演讲和项目 README 里反复给过同一个观察：大多数自称「AI Agent」的产品并没有那么 agentic——它们大部分是确定性代码，LLM 步骤被撒在少数关键节点上，让体验显得神奇。他自述试遍了主流框架，从 plug-and-play 的 crew/langchains 系到「极简主义」的 smolagents，再到自称生产级的 langraph、griptape；也和不少 YC 内外的创始人聊过，这些人大多在自研技术栈——在生产环境面向客户的 Agent 里，他很少看到框架。

这 12 条原则就是从「大量软件工程」这个方向上总结出来的。共同方向是把控制权从模型收回代码，具体收回哪些环节见下文逐条说明。这个方向和「让 Agent 更智能」是正交的——12-Factor 不涉及提示词技巧，关注的是怎么把不确定性约束在可控范围内。

## 输入层：提示词、上下文与工具调用

### Factor 1 — 自然语言 → 工具调用

这条原则解决的是 Agent 最基础的一步：把一句自然语言翻译成结构化的工具调用。官方文档给的原始例子是把「帮 Terri 创建一个 750 美元的赞助付款链接」翻译成一个描述 Stripe API 调用的 JSON 对象——金额、客户 ID、产品 ID、备注各就各位，剩下的活交给确定性代码。原子化地看，这就是一次翻译：输入是一句话，输出是一个可解析的对象。

如果工具返回的是自由文本，下游代码就得写正则去提取、处理转义、应对模型偶尔加一句解释；如果工具返回的是 JSON，下游直接用现成的解析器，失败也能在 schema（模式定义）校验层统一拦截。把工具当「魔法能力」来设计、让它吐自由文本，调用链路就会变得不透明。
...
只返回 JSON，字段：intent, severity, comment。"""
    for attempt in range(3):
        try:
            ...
            time.sleep(2 ** attempt)  # 第 1、2 次重试前分别等 1s、2s
            continue
        try:
...
拿到生产环境跑时，有三个工程细节需要单独处理。第一是超时——OpenAI 调用默认超时偏长，显式设置 `timeout=30`，避免单个慢请求把 Agent 调用链拖垮。第二是重试退避——两次重试前分别等 1 秒、2 秒，给上游留恢复窗口，避免雪崩；两次都失败就降级，不要无限重试。第三是可观测性——每次校验失败打一条结构化日志（含 attempt、错误类型、原始输出），事后能统计 schema 失败率，定位是提示词漂移还是模型本身问题。这三点不是 Factor 1 强制要求的，但缺了任何一条，线上稳定性都会出问题。

### Factor 2 — 掌控你的提示词

官方对这条的原则性表述是一句告诫：不要把提示词工程外包给框架。一些框架提供「黑盒」封装——传入 role、goal、tools 就能得到一个 Agent，起步很快，但要调到刚好对的 token 却很难：你既看不到完整提示词，也改不动它的内部拼接。官方的态度是反过来：把提示词当一等公民的代码，自己拥有它。
...
作者把「Context Engineering（上下文工程）」放在比「Prompt Engineering（提示词工程）」更本质的位置。上下文窗口的大小和内容布局，直接决定了模型能看到什么、记住什么、做出什么判断。如何压缩历史、选择检索片段、组织消息顺序——这些比提示词里的措辞影响更大。
...
官方文档给这条的概括是：工具没那么复杂，本质就是「LLM 的结构化输出触发一段确定性代码」。配合的一句话更直接：LLM 决定做什么，代码控制怎么做。模型返回 `create_issue` 的意图，执行哪个 API、怎么处理失败、要不要真的执行，都是代码的事——「模型调用了工具」并不意味着你必须原样执行对应的函数。
...
官方把这个问题拆成两个概念：执行状态是「当前步骤、下一步、等待状态、重试计数」这类流程元数据；业务状态是「到目前为止发生了什么」——消息列表、工具调用与结果。传统基础设施习惯把两者分开管，官方的建议是能统一就统一：很多执行状态本来就只是「发生过什么」的元数据，完全可以从上下文窗口推断出来，不需要另立存储。

统一之后的好处官方列了七条：状态有单一事实来源；线程可以整体序列化、反序列化；调试时全部历史在一处可见；加新状态只需加新事件类型；可以从任意点恢复；可以复制任意点分叉出新分支；还能低成本转成人类可读的界面或监控视图。这七条里的「分叉」对调试尤其有用——把某个状态点复制出来重放，就是一次最便宜的复现实验。
...
官方的出发点很朴素：Agent 就是程序，程序就该有符合直觉的启动、查询、暂停、恢复、停止接口。用户、应用、流水线、其他 Agent 都能用简单 API 把 Agent 拉起来；长任务执行中能随时暂停；webhook 之类的外部触发能让它从断点继续，而不需要和编排器深度耦合。

生产级 Agent 必须支持中断和恢复。运行中的 Agent 可能因为人工审批、外部依赖超时、资源限制等原因停下来，不能每次中断都从零开始。审查一个 500 文件的大 PR，跑到第 200 个文件时进程被 OOM 杀掉——从头重跑既浪费 token 又拖慢响应。把执行状态持久化到外部存储，恢复时从断点继续，是生产环境的基本要求。

官方文档还点名了一个常见缺口：多数 AI 编排器支持暂停和恢复，但暂停点落不到「工具选择」和「工具执行」之间——而这恰恰是在高风险工具真正运行之前做审查、审批最需要的位置。评估框架时值得专门验证这一条。
...
官方文档对这条的论证从 LLM API 的一个细节开始：模型返回的是明文内容还是结构化数据，取决于第一个 token——一次高风险的 token 选择。他们的建议是干脆让模型永远输出 JSON，用 `intent` 字段声明意图，比如 `request_human_input` 或 `done_for_now`，把「要不要联系人类」变成 JSON 里的一个普通取值。这不是说一定能带来性能提升，但它让你可以自由尝试各种「怪招」而不被工具调用机制绑住。

Agent 需要人工介入时，不应该靠「在日志里打印一句话」。应该通过标准化的工具调用机制发出请求、等待响应、超时处理——和调用任何其他工具一样。日志里打一句话，没人会看。把人工介入做成工具调用，请求会进队列、有超时、有重试、有审计记录。人和其他工具在 Agent 眼里没有区别，都是「调用 → 等响应 → 处理结果」。

官方给人类交互工具定义的数据结构值得直接借用：`question` 和 `context` 之外，还有 `urgency`（low/medium/high）和 `format`（free_text、yes_no、multiple_choice 加选项列表）。让提问的人先想清楚「紧急程度」和「希望对方用什么格式回答」，人的响应速度和后续解析都会受益。
...
掌控控制流的回报不只是稳定。官方列了一张「值得自己做一遍」的清单：工具结果的摘要或缓存、对结构化输出跑 LLM-as-judge、上下文压缩与记忆管理、日志追踪与指标、客户端限流、可持久化的 sleep/pause/等待事件。这些能力只有在控制流归代码时才插得进去。

官方给的控制流模式里，分支只有两种走向，对应两类工具调用：同步意图（如 `fetch_git_tags`）把结果追加进上下文后 `continue`，直接进入下一轮推理；异步意图（如 `request_clarification`、需要审批的 `deploy_backend`）则把线程存档、`break` 出循环，等 webhook 带着人的响应回来再续。

官方还留了一句对框架生态的批评，值得每个选型者记住：Dex 对所有 AI 框架的头号需求，是能在「工具选择」和「工具执行」之间打断 Agent。没有这个粒度的可恢复性，就没法在工具真正运行前做审查和审批，你被迫二选一——要么用 `while...sleep` 占着内存等长任务跑完，进程一断就从头再来；要么把 Agent 限制在研究和摘要这类低风险调用上。
...
def llm_judge(code: str) -> LintDecision:
    ...
    reason 以 critical: 开头是代码侧约定的严重问题标记，
    下面的流水线靠这个约定决定是否提前终止。
    """
    resp = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": f"代码是否有问题？只返回 JSON：has_issue, reason（严重问题请让 reason 以 critical: 开头）\n\n{code}"}],
        response_format={"type": "json_object"},
    )
...
`while` 循环、终止条件、汇总逻辑都是代码写的，LLM 只在 `llm_judge` 里判断「这段代码是否有问题」。如果让模型决定「下一步审查哪个文件」「要不要继续」，执行路径就不可控了。
...
官方强调这是 Agent「自愈」能力的来源：短任务里模型调用的工具失败了，好的模型有相当大概率能读懂错误信息或堆栈，在下一个工具调用里改对。官方的示例代码就是把异常格式化后作为一个 `error` 事件追加进线程事件列表，然后循环重试。同时官方建议给特定工具加一个错误计数器，把单个工具的连续尝试限制在 3 次左右——盲目重试和自愈之间的界线，就靠这个计数器划出来。
...
官方把这条的收益列成三层。第一层是「用户在哪，服务就在哪」：让用户从 Slack、邮件、短信或任何他们在用的渠道触发 Agent，Agent 也从同一渠道回应——这样的产品更像一个数字同事，而不是一个要专门打开的网页。第二层是「外循环 Agent」：Agent 可以被事件、定时任务、故障告警这些非人类触发源拉起，独自工作 5 分钟或 90 分钟，走到关键节点再找人要反馈或审批。第三层最反直觉也最有价值：当 Agent 能随时把人快速拉进来兜底，你就敢给它接发外部邮件、改生产数据这类高风险工具——可审计的人工介入通道，是放权给 Agent 的前提。
...
官方文档写到这里时自嘲了一句「这一条 mostly just for fun」——前文已经超过一千行 Markdown，收尾这一条把整套思路压成函数式编程里的 fold：状态在事件序列上折叠，每一步的输出只取决于上一步的状态和这一条事件。语气轻松，抽象层级却是全文最高的：它给前 11 条约束提供了一个可以整体推理的数学形状。
...
`review_reducer` 是纯函数，副作用以字符串描述形式返回，由外部代码执行；测试时只测 reducer，不测副作用执行。

## 附录：Factor 13 — 预取上下文

Factor 13 不在正式 12 条里，官方以附录（appendix-13-pre-fetch）的形式收录。思路一句话能说清：如果你已经知道模型大概率会调用某个工具，就直接在代码里预先调用，把结果塞进上下文，别浪费一次 token 往返让模型自己 fetch。

官方原文用一段伪代码 diff 呈现同一个道理：删掉「部署时你可能需要拉取 git tags 列表」这类伪提示，换成直接写好的「当前 git tags 是：{{ git_tags }}」，同时从 intent 枚举里删掉 `list_git_tags`——工具定义和往返一起消失。
...
下面是把这个思路落成 Python 的对比示例，展示「让模型自己 fetch」和「代码预取」的差异：
...
如果你的团队还在选框架阶段、Agent 还没上线，不如先把 Factor 8 和 Factor 4 落实——这两条能帮你在选框架时就排除掉一批把控制流交给模型的方案；选型时再顺带验证 Factor 6 说的「工具选择与执行之间能否暂停」。
...
| 框架声称支持暂停恢复，但审批总在工具执行之后 | 违反 Factor 6 的暂停粒度要求 | 验证暂停点能否落在工具选择与工具执行之间；不能的话换实现或手写编排 |
| Agent 行为突然变差，找不到原因 | 违反 Factor 2，提示词改动没记录 | 把提示词移进 Git 仓库，用 `git log` 查最近改动 |
| 人工介入请求丢失 | 违反 Factor 7，靠日志通知人 | 改成工具调用机制，请求进队列，有超时和重试 |
| 模型反复调用同一个失败工具 | 违反 Factor 9，错误没写进上下文 | 把工具失败的错误信息追加到上下文窗口，并给单个工具加连续错误计数上限 |
...
**LLM 变强之后，这 12 条还有意义吗？** 官方在 Factor 10 的问答里给过正面回答：有。模型变强的表现之一是能处理更长的上下文、覆盖更大 DAG（有向无环图）的更多环节，而「小而专注」的做法保证你今天就能拿到可靠结果，同时让 Agent 的职责范围随模型能力渐进扩大——而不是某天一夜之间把整个流程交给模型。官方还引了 NotebookLM 团队的说法：最有魔法的时刻，出现在贴近模型能力边缘的地方。找到那个边界并稳定守住它，比赌模型明天更聪明更可靠。
...
**Factor 13 为什么只是附录？** 因为它依赖前置条件——你得先有 Agent 运行一段时间、统计出工具调用频率，才知道哪些工具值得预取。新项目没数据，预取就是瞎猜。所以 Factor 13 放在最后，作为优化手段而不是基础约束。
...
| 长任务中断后能否从保存点恢复，而不是从头重跑？暂停点能否落在工具执行之前？ | | [Factor 6](#factor-6--简单-api-的启动--暂停--恢复) |
...
| 自测项 | 是/否 | 答否时回看 |
|--------|-------|------------|
| 是否统计过 Agent 的工具调用频率，把高频工具改成代码预取？ | | [Factor 13](#附录factor-13--预取上下文) |
...
区别：Factor 1 约束单次工具调用的输出格式（要结构化），Factor 8 约束整个流程的编排权（在代码手里，不在模型手里）。一个 Agent 可以输出完全结构化（满足 Factor 1），但下一步做什么由模型决定（违反 Factor 8）——比如提示词里写「根据用户意图决定下一步调用哪个工具」，这就是把控制流交给了模型。
</details>
...
判定边界：工具在历史调用中被命中的频率是否足够高（比如 >90%），以及预取的数据量是否在上下文预算内。不应该预取的情况：① 工具调用频率低（<10%），预取了反而浪费上下文预算；② 预取的数据量很大（比如整个仓库的文件列表），会挤占上下文窗口；③ 数据实时性要求高（比如当前库存量），预取的数据可能过期。这些百分比只用于说明决策边界，具体阈值取决于每次预取的数据量与失败代价，按你自己的调用统计校准。
</details>
...
4. 预期结果：`review_reducer` 是普通的纯 Python 函数，不涉及模型调用，同样的输入跑两次输出必然一致。temperature 只影响 reducer 外部承载它的 LLM 调用，不是本步骤验证的对象。
...
2. 打开 `content/` 目录，找到 Factor 2、3、8、12 对应的文档（文件名形如 `factor-02-own-your-prompts.md`）
...
1. **主要来源**：humanlayer/12-factor-agents 仓库的公开文档、Dex（Dexter Horthy）在 AI Engineer World's Fair 的演讲视频（17 分钟）、以及官方仓库中的代码示例。这些材料代表了作者团队从生产环境实践中提炼的工程判断，但样本选择存在主观性，结论推广需要结合团队自身场景。
...
3. **适用性边界**：12-Factor Agents 面向的是「需要稳定交付给真实用户的 LLM 应用」，对于原型验证、纯研究、单轮结构化提取等场景，部分原则的边际收益会递减。正文与自测题中的预取命中率数字（>90% 预取、<10% 不预取）只用于说明决策边界，并非来自实测统计，真实阈值应结合工具命中率与预取代价确定。

4. **未覆盖话题**：本文明确不讨论 MCP（Model Context Protocol）协议层、框架横向评测、模型训练/Fine-tuning，这些话题的判断需要参考其他专门资料。

5. **版本与时效性**：仓库创建于 2025 年 3 月 30 日，最后一批提交停在 2025 年 9 月 21 日；本文的星标数、fork 数、文件清单与文档内容均以 2026 年 9 月 7 日快照为准。原则本身是稳定的工程判断，不依赖仓库后续更新；若官方文档后续有调整，以仓库最新版本为准。
...
GitHub: [humanlayer/12-factor-agents](https://github.com/humanlayer/12-factor-agents)（仓库以 Markdown/MDX 文档为主，配套 Mintlify 文档站；代码 Apache 2.0，文档内容 CC BY-SA 4.0）

- [演讲视频：AI Engineer World's Fair（17 分钟）](https://www.youtube.com/watch?v=8kMaTybvDUw)（链接有效性以发布时为准）
- [官方 YouTube Deep Dive 解读视频](https://www.youtube.com/watch?v=yxJDyQ8v6P0)
...
- [create-12-factor-agent 脚手架共建讨论帖](https://github.com/humanlayer/12-factor-agents/discussions/61)：官方在 README 中征集贡献的脚手架项目，源码在仓库 `packages/create-12-factor-agent` 目录；截至 2026 年 9 月尚未发布到 npm，无法直接 `npx` 试用
