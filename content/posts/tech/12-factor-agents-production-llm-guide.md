...
12-Factor Agents 给出 12 条工程约束，每条约束把 LLM 的某类不确定性交给代码处理。这组约束从生产环境里长出来，和 Agent 框架的区别在于，它不提供运行时，只提供约束——每条原则都在回答同一个问题：什么东西该由代码管、什么东西可以交给模型。
...
这 12 条原则就是从「大量软件工程」这个方向上提取出来的。12 条原则的共同方向是把控制权从模型收回代码，具体收回哪些环节见下文逐条说明。这个方向和「让 Agent 更智能」是正交的——12-Factor 不涉及提示词技巧，关注的是怎么把不确定性约束在可控范围内。
