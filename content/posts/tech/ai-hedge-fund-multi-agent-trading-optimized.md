...
### 5.2 状态设计：AgentState 决定数据如何流转

所有节点之间传递的状态都由 `AgentState` 定义：
...
1. **`messages` 使用 `operator.add`**：意味着每个节点追加消息，而不是覆盖。这样可以保留完整分析链路，方便调试。
2. **`data` 和 `metadata` 使用 `merge_dicts`**：意味着每个节点可以往状态里写入自己的分析结果，而不会清空前一个节点的数据。
3. **状态是类型化的**：使用 `TypedDict` 而不是普通 `dict`，可以在开发阶段捕获字段名拼写错误。

### 5.3 数据访问层：统一封装外部 API

所有对外部金融数据的请求都通过 `src/tools/api.py` 完成。这样做的价值是：
...
理解架构的最好方式是跟踪一个具体任务：当用户运行 `--tickers AAPL,MSFT` 时，系统内部发生了什么。

### 6.1 初始化阶段

1. `main.py` 解析命令行参数，确定要分析哪些 `ticker`
2. `create_workflow()` 根据 `--analysts` 参数选择要启用的 Agent 节点
3. 初始化 `AgentState`，设置初始 `data` 和 `metadata`

### 6.2 分析阶段

1. `start_node` 把 `ticker` 列表写入状态
2. 每个分析 Agent 节点依次执行：
   - 从 `src/tools/api.py` 获取对应 `ticker` 的财务数据、价格数据、新闻数据
   - 调用 LLM 生成分析观点
   - 把分析结果写入 `AgentState["data"][analyst_name]`
3. 所有分析 Agent 执行完毕后，状态里已经积累了多个视角的观点

### 6.3 风控与决策阶段

1. `risk_management_agent` 读取状态里的所有分析结果，以及当前 `portfolio` 状态
2. 计算波动率、相关性、当前仓位，产出"允许动作集合"
3. `portfolio_manager` 读取"允许动作集合"和所有分析结果，调用 LLM 做最终选择
4. 输出最终交易决策（但不实际下单）

### 6.4 为什么这个流程值得学

这个流程最值得借鉴的地方是：**分析可以并行，但决策必须串行且受约束**。

- 分析阶段：多个 Agent 可以并行（取决于 LLM 调用方式），因为它们只读取数据，不修改共享状态
- 风控阶段：必须等所有分析完成后才能计算，因为它要综合所有观点
- 决策阶段：必须等风控完成后才能执行，因为它要在"允许动作集合"里选择

---

## §7 项目边界与当前限制

在决定采用这个项目的设计思路之前，你需要清楚它的边界在哪里。

### 7.1 它不做什么

- **不实际下单**：项目明确声明"用于教育和研究用途"，不会连接券商 API 做真实交易
- **不保证盈利**：回测结果仅供参考，不代表未来收益
- **不提供投资建议**：所有输出都应视为技术演示，而不是投资指导

### 7.2 它的当前限制

- **数据依赖外部 API**：需要配置 `FINANCIAL_DATASETS_API_KEY`，且受 API 速率限制
- **LLM 调用成本**：每次运行都会调用 LLM API，成本取决于使用的模型和分析 Agent 数量
- **回测不等于实盘**：回测使用历史数据，不考虑滑点、手续费、流动性等实盘因素
- **Agent 观点质量取决于 Prompt 和 LLM 能力**：如果 LLM 本身对某个领域理解有限，Agent 的观点质量也会受限

### 7.3 什么场景适合借鉴它的设计

- **多角色决策系统**：例如内容审核、漏洞优先级评估、销售线索评分
- **需要先分析后决策的工作流**：例如代码 Review、设计方案评审、简历筛选
- **需要可解释性的 AI 系统**：因为每个 Agent 的观点都记录在 `AgentState["data"]` 里，可以回溯
...
1. Python 3.12+（项目使用 Poetry 管理依赖）
2. 一个大模型 API Key（支持 OpenAI、Groq、Anthropic、DeepSeek、Moonshot Kimi 等）
3. 一个金融数据 API Key（`FINANCIAL_DATASETS_API_KEY`）
...
# 至少配置一个 LLM_PROVIDER 和 FINANCIAL_DATASETS_API_KEY
...
回测模块会复用 `run_hedge_fund()` 作为决策引擎，这意味着研究态运行与回测态运行共享决策逻辑。

### 8.5 常见问题排查

| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| `ModuleNotFoundError` | 依赖未安装完整 | 运行 `poetry install --no-cache` |
| `API key not found` | `.env` 文件未配置或路径错误 | 确认 `.env` 在项目根目录，且包含所需 Key |
| `Rate limit exceeded` | API 调用超限 | 减少分析的 `ticker` 数量，或增加 `--end-date` 与 `--start-date` 的间隔 |
| 输出为空 | LLM 未返回有效响应 | 检查 LLM API Key 是否有效，或尝试换一个模型 |
