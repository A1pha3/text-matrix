...
- 理解 Home Assistant 的实体（Entity）/状态（State）数据模型及其在统一多品牌设备中的作用
- 掌握集成（Integration）的加载机制、配置模式和通信路径
- 了解自动化引擎（Automation Engine）的触发-条件-动作模型与事件总线的配合关系
- 认识本地优先（Local-First）设计对隐私的含义及其代价
- 能从头追踪一次完整自动化从触发到执行的数据流
