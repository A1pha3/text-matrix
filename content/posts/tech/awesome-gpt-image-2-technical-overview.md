...
Awesome GPT Image 2 是一个把"提示词众包投稿"工程化的开源项目。它用 GitHub Issue 收稿、用 CMS 做单一数据源、用 TypeScript 脚本生成 17 种语言 README，把 4000+ 提示词维护成了一份可被自动化流水线持续产出的产物。本文拆解它的三条并行机制——内容生产、自动化同步、多语言发布——并给出可复用的提示词结构模板与采用建议。
...
先看清三条互不阻塞的工作流，才能理解这个仓库为什么能在 4000+ 提示词规模下保持低维护成本：
...
机制 A 决定内容质量，机制 B 决定数据一致性，机制 C 决定发布覆盖面。三者解耦后，任何一个环节的延迟都不会阻塞另外两个。

## GPT Image 2 的能力边界

GPT Image 2 是 OpenAI 于 2026 年发布的图像生成模型（社区测试反馈，OpenAI 官方未公开完整技术报告）。相比上一代，社区反馈在以下场景有明显改善：

**像素级文字渲染**。在中文、英文、日文上达到 native 水准，无错字、无字形扭曲。文字渲染一直是图像生成模型的痛点，GPT Image 2 在这张图上把可用阈值拉到了商业排版级别。

**跨图像一致性**。同一角色、风格、IP 在多张图之间保持像素级一致。故事板、IP 形象设计、产品系列图这类需要多图协同的商业场景因此可以走通自动化流程。
...
为什么选 TypeScript？因为这个仓库的核心产物是结构化数据（提示词 JSON Schema）和文本（多语言 README），TypeScript 的类型系统能在脚本层约束提示词字段，避免 CMS 数据格式漂移导致 README 生成失败。`@octokit/rest` 用于与 GitHub Issue 交互，`node-fetch` + `qs-esm` 用于调用 CMS API，`tsx` 提供无需编译的 TypeScript 直接执行能力。
...
脚本通过 GitHub Actions 每日定时运行，确保 README 与 CMS 数据保持同步。为什么用每日定时而非 Webhook 触发？因为多语言 README 的更新对实时性要求不高，定时批量生成可以避免高频 Webhook 带来的并发与限流问题。
...
整条链路从打标签到 README 上线，最长延迟约 24 小时（取决于定时任务节奏）。Issue 在这里充当人类可读的投稿入口，CMS 充当机器可读的数据源，README 充当面向读者的产物，三者职责分离。
...
这种设计让提示词模板可以被快速迭代使用——把"产品名""标语""背景色"等高频变量参数化后，同一模板可以服务多个具体需求，无需重复编辑提示词正文。
...
支持的语言：英文、简体中文、繁体中文、日语、韩语、泰语、越南语、印地语、西班牙语（拉美）、德语、法语、意大利语、葡萄牙语（巴西/欧洲）、土耳其语。

每种语言的 README 都有对应的文件（`README_zh.md`、`README_ja-JP.md` 等），内容通过 CMS 中的 locale 字段区分。英文作为默认语言，中文版本（简体和繁体）由社区贡献翻译。这种"社区翻译 + 自动化生成"的模式把翻译工作量分散到社区，同时通过 CMS 的 locale 字段保证生成阶段的一致性。
...
4. 用 `{argument}` 把高频变量参数化，模板复用率会显著提升
