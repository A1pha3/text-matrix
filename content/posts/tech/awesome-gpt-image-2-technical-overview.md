...
仓库的核心产物是结构化数据（提示词 JSON Schema）和文本（多语言 README）。TypeScript 的类型系统在脚本层约束提示词字段，避免 CMS 数据格式漂移导致 README 生成失败。`@octokit/rest` 用于 GitHub Issue 交互，`node-fetch` + `qs-esm` 用于 CMS API 调用，`tsx` 提供无需编译的 TypeScript 直接执行能力。
...
1. 调用 `fetchPromptCategories()` 获取 CMS 分类体系
...
1. `type` 字段决定图像整体形态，选择准确的类型关键词是控制输出的第一步
2. `style` 字段影响艺术风格，应与使用场景匹配
3. 结构化参数（`header`、`layout`、`footer`）让提示词具备版式控制能力，减少随机性
...
1. **CMS 依赖**：README 自动生成依赖 CMS API，CMS 不可用时自动化流程中断，项目没有提供降级方案。
2. **提示词非代码**：项目收录的是提示词本身，不包含 API 调用代码，用户需自行接入 GPT Image 2 API。
3. **审核周期**：社区投稿需 48 小时审核，不适合热点内容的快速更新。
4. **内容合规性**：提示词来自社区，仅供教育目的，商业使用需遵守 CC BY 4.0 的署名要求。
...
- **AI 图像生成使用者**：直接用 Web Gallery 搜索比读 README 更高效；把高频提示词复制到 Raycast Snippets，配合 `{argument}` 参数化使用。
- **提示词工程师**：重点参考 JSON Schema 设计——`type` / `style` / `layout` 三层结构 + 动态参数，是把提示词从"自然语言"升级为"可工程化产物"的可行路径。
- **开源项目维护者**：可借鉴"GitHub Issue 投稿 + CMS 单一数据源 + 定时生成多语言产物"三段式架构，适用于提示词库、代码片段库、设计资源库等场景。
