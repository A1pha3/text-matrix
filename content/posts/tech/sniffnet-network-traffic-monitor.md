...
Sniffnet 是一款开源的网络流量监控应用，GitHub 标星数超过 36,000，Fork 数超过 1,400。定位是面向普通用户提供直观的网络流量可视化。
...
pcap 的 `snaplen` 设置为 65535，即最大捕获长度。对于正常的以太网帧（通常 <1500 字节），每个数据包都有充足的缓冲空间，同时不会因设置过大而浪费内存。
...
Sniffnet 是 Rust 桌面应用开发的一个成熟案例。Rust + iced 这套组合做到了 LTO 优化、异步并发、内存安全带来的高性能，Windows/macOS/Linux 一套代码同时保留各平台的差异化能力（进程关联、capability 管理），以及模块化架构、单向数据流、6000+ 条协议识别库支撑的可维护性。
