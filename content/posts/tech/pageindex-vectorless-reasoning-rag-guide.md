...
1. 解释为什么传统基于向量的 RAG 在复杂推理任务中存在瓶颈，以及 PageIndex 的推理型检索如何绕过这一限制。
2. 理解 PageIndex 的核心设计：不建向量索引、不做文档分块、直接利用 LLM 推理能力做上下文感知识别。
3. 掌握 PageIndex 的 MCP 协议集成方式和 API 调用方法，能够在自己的应用中添加 PageIndex 支持。
4. 判断 PageIndex 适合哪些场景，不适合哪些场景，以及它与普通向量 RAG 的取舍。
...
PageIndex 的核心思路是：既然 LLM 本身具备强大的上下文理解和推理能力，为什么不直接让 LLM 在检索时"读懂"文档结构，而不是依赖向量相似度？
...
PageIndex 代表了 RAG 领域的一个新方向：不是通过更精确的向量或更复杂的分块策略来改进检索，而是**将检索本身变成一个 LLM 推理任务**。这种思路在复杂推理、长文档理解、多语义场景下有显著优势，但代价是更高的推理成本和延迟。

它的关键价值主张是**简化 RAG 基础设施**——对于不想维护向量数据库、不想调优 Embedding 模型的团队，PageIndex 提供了一种"LLM 即检索引擎"的一体化方案。

如果你对 RAG 的传统范式已经熟悉，想探索下一代检索增强生成的方向，PageIndex 是一个值得关注的项目。
