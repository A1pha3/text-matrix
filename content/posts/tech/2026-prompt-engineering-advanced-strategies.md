...
---
> 提示词工程没死，只是长胖了。两年前讨论怎么写好一段话，现在要讨论一组配置：任务怎么定义、上下文怎么组织、工具什么时候启动和停下。措辞改到一定程度就封顶了，剩下的分数不在句子里面。
>
> 这套东西不是给所有 prompt 用的。一次性提问照旧直接写，它服务的是上线后会被反复调用的生产级 prompt——系统提示词、Agent 指令、评测模板。读完你得到的是一套定位问题层的顺序，而不是更多金句。
>
...
2. [系统全景：四个策略各在什么层级解决问题](#2-系统全景四个策略各在什么层级解决问题)
...
这篇笔记写给已经能把 prompt 写出「能跑的第一版」、却在反复调整中始终不稳的工程师。不要求先读过论文——文中提到的 ReAct、Chain-of-Thought、Lost in the Middle 都会在对应小节给出可查证出处；只需要熟悉 token、上下文窗口、工具调用这三个基础概念。
...
Anthropic 的 [context engineering 文章](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) 点明了这一点：上下文是有限资源，应该用尽量少但尽量高信号的 token 去完成任务。「少」指的是不要用低价值背景、重复的工具输出、陈旧的历史消息去挤占注意力预算。注意力预算有限，根子在 Transformer 的自注意力机制：每个 token 通过 query-key 点积与其他 token 计算注意力权重，上下文越长，单个 token 分到的注意力越分散。低信号内容推高的是分母，拉低的是高信号内容被注意到的概率——这是工程化解释，严格机制见 Transformer 原论文。缩减字数本身不解决问题，关键是区分硬约束和低信号背景——只删背景，不删约束。

### 1.4 动手调 prompt 之前，先把三样东西备好

Anthropic 的 [prompt engineering 概览](https://docs.anthropic.com/en/docs/intro-to-prompting) 给了三样前提：
...
让模型扮演「资深专家」很常见，但单角色有一个通病：输出经常表面稳重、实际保守——每个方案讲两句优点，最后落回「要结合业务场景综合判断」。这类回答在逻辑上挑不出错，却没法拿来决策。它没把关键冲突和代价展开。单角色容易保守，是因为模型在没有明确评价函数冲突时，会回避任何可能被判定为「片面」的取舍——这类圆滑回答在训练数据里最常见，模型自然往这个方向收敛。
...
拿 API 网关选型演示一遍：让「成本控制角色」（KPI：年度运维总费用）和「可靠性角色」（KPI：全年可用性 SLO）分别按自己的评价函数给出推荐，分歧很快会暴露一个评审会上容易被略过的前提——可靠性角色推荐的自建方案需要额外投入两名 SRE，成本角色推荐的托管方案在高并发下有额外限流费用。合理的结论往往落在折中方案：先用托管方案过渡，同步搭建自建方案的灰度环境，再根据监控数据决定切换时间。
...
### 4.1 压缩的目标是「硬信息密度」，字数只是表象

「压缩」容易被理解成删字数。但问题在于重要信息和次要信息混在一起——模型分不清哪些内容不能丢。

Compression Protocol 要做的事：把任务目标、成功标准、硬约束、禁止事项、输出要求、停止条件压成结构化锚点，并在长上下文里把最关键的一两条放到开头或结尾这类更容易被注意到的位置。

Anthropic 的 [context engineering 文章](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) 点出了两个直接相关的原则：系统提示应该清楚、直接，保持「最小但充分」的信息量；few-shot 示例应该挑代表性的 canonical examples，不要把所有边缘情况塞进去。这两条原则落到 Compression Protocol 上，就是把硬约束从背景里提炼出来、独立成块，让模型在长上下文里也能稳定看到。独立成块有效，是因为 Transformer 的注意力机制对每个 token 分配的权重相互竞争——硬约束混在背景段落里时，会被周围语义噪音稀释；独立成块后，约束更容易被稳定注意到，而不是埋在段落里靠运气。
...
这套模板依赖前面的结构化分层——硬约束先独立成块，重复锚点才有东西可重复。如果模板后面还要接很长的外部资料（检索结果、文档、日志），可以把【任务】和【硬约束】在资料末尾再写一遍：长上下文里，开头定下的锚点容易被中间内容冲淡，结尾重复相当于给模型第二次对齐的机会。上下文本来就短时不必重复，那只会制造冗余。
...
def react_loop(
    ...
    关键设计：
    - max_steps 防止无限循环（默认 10；正式环境建议收紧到 5-8，视任务复杂度调整）
...
骨架里四个关键设计：`max_steps` 兜底防止无限循环；`thought.is_final` 是停止规则的代码化表达，让模型主动收敛而不是被动超时；`evidence` 列表让每一步推理都能引用前序观察，形成可追溯的决策链；未知工具和调用异常都有处理路径。骨架按 Python 3.10+ 的类型标注语法写（`dict[str, Any] | None`），`llm_call` 与 `tools` 由调用方注入。生产环境还需要补上 token 用量监控、工具调用重试策略（建议 2 次指数退避）和一条熔断规则——单次循环 token 消耗超过阈值（比如 8000）时触发人工介入。
...
团队的第一反应几乎总是改句子。比如把「帮我优化首页」换成「请作为资深前端工程师深入优化首页性能和体验」。这种改写有时会改善风格，但「优化」到底指加载更快、转化率更高，还是交互更稳——这件事没定义清楚之前，模型仍然是在猜。

一份能用的规格至少回答 5 个问题：目标对象是谁、输出长什么样、绝对不能做什么、什么条件算完成、证据不足时该怎么办。这 5 个问题没写清之前，继续在写法层折腾很少会带来决定性收益。

写代码审查 prompt 的团队经常碰到这种事：反复调整措辞，输出质量始终不稳。回到规格层重写，把成功标准从「给出有用的建议」改成「每条建议必须附带代码位置引用和不少于一个具体改写示例」之后，输出通常会立刻收敛。这类问题的根因通常在成功标准是否可验证——可验证的标准让模型从「猜什么是好建议」变成「按字段填空」，措辞调整无法替代这种结构化约束。

### 6.2 上下文层——今天最容易被低估的故障源

在真实系统里，模型输入从来不只是 prompt 文本。它还包括检索片段、工具返回值、消息历史、系统状态、用户权限、缓存结果与中间记忆。任何一环变脏，最终输出都会漂移。

上下文层值得单独说，原因也在这里。你提供的信息越多，注意力竞争就越激烈；「能不能塞得下」早已经不是问题，问题在于哪些信息值得留下。
...
多数情况下，前三层走完，问题就已经露出来了。很多「prompt 失效」的根因在规格层或上下文层，写法层只是表象。

症状和层的对应关系，回到第 2 节那张速查表对照即可，这里不再重复。
...
### 7.1 反例：跳过规格层直接上 Expert Panel

顺序错了，四个策略各自再对也救不回来。一个常见反例是「代码评审助手」：第一版直接上 Expert Panel，让「架构师」「安全工程师」「性能工程师」三个角色讨论每一段代码。输出冗长、互相重复，开发者抱怨「读完不知道该改哪一行」。常见的下一步操作是再加第四个角色「平衡观点」，输出更长。
...
| **四层框架** | 前期诊断时间增加（几十分钟量级），团队需要统一术语 | 是否减少了盲目改写，是否更快定位责任层级 |

当一个策略的代价已经明显高于收益时，先退回更简单的方案；继续微调只会让代价继续累积。

## 9. 结语：下一步做什么

四个策略怎么用只是表面，更有用的是知道什么时候不该用。

两年前的 prompt engineering 指南翻开，大概率会教你写「你是一位资深专家」起手式。今天再翻开，写法还在，前面多了三层：先定义成功标准到可验证的程度，再组织上下文到高信号密度，最后把工具的启停条件写到不会失控。措辞没有归零，但有效区间收窄了——规格和上下文没对齐的时候，改措辞能补回来的差距通常很有限。

再画一条更务实的线：这套框架不要求每次写 prompt 都走完全流程。三行的一次性提问直接写就行，不需要搬出四层框架当仪式。这套方法的适用场景是**需要反复迭代的生产级 prompt**——系统提示词、Agent 指令、评测模板——这些一旦上线就会被反复调用，前期多花半小时做设计，能省掉后面一轮又一轮的调试和误判。

四条顺序不能随便换，原因在第 2 节和第 7 节已经讲过：规格没定就去制造冲突，等于放大模糊；上下文没清就去加工具，等于给噪音接上更多动作。顺序颠倒时，后面的策略反而帮倒忙。
