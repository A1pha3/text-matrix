...
>
> 提示词工程没有被淘汰。它只是从一段话扩展到了一组配置——任务怎么定义、上下文怎么组织、工具什么时候启动和停下。措辞变好能把 87 分推到 93 分，但剩下的分数不在句子里。
>
...
2026 年的提示词工程，变化不在理论，在工程。简单任务里措辞还是第一位的，但一旦任务涉及多轮交互、外部工具或长上下文，决定输出的就不再是「哪句话更像专家」，而是模型拿到了什么材料、这些材料排在什么位置、哪些约束被标成了不可违反。这两层合起来，重点落在三组判断上：哪些信息必须进上下文，哪些适合运行时按需取用，哪些动作需要被循环验证。

举个例子：「prompt 改了十版还不稳」——八成是没定义成功标准，或者上下文里塞了太多低信号信息。这时候继续找「最合适的那句话」，等于在错误的层面上反复改句子。
...
[Lost in the Middle](https://arxiv.org/abs/2307.03172) 讨论了一个很务实的问题：模型能接收更长的输入，但不会稳定利用每一部分。论文在多文档问答与键值检索任务中发现，相关信息出现在上下文中部时，性能常常低于出现在开头或结尾时。这个结论来自论文中的实验设置与评测模型，不能粗暴外推成「所有模型在所有长上下文场景都一样差」，但它指向了一个工程现实：**上下文长度增加不会自动带来等比例的信息利用率。**

Anthropic 的 context engineering 文章把这一现象说得更直白：上下文是有限资源，应该用尽量少但尽量高信号的 token 去完成任务。「少」指的是不要用低价值背景、重复的工具输出、陈旧的历史消息去挤占注意力预算。注意力预算有限的原因在于 Transformer 的自注意力机制——每个 token 通过 query-key 点积与其他 token 计算注意力权重，上下文越长，单个 token 分配到的注意力越分散，低信号内容会稀释高信号内容被关注到的概率（这是工程化解释，严格机制见 Transformer 原论文）。缩减字数本身不解决问题，关键是区分哪些信息是硬约束、哪些是低信号背景——只删背景，不删约束。

### 2.4 动手调 prompt 之前，先把三样东西备好

Anthropic 的 prompt engineering 概览给了一份很实用的前提清单：
...
这与 [Chain-of-Thought](https://arxiv.org/abs/2201.11903) 的边界需要单独画一条：CoT 把中间推理步骤展开，ReAct 把推理和外部行动交错起来。两者不互斥——CoT 偏一次性展开推理，ReAct 偏边思考、边观察、边修正。
...
骨架里四个关键点：`max_steps` 兜底防止无限循环；`thought.is_final` 是停止规则的代码化表达；`evidence` 列表让每一步推理都能引用前序观察；未知工具和调用异常都有处理路径。生产环境还需要补上 token 用量监控和工具调用重试策略。
...
ReAct 最怕的不是「不够主动」，而是「循环过头」——后者会直接把成本拉爆。一个能上线的 ReAct 工作流，至少要提前定义三件事：
...
很多 prompt 调不好，是因为团队连问题出在哪一层都没分清。有人一直改措辞，有人一直堆示例，有人一直换角色设定——但故障点可能在目标定义、上下文供给或业务意图，写法只是表象。
...
顺序错了，四个策略各自再对也救不回来。一个有过类似遭遇的团队：做「代码评审助手」，第一版直接上 Expert Panel，让「架构师」「安全工程师」「性能工程师」三个角色讨论每一段代码。输出冗长、互相重复，开发者抱怨「读完不知道该改哪一行」。团队的反应是再加第四个角色「平衡观点」，输出更长。
...
把一本 prompt engineering 指南摊开，会发现它跟两年前已经不太一样了。写法还在，只是前面多了三层——先定义成功标准到可验证的程度，再组织上下文到高信号密度，最后把工具的启停条件写到不会失控。一段精致的提示词曾经能拉起一个任务，但现在系统里同时跑着 Agent、多轮对话、长上下文和检索增强，决定结果的重心早就从那一段话移到了它背后的配置上。

措辞的价值没有归零，但区间收窄了。规格和上下文没对齐时，措辞调整的边际收益通常超不过 5 分——不值得把它当主要方向。等这几层都对齐了，措辞负责的是那把 87 推到 93 的精细活。

这篇文章里讲的东西，最终能落到日常工程里的判断力，拢共就四条：

1. 规格层有冲突时，主动制造角色分歧比等模型自己暴露权衡更省事。
2. 硬约束被噪音冲淡时，把任务目标和停止条件压成结构化锚点，比堆背景更有用。
3. 需要外部证据时，让模型边查边修正，比一次性猜对更可靠。
4. 结果不稳定时，先定位故障出在规格、上下文还是工具，再决定改哪里。

四条顺序不能随便换。第一条定义 "做完了" 是什么，第二条定义什么信息必须留在上下文里，第三条定义观察和修正的节奏，第四条是刹车——告诉你什么时候该停手诊断，而不是继续改句子。

当你能用四层框架定位问题、用 Compression Protocol 压出高信号上下文、用 ReAct 驱动工具循环、在需要的时候让 Expert Panel 暴露冲突——「prompt 怎么写」就不再是一个措辞问题。它变成了规格、上下文、工具边界和停止规则的配置问题。
