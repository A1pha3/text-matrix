...
骨架里四个关键设计：`max_steps` 兜底防止无限循环（正式环境建议根据任务复杂度收紧到 5-8）；`thought.is_final` 是停止规则的代码化表达，让模型主动收敛而不是被动超时；`evidence` 列表让每一步推理都能引用前序观察，形成可追溯的决策链；未知工具和调用异常都有处理路径。生产环境还需要补上 token 用量监控、工具调用重试策略（建议 2 次指数退避）和一条熔断规则——单次循环 token 消耗超过阈值（比如 8000）时触发人工介入。
