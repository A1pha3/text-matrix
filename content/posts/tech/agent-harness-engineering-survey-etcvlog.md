...
T 层的关键在于工具 Schema 的描述质量。协议本身只是载体，描述质量才决定模型能不能正确调用工具。一个描述模糊的工具，模型要么不敢调，要么调错参数；一个描述过细的工具，又会挤占上下文窗口。这是 T 层和 C 层的耦合点。
...
V 层有 21 个主项目，数量仅次于 L 层。SWE-bench、AgentBench、WebArena、GAIA 等评测体系都在这一层。但这里有个容易被忽略的边界问题：这些 benchmark 测的是模型 + 框架的联合表现，而非框架本身。同一个 SWE-bench 分数，可能来自更强的模型，也可能来自更聪明的上下文压缩——单看分数推不出框架质量。这意味着 V 层的工程价值在于建立可归因的回归反馈——哪次改动让哪个子任务的通过率掉了，是模型问题还是框架问题。"跑分"本身并不能反映框架质量。论文也指出，当前可观测性采纳广泛但离线评估少见，这个差距是 V 层的具体切入点。
...
在生产中，C 层压缩策略不当会让长任务"失忆"。典型表现是 Agent 跑到第 20 步时忘了第 3 步的关键约束（如"不要修改 tests/ 目录"），开始违反早期指令；另一种表现是压缩保留了高频但低价值的信息（如重复的工具调用日志），丢掉了低频但关键的事实（如用户的核心需求）。

### 3. Trace 原生故障诊断

Trace 应该成为系统计算结果分数、轨迹质量、失败归因和回归测试的主要对象，而非仅作为事后调试材料。当前可观测性采纳广泛但离线评估少见，这个差距是具体切入点。

实际项目中常见的症状是：团队"先跑起来再说"，等事故发生时发现没有 Trace、没有成本归因、没有失败率统计——复盘时只能靠日志猜测，无法定位是模型退化、框架 bug 还是上下文污染。
...
这类技术债的典型表现是：2023 年为 GPT-3.5 写的复杂重试逻辑、格式校验、思维链引导，到 2026 年模型已经能原生处理，但这些 wrapper 还在跑——增加延迟、挤占上下文、制造不必要的失败路径。
