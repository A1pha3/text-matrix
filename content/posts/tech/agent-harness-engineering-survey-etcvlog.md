...
T 层真正决定成败的是工具 Schema 的描述质量。协议只是载体：描述模糊的工具，模型要么不敢调，要么调错参数；描述过细的工具，又挤占上下文窗口。这是 T 层和 C 层的耦合点。

### C – Context & Memory Management（上下文与记忆管理）

C 层在论文的生态统计中只有 9 个主项目，数量最少。论文给出的解释不是它不重要：上下文和记忆通常嵌入在 L 层框架内部（如 LangGraph 的 state、AutoGen 的 memory），很少作为独立组件发布。技术形态上，模型在短时、会话级和持久化三个层面看到什么，对应长期上下文、上下文漂移缓解、状态持久化与恢复三类技术。
...
- **2023–2024 年（CAMEL/ChatDev/MetaGPT）**：多 Agent 角色分工，用"角色扮演"模拟团队协作，复杂度上去了，但调试困难。
...
Agent 跑了 30 秒的 `run_tests`，但没人知道是哪几个测试慢、是不是卡在网络拉依赖——这是 O 层缺位的典型症状。O 层要捕获的信号包括 Trace、成本、失败率和可靠性，形态上分 Trace 平台、Agent 专用运维工具、成本追踪、统一可观测性四类。

O 层在开源生态中只有 15 个主项目，更多出现在商业平台和 SDK 功能里。团队通常先跑起来再考虑观测，代价是等事故发生时往往没有 Trace 可查——这是 2025–2026 年 Agent 事故复盘时最常见的痛点。

### V – Verification & Evaluation（验证与评估）

V 层有 21 个主项目，数量仅次于 L 层。SWE-bench、AgentBench、WebArena、GAIA 等评测体系都在这一层。但有一个边界问题容易被忽略：这些 benchmark 测的是模型 + 框架的联合表现，而非框架本身。同一个 SWE-bench 分数，可能来自更强的模型，也可能来自更聪明的上下文压缩——单看分数推不出框架质量。V 层的工程价值因此落在建立可归因的回归反馈上——哪次改动让哪个子任务的通过率掉了，是模型问题还是框架问题。论文也指出，当前可观测性采纳广泛但离线评估少见，这个缺口恰好是 V 层的切入点。
...
Trace 应该成为系统计算结果分数、轨迹质量、失败归因和回归测试的主要对象，而不只是事后调试材料。当前可观测性采纳广泛但离线评估少见，这个缺口是最直接的切入点。

团队"先跑起来再说"的常见后果是：等事故发生时发现没有 Trace、没有成本归因、没有失败率统计——复盘时只能靠日志猜测，无法定位是模型退化、框架 bug 还是上下文污染。
...
采用顺序建议：先 E 和 T，再 L 和 C，最后 O、V、G。E 和 T 决定系统能不能跑起来；L 和 C 决定能不能跑长任务；O、V、G 决定能不能上生产。跳过 O/V/G 直接上生产，是 2025–2026 年 Agent 事故复盘里的常见路径。
...
- **C 层**：通常嵌入在 L 层框架内部（LangGraph state、AutoGen memory），不可导出。代价：换框架等于重写状态层，迁移成本极高。
- **L 层**：大多数框架有显式设计（循环、状态机、流水线）。这是七层里最成熟的。
- **O 层**：大多数框架缺位，或者只有简单的日志。代价：出问题时没有 Trace，无法定位是模型退化、框架 bug 还是上下文污染。
- **V 层**：部分框架有内置评估（如 CrewAI 的 Flow 验证），但回归测试通常需要外接 benchmark。代价：升级框架或模型后，无法快速判断是不是引入了回归。
...
- L 层：用 LangGraph 或 AutoGen 搭显式状态机，替换掉 ReAct 式的隐式循环。
...
**第 3 阶段（6-12 个月）：O + G**
- O 层：接入 Trace 平台（如 LangSmith、Helicone），记录每次工具调用的延迟、成本、失败率。
- G 层：引入 AGT（Agent Governance Toolkit）或自建策略引擎，在代码层拦截越权操作。
...
| 层 | LangGraph | AutoGen 0.4 | CrewAI |
|---|------------|---------------|--------|
| E | 依赖外部（Docker、Fly.io） | 内置 Agent Runtime | 依赖外部 |
| T | MCP 支持（0.4+） | MCP 支持（0.4+） | 私有协议（工具装饰器） |
| C | state schema（TypedDict） | memory 模块（可插拔） | 隐式（存在 Agent 属性里） |
| L | 显式状态机（StateGraph） | 显式状态机（Orchestrator） | 隐式流水线（Flow） |
| O | LangSmith（商业） | 内置 Trace | 第三方集成 |
| V | 外接 benchmark | 外接 benchmark | 内置 Flow 验证 |
| G | 外接（如 AGT） | 内置安全护栏 | 外接 |

迁移成本最高的层：
- **C 层**：每个框架的 state schema 不兼容，换框架等于重写状态层。
- **L 层**：每个框架的状态机抽象不兼容（StateGraph vs Orchestrator vs Flow），换框架要重写编排逻辑。
- **T 层**：如果用了私有协议（如 CrewAI 的工具装饰器），换框架要重写工具适配层；如果用了 MCP，可以复用。
...
3. **接一个 O 层的 Trace 平台**。当 Agent 日均调用量超过 100 次时，接入 LangSmith 或 Helicone，记录每次工具调用的延迟、成本、失败率，为后续优化提供数据。
4. **建一个 V 层的回归测试套件**。当 Agent 开始进入生产环境时，建 SWE-bench 风格的回归测试，确保升级模型或框架后不引入静默回归。
5. **补一个 G 层的策略引擎**。当 Agent 持有高风险工具权限（如 `drop_table`、`send_email`、`execute_code`）时，引入 AGT 或自建策略引擎，在代码层拦截越权操作。
...
本文基于 *Agent Harness Engineering: A Survey*（CMU、耶鲁、Tulane、UAB、Virginia Tech、东北大学（Northeastern）、Stanford 与 Amazon 等机构联合团队）编写，论文于 2026 年 5 月 14 日在 OpenReview 发布（ID: eONq7FdiHa），最后一次修改为 2026 年 5 月 15 日。

以下信息会随时间变化：

- ETCLOVG 七层的定义和边界可能在后续版本中调整，以 [OpenReview 页面](https://openreview.net/forum?id=eONq7FdiHa) 的最新版本为准。
- Awesome-Agent-Harness 开源项目目录会持续更新，各层项目数可能变化，以 [项目 GitHub 仓库](https://github.com/Picrew/awesome-agent-harness) 的最新提交为准。
...
- ETCLOVG 与 Anthropic 的 Agent Skills、Microsoft 的 AGT 的具体集成方式。

### 争议与不同意见

- ETCLOVG 的七层划分存在争议，部分研究者认为 O（可观测性）和 V（验证与评估）的边界不够清晰，在实践中经常混在一起。
- "Harness Engineering"作为一个独立工程阶段，是否存在争议：部分团队认为 Context Engineering 已经覆盖了大部分 Harness 问题，不需要单独拆出一层。
- 开源生态的项目编码方法存在主观性：某些项目同时跨越多个层，编码时的主观判断会影响统计结果。
