...
draft: false
categories: ["技术笔记"]
tags: ["Claude Code", "AI 编程", "gstack", "YC", "开发工具"]
---
...
本文从设计理念、架构解析、技能速览到安装配置，对项目做一次完整解读。
...
ngrok 只转发隧道监听器的端口。即使远程攻击者拿到了隧道 token，也无法访问本地监听器上的 `/health` 或 `/cookie-picker`。物理端口分离，比 HTTP Header 检查可靠得多。
...
代码里存在的命令，必然出现在文档里；代码里不存在的命令，文档里不会出现。CI 可以用 `gen:skill-docs --dry-run` + `git diff --exit-code` 检测文档与代码的漂移。
...
gstack 解决的根本问题是：**AI 编程工具目前太像助手，不够像团队**。
