...
author: "钳岳"
canonical: "https://txtmix.com/posts/tech/apache-maka-local-first-agent-workspace/"
description: "Apache 孵化器项目 Maka 把 Agent 执行记录当作一等公民：模型消息、工具调用、权限决策与终止事件全部落为只追加日志，桌面端、CLI、Bot 与评测共用同一个 Runtime Host。本文拆解其架构、一次任务的完整流转与上手路径。"
draft: false
categories: ["技术笔记"]
tags: ["AI Agent", "Apache", "开源", "架构", "本地优先"]
---
...
1. **测出来，而不是说出来（Measured, not claimed）。** Maka 与其他 Agent harness（执行体）在同一模型、同一官方验证器（verifier）下对测，逐任务结果随报告一起发布在仓库 `docs/eval/` 目录。这是当前 README 摆在第一条的卖点：性能靠公开的运行记录说话。
2. **日志即运行时（The log is the runtime）。** 每条模型消息、工具调用、权限决策与终止事件都是只追加的 RuntimeEvent。UI、下一条 prompt、崩溃恢复都只是这份日志的投影，日志之外没有唯一副本。
3. **你的机器，你的模型（Your machine, your model）。** 会话、设置与运行记录留在本机，模型连接由用户自带。
4. **一个执行权威（One Runtime Host）。** 桌面端、TUI/CLI 与评测都是同一执行权威的瘦客户端；评测只拥有实验与分数，不另起一套执行环境。
...
ARCHITECTURE.md 的拓扑在这条链上补了三笔：入口一栏多了 Bot；SessionManager 之下挂了 Agent Graph 控制平面，用子 Session 调度有依赖的工作；投影一栏多了 Recovery，崩溃恢复同样从日志投影而来。更结构性的变化是状态根（State Root）的引入——每个 State Root 有自己的 Runtime Host 作为执行与写入权威，多个 Host 可以各自持有不同的 State Root，Peer Mesh 负责端点成员关系与连接，但不合并各自的执行权威。分布式是多 Host 各管各的日志，不是把执行权威摊开。
...
一次性调用可以走 `npx --yes --package maka-agent@nightly maka`。CLI 的发布门禁在 Linux x64/arm64、macOS arm64 与 Windows x64 上验证安装包矩阵（Node.js 22.19 起，重点验证 24）。
...
TUI 里也可以用 `/graph on`、`/graph off`、`/graph <task>` 切换 Graph 模式；非交互的 `--graph` 运行会等持久化 Graph 跑完再打印最终 supervisor 输出。`--graph` 的实现算子使用隔离的 Git worktree，因此要求源项目是干净的 Git 工作树。注意开发 CLI 用 `Maka Dev` profile，与发布版 `maka` 二进制的 `Maka` profile 不互通。
...
契约（core）与实现（runtime/storage）分离，桌面与 CLI 共享同一套包——这是"多个入口一条脊柱"在代码层的体现。`runtime-host` 把执行内核收敛为单一持有者，`mcp`、`computer-use` 作为独立包接入，`native` 把网络与 Git 存取下沉到 Rust，避免能力长进主进程里。
...
- 架构总览：仓库内 `ARCHITECTURE.md`（系统地图、代码边界与 `docs/architecture/` 下的深潜文档，多数中英双语）
- 产品设计：仓库内 `DESIGN.md`；安全策略：`SECURITY.md`
- 中文文档：仓库内 `README.zh-CN.md`
- CLI 指南：`packages/cli/README.md`（npm 安装与 dist-tag 说明）
