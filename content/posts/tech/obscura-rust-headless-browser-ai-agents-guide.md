...
Obscura 是 Rust 生态中一个值得关注的无头浏览器项目。它的关键价值在于：以远低于 Chrome 的资源占用，实现了同等的 JS 执行能力和 CDP 协议覆盖，同时内置了反指纹和追踪屏蔽能力。对于需要大规模浏览器自动化的 AI Agent 项目，以及对性能敏感的网络爬取场景，Obscura 提供了一个轻量级的替代方案。
