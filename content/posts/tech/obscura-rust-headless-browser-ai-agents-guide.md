...
source_key: "gh:h4ckf0r0day/obscura"
...
`Fetch` 域的完整实现——这意味着 Obscura 支持对 HTTP 请求和响应进行实时拦截、修改、替换或主动失败，是做反向代理测试或数据脱敏的强大工具。
...
Obscura 是 Rust 生态中少见的无头浏览器项目。它做的事很明确：以远低于 Chrome 的资源占用，实现同等的 JS 执行能力和 CDP 协议覆盖，同时内置了反指纹和追踪屏蔽能力。对于需要大规模浏览器自动化的 AI Agent 项目，以及对性能敏感的网络爬取场景，Obscura 提供了一个轻量级的替代方案。

如果你正在用 Puppeteer 或 Playwright 构建爬虫或 Agent，可以试试把底层浏览器换成 Obscura——多实例并行场景下的内存和启动速度差距最明显。
