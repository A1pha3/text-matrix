...
**第 12 章未来趋势**：具身智能、模型式强化学习、自我博弈、多智能体、离线 RL、规模化趋势。最后这章不是泛泛的趋势预测，而是对每个方向的论文脉络和技术瓶颈做了整理。
...
传统 RL 的训练循环是"环境 → 状态 → 动作 → 奖励 → 下一状态"。Agentic RL 在这个循环上叠了一层：动作空间不再是离散或连续的动作向量，而是自然语言输出 + 工具调用，奖励也不是环境自动返回的标量，而是需要 LLM-as-Judge 或结构化验证器（如测试用例通过率）来生成。这意味着 Agentic RL 的训练稳定性远比传统 RL 脆弱——课程第 10 章花了不少篇幅讨论这个问题。
