...
CopilotKit 解决的不只是"做一个聊天框"，而是**让 Agent 成为应用的一等公民**——传统 chatbot 只能输出文本，CopilotKit 让 Agent 能渲染 UI、操作状态、暂停等待输入。
...
**协议级采纳 vs 单一产品**：即使你不用 CopilotKit 的 UI 库，只要你的 Agent 实现 AG-UI 协议，也可以被任何支持 AG-UI 的前端消费。CopilotKit 在赌的是**协议生态**。
...
**这是仓库工程化的一个范本**：**自描述的 monorepo**——仓库本身就是 AI Agent 学习的知识源，代码之外还有完整的使用路径。
