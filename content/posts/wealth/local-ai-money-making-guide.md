...
5 月 10 日，unix.foo 的文章 Local AI Needs to be the Norm 登上 Hacker News 首页。到我整理这篇文章时，讨论页大约有 518 分、248 条评论。原文里有产品意义的判断，藏在一句容易被跳过的观察里：很多 AI 功能，本来只是应用里的一个小能力，却被做成了依赖外部模型、账单、日志留存和后端兜底的分布式系统。
...
这也是原文在苹果生态里强调系统模型接口和结构化输出的原因。对产品来说，"模型能稳定产出界面能直接消费的字段"才是关键。一旦输出可预测，AI 功能就更像一个子系统，而不是一个随时失控的聊天框。边界说明：原文展示的工具链主要成立于苹果生态，换到 Windows、Linux 或跨平台应用，工程难度通常会高不少。
