...
1. 手算任意相关结构下的组合 IR，并解释 √N 为什么会被相关性吃掉；
2. 拆解一篇量化论文或一份业绩报告时，立刻定位它有没有做过因子剥离、归因是否干净；
3. 用三个问题判断一个量化团队到底是"配方作坊"还是"工厂"。
...
三条线之间只有一个变量：信号之间的两两相关性 ρ。组合 IR 的一般式是：
...
真正扎眼的是后两条曲线：相关性一旦大于零，组合 IR 随 N 增加趋于一个平台，N→∞ 时的极限是：
...
最后一组数字最扎眼：30 个 IR 0.4 的信号，相关性只要 0.5，有效 N 就从 30 塌缩到 2 左右，组合 IR 还不如一个普通的单一信号。你以为自己持有一个多元化的信号组合，实际上持有的是同一个赌注的三十份复印件。
...
把 2007 年 8 月 6 日-17 日这段时间（全球 quant 拥挤交易踩踏的高峰期）拿出来单独看：多只量化基金在几天内亏损 5%-30%（[Khandani & Lo 的复盘论文](https://web.mit.edu/alo/www/Papers/august07.pdf)记录了一批历史上盈利最稳定的量化多头空头基金正是这批受伤最重的），而它们之间股票持仓的相关性从年初的 0.18 飙到 8 月 9 日的 0.83。

把这件事写成算术：假设你有 50 个因子信号，年初两两相关 0.18（教科书级别"多元化"），组合 IR 应该是 $0.4 \times \sqrt{50/(1+49 \times 0.18)} = 0.4 \times \sqrt{5.09} \approx 0.90$。但到了 8 月压力日，相关性瞬间变成 0.83，组合 IR 跌到 $0.4 \times \sqrt{50/(1+49 \times 0.83)} = 0.4 \times \sqrt{1.20} \approx 0.44$——**缩水超过一半**。而回撤幅度常常是 IR 收缩的 3-5 倍，叠加杠杆后足以击穿风控线，这正是 2007 年 8 月那批高杠杆 quant 基金被迫清盘时看到的量级。
...
读过《神经网络的四重境界》（[Wayland Zhang 的原文](https://www.waylandz.com/blog/four-realms-of-neural-networks/)）的读者，可能已经闻到什么。那篇文章一境一境地往上读神经网络：方程、流形、联络、测量。现在把同一双眼睛转向市场——你会发现本文的三条纪律不是三条独立的经验法则，而是同一套几何在金融里的显形。逐境走一遍。
...
**工厂型系统**是这样工作的：候选信号从数据表面被系统性地枚举出来，每一个都走完全相同的验证流程——因子剥离、样本外、净成本、尾部相关——绝大多数被淘汰，极少数进入候选池，等待人的最终准入决策。信号会衰减、会被淘汰，但**生产线本身在复利**：每一轮筛选留下的不只是幸存者，还有"这一族方向已被证伪"的负知识，让下一轮枚举更聪明。这种组织形态，就是本文所说的工厂模式（factory）。
...
3. **他们的验证规则在看到数据之前就冻结了吗？** 工厂：评估窗口、阈值、成本假设全部预注册，哈希进账本；作坊：研究员发现候选在某个窗口表现更好，调整窗口重新跑——这就是过拟合的标准操作。
...
把前八节串成一根链条：物理（信号的几何独立性把 √N 钉死在相关性矩阵上）把配方的强度逼出场景；工程（因子剥离、尾部检验、正交化）把单一策略的兑现节奏拉长；四重境界把工厂的几何直觉还原成可执行的步骤；工厂把研究从灵感变成纪律；垂直整合是工厂落地的架构前提——2026-2028 三年窗口里，最值得跟踪的不是哪只基金今年赚了多少，而是哪家工厂的产能曲线第一次和人头数解耦。
...
1. **从等权到最优权**：Grinold & Kahn《Active Portfolio Management》的组合构建章节；Black & Litterman（1992）的原始论文，看主观观点如何进均值方差优化。
2. **过拟合与统计陷阱**：López de Prado《Advances in Financial Machine Learning》里 Deflated Sharpe Ratio 与回测过拟合的章节——这是第六节"预注册"纪律的学术依据。
3. **流动性与执行**：Pedersen《Efficiently Inefficient》及其论文 "When Everyone Runs for the Exit"——"陆地神仙境"的市场版本在那里有更完整的处理。

站内相关阅读：

- {{< postsRef "quant-weak-signal-aggregation-and-signal-factory" "弱信号聚合与信号工厂：用数学和工程构建量化交易的护城河" >}}——本系列第三篇，本文的骨架来源。
- {{< postsRef "mathematical-foundations-deep-learning-agi-chern-simons" "AGI 的数学基础到底是什么：从流形假设、对称性到信息几何" >}}——"四重境界"框架在深度学习语境下的校准版本。
...
- 2007 年 8 月 quant unwind 复盘：[Khandani & Lo (2007)](https://web.mit.edu/alo/www/Papers/august07.pdf)；Pedersen (2009)，"When Everyone Runs for the Exit"。
- 组合优化与收缩估计：Black-Litterman 模型；[Ledoit & Wolf (2004) 协方差收缩估计](https://doi.org/10.1016/S0047-259X(03)00096-4)。
- 因子模型起点：[Fama & French (2015) 五因子模型](https://doi.org/10.1016/j.jfineco.2014.10.010) + Carhart 动量因子，是第 2.5 节因子库的学术参考。
- 数值流形法（NMM, Numerical Manifold Method）：计算力学的经典方法，用于第五节"用几何读市场"的类比基础。
- 四重境界框架：[Wayland Zhang，The Four Realms of Neural Networks](https://www.waylandz.com/blog/four-realms-of-neural-networks/)。
