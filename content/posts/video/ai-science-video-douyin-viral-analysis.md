...
> **学习目标**：拆解一个现象——某种固定格式的 AI 科普视频，为什么能在抖音 10 天涨粉 5.7 万，并把可复制的部分提炼出来。下面各级标题即目录（TOC）：先看视频与数据（概述），再拆主题（一）、知识管理工具（二）、创作公式（三）、平台算法（四）、涨粉变现（五）、可复制要素（六），最后收束（七）。
...
- **「复刻 = 把爆款视频抄一遍」**——不对。可复制的是**选题逻辑 + 结构模板 + 开头钩子**，而不是具体文案；照抄内容只会被算法当搬运。
- **「涌进流量靠辐发时间」**——不对。决定能不能进下一个流量池的是**完播率与互动率**，不是发布时间本身。
- **「工具能替代选题」**——不对。BiliSummary 这类工具提高的是**整理与复盘效率**，优质选题与表达依然稀缺。
...
- [bilibili-api-python](https://github.com/Nemo2011/bilibili-api)：文中用于视频下载 / 字幕获取的开源库
- [FastAPI](https://fastapi.tiangolo.com/) ｜ [pywebview](https://pywebview.flowrl.com/)：BiliSummary 后端与桌面壳所用框架
- [PyAV](https://github.com/PyAV-Org/PyAV)：FFmpeg 的 Python 绑定，用于音频处理
