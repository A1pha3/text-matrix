...
缺少基础功的开发者的做法是："帮我写个用户模块。"AI 返回一套看起来能用的混乱 CRUD，可扩展性、安全性、错误处理全都欠考虑。有基础功的开发者会这样描述："实现一个用户注册流程，包含邮箱验证、密码强度校验（8 位以上，大小写加数字），用 Strategy 模式（策略模式）支持多种验证方式，Repository 模式（仓储模式）隔离数据访问。"AI 生成的代码层次清晰、职责分明，可以直接进 review。
