...
Claude Mythos Preview 在安全研究上表现突出，背后是三个能力的叠加：编程能力（代码理解→安全漏洞识别）、自主性（独立调查→漏洞链构建）、长周期任务执行（持续分析→深度挖掘）。Anthropic 没有专门训练它做网络安全，但编程能力的外溢效果让它在这条路上也走得远。
...
AI 辅助安全研究的流程正在跑通：先让模型对目标代码库做大规模扫描，找出潜在漏洞；再让模型自动关联分析，把多个漏洞串成攻击链；最后通过负责任披露流程通知维护者、等待修复、协调发布。高危漏洞优先通知、加速修复；常规漏洞走标准披露流程。
