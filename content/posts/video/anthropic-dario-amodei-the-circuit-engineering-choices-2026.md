...
Bloomberg 主持人 Emily Chang 直接问"你们内部怎么决定哪些模型可以发布、哪些要锁起来"，Dario 的回答是 Mythos 因为能自主走完 cyber kill chain（网络杀伤链，从漏洞发现一路到武器化利用的完整攻击链路）被锁——这是 model-level catastrophic risk（模型级灾难风险）评估的具体输出。八个取舍都按这条标准展开：信号是什么、判断是什么、代价是什么。
