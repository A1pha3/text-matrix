...
> **难度**：⭐⭐⭐⭐
>
> **幻灯片 PDF（43 页）**：https://louiskirsch.com/assets/louis_at_the_recursive_workshop_ICLR_2026.pdf
> **SlidesLive 录像**：https://slideslive.com/39063672?t=1425
> **B站转载**：https://www.bilibili.com/video/BV1YRVy6nEzT/
> **RSI 2026 Workshop 主页**：https://recursive-workshop.github.io/
>
> 演讲者 Louis Kirsch —— IDSIA 博士（导师 Schmidhuber）；前 Google DeepMind「AI Scientist」团队创建者与负责人；现 Inherent 联合创始人兼 Chief Superintelligence Officer。时长 27:57，现场 2026-04-26，ICLR 2026 RSI Workshop 首场受邀报告（Invited Talk 1，里约热内卢 101-D 厅）。
>
> **写作依据，以及一处必须先讲的更正**：本文依据 Kirsch 个人站公开的 43 页幻灯片原件（已逐页核对文本）、Workshop 官网、Kirsch 个人站与相关 arXiv 论文写成，未拿到逐字字幕，所有口头引用为据幻灯片的综合转述。此前网络上流传过一套"RSI 五判据"——可修改范围、可信评估、可持续增长、资源有界、状态可读写，并说它出自这场演讲。**43 页幻灯片里没有这五条。** 演讲里真正的那张清单是"如何保证改进"的五类机制（见第四节），不是"五判据"。

---

这场报告没有推出新算法。它做的是把"递归自我改进"从一句口号拆成三个可以分头讨论的工程问题：把人移出外层循环、保证每次改动是真改进、以及拐点之前那堆没解决的开放难题。

---

## 一、这场演讲到底在讲什么

Kirsch 开场先抛了一张 Schmidhuber 的老图。把人类历史上的重大里程碑排在时间轴上，它们大致落在一条指数曲线上，外推下去，收敛点落在 2040 年前后——Schmidhuber 半开玩笑地把那个收敛点记作 Ω，也就是超级智能。这张图不是论证，是一个坐标：它把整场演讲要谈的东西，钉在"通往 Ω 的最后一段路由什么驱动"这个问题上。
...
有个观点最容易被断章取义，值得单独拎出来：**他没有说逃逸速度已经发生**。幻灯片里的原话恰恰相反——当前的 RSI 系统"过早地停止改进，仍然需要人类介入"。他所谓的逃逸速度，是那个"人类不再需要亲自推动进步"的拐点；他的判断是我们正在逼近它，不是已经越过它。整场报告的张力就在这里：技术栈在快速就位，可临界点还没到，中间缺的东西恰恰是最难的。
...
| RSI 定义：把人移出外层循环 | 第三节 |
| "如何保证改进"的五类机制 + FME | 第四、五节 |
| 逃逸速度：还没跨过的拐点 + 怎么度量 | 第六节 |
| Lessons Learned + Open Problems | 第七、八节 |
...
Kirsch 给 RSI 的定义很短：一个系统对自己做研究，产出一个更强的自己（幻灯片原话是 "a system that does research on itself to create an even better version of itself"）。关键在"更强"怎么理解。他画了一道梯子：最底下是人类手写更新规则，往上一层是元学习（meta-learning，系统学怎么更新自己），再往上是元-元学习（系统学"怎么学会更新自己"）……每往上爬一层，就从外层循环里拿掉一点人类的参与。RSI 就是把这道梯子推到底——用他幻灯片上的话说，目标是"最小化对人类工程的依赖"，一个 RSI 系统"能无界地改进自己的能力，且理想情况下无需人类介入"。
...
名字不需要全记住，记住这层门槛就够：RSI 要的不是"变强"，是"变得更能变强"。

---

## 四、真正的清单：怎么保证"自我改进"不是在自欺

这场报告里最该被截图的那张幻灯片，标题叫 **Guaranteeing improvement**（如何保证改进），出处标着 Kirsch 博士论文《Automating AI Research》第 7 章。它回答的问题很具体：一个系统在改自己，你凭什么相信每次改动真的是改进？Kirsch 把已有的答案归成五类，每一类都是一种"用什么代价换什么保证"的赌注：
...
五行从上往下，其实是同一根轴上的滑块：**越往上越严格、越往下越实用**。Gödel Machine 给你数学级别的保证，代价是它在现实里几乎跑不起来；STOP 什么都不保证，换来的是立刻能用。中间那三档——回滚、进化、按 fitness 分配算力——才是 2025 年真正在跑的东西，因为它们放弃了"每一步都可证明"，改成"允许犯错，但留一套机制把错的收回来"。
...
面对一个号称会自我改进的系统，别问它"聪不聪明"，问它这一条：**你落在表里哪一档？** 停在"不做保证"那档的系统，跟一个会不小心把自己改崩、还没有 Ctrl+Z 的脚本没有本质区别。这张表，而不是那套并不存在的"五判据"，才是这场演讲真正留下的清单。

---

## 五、FME：Kirsch 自己的赌注

上面那张表的最后一行——按 fitness 分配算力——是 Kirsch 自己押的方向，全名叫 **Fitness Monotonic Execution（FME，适应度单调执行）**，出自他和 Schmidhuber 2022 年那篇 [Eliminating Meta Optimization Through Self-Referential Meta Learning](https://arxiv.org/abs/2212.14392)（arXiv 2212.14392），博士论文第 7 章有完整版。
...
但 FME 有一道它自己迈不过去的坎，Kirsch 在演讲里也坦率地把它单拎出来问了：**这个 fitness / reward 到底从哪来？**（幻灯片原话：What is the reward, e.g. in FME? Where does this come from?）FME 解决的是"拿到 fitness 之后怎么保留"这一层，它默认 fitness 是给定的。可在真正开放的自我改进里，没人从外面递给你一个可信的分数。

---

## 六、逃逸速度：一个还没跨过的拐点

演讲的正式标题就是"逃逸速度"，Kirsch 用这个物理隐喻收束整场：

- 引力 ≈ 人类驱动研究的那层阻力——读论文的速度、设计实验的带宽、验证一个想法的成本；
- 火箭速度 ≈ "AI 改 AI"这个反馈环的转速；
- 逃逸速度 ≈ 反馈环快到能自我维持的那个临界点，过了它，进步不再需要人类亲自去推。

限定词还是第一节那一个：Kirsch 说的是**还没到**。幻灯片里 "Thesis: Escape velocity" 连占两页，分工明确——一页摆现状，当前 RSI 系统"过早地停止改进，仍然需要人类介入"（p20）；一页给定义，逃逸速度是"人类不再被需要来交付进步"的那个拐点（p21）。他的判断是方向性的：逼近，而非越过。至于何时越过，他没有给时间表。
...
报告中段有一节就叫 "Lessons Learned"，是 Kirsch 从自己下场做 AI Scientist 的经历里拧出来的三条。

**一，基座模型的分量压倒一切（Base models matter a lot）。** 底座模型的能力，很大程度上决定了整个自我改进循环的天花板。你没法靠外面那层脚手架，把一个弱底座"救"成强系统。
...
报告的最后三分之一叫 "Open Problems"，也是最实在的一段——全是当前没人能完整回答的硬问题。
...
> "How can we craft rigorous benchmarks for a system that controls its own reward?"

这句话和 FME 的"reward 从哪来"是同一道题。区别在于，在演讲里它是一个待解的开放问题，在 Inherent 那里成了立身之本的赌注。B 站转载给这场演讲起的标题是"递归式自我提升：回顾和展望"——"展望"两个字真正的落点在这里：Kirsch 不是在预测别人会怎样，他把自己接下来几年押在了"组织层的集体 RSI"上。

---

## 十、常见误读

这场报告被转述得很多，几处走样传得很广。对着幻灯片逐条校一遍：

- **误读一：它宣布逃逸速度已经发生。** 恰好相反。Kirsch 的论点是当前 RSI"过早停止改进、仍需人类介入"，拐点在前方，不在身后。
- **误读二：演讲给了一套"RSI 五判据"。** 43 页幻灯片里没有这五条。演讲里真正的那张清单是第四节的"如何保证改进"（不做保证 / 形式化证明 / 回滚 / 进化 / 按 fitness 分配算力）。
- **误读三：Kirsch 反复引用 FunSearch 和 AlphaEvolve。** 这两个不在他的幻灯片里；他举的例子是 STOP、Darwin Gödel Machine、GPTSwarm、ADAS、Sakana AI Scientist、MLGym、EvoTune。
...
**做 AI Agent 工程的**：把第四节那张"如何保证改进"的表当 checklist 用。现在大多数 agent 框架其实停在最上面那档——"不做保证"：能自己改提示词和工具，但改坏了没有回退。往 RSI 挪一步，先补上"回滚有害改动"（Success Story Algorithm 的思路，留检查点），再接一个可信的 benchmark 闭环（Darwin / Huxley-Gödel Machine 的做法）。别一上来就追"让 agent 改自己权重"，那是更靠后的事。排查时先定位你的循环里 `guarantee_improvement` 落在表里哪一档，缺回退就先补回退，而不是继续加工具。
