...
draft: false
description: "硅谷101 对话叶奇意（Kiwi）：从亲历美团龙珠押注月之暗面 A1 轮，到拆解 Kimi K3 的技术分量、两代「AI 四小龙」之间的人才迁徙，以及算力受限下中国 AI 的出路讨论。技术数据已对照 Kimi-K3 官方仓库核实。"
...
cover: "https://i0.hdslb.com/bfs/archive/31ca77bfb84d20fafde64c49c18026a6ed3138bf.jpg"
---
...
**CV 时代的亲历者。** 商汤、旷视、依图、云从四家公司在 2014 到 2019 年间先后崛起，以 CV 技术为核心，一度代表中国 AI 的最高水平：商汤（SenseTime）一度是全球估值最高的 AI 独角兽，旷视的 Face++ 在人脸识别领域名噪一时，依图在医疗影像和金融领域布局广泛，云从在安防和航空场景深耕。
...
MXFP4 量化是另一个容易被忽略的细节。MXFP4 / MXFP8 属于微缩浮点（microscaling floating point）格式，用更少的位宽存储权重和激活值。K3 从 SFT（监督微调）阶段就开始做量化感知训练——量化不是部署时对成品的事后压缩，而是训练时就让模型适应低精度运算——部署时可以用 4 bit 权重和 8 bit 激活值运行，官方的说法是"广泛的硬件兼容性"。对一个 2.8T 参数的模型来说，能跑和跑得起是两个完全不同的问题。

### Benchmark 表现

K3 在多个维度上与全球顶尖闭源模型正面竞争。官方对照表还包含 Claude Opus 4.8 和 GPT-5.5 两列，下表摘出四个有代表性的对手：
...
SWE-Marathon 上的表现值得展开。它测试模型在长时间、多步骤软件工程任务中的持续表现，要求模型像真实工程师一样在大型代码库中工作。K3 的 42.0 分领先 Claude Fable 5 七个百分点，领先 GPT-5.6 Sol 三个百分点。这个领先要打些折扣地读：K3 与 Claude Fable 5 都用 Claude Code harness 评测，GPT-5.6 Sol 用 Codex harness；官方脚注同时说明，Fable 5 在 35% 的任务上命中 fallback，可能拉低了它的实测分数。

FrontierSWE 上 K3 拿到 81.2，低于 Claude Fable 5 的 86.6，但远超 GPT-5.6 Sol 的 71.3。Agents' Last Exam 是综合智能体能力的测试，K3 的 28.3 分仅次于 GPT-5.6 Sol 的 29.6，超过 Claude Fable 5 的 25.7——后者在榜单上带一个脚注标记：该成绩在 xhigh 推理档位下取得，且 40% 的任务被标注为降级。
...
美国对中国的芯片出口管制始于 2022 年 10 月，经过多次升级后，英伟达面向中国市场能出口的最高端 GPU 是 H20。H20 的算力约为 H100 的 15%-20%（具体取决于计算精度和工作负载类型），但内存带宽不降反升——H20 配 4.0 TB/s 的 HBM3 显存，高于 H100 SXM 的 3.35 TB/s，这是英伟达为大模型推理场景做的取舍。算力落差意味着在相同预算下，中国 AI 公司能获得的训练计算能力远低于美国同行；带宽优势则部分解释了为什么 H20 上跑出来的推理评测仍有参考价值。K3 技术报告把 SWE-Marathon 的评测环境从 H100 重新校准到 H20，就是这种差距落在具体产品上的一个注脚。
...
1. **K3 是全球首个开源 3T 级别模型，在 H20 降配 GPU 上达到了与 Claude Fable 5、GPT-5.6 Sol 正面竞争的水平。** SWE-Marathon 42.0 分领先 Claude Fable 5 七个百分点——不过官方脚注注明 Fable 5 在该榜 35% 的任务上命中 fallback，分数可能被低估；KDA + Stable LatentMoE 带来约 2.5 倍扩展效率提升，MXFP4 量化感知训练降低部署门槛。算法创新可以在一定程度上弥补硬件差距。
...
本文的模型规格、benchmark 分数与 H20 校准脚注，已于 2026-09-23 对照 [Kimi-K3 官方仓库 README](https://github.com/MoonshotAI/Kimi-K3) 逐项核实。视频信息卡中的播放量、粉丝数与互动数据为发稿时抓取的动态数值。嘉宾原话目前均以视频简介和弹幕线索间接还原，如硅谷101 后续放出完整字幕或文字稿，核实后会重新修订本文。
