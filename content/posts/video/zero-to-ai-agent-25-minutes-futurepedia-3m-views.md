...
# 25 分钟从零到 AI Agent：Futurepedia 爆款教程拆解
...
下面把视频中每一步拆开，同时补上视频来不及展开的原理（Agent 和自动化到底差在哪、Memory 为什么重要、API 在干什么），以及视频里踩过的真实调试坑。
...
Futurepedia 的「From Zero to Your First AI Agent in 25 Minutes (No Coding)」是目前 YouTube 上播放量最高的 AI Agent 入门视频。9.2 万点赞，在 Futurepedia 频道所有视频中排第一。它做对了几件事：
...
Agent 不一样。同样是「每天早上推荐跑步路线」，Agent 会先看你的日历有没有空，再看天气适不适合跑，再根据你之前存的路线数据挑一条最合适的，最后才给你发邮件。如果今天日历满了，或者空气质量爆表，它可以决定不发推荐——它根据不同的情况做出不同的决策。
...
这里值得说清一件事：Brain 并不知道「有哪些工具」。它在每次推理时，收到一份用 JSON Schema 描述的工具清单，相当于「这张表里列了 5 个可以调用的函数」。当它决定要用其中一个时，不是在脑内执行，而是输出一个结构化的调用请求，由 n8n 去真正调用，再把结果回传给它。弄清这层，后面调试「Agent 不调用工具」时才有方向。
...
记住这四个概念之间的关系就够了：Brain 决定怎么想，Memory 决定记住什么，Tools 决定能做什么，Guardrails 决定不能做什么。往下走到前置知识和实战，就是把这四样落到 n8n 的画布里。
...
n8n（源自 "nodemation"——node 和 automation 之间相隔八个字母，读作 "n-eight-n"）是一个开源的工作流自动化平台。可以把它理解成一个可视化的「流程编排器」——屏幕上是一张画布，你往上面拖节点（Node），用线连起来，数据就顺着线从一个节点流向下一个节点。（内容基于 n8n 1.x 版本界面，后续版本界面可能略有差异。）
