...
而且全程没有一行代码。用的是 n8n——一个可视化自动化平台。不需要装 Python、不需要配环境、不需要理解框架的 API，跟着拖拽就能跑。门槛从「学一门编程语言」降到了「会用流程图」。
...
**用真实场景驱动。** 视频没有构建 demo 级的「Hello World」，而是做了一个有实际用途的东西：一个每天早上 5 点自动检查日历、天气、空气质量，从跑步路线表里挑一条最合适的路线，然后发邮件告诉你。场景足够复杂，能展示 Agent 的真正能力，又足够直观，任何人都能理解它在干什么。
...
区分两者的一种快速方法：**自动化执行的是你写死的流程，Agent 执行的是你给定的目标。** 流程是确定的，目标下的路径不确定。

### Agent 的三个关键组件

视频把 Agent 拆解成三个部分，这个框架简洁且足够准确：
...
**Memory（记忆）**——Agent 对话过程中的上下文信息。如果用户说了三句话，Agent 需要记住前两句才能理解第三句。视频里配置了 5 条消息的上下文窗口——Agent 能记住最近 5 轮交互。5 条是权衡的结果：短了丢失上下文，长了增加 Token 消耗和推理延迟。
...
四个概念合起来就是 Agent 的完整心智模型：Brain 决定怎么想，Memory 决定记住什么，Tools 决定能做什么，Guardrails 决定不能做什么。接下来的前置知识和实战，就是把它们落地到 n8n 里的过程。
...
API（Application Programming Interface，应用程序接口）是两个软件之间对话的方式。打个比方：API 就像自动售货机——你投币、按按钮（发请求），机器出货（返回结果）。你不需要知道机器里面的齿轮怎么转，只需要知道「按这个按钮，会得到这个饮料」。视频用了类似的方式把 API 解释给没有技术背景的观众。
...
这个工具说明了一个实际的问题：**当预置连接器不够用时，HTTP Request 节点可以对接任何有 API 的服务。** 这也是前面花时间讲 API 的原因——理解了 HTTP 请求，你就能接任何工具。
...
这个工具相当于 Agent 的「知识库」。它不知道哪些跑步路线可选，你得给一张表。这种设计思路值得记住：当你希望 Agent 在特定范围内做决策时，与其让它漫无目的地搜索，不如把选项列出来。
...
视频里提到一个实用技巧：如果你不确定怎么写好 Prompt，可以先让 ChatGPT 帮你生成一个结构化 Prompt，再根据需求微调。用 AI 写 AI 的指令，在实务中很常见。
