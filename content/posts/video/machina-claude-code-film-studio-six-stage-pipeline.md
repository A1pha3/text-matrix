...
- 看清 Machina 这套系统的真正主张：把 Claude Code 改造成能全天候运转的 AI 短片工作室，而不是又一个「提示词技巧合集」
- 说出「loop engineering（循环工程）」这个新词到底在指什么——它和传统自动化、传统脚本批处理的关键区别
- 走通六阶段管线的每一段：萃取风格契约、批量出帧、Agent 写分镜、运动语法、并行子代理舰队、文本化剪辑
- 复述五条立刻能用的提示词规则（每镜头一个动词 / 物体承载情绪 / 相机运动与主体运动分句等）
- 评估这套系统在自己的工作流里能用几分、不能照搬的几分
...
如果把这六步压成一张工作流地图，它更像一个可观测的生产系统，而不是一条把 prompt 丢进黑箱等结果的流水线：
...
AI 视频把这个循环压扁了：描述一个镜头，几分钟生成，看了之后改三个词，重新生成。「试错成本」降到了足够低，「什么都试一遍」反而成了策略。
...
Machina 给这个新技能取了个名字：**loop engineering（循环工程）**。它不是写脚本，不是写批处理，而是「把要 Agent 重复跑到满足条件为止的任务」当成一个工程对象来设计。
...
按 Higgsfield Skills 仓库的 README，官方现在也提供 `npx skills add higgsfield-ai/skills`、`gh skill install` 和 setup 脚本等安装路径，安装过程中会顺带处理 CLI 与认证。Machina 保留拆开的三步，反而更适合已经决定走 CLI-first 的人，因为你能清楚地区分哪一步在装命令行、哪一步在做登录、哪一步在把技能接进自己的 Agent harness。
...
锁死模板的意义是：跨镜头的一致性来自重复的参考图和重复的语言，绝不是来自 seed 或运气。
...
舰队生死系于一条不性感的规矩：**尊重并发上限**。视频模型有同时运行任务数的天花板，Agent 检查活跃任务，只在有空槽时提交。带节流的 Agent 跑一整夜能跑完；朴素循环一小时就撞限速挂掉——而限速本身正是「让它能跑一整夜」的原因。
...
每段音频在和配乐打架的地方静音、在加真实感的地方保留——意味着整份剪辑可复现、可秒改，不用时间线软件，不用剪辑师。
...
如果你期待 Agent 独立从剧本跑到成片，排查第一步不是换模型，而是先看有没有人为挑选节点。Machina 反复强调的是“Agent 负责吞吐，人负责判断”。

### 错误二：在视频层才开始找构图

如果一条镜头反复烧积分却总像废片，先回到静帧层排查。构图、角色比例和场景关系没在 stage 2 解决，stage 4 只会把问题放大。

### 错误三：一个镜头塞进三个动作

常见症状是人物发飘、运镜发抖、事件没有主次。排查办法不是继续加形容词，而是删动作，把镜头压回一个主事件。

### 错误四：没有日志和节流就上舰队

子代理并行看起来很爽，但最常见的故障也出在这里：任务撞上并发上限、失败重试没有记录、第二天根本不知道哪条 prompt 产出过赢家。排查时优先补任务日志，再谈调度策略。

## 七、自测与练习

### 自测 1

如果现在让你复述 Machina 这套系统的核心，你能不能不用“电影感”“全链路”“工作流”这些大词，只用“哪些工件被保存下来、谁在什么时候做决定”把它讲清楚？

### 练习 1

选一条你最熟悉的短视频题材，只写 6 镜头镜头表，不写 prompt。等镜头表成立以后，再补风格契约和模型选择。这个顺序能逼你先做导演，再做提示词工程。

### 进阶练习

把“镜头循环”和“舰队循环”拆开实现。先手动完成 3 个镜头的静帧筛选，再让 Agent 只接管提交、轮询和记录，别一开始就把所有决策一起自动化。
...
所有人都能租到同一批模型，所以片子最后好不好，差异主要不在模型名字，而在你有没有把品味、参考、重试和取舍写成一套可复用流程。
...
进阶阅读可以接着看两篇站内文章：[Anthropic Claude Code：把 Loop Engineering 拆成你今天就能用的四块](../tech/anthropic-claude-code-loop-engineering-2026.md) 适合补 agent harness 视角；[Seedance 2.0 视频制作实战指南：从提示词到分镜的全流程教程](./seedance-2-video-production-guide.md) 更适合补镜头与提示词这一侧的基本功。

## 参考资料

- Machina，How to build an AI video studio in Claude Code，X，2026-07-28
- Higgsfield AI Skills README，安装路径、跨 Agent 技能形态与 Quick Reference
- Higgsfield CLI 仓库，命令行入口与认证流程说明

品味进去，片子出来。

**第二，把循环当成工程对象来设计。** 不是所有事都该跑成 while 循环，但只要你发现一件事需要「重复跑到某个条件满足」——多模型选最佳、多版本挑赢家、多角度试构图——它就是 loop engineering 的候选。设计循环时至少问三个问题：退出条件是什么？并发上限在哪？这一轮日志怎么变成下一轮的起点？

**第三，约束用正面语言写。** 这条对任何用模型生成内容的人都有效：`stable picture, sharp clarity` 比 `no blur, no shaking` 有用得多。每镜头一个动词、主体运动和摄影机运动分两句、120 到 280 词的长度——这些是写给模型的提示词工程，但同样适用于写给人看的需求文档：明确说什么，比明确说不要什么，更容易产出你想要的结果。

最后一句留着：**便宜生成没让好内容变容易，它把整份工作挪进了品味。** 谁能把品味结构化（风格契约、参考表、节拍表），谁就能跑赢这一轮。
