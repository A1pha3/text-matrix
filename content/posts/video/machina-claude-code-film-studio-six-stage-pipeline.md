...
所有人都能租到同一批模型，所以片子最后好不好，只看你喂给系统什么。
...
Machina 这套东西不是某个具体工具的功能清单，而是「loop engineering」这种新工程范式的活样本。把它的设计哲学抽出来，三件事在自己的工作流里就能直接用。

**第一，把任务切成人和 Agent 各自的活。** 不是每件事都要 Agent 做，也不是每件事都该自己点按钮。「品味 vs 吞吐」是一个可以套到任何内容生产场景的切法：判断、挑选、定方向归人；写提示词、提交、轮询、重试、组装归 Agent。
