...
description: "Anthropic Claude Code 负责人 Boris Cherny 说：别再写代码了，去构建一个能自我编写的图。本文沿 Agent、Agentic Loop、Graph 三级台阶，拆解 Graph Engineering 如何改写 AI 编程的规则。"
...
说这话的是 Boris Cherny，Claude Code 的创建者与负责人。他在一段约 28 分钟的视频里捅破了过去两年 AI 编程圈的一层窗户纸，视频后来被 @Mahaximus_ 转发到 X 并发酵。他的判断是：大多数人用 AI 写代码，本质上还是在给模型写 Prompt（提示词）——像给一位天赋异禀的实习生反复口述需求，而这位实习生每次对话开始时，都得从零理解上下文。

阅读目标：读完本文，你应该能回答三个问题——Agent、Agentic Loop、Graph 三级台阶各解决什么问题；一张能学习的图由哪四个组件构成；自己的第一个图该从哪个任务下笔。

Anthropic 内部在做另一件事：搭图（Graph）。图是由多个智能体（Agent）节点组成的系统，有记忆，能从错误中学习，每一次运行都比上一次更聪明。传统智能体执行一次任务就结束；智能图是一个不断进化的系统。

按视频里的说法，Claude Code 本身就是用这种方式构建出来的。

## 从 Agent 到 Loop 再到 Graph

要理解图工程，得先回到它之前的两个台阶。

**第一级：单次 Agent。** 2023 年 AutoGPT 让「自主智能体」出了圈，但多数开发者日常停留的形态比它朴素：一次对话，接收指令、调用模型、执行动作、返回结果、结束。能跑，但不积累。今天帮你修好的那个 bug，明天它会换一种方式再犯一遍。

**第二级：Agentic Loop（智能体循环）。** 给 Agent 装上循环，让它在「规划 → 行动 → 观察 → 修正」里多转几圈，直到任务收敛。这是从「执行命令」到「解决问题」的升级。Anthropic 官方博客《Building Effective AI Agents》对 Agent 的定义就落在这里：一个由 LLM（大语言模型）驱动的循环，模型动态地指导自己的流程和工具使用，每一步都从环境中拿到「地面真相」（ground truth）——工具调用的返回、代码执行的结果——再决定下一步。

与之相对，工作流（Workflow）是通过预定义代码路径编排模型和工具的系统，控制权在程序里，不在模型手里。
...
这四样东西拼在一起，回答了一个所有写过 Agent 的人都遇到过的问题：为什么 Demo 惊艳，一上生产就翻车？因为你交付的是一个聪明瞬间，而生产环境要的是一台日复一日稳定出活的机器。
...
这就是模型上下文协议（Model Context Protocol，MCP）要补上的。随视频一同流传的课程大纲里，「如何构建 MCP」排在「Graph Engineering」之前整整一节课——先有通信层，再谈组网。
...
1. 工作流和智能体的分界线是什么？控制权在哪一侧？
2. 五种官方工作流模式里，哪一种最适合「涉及多文件、无法预先穷举改动路径」的编码任务？
3. 验证循环和投票（Voting）机制是什么关系？
4. 你的第一张图为什么应该从单个带循环的 Agent 开始，而不是直接搭多节点？
...
编译器把程序员从机器码里解放出来，没人再说「真正的程序员都手写汇编」；垃圾回收把我们从手动内存管理里解放出来，也没人怀念 malloc 和 free 的年代。抽象层每上移一次，都会带来同一类焦虑——「那我们还能干什么」——和同一个答案：去设计上一层的东西。
...
- **重读《Building Effective AI Agents》**：正文「Agents」一节给出了一张 coding agent 的高层流程图，附录「Agents in Practice」补了客户支持与编程两个实战案例，可以和本文的五种积木对照，看生产系统如何把它们组合起来。

---

> **资料说明**：本文中 Boris Cherny 的引言、Agent 与 Smart Graph（文中作「智能图」）的对比框架、课程大纲来自 X 平台传播的原视频及推文内容；五种工作流模式、Agent 与 Workflow 的定义、三条设计原则引自 Anthropic 官方博客《Building Effective AI Agents》；场景对比与上手路径为基于上述资料的推演与延展，非视频原话。
>
