...
summary: "把 Seedance 2.0 当成镜头生成器而不是成片导演：先分镜、再提示词，用五模块结构、三类参考素材包和参数控制稳住出片质量。从创意到可剪辑素材的每一步都拆成可执行规则，附四套模板与排查清单。"
---
...
- 第一次用：按 1（边界）→ 2（流程）→ 3（提示词）→ 5（一致性）→ 8（排错）走一遍
- 想直接上手：从 7 的四套模板改起，再回头对照 3 的五模块结构
- 已经会写提示词但出片不稳定：重点看 5 的一致性控制与 9 的制作 SOP
...
补一个版本快照，帮你校准时间感：本文写于 2026 年 3 月，Seedance 2.0 发布不久，单段输出就是数秒到十余秒的量级。到 2026 年 9 月，官方已发布 Seedance 2.5——单段可直出 30 秒，参考素材扩展到最多 50 个全模态素材，还内置了局部编辑能力。方法不变，量级在变：读这篇文章时，以你当下所用版本的实际档位为准。

### 1.4 三个最常翻车的参数：分辨率、随机种子、时长

Seedance 2.0 这类模型的可调参数里，翻车率最高的是分辨率、随机种子（seed）和时长三个。它们各自的坑不一样：

| 参数 | 翻车方式 | 处理原则 |
| ---- | -------- | -------- |
| 分辨率 | 选错档位导致画面糊或构图被裁 | 先低分辨率验证运动和构图，定稿再升档 |
| 随机种子 | 不定 seed，同一提示词每次结果都漂移 | 入口提供 seed 就锁定关键镜头，保证可复现；没开放就把已验证的提示词原样存档，当作你自己的"种子" |
| 时长 | 一拉长，一致性迅速崩坏 | 单段控制在数秒到十余秒，长镜头拆段生成 |

**如果你的入口提供 seed，它是跨镜头一致性的地基。** 每次生成都相当于一次新的采样，模型会重新"理解"你的要求。锁住 seed，同一段提示词才能产出可对比、可复用的结果；不锁，你连"这次改得有没有用"都判断不了——优化只能建立在可复现的基线上。
...
要理解为什么多镜头叙事容易崩，先要知道 Seedance 2.0 这类模型的底层能力边界：官方宣传的"原生音画同步与多镜头长叙事"，指的是单段生成里能保持角色、风格统一，是"关卡内"一致；跨段生成时，没有显式锚点，一致就要靠你在输入侧补。**模型承诺的是单段稳定，不是你整条视频的连续性。** 音频和视频联合生成同样是这个逻辑：单段内音画同步由模型兜底，跨段的声音风格仍要靠你统一设定。
