...
把 AI 视频工具当"一键出片"的机器，是新手最容易踩的坑。**视频越长，变量越多；变量越多，一致性越差。** 角色表情、服装、景别、机位、光线、动作节奏，任何一个环节漂移，长镜头都会失控。

办法是把每次生成当作一个"可剪辑镜头"，成片质量建立在分镜设计和后期拼接上。目标从"完美成片"改成"稳定可用素材"——这就是"剪辑师思维"。

### 1.2 影响成片质量的三个因素

很多人把问题归结为"提示词不够强"。但 Seedance 2.0 的结果由三项共同决定，提示词只是其中之一：
...
五步里最容易被跳过的是第二步。很多人看到想法直接写提示词，但**没有分镜就没有稳定的输出标准**。镜头目的都没定义，模型当然随机发挥。
...
回答不了这四个问题，说明不是提示词的问题，是镜头定义不够清晰。
...
最后一个模块不只是清晰度，还包含了光线、色彩、质感、景深等统一的输出要求。

### 3.2 主体：写到足以排除歧义

主体描述的目标是"写到足以排除歧义"。结构：身份 + 外观关键特征 + 当前场景。
...
技术收尾的作用是把前面的内容统一收束。不是迷信魔法词，而是让模型在清晰度、光线和质感上朝同一个方向收敛：
...
图生视频的目标不是重新定义主体，而是让主体动起来。提示词要更短、更克制，只保留三类信息：
...
**文生视频靠描述完整度，图生视频靠约束克制度。** 同样的写法在文生视频里效果不错，换到图生视频就跑偏——不是模型变差了，是输入策略错了。
...
AI 视频生成不是在调用固定演员和固定片场，而是在每次采样时重新"理解"你的要求。约束不够稳，漂移就会发生。
...
- **场景包**（锁定环境结构）：选能代表空间关系的参考图，不是只选漂亮图。
...
真正重要的是：**不要让角色图、场景图、风格图在同一张图里互相打架。**
...
真正影响成片质量的，是镜头之间能不能接起来，而不是某一个镜头有没有单独惊艳。分镜的作用是提前控制镜头之间的关系。至少要控制四个连续性变量：
...
出现以下任何一种情况，拆镜头而不是强行塞进一段生成：

- 既想展示环境，又想展示表情，还想展示产品细节
- 人物需要完成明显动作链（走近、停下、抬手、转头）
- 提示词里出现大量"同时""然后""接着"

提示词里时间顺序词太多，说明你在让一个镜头承担过多叙事任务。
...
这些模板不是复制粘贴就能直接出片，它们体现的原则是：每个镜头只服务一个明确目标。
...
这是最容易被误判的失败。结果看起来很"电影"，但没有完成镜头任务。

**原因**：风格词过强，压过了动作和主体；镜头目的本身没有写清楚；提示词追求华丽而不是可控。
...
**新手最该先练什么？** 先练分镜拆解，而不是背提示词。不会拆镜头的人，抄到好提示词也难稳定复现。

**一条视频应该先做文生还是图生？** 如果还在找视觉方向，先用文生视频探索；如果已有角色或产品主视觉、需要保持一致性，就尽快切到图生视频。

**什么时候该放弃继续调一个镜头？** 连续几轮都在改风格词，但主体、动作和镜头目的仍然不清楚时，停下来重写镜头定义，而不是继续在同一版本上硬调。

**怎样提高整条片子的统一感？** 最有效的方式不是给每一镜都加华丽词，而是提前固定三件事：统一风格锚点、统一参考素材、统一分镜节奏。

---

能不能稳定出片，靠的不是某个"魔法词"，是你有没有把视频制作当成一个可拆解、可迭代的流程。