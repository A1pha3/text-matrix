...
> 2026 年 4 月，Jeff Su 在 YouTube 发布了 [AI Agents Clearly Explained](https://www.youtube.com/watch?v=HvD2RGJfp6s) 视频，播放量超过 400 万。以下基于该视频及配套文章，系统拆解 AI Agent 的三层进化路径。

---

Jeff Su 的 AI Agents, Clearly Explained 发布于 2025 年 4 月 8 日，时长 10 分 09 秒，并配有一篇面向非技术读者的说明文章 AI Agents for Curious Beginners。这套内容之所以出圈，是因为它把最容易被营销话术搅浑的三个概念重新排好了顺序：LLM、AI Workflow、True Agent。

这也是这支视频最有价值的地方。很多入门文章会把 "能调工具" 和 "能自主决策" 混为一谈，导致初学者以为接入 RAG、加几个 API、套一个框架，就已经做出了 Agent。Jeff Su 的讲法更朴素，但也更实用：差别在于谁决定下一步。
...
这也是 Jeff Su 为什么反复强调 "passive systems"。如果只是让模型根据一个 prompt 生成文字，仍然是一个更强的输入输出系统，算不上能围绕目标持续推进任务的执行体。
...
视频第二层讲的是 AI Workflow。Jeff Su 给出的判断是：工作流已经能连上外部工具、能访问外部信息、也能完成多步任务，但路径仍然是人预先写好的。
...
这比"能不能调 API"更接近工程上的实质。一旦决策权从人手里转移到模型手里，系统能力会立刻发生三件变化：

- 它不再只能沿着一条预设路径走下去，而是会根据中间结果选择下一步。
- 它会把工具调用当成解决问题的一部分，而不是固定脚本中的一个节点。
- 它需要在循环里判断任务是否完成、是否失败、是否需要重试。

Jeff Su 用 Andrew Ng 展示过的视觉代理示例来解释这件事：用户只给出一个目标，比如找出视频里出现滑雪者的片段，系统要自己判断什么是滑雪者、如何搜索、如何筛选、如何索引，最后再返回结果。这里真正发生变化的，是模型开始在工具和环境之间做决策。
...
不过这里最好避免拟人化。说 Agent "会思考" 很顺嘴，但工程上更准确的说法是：模型在 prompt、状态和工具反馈的约束下，交替生成推理痕迹与行动决策。把它说成自主意识，没有必要，也不准确。
...
所以，对这支视频最合理的使用方式是：看完以后知道接下来该学什么、不该把什么混为一谈。
