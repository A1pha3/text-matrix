...
2025 年 5 月 23 日，高通骁友会 5 周年线下派对在北京举办。硅谷 101 在现场录了一期《对话高通：智能体爆发、6G 与 Physical AI 背后的大赢家》（BV1fuVZ6bEAD，35 分钟），嘉宾是高通公司全球副总裁、中国区研发负责人、IEEE Fellow 徐晧。节目信息密度很高，但核心判断只有一条：

> 「AI 正在从云端走向终端，手机、汽车、眼镜、机器人……所有这些终端正在被重新定义。」

把这话和 2026 年 1 月 CES、3 月 MWC 高通的两场主题演讲拼起来看，2026 年确实是端侧 AI 破局的元年——不是「云端 AI 升级了」，是**端侧能稳稳跑 7B-13B 模型，端云协同的智能体调度从演示走到能落地的应用场景**。

本文以这期节目为骨架，叠加徐晧 2024 年公开访谈、骁友会活动现场报道、CES 2026 / MWC 2026 高通主题演讲，以及 Snapdragon X Elite / 8 Elite NPU 公开数据，把「端侧 AI 智能体元年」从芯片、通信、操作系统、智能体四层串起来讲。文中所有数字、产品名、时间表均来自公开报道，不外推任何未发布产品。
...
| --- | --- | --- | --- |
| 1. **端侧算力层** | NPU（神经处理单元）+ CPU + GPU 异构计算 | Snapdragon X Elite / 8 Elite 系列 + Hexagon NPU | 端侧能跑 7B-13B 模型，45 TOPS |
...
```mermaid
flowchart LR
  A[Snapdragon X/8 Elite · 45 TOPS NPU<br>端侧 7B-13B 模型] --> E[端云协同 OS<br>Agent Runtime]
  N[Hexagon NPU · Adreno GPU · Oryon CPU<br>异构计算] --> E
  H[FastConnect Wi-Fi 7 · 5G Advanced] --> E
  G[6G 2028-2030 · 低时延+通感] -.未来.-> E
  E --> F[OEM Agent 应用层]
```

_示意图：端侧 AI 三层基础设施——芯片 NPU 异构计算、连接层 5G Advanced / 6G、智能体调度 OS；三层同时就绪，才让 2026 年成为「智能体元年」，Agent Runtime 是中间抽象层。_
...
| 2024-10 | Snapdragon 8 Elite Hexagon | 45 TOPS | 7-13B 模型，端侧大模型稳态 |
| 2024-10 | Snapdragon X Elite（PC 平台） | 45 TOPS（NPU） | 官方宣称支持 13B 模型，约 30 tokens/s |
...
具体例子：把 Llama 3 8B 模型从云端搬到 Snapdragon X Elite 跑（以下为分工与功耗的示意估算）：

- **CPU**：负责 prompt 解析 + 输出文本的 token 化
- **NPU**：负责 8B 模型的矩阵推理（峰值 45 TOPS，实际平均 15-20 TOPS）
- **GPU**：负责多模态输入（图片 / 视频预处理）
- **Memory**：端侧 LPDDR5X 内存带宽 100+ GB/s（瓶颈点）

如果只用 CPU 跑 8B 模型，功耗要 30+ 瓦（风扇起飞）。如果用 NPU + CPU 异构，功耗降到 5-8 瓦，笔记本才能撑 8-10 小时续航。这就是「**端侧 AI 跑得动**」的工程意义。
...
高通在 2024-2025 年这代 Snapdragon 上的工程优化是「**用 NPU 异构 + 量化 + 投机解码三件套**，把 7B-13B 模型在 8 瓦功耗下跑到约 30 tokens/sec」。这是官方给出的端侧生成速度，意味着端侧 Agent 能「**实时响应**」用户指令。
...
- **Release 18**：2024 年 3 月功能冻结，AI/ML for RAN 起步、上下行 MIMO 增强、网络节能
- **Release 19**：2025 年 9 月功能冻结，沉浸式通信（XR）、AI 增强的 RAN、ISAC 研究立项
- **Release 20**：2027 年功能冻结，5G Advanced 完整能力，同时并行开展 6G 研究
- **6G Release 21**：首个 6G 规范版本，Stage-3 功能冻结约在 2028 年底
...
6G 时间表（依据 3GPP 2026-06 公布的 Release 21 时间线）：

- **2026-2027**：6G 研究阶段（Release 20 内完成各项 6G 研究课题）
- **2027-2028**：Release 21 规范阶段，Stage-3 功能冻结约在 2028 年底
- **2030+**：商用

徐晧在 5 周年现场对 6G 的描述是「不是为了人上网更快，是为了让 AI 智能体之间通信」。这句话点出了 6G 与 5G 的分野——6G 的服务对象从人换成 AI 智能体，协议栈也因此重新设计。

### 3.3 骁友会现场：4 个真实的端侧 AI 体验
...
拆开看一次任务的理想流转。下面是一段示意推演（时间为假设值，非现场实测），用来演示端侧各环节如何接力：用户在骁友会现场戴上 AR 眼镜说一句「导航到最近的中关村咖啡店」，从麦克风收到语音到镜片出现导航指引，理想情况下 1 秒内走完这条路径：
...
整条路径里**端侧承担绝大部分环节**——声学模型、LLM 意图解析、视觉识别、排序、SLAM 叠加都在端侧；云端只跑「查地图」这一件事。上面的时间、功耗、流量都是示意假设，真实取值取决于具体模型与网络，但“端侧为主、云端兜底”的分工结构是确定的。

这是「智能体元年」理想的形态：大部分环节端侧完成，1 秒内给答案，功耗低到眼镜电池能撑住。如果任何一环离开端侧（去云端跑 LLM、视觉模型、SLAM），网络延迟、流量、隐私三者会同时失控。这也是「智能体元年」真正的工程门槛：芯片 + 通信 + 操作系统**三层同时就绪**。
...
### 5.3 高通在 Physical AI 押注的「第二增长曲线」

高通靠 Snapdragon 8 系列 + 调制解调器在 5G 时代把营收做到 2025 财年的 442.84 亿美元。「个人 AI」这条赛道对高通来说只是维持现状——手机 / PC 还是高通的，但 AI 化是延续而非新增。

「**Physical AI**」是高通押注的第二条增长曲线：

- **汽车 Snapdragon 8295 / Cockpit AI**：高阶车机 AI 芯片的主要玩家。2026 年 7 月，高通被 BMW 选为未来十年座舱计算与自动驾驶的主计算芯片供应商，车机这条线的确定性被进一步锁定
- **机器人 Robotics RB6**：与上一篇文章《机器人「肉身」的工程化》主线对应，**芯片是机器人的大脑，机器人公司是高通 Physical AI 客户**
- **XR AR1 / AR2**：智能眼镜 AR 芯片的先行者
...
## 六、如何判断这一波是真是假：4 个观察维度

**NPU 算力**——端侧能否跑稳 7B-30B 模型，45+ TOPS 是及格线。Snapdragon 8 Elite 这一代过了。

**端云协同**——是否有 3 种模式（端侧为主 / 云端为主 / 端云协同推理）落地的产品，而不是「上云下云来回跑」的伪协同。

**Physical AI 出货**——车、机器人、XR、工业，是否有真实出货。2026 年盯高通 Robotics RB6 和车机 8295 的出货量。

**6G 进度**——3GPP Release 21 是否在 2028-2030 年如期出标准。6G 是 Physical AI 的通信底座。

把 4 个维度乘起来看，**高通是 2026-2030 年端侧 AI 浪潮里最被低估的玩家**——芯片、通信、操作系统三层都有布局，骁友会的 4 个体验不是营销秀，是三层基础设施**同时跑通**的早期形态。

徐晧在现场反复说：「**这一波端侧 AI 和 2010-2015 年的移动互联浪潮一样，是 24 个月起步的工程量。**」——**端侧 AI 不是 ChatGPT 加个 App，是芯片 + 通信 + OS 三层基础设施的同步就绪**。

---

## 七、结语

2026 年的端侧 AI 智能体元年，模型聪明不一定赢，端云协同稳才赢。端侧稳，是 NPU 算力、内存带宽、功耗控制三角平衡，能跑 7B-30B 模型；云端稳，是 5G Advanced 到 6G 的路线图清晰，端云能无缝切换。

骁友会 5 周年的 4 个体验，CES 2026 / MWC 2026 高通把 AI 切成「个人 AI + Physical AI」两块，都是同一张明牌：**2026-2030 年端侧 AI 浪潮是 Physical AI 的故事**。SpaceX 的故事是去火星，高通的故事是去端侧——两条路都还没走完，但方向已经清楚。剩下的就是工程。
...
- **「模型越聪明，端侧就越赢」**——不对。端侧的胜负手是**端云协同稳不稳**（算力/内存/功耗三角平衡 + 无缝切换），不是单看模型分数。
- **「6G 是为了让人上网更快」**——不对。徐晧的定义是 6G 主要为**AI 智能体之间通信**（低时延 + 通信感知），是为 AI 时代重新设计的网络。

## 九、自测

读完能回答这几个问题，就算没白看：

- 端侧 AI 三层基础设施（算力 / 连接 / 调度）各自的关键载体是什么？
- 「个人 AI」和「Physical AI」在模型、实时性、硬件上差在哪？
- 端云协同 3 种调度模式各自适合什么设备？
- 判断端侧 AI 是不是真趋势，看哪 4 个维度？

## 十、参考来源

- 硅谷 101《对话高通：智能体爆发、6G 与 Physical AI 背后的大赢家》：[BV1fuVZ6bEAD](https://www.bilibili.com/video/BV1fuVZ6bEAD)（2026-06 上旬发布，嘉宾徐晧）
- 公开对谈《对话 IEEE Fellow 高通徐晧：如何看待 5G、AGI、人机交互革命》：[BV1YMfnYNEco](https://www.bilibili.com/video/BV1YMfnYNEco)（2024 年发布）
...
- 上篇文章《机器人「肉身」的工程化：宇树半马破纪录、智元量产和特斯拉 Optimus 留下的不是空翻，是供应链重组》
