...
传统 LLM（大语言模型）的使用方式是一次性的：你给一个 prompt（提示词），它返回一个回复，对话结束。"总结这篇文档"或"翻译这段文字"这种任务，单次问答够用。但换成"帮我整理这个季度的销售数据，标出异常，然后给每个客户经理发一封邮件"，单次问答就不够了。

AI Agent（智能体）把问题从"生成回复"变成了"完成任务"。Jeff Su 那支播放量超过 421 万的视频用一个类比说明了区别：传统 AI 是指路，Agent 是开车——它需要自己规划路线、识别路况、处理意外，并在到达后确认完成。这个类比不算精确，但抓住了核心：**Agent 不是一次回答，而是一个带状态、带工具、带反馈循环的执行过程。**
...
- [一、Agent 不是"会调工具的 LLM"](#一agent-不是会调工具的-llm)
- [二、规划：从 Chain-of-Thought 到分层分解](#二规划从-chain-of-thought-到分层分解)
- [三、工具调用：Agent 的双手](#三工具调用agent-的双手)
- [四、记忆系统：Agent 的短期与长期](#四记忆系统agent-的短期与长期)
- [五、多智能体协作：角色分工与通信开销](#五多智能体协作角色分工与通信开销)
- [六、框架选型对照](#六框架选型对照)
- [七、工程实践：构建 Agent 的常见陷阱](#七工程实践构建-agent-的常见陷阱)
- [八、亮点与争议](#八亮点与争议)
- [九、读者判断：谁该看原视频](#九读者判断谁该看原视频)
...
还有一个演进值得知道：ReAct 原论文里的 Thought/Action 是纯文本格式，靠提示词约定和外部解析器从中抽取调用指令。2026 年的主流做法已经换成模型原生输出结构化的工具调用，推理部分由模型的推理通道或正文承载。格式换了一代，"推理-行动-观察"这个循环骨架没变——变的是指令怎么传，不是循环怎么转。

2026 年主流的 Agent 框架——LangGraph 的状态机、CrewAI 的层级调度、AutoGen 的对话驱动——底层都能追溯到这种循环结构。差别在于循环的编排方式：图结构（LangGraph）、对话轮次（AutoGen）或层级调度（CrewAI）。
...
- **目标验证**：Agent 自己判断任务是否完成。问题有两类：一是自评可能过于乐观或过于保守；二是循环拉长后会"跑偏"——后期的步数花在与原始目标无关的调用上，越走越远。
...
最简单的规划形式，让模型在同一个上下文中逐步推理。没有分支，没有回退，适合"步骤明确、依赖清晰"的任务。CoT（思维链）的局限在于，一旦中间某一步出错，后续全部跑偏，且没有纠错机制。

### 任务分解（Task Decomposition）

把大任务拆成子任务，可以顺序执行，也可以并行。Plan-and-Execute 模式就是典型的任务分解：一个 Planner 负责拆解，Executor 负责执行，Supervisor 监控进度。

任务分解的工程难点不在"拆"，而在**依赖管理**。子任务之间可能存在数据依赖（B 需要 A 的输出）、资源依赖（两个任务不能同时使用同一个工具）、以及决策依赖（C 是否需要执行取决于 B 的结果）。主流框架用图结构来表达这些依赖，多数场景下是 DAG（有向无环图）；LangGraph 的图走得更远，允许环存在——反思和重试需要回到之前的节点，无环图装不下这种流程。
...
**2. 并行调用**
模型一次调用多个工具，运行时并行执行。适合"查多个数据源再综合"的场景。OpenAI 的 function calling API（应用程序接口）在 2023 年 11 月的 DevDay 上加入并行调用，一条回复可以携带多个工具调用；Anthropic 的 tool use 在 2024 年跟进，同样原生支持并行。如今这已是主流模型 API 的标配能力。
...
接工具的方式也在收敛。2024 年 11 月 Anthropic 发布 MCP（Model Context Protocol，模型上下文协议），把"每个工具写一份胶水代码"变成"实现一个协议服务器，任何支持该协议的 Agent 都能调用"；2025 年 OpenAI 和 Google 相继宣布支持，主流 Agent 框架和编码工具也接了进来。协议化不改变上面三种调用模式——模型照样输出一条调用指令——改变的是接入成本：工具写一次，处处可连。
...
短期记忆就是 LLM 的上下文窗口。2026 年主流模型的窗口从 128K 到 1M token（词元）不等，长的一档足以一次读入几本书。但长上下文不等于高质量记忆——模型在长上下文中检索信息的能力有限，相关信息越分散、窗口越满，漏看和串味的概率越高。所以业界的做法不是单纯堆窗口，而是做上下文工程：定期把陈旧的历史压缩成摘要，把大段内容外置到文件或检索库，窗口里只留索引和当前任务需要的那部分。

### 长期记忆（外部存储）

当任务跨多个会话，或者需要积累领域知识时，Agent 需要外部存储。最常用的实现是向量数据库 + RAG（检索增强生成）：把历史信息向量化存储，在需要时检索相关片段写回上下文。
...
6. **协作**（本节三种模式）：如果团队用 Supervisor 模式，一个调度 Agent 把"查数据"和"写邮件"分给两个 Worker，并汇总结果；若用单 Agent，这一步省掉，直接在第 5 步收尾。
...
- **AutoGen** 的对话驱动抽象适合研究和多轮交互实验。它的归属近期有变化：2025 年 10 月微软发布 Microsoft Agent Framework，定位为 AutoGen 与 Semantic Kernel 的接替者，新项目值得先了解它再决定是否直接用 AutoGen。
...
Agent 说"任务完成了"不一定真的完成了。在关键节点设置人工确认（human-in-the-loop）或自动验证（assertion check，断言检查），是生产级 Agent 的标配。
...
1. **第一个版本：单 Agent + 固定工具链 + 明确任务定义**。这也是本节四个陷阱的预防位置——任务定义够具体、错误恢复够用、token 有预算。此时不引入反思，更不引入多 Agent。
...
视频最值得看的地方在开头那个类比：把"指路"和"开车"分开，一次性讲清了 Agent 和普通 LLM 的本质差别。这个画面感让没接触过 Agent 的人也能建立直觉，后面所有技术细节都绕着这个直觉展开。

有几个点视频是简化过的，读上面这篇推演时要补回来：

- 视频把工具调用讲得偏顺利，弱化了错误恢复。真实生产里工具超时、返回格式异常、模型陷入循环，每一样都要单独处理，这部分的工程量往往比"跑通 happy path"更大。
- 视频把多 Agent 讲得像自然的分工，容易让人低估通信开销。第五节里提到的 token 膨胀不是稳定值，但也不该被乐观忽略——中间结果来回回写是真实成本。
- 视频没有展开终止条件的设计。循环里最难的不是"怎么想"，而是"何时停"，这一节在第一节单独补了。

## 九、读者判断：谁该看原视频

如果你已经接触过 Agent、写过简单的工具调用，这份推演基本够用，原视频可以跳过，重点读第一节的循环终止条件和第五节的多 Agent 成本。

如果你对"Agent 到底是什么"还没有一个整体画面，或者想用几分钟建立直觉，建议去看原视频——它的类比和节奏比文字更直接。看的时候带着两个问题：作者把 Agent 简化掉了什么，这些简化在真实部署里又该怎么补回去。
...
1. 只接了工具的 LLM 调用和 Agent 的分界线在哪里？
2. 三种循环终止策略里，为什么不建议只用 Agent 自评？
3. 业务流程确定的场景，工具链应该动态还是固定？为什么？
4. 决定拆成多 Agent 之前，先要确认瓶颈在"能力"还是"流程"——这句话的判据是什么？

答案分别藏在第一节、第一节末尾、第三节链式调用、第五节末尾，回查时顺便检验自己能否直接说出结论。

## 进阶方向

- **LangGraph 的状态与检查点机制**：本文第四节的工作记忆只讲了概念，LangGraph 文档里的 checkpoint 和持久化是它最成熟的落地形态。
- **Reflexion 原论文**：第二节把反思压缩成了三步，论文里的语言强化学习视角解释了为什么"把失败写成文字喂回去"有效。
- **多智能体协作成本实测**：第五节的 token 倍数是经验估算，拿一个真实任务分别跑单 Agent 和三 Agent，自己量一次通信开销，比任何估算都可信。
- **框架源码里的循环实现**：挑 LangGraph 或 CrewAI 读一遍它的调度循环，看终止条件和错误恢复在生产框架里是怎么写的。

---

## 参考资料

- Jeff Su 视频「AI Agents, Clearly Explained」（本文的解读对象，播放量数据以发布时平台显示为准）
...
- MCP（Model Context Protocol）官方站点：https://modelcontextprotocol.io/

*文章基于 Jeff Su 视频「AI Agents, Clearly Explained」（421 万播放）整理；正文的五条主线拆解与工程实践来自公开技术资料，超出原视频口播范围，故未标注具体视频时间点。*