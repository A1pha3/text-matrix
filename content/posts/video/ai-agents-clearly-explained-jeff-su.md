...
**2. 并行调用**
模型一次调用多个工具，运行时并行执行。适合"查多个数据源再综合"的场景。OpenAI 的 function calling API（应用程序接口）从 2024 年起就支持并行调用，Anthropic 的 tool use 也在 2025 年跟进。
...
短期记忆就是 LLM 的上下文窗口。2026 年主流模型的上下文已经达到 128K-200K token（词元），足以容纳整本书。但长上下文不等于高质量记忆——模型在长上下文中检索信息的能力仍然有限，尤其是当相关信息分散在各处时。
