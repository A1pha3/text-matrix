...
张小珺和苏煜这期长访谈没停在热词复述上，把 Agent 拉回工程语境里讨论。问题不是 Agent 像不像人，而是它能不能作为行动系统稳定运行。一个系统如果能感知环境、决定下一步、调用工具、读回结果、在出错时停住并把控制权交还给人，它才开始像 Agent。

沿着这个坐标回看，Agent 史就不会只剩下 2023 年以后的那点热闹。苏煜把几条原本分散的技术线重新并到一起：逻辑代理把下一步写进规则，神经代理从数据里学下一步，语义解析把语言翻成可执行表示，Language Agent 则让语言直接参与计划、工具调用、记忆和协作。问题从"模型会不会回答"换成了"系统能不能在开放环境里连续做事"。
...
2023 到 2024，大家第一次相信 LLM 可以拿来当控制器。2025 到 2026，Agent 的执行栈终于开始长成产品栈。这不是某个单点突破，而是几层东西前后脚开始到位：

| 层 | 之前更像什么 | 到 2025-2026 发生了什么 | 这意味着什么 |
| ---- | ---- | ---- | ---- |
| 环境接口 | Prompt 内部演示、封闭 benchmark | 浏览器、桌面、代码仓库、文件系统开始作为公开接口和评测环境进入主流 | Agent 第一次要对真实软件负责 |
| 运行时 | demo 脚本、临时 glue code | Responses API、Agents SDK、ADK、Agent Framework 之类的运行时开始把工具、会话、工作流、checkpoint（检查点）、telemetry（遥测）做成基础设施 | 团队开始能系统化开发、运维和审计 Agent |
...
苏煜没有从产品热词切入 Agent，而是从几条长期技术路线的汇合处切入。符号规划、强化学习、语义解析、语言模型、工具使用、计算机界面和持续学习，今天都被"数字工作者"这个问题拉到了一起。

## 为什么是苏煜来讲这段历史（00:02:00 起）
...
今天 Agent 系统里的状态机、规划器、工具 schema、权限规则、回滚策略，仍然带着这一代思想的影子。区别在于工程系统不再假设所有世界知识都能提前写进规则库。
...
## Semantic Parsing：被低估的中间桥梁（00:45:18 起）

Semantic Parsing 在这条历史里容易被跳过，因为它不像 AlphaGo 那样好传播，也不像 ChatGPT 那样有全民体验。但在苏煜这条技术线上，它是一条中间桥梁。
...
苏煜参与过的交互式语义解析工作，就把 parser 放进更像 Agent 的框架里：系统维护状态，判断哪里需要用户反馈，并用自然语言提出澄清问题。这已经接近现代 Agent 的基本动作：保持状态、发现不确定性、请求外部反馈、再执行。
...
## Language Agent：过去三年为什么突然加速（00:29:28 起）

苏煜在节目里强调，过去三年 Language Agent 的发展速度比之前几十年都快。不是"模型更大了，所以 Agent 自然出现了"，而是四个部件同时跨过了可用门槛。
...
"会想"这个说法太粗了。计划、行动、反馈、工具说明和人类协作开始被写进同一段上下文、同一套工具协议和同一组日志里。系统连接成本降了，但错误传播也因此更容易被放大。

## OpenClaw Moment：界面边界开始松动（00:48:56 起）

节目里 OpenClaw Moment 这个词容易被单独拎出来。它适合拿来类比 ChatGPT Moment，但也容易被写过头。

ChatGPT Moment 的关键，是普通人突然意识到：模型的语言能力已经跨过一个主观可用门槛，可以参与写作、总结、翻译、编程、检索和知识组织。OpenClaw Moment 如果成立，指的不是某一个项目完成了通用智能，而是普通用户开始意识到：Agent 可以进入我的数字环境，替我操作真实软件，而不是只给我建议。
...
OpenClaw Moment 的入口变化更值得盯：数字 Agent 正在从"聊天框里的建议"变成"带执行权限的工作台"。但离站稳还早。它能不能在高频、长期、有风险的工作里稳定下来，最后要看真实任务里的失败率和接管成本。
...
## 当前瓶颈：长程可靠性（01:44:34 起）

节目后半段谈到 Agent 瓶颈时，话题已经从模型能力转到工程约束了。Language Agent 能不能用，取决于这些能力怎么被约束、验证和复用。

### 先别把 workflow 和 agent 混成一件事

很多团队踩坑的地方，是把本该写成 workflow 的任务硬做成了 agent。Microsoft 在 Agent Framework 文档里说：如果一个任务可以写成函数，那就先不要用 AI agent。开放任务和固定流程，本来就该用不同的控制方式。

固定步骤的任务，更适合显式工作流、类型约束、checkpoint 和可预测的失败处理；开放任务、目标会变化、需要边走边判断的任务，才更需要 Agent 的规划与工具选择能力。很多"Agent 失败案例"是任务选型一开始就错了。

### 长程任务会放大小错误

单步回答错了，可以重问；多步任务第 7 步错了，后面 40 步可能都在错误前提上继续执行。Agent 的长程任务难在步骤多，也难在每一步都会改变环境。点错按钮、删错文件、用错账户、误解网页提示，都把状态写进了现实系统。

因此，Agent 系统要有显式的停止条件、预算限制、回滚策略和人工接管点。缺了这些东西，执行能力越强，风险越难收。

### 可靠性不能靠"模型更大"自动解决

OpenAI 在 Operator 和 Computer-Using Agent 相关资料里反复强调 research preview、用户确认和安全隔离。Anthropic 2024 年发布 Computer Use 时也说得很直接：这一能力仍处在实验阶段，会笨拙，也会出错。让模型操作计算机，已经比让模型回答问题更接近真实责任。
...
把 memory 理解成"加一个向量数据库"会误导。记忆至少分三类：
...
持续学习的难点也在这里。一个 Agent 什么都记，会变慢、变乱、变危险；什么都不记，就永远只是一次性脚本。好的记忆系统要决定什么值得进入长期记忆，什么时候应该遗忘，什么时候必须让用户确认。

### Benchmark 只能说明一部分

SWE-bench、WebArena、OSWorld、TAU-bench 这些评估把 Agent 从主观演示拉回可比较任务。但数字不能直接换算成生产可靠性。一个模型在某个评测基准上提升，可能说明它更会操作浏览器、更会修代码、更会调用工具；不能直接说明这个 Agent 能安全处理企业财务、医疗记录或客户合同。

写评测时，先问三件事：测的是什么，数字主要反映哪一层能力，不能推出什么结论。

## 大厂押注：模型、工具、运行时和工作流正在合流（01:47:09 起）

2025 到 2026 年的大厂动作放在一起看，它们已经从单纯发模型往外走了：工具、状态、权限、观测、评估和部署运行时，都被补进了 Agent 产品栈。
...
模型层当然还重要，但产品能不能站住，越来越取决于工具接入是否安全，状态是否可恢复，执行是否可观测，错误是否可控，长期记忆能不能慢慢沉淀成经验。

## 中美扩散路径：一个从技术社区外溢，一个从应用场景提速（00:55:10 起）

节目里关于中美科技辐射的讨论，更像观察。美国生态往往先从模型、框架、论文、开发者工具和创业公司扩散；中国生态则更容易从内容平台、超级 App、企业应用和普通用户场景里快速试错。
...
苏煜说的"边界消弭"也体现在这里：模型公司、应用公司、开发工具公司、企业软件公司之间的分工正在重叠。谁能更快拿到真实任务反馈，谁就更可能把 Agent 从演示推到工作流。

## 如果你要学习或采用 Agent，顺序别反过来

这期访谈对开发者和产品负责人有一个具体提醒：别急着做"通用 Agent"。
...
这些事情不能后补。没有评估和观测，Agent 越强，排错越困难；没有权限边界，工具越多，风险越大；没有记忆治理，持续学习容易变成持续污染。

## 如果你想继续追这条线，推荐按这个顺序补资料

如果对 Agent 技术史起了兴趣，继续往下读时，最好不要从产品宣传开始。路线通常是：
...
按这个顺序，读者更容易把"历史路线""论文机制""评测环境"和"产品包装"拆开，不会把所有新名词都当成同一种进展。

## 谁该看原视频，谁读这篇文章就够

这期访谈主线是把 Agent 技术史和 2026 的产品形态接起来，而不是给某个产品做上手教程。如果你正打算选型 Agent 平台、判断某个"数字员工"项目值不值得投入，文章里关于执行栈、权限边界、可观测性和记忆治理的部分，基本够你拿去做判断。

该回去看原视频的情况，通常是这几类：

- 你想听到苏煜对某个具体工作（Mind2Web、SeeAct、HippoRAG、交互式语义解析）的口头展开，文章只做了压缩；
- 你想核对"边界的消弭""GUI vs. CLI"这些段落里，哪些是苏煜的原话、哪些是作者的转述与判断；
- 你关心 01:52:47 之后苏煜对"见证 Agent 完整周期"和 conceptual framework 的现场收束，这段在文章里没有逐句展开。

反之，如果你只是想快速知道"Agent 为什么在 2026 变成系统问题、OpenClaw 和 NeoCognition 各自押什么"，读这篇文章就够了，不必再花两小时。

## 这场访谈留下的问题

这期节目最后留下的是一条一直没断过的问题链。产品名会换，问题本身没换：Logical Agent 说明行动需要形式化，Neural Agent 说明规则覆盖不了真实世界，Semantic Parsing 说明语言可以变成程序，Language Agent 又把线索重新拧到一起：一个会使用语言的模型，能不能在真实环境里长期、可靠、可审计地行动？
...
到 2026 年，再看 Agent，先放下产品名，直接看那条执行链：语言、工具、记忆、权限、评估和反馈能不能连稳。分水岭是它能不能在长任务里留下可复查的轨迹，知道什么时候该停，什么时候该把控制权还给人。只要链路连不稳，再响的"通用数字 Agent"也还是只能停在演示里。
