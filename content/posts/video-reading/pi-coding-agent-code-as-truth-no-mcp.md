...
这更像一次对「AI Agent 复杂度爆炸」的明确反抗，而不是某个功能点的取舍。
...
这两个人背景差异本身就是信号：一群从 AI 巨头出来的工程师做 Agent，目标是占领市场；两个从 libGDX / Flask 时代走出来的传奇开发者也在做 Agent，目标是让自己和朋友用得顺手。这两种状态，对「AI Agent 应该长什么样」的回答不会一样。
...
Mario 反对的不只是「RAG 这个具体技术」，是「让模型记住你的整个代码库」这件事本身。
